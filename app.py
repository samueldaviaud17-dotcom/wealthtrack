...
<div style="display:flex;align-items:baseline;gap:6px">
  ...
  <span style="font-size:12px;color:{_score_col};font-weight:600">{_score_lbl}</span>
            ...
            _help_score_q = (
                f"Score de qualité fondamentale basé sur {hist_d.get('n',5)} ans (Yahoo Finance). "
                ...
                f"Grille : ≥8 Excellente · ≥6 Bonne · ≥4 Correcte · <4 Faible. "
                f"Ce score mesure la qualité historique, pas la valorisation."
            )
            st.metric(label="ℹ️ Score qualité",
                value=f"{_score:.1f}/10 — {_score_lbl}",
                help=_help_score_q,
        ...
        kpi_r = st.columns(4)
        _h_pp  = (f"Formule : VI / (1+{_perf_v:.0f}%)^{_n_yr} x (1-{_ms_v:.0f}%) = {pp_base_kpi:,.0f}€. "
                  f"Prix maximum pour atteindre {_perf_v:.0f}%/an sur {_n_yr} ans avec marge sécu {_ms_v:.0f}%. "
                  f"Cours < Prix a payer = opportunite. Cours > Prix a payer = trop cher.")
        _h_up  = (f"Formule : (Prix a payer - Cours) / Cours = ({pp_base_kpi:,.0f} - {cours_kpi:,.0f}) / {cours_kpi:,.0f}. "
                  f"Positif = action sous-evaluee vs objectif rendement. "
                  f"Negatif = action trop chere.")
        _h_vi  = (f"Formule : EPS {_YR+_n_yr} projete x PER {_per_v}x. "
                  f"Valeur estimee dans {_n_yr} ans (croissance CA {_g_v:.0f}%/an, PER {_per_v}x). "
                  f"Ne tient pas compte du rendement exige ni de la marge de securite.")
        _h_eps = ("EPS = Benefice net / Nombre d'actions. "
                  "Benefice genere par action sur le dernier exercice (Yahoo Finance). "
                  "Point de depart de toutes les projections.")

        with kpi_r[0]:
            st.metric(label="💰 Prix à payer BASE",
                value=f"{pp_base_kpi:,.0f} €" if pp_base_kpi else "—",
                delta="avec marge de sécurité", delta_color="off", help=_h_pp)
        with kpi_r[1]:
            st.metric(label="📈 Upside BASE",
                value=f"{upside_kpi:+.1f}%" if upside_kpi is not None else "—",
                delta="vs cours actuel", delta_color="off", help=_h_up)
        with kpi_r[2]:
            st.metric(label="🔍 Valeur intrinsèque",
                value=f"{vi_base_kpi:,.0f} €" if vi_base_kpi else "—",
                delta="scénario BASE", delta_color="off", help=_h_vi)
        with kpi_r[3]:
            st.metric(label="📊 EPS actuel",
                value=f"{hist_eps_kpi:.2f}" if hist_eps_kpi else "—",
                delta="€/$ par action", delta_color="off", help=_h_eps)
        st.markdown("<br>", unsafe_allow_html=True)
