...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        # FX : taux moyen EUR.USD du fichier de l'année sélectionnée
        # On évite le taux live qui peut être mal orienté selon la version de yfinance
        _fx_yr_data = st.session_state['ibkr_data'].get(_rpt_yr, {})
        _fx_rpt_raw = _fx_yr_data.get('fx', 0)
        _fx_rpt = _fx_rpt_raw if 0.8 < _fx_rpt_raw < 2.0 else 1.10
