...
tab1, tab5, tab2, tab3, tab4 = st.tabs(["🏠 Vue d'ensemble", "📊 Performance", "📈 Bourse", "₿ Crypto", "⚙️ Options"])
...
with tab1:
    ...
    with L:
        sec("Évolution mensuelle 2026","📅","#D29922","#1C1A0F")
        ...
        sec("Variations mensuelles","📊","#D29922","#1C1A0F")
        ...
        sec("Évolution annuelle","📈","#D29922","#1C1A0F")
    ...
    with R:
        ...
        if vv:
            ...
            st.plotly_chart(fig3,use_container_width=True,config={'displayModeBar':True,'scrollZoom':True,'modeBarButtonsToRemove':['lasso2d','select2d','hoverClosestCartesian','hoverCompareCartesian','toggleSpikelines'],'displaylogo':False})
...
with tab2:
    ...
    with L2:
        sec("DCA Mensuel 2026 — CTO & PEA","💸","#06B6D4","#0A1A1C")
        ...
        if xc:
            sec("DCA Cumulé 2026","📈","#06B6D4","#0A1A1C")
    ...
    with R2:
        sec("Évolution du solde (CTO + PEA)","📊","#06B6D4","#0A1A1C")
        ...
        # Dividendes
        sec("Dividendes 2026","💰","#06B6D4","#0A1A1C")
        div_rows=[]
        for i in range(15):
            r_=11+i
        ...
        sec("Solde mensuel CTO & PEA","📋","#06B6D4","#0A1A1C")
        solde_rows=[]
        for i in range(12):
            m=mois_f[i]
...
with tab3:
    ...
    with L3:
        sec("Portefeuille Crypto","₿","#F7931A","#1C100A")
        cdata=[]
        ...
        sec("DCA Mensuel 2026","💸","#F7931A","#1C100A")
    ...
    with R3:
        ...
        sec("Gains Staking","🌾","#F7931A","#1C100A")
...
with tab4:
    ...
    sec("Options 2026 — Stratégie de la Roue — Primes mensuelles","⚙️","#8B5CF6","#120A1C")
...
with tab5:
    ...
    with Lp:
        sec("Performance par enveloppe","🏆","#D29922","#1C1A0F")
        poches=[
...
<span style="color:{C['green']};font-weight:700">+{tp:.1f}%</span></div>""",unsafe_allow_html=True)
        ...
        # Évolution annuelle
        sec("Évolution annuelle du patrimoine","📈","#3FB950","#0A1C0F")
        tots=[n(v(df_ea,11+i,9)) for i in range(5)]
        yrs=["2022","2023","2024","2025","2026"]
        tv=[x for x in tots if x>0]; yl=yrs[:len(tv)]
        if tv:
    ...
    with Rp:
        sec("CAGR vs Benchmarks","⚡","#3FB950","#0A1C0F")
        benchs=[
...
<div style="height:100%;width:{bv/mx*100:.0f}%;background:{bc};border-radius:3px"></div></div></div>""",unsafe_allow_html=True)
        ...
        sec("Alpha vs Benchmarks","📐","#3FB950","#0A1C0F")
