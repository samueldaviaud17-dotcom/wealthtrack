...
<span style="font-size:12px;font-weight:700;color:{_vcol}">{'+' if _snap['var_pct']>=0 else ''}{_snap['var_pct']:.2f}%</span>
  ...
  <div style="font-size:10px;color:{C['muted']}">objectif {fmt(_obj_pat)}</div>
    ...
    with _o2:
        ...
        _dca_total_real = _dca_b_real + _dca_c_real + _dca_o_real
        ...
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px">
  <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{_dca_col}">{fmt(_dca_moy_mois)}<span style="font-size:11px;color:{C['muted']}"> /mois</span></div>
  <div style="font-size:10px;color:{C['muted']};margin:2px 0 2px">Total YTD : {fmt(_dca_total_real)} · <span style="color:{_dca_col};font-weight:700">{_pct_dca:.0f}%</span> objectif</div>
  <div style="font-size:9px;color:{C['muted']};margin-bottom:5px">Bourse {fmt(_dca_b_real)} · Crypto {fmt(_dca_c_real)} · Options {fmt(_dca_o_real)}</div>
  <div style="background:{C['border']};border-radius:3px;height:5px;margin-bottom:4px">
    <div style="background:{_dca_col};height:100%;width:{min(_pct_dca,100):.0f}%;border-radius:3px"></div>
  </div>
  <div style="font-size:10px;color:{C['muted']}">objectif {fmt(_dca_mois_obj)}/mois · {_nb_mois} mois</div>
</div>""", unsafe_allow_html=True)
  ...
  <div style="background:{C['border']};border-radius:3px;height:5px">
    ...
    # Card 4 — Avance / retard sur l'objectif patrimoine vs avancement de l'année
    with _o4:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
