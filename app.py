...
with tab1:
    ...
    if YF_AVAILABLE:
        st.markdown("""<style>
/* Supprime le padding Streamlit sous les graphiques dans l'expander indices */
div[data-testid="stExpander"] div[data-testid="stPlotlyChart"] {
    margin-bottom: -32px !important;
    padding-bottom: 0 !important;
}
div[data-testid="stExpander"] div[data-testid="stPlotlyChart"] > div {
    padding-bottom: 0 !important;
}
</style>""", unsafe_allow_html=True)
        with st.expander("📊 Indices de référence — variation journalière", expanded=True):
            ...
            <span style="font-size:12px;font-weight:700;color:{_vcol}">{'+' if _snap['var_pct']>=0 else ''}{_snap['var_pct']:.2f}%</span>
                    ...
                    # Normaliser la série en % par rapport au premier point
                    _serie_raw = _snap['serie']
                    _s0 = _serie_raw[0] if _serie_raw[0] != 0 else 1
                    _serie_pct = [(v / _s0 - 1) * 100 for v in _serie_raw]
                    _fig_spark = go.Figure(go.Scatter(y=_serie_pct, mode='lines',
                        line=dict(color=_vcol, width=1.5)))
                    _fig_spark.update_layout(height=120, margin=dict(l=0,r=0,t=0,b=0),
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    st.markdown(f"""<div style="background:{C['card']};border:1px solid {_vix_col}44;border-radius:8px;padding:10px 14px 4px 14px">
        ...
        <div style="padding:2px 8px;border-radius:4px;background:{_vix_col}22;display:inline-block;margin-top:4px">
        <span style="font-size:11px;font-weight:700;color:{_vix_col}">{_vix_label}</span>
        </div>
        <div style="font-size:9px;color:{C['muted']};margin-top:2px">&lt;15 Calme · &lt;20 Normal · &lt;30 Prudence · &lt;40 Stress</div>
        </div>""", unsafe_allow_html=True)
                    # Sparkline VIX normalisée
                    _vix_serie = _vix_snap['serie']
                    _v0 = _vix_serie[0] if _vix_serie[0] != 0 else 1
                    _vix_pct = [(v / _v0 - 1) * 100 for v in _vix_serie]
                    _fig_vix = go.Figure(go.Scatter(y=_vix_pct, mode='lines',
                        line=dict(color=_vix_col, width=1.5)))
                    _fig_vix.update_layout(height=120, margin=dict(l=0,r=0,t=0,b=0),
                        showlegend=False, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                        xaxis=dict(visible=False, showgrid=False, zeroline=False),
                        yaxis=dict(visible=False, showgrid=False, zeroline=False))
                    st.plotly_chart(_fig_vix, use_container_width=True,
                        config={'displayModeBar':False}, key="spark_VIX")
