...
with tab3:
    ...
    with L3:
        ...
        if _xc_ev:
            ...
            _fig_c_ev = go.Figure()
            _fig_c_ev.add_trace(go.Scatter(
                x=_xc_ev, y=_yc_ev, mode='lines+markers+text',
                line=dict(color=C['gold'], width=2.5, shape='spline'),
                marker=dict(color=C['gold'], size=7, line=dict(color=C['bg'], width=2)),
...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        _now_an   = _dt_an.now()
        _mois_cur = _now_an.month
        _yr_cur   = _now_an.year

        # ── Trades du mois courant ─────────────────────────────
        _all_trades_an = []
        ...
        for _t in _all_trades_an:
            try:
                ...
                if _dt_t.month == _mois_cur and _dt_t.year == _yr_cur:
        ...
        # ── Meilleur trade du mois (rendement) ────────────────
        ...
        sec(f"Rapport du mois — {_MOIS_FR_AN[_mois_cur-1]} {_yr_cur}", "📊", "#FDA4AF", "#1C0A12")

        _mois_partiel = _now_an.day < 28
  ...
  <div style='display:flex;justify-content:space-between;align-items:center'>
    ...
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
        ...
        _mois_label = _MOIS_FR_AN[_mois_cur-1]
        _yr_label   = str(_yr_cur)
  ...
  <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px'>
    <div>
      ...
      <div style='font-size:30px;font-weight:700'>{_mois_label} {_yr_label}</div>
  ...
  <div style='display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:4px'>
    <div>
      <div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Top tickers du mois</div>
  ...
  {_best_html}
        ...
        try:
            ...
            def _make_rapport_png():
                ...
                def rect(x,y,w,h,col,radius=8):
                    d.rounded_rectangle([x,y,x+w,y+h], radius=radius, fill=col)

                def txt_at(x, y, s, col, size=13):
                    try:
                        fnt = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", size)
                    except:
                        fnt = ImageFont.load_default()
                    d.text((x, y), str(s), font=fnt, fill=col)

                # Header
                txt_at(28, 24, "SAMINVEST · RAPPORT MENSUEL", mut, 11)
                txt_at(28, 44, f"{_MOIS_FR_AN[_mois_cur-1]} {_yr_cur}", txt, 30)
                if _mois_partiel:
                    txt_at(28, 84, "⚠ Mois en cours — données partielles", (210,153,34), 11)
                nc = grn if _pm_net_eur >= 0 else red
                txt_at(W-220, 24, "PRIMES NETTES", mut, 11)
                txt_at(W-220, 44, f"{_pm_net_eur:+.2f} €", nc, 26)

                # 3 cartes
                card_y = 110; card_h = 80; gutter = 16
                cw = (W - 2*28 - 2*gutter) // 3
                for i, (lbl, val, col) in enumerate([
                    ("PRIMES BRUTES", f"{(_pm_brutes_usd-_pm_rachats_usd)/_fx_an:+.2f} €", grn),
                    ("FRAIS",         f"-{_pm_frais_eur:.2f} €",                            red),
                    ("TRADES",        str(_pm_nb_trades),                                   blu),
                ]):
                    cx = 28 + i*(cw+gutter)
                    rect(cx, card_y, cw, card_h, car)
                    txt_at(cx+12, card_y+10, lbl, mut, 10)
                    txt_at(cx+12, card_y+30, val, col, 18)

                # Top 5 mois + Hall of fame
                col_y = 210
                txt_at(28, col_y, "TOP TICKERS DU MOIS", mut, 10)
                txt_at(W//2+10, col_y, "HALL OF FAME — DEPUIS LE DÉBUT", mut, 10)
                medals = ['🥇','🥈','🥉','4.','5.']
                for side, top5 in enumerate([_top5_mois, _top5_alltime]):
                    x_base = 28 if side==0 else W//2+10
                    max_v  = max((v for _,v in top5), default=1)
                    for i, (_tk, _val) in enumerate(top5):
                        ry = col_y + 22 + i*36
                        vc = grn if _val >= 0 else red
                        bar_w = int(max(_val/max_v, 0.05) * 280) if max_v > 0 else 14
                        txt_at(x_base, ry+2, f"{medals[i]} {_tk}", txt, 12)
                        rect(x_base+80, ry+4, bar_w, 8, vc, 4)
                        v_eur = _val/_fx_an if side==0 else _val
                        txt_at(x_base+370, ry+2, f"{v_eur:+.2f} €", vc, 12)

                # Meilleur trade
                if _best_trade:
                    by = H - 80
                    d.line([(28, by), (W-28, by)], fill=brd, width=1)
                    txt_at(28, by+10, "⚡ MEILLEUR TRADE DU MOIS", mut, 10)
                    bt_lbl = f"{'CALL' if _best_trade.get('call_put','')=='C' else 'PUT'} ${_best_trade.get('strike','')} · exp. {_best_trade.get('expiration','')}"
                    txt_at(28, by+26, f"{_best_trade.get('ticker','')}  {bt_lbl}", txt, 12)
                    txt_at(W-120, by+20, f"{_best_rdt:.1f}%", grn, 20)
            ...
            _mois_fn  = f"rapport_{_MOIS_FR_AN[_mois_cur-1]}_{_yr_cur}.png"
            st.download_button(
