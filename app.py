...
with tab1:
    ...
    # Options : valeur directement depuis l'app (capital_reel.json), SANS lien Sheet
    ...
    _opt_sheet_val = sv(df_pf, 8, 3)  # valeur Options dans Sheet (à soustraire)
    _opt_sheet_inv = sv(df_pf, 8, 2)  # investi Options dans Sheet (à soustraire)
    # Soustraire la part Options du Sheet, ajouter la valeur app
    if _opt_sheet_val > 0:
        pat = pat - _opt_sheet_val
    if _ibkr_cr > 0:
        pat = pat + _ibkr_cr
    if _opt_sheet_inv > 0:
        inv = inv - _opt_sheet_inv
    if _ibkr_inv > 0:
        inv = inv + _ibkr_inv
...
with tab4:
    ...
    with _cr_col1:
        ...
        # Colonnes internes : card + crayon dans le coin droit
        _card_col, _pencil_col = st.columns([7, 1])
        with _card_col:
            st.markdown(card_editable(
                "CAPITAL RÉEL", f"{_cr_val:.2f} €",
                "← cliquer ✏️ pour modifier", C['teal'], "🏦"
            ), unsafe_allow_html=True)
        with _pencil_col:
            # CSS pour aligner verticalement le popover avec la card
