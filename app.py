...
def _compute_tri_global_et_benchmarks():
    ...
    # Année en cours depuis Sheet Bourse/Crypto (flux déjà écoulés)
    _YR_NOW = _now.year
    try:
        _df_b = fetch("📈 Bourse")
        _df_c = fetch("🪙 Crypto")
        _dca_cto_cur  = [n(v(_df_b, 11+i, 2)) for i in range(12)]
        _dca_pea_cur  = [n(v(_df_b, 11+i, 3)) for i in range(12)]
        _dca_cryp_cur = [n(v(_df_c, 11+i, 2)) for i in range(12)]
        for i in range(12):
            _d = datetime(_YR_NOW, i+1, 4)
            if _d >= _now: break
            _ab = _dca_cto_cur[i] + _dca_pea_cur[i]
            _ac = _dca_cryp_cur[i]
            if _ab > 0: _bourse_cfs.append((_d, _ab))
            if _ac > 0: _crypto_cfs.append((_d, _ac))
    except Exception:
        # Fallback : lissage uniforme sur les mois écoulés de l'année en cours
        _mois_ecoules = _now.month - 1
        if _mois_ecoules > 0 and _pi_bourse > 0:
            # Estimer le total 2026 en extrapolant le rythme des années précédentes
            _moy_b = 591 + 120  # CTO + PEA moy 2026 estimée
            _moy_c = 200
            for i in range(_mois_ecoules):
                _d = datetime(_YR_NOW, i+1, 4)
                _bourse_cfs.append((_d, _moy_b))
                _crypto_cfs.append((_d, _moy_c))
