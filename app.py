...
with tab1:
    ...
    # Appliquer si Capital Réel saisi — sans condition sur Sheet (qui peut être 0)
    if _ibkr_cr > 0:
        _base_opt = _opt_sheet_val if _opt_sheet_val > 0 else 0
        pat = pat - _base_opt + _ibkr_cr
    if _ibkr_inv > 0:
        _base_inv = _opt_sheet_inv if _opt_sheet_inv > 0 else 0
        inv = inv - _base_inv + _ibkr_inv
