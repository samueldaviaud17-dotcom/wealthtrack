...
with tab9:
    ...
    st.markdown(f"""<div style='font-size:12px;color:{C['text']};line-height:1.8'>

<b>⚠️ Limite connue — retraits non gérés (TRI Global)</b><br>
Aucun retrait effectué à ce jour → calcul correct pour l'instant. Si un retrait a lieu un jour, il
faudra : (1) l'ajouter comme cashflow positif dans le calcul TRI global, et (2) simuler une vente
de parts au même montant/date dans chaque indice simulé. Le moteur XIRR gère déjà nativement les
flux positifs intermédiaires — seule la collecte de la donnée "retrait" manque.
...
mouvements, valeurs Bourse/Crypto...), avec archivage JSON des valeurs Sheet actuelles comme
"point zéro", et formulaires Streamlit structurés (pas de saisie en langage naturel — trop risqué
pour du financier). Une fois en place, les retraits deviennent un non-sujet : ils seront captés
nativement comme <code>depots_detail</code> l'est déjà pour IBKR. Question ouverte à trancher
avant de se lancer : le stockage persistant (Streamlit Cloud = filesystem éphémère → JSON committé
via API GitHub, ou SQLite + service externe).

<br><br><b>📌 Dépendances Sheet restantes (Performance / Vue d'ensemble)</b><br>
...
À traiter dans le grand chantier ci-dessus.

<br><br><b>🎨 Chantier Visuel — Vue d'ensemble</b><br>
Cards "Objectif patrimoine fin d'année" + jauge + "Objectif DCA mensuel" + barre "Année écoulée" :
revoir la mise en page (alignement, tailles, cohérence avec le reste de l'app). Pas encore détaillé
précisément — à affiner ensemble.

<br><br><b>📋 Chantiers validés — priorisés (12/06/2026)</b><br>
- <b>🔴 PRIORITÉ — Export/Backup JSON</b> : bouton "exporter mes données" (capital_reel.json,
depots_detail, historique IBKR...) téléchargeable depuis l'app + fonction d'import pour restaurer
en cas de perte serveur. Filet de sécurité, peu coûteux à implémenter.<br>
- <b>Alertes visuelles in-app</b> (pas d'email — nécessiterait un cron externe, hors scope) :
bandeau/badge si position options proche ITM, DCA manqué, etc. Visible à la prochaine ouverture
(usage mobile fréquent confirmé).<br>
- <b>Historique TRI dans le temps</b> : snapshot périodique (granularité à définir ensemble —
mensuel probable, affichage possiblement trimestriel/semestriel pour lisser).<br>
- <b>Onglet "Fiscalité" dédié</b> : nécessite de tracker l'historique des ventes Bourse/Crypto
(date, qté, prix vente, PRU au moment de la vente, PV/MV réalisée — pas suivi aujourd'hui, contrairement
aux Options où l'historique existe déjà). Calcul PFU/flat tax. Gros morceau structurant.<br>
- <b>Rebalancing assistant</b> : à creuser ensemble, pas prioritaire actuellement.<br>
- <b>Mobile/responsive</b> : usage mobile fréquent confirmé → tester les tableaux HTML
(`overflow-x:auto`) en conditions réelles, ajuster font-size/padding si besoin.<br>
- <b>🎨 Chantier Visuel Vue d'ensemble</b> : revoir mise en page cards Objectifs/jauge/Année écoulée
(détails à définir ensemble).
