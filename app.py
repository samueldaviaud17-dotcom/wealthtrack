...
with tab_budget:
    ...
    with st.expander(f"📊 {_ML.get(_sel,'').upper()} — Suivi mensuel du budget", expanded=True):
        ...
        st.markdown(
            f"<div style='background:linear-gradient(90deg,#0A1218,#0A1A18);border-left:3px solid #58A6FF;"
            f"padding:8px 14px;border-radius:6px;margin:8px 0 0'>"
            f"<span style='font-size:13px;font-weight:700;color:#58A6FF'>✏️ Gérer les transactions</span>"
            f"<span style='font-size:10px;color:{_C_MUTED};margin-left:8px'>Créer · Modifier · Supprimer</span></div>",
            unsafe_allow_html=True)
        if st.toggle("",key="tog_gerer_tx",label_visibility="collapsed"):
            _action_tx=st.radio("",["➕ Créer","✏️ Modifier","🗑️ Supprimer"],horizontal=True,key="tx_action",label_visibility="collapsed")

            if _action_tx=="➕ Créer":
                with st.form("form_tx_create",clear_on_submit=True):
                    _cA,_cB=st.columns(2)
                    with _cA:
                        _fd=st.date_input("Date",value=_now_p.date(),key="fb_date_c")
                        _ft=st.selectbox("Type",_TYPES,key="fb_type_c")
                        _fc=st.text_input("Catégorie",key="fb_cat_c")
                    with _cB:
                ...
                if _txs:
                    ...
                    with st.form("form_tx_mod",clear_on_submit=False):
                        ...
                        with _cA2:
                            ...
                            _fm_cat=st.text_input("Catégorie",value=_tx_obj['categorie'],key="fm_cat")
                        with _cB2:
                        ...
                        _fm_sub=st.form_submit_button("💾 Sauvegarder modification",use_container_width=True)
                    if _fm_sub:
        ...
        st.markdown(
            f"<div style='background:linear-gradient(90deg,#0A1218,#140A18);border-left:3px solid #F59E0B;"
            f"padding:8px 14px;border-radius:6px;margin:8px 0 0'>"
            f"<span style='font-size:13px;font-weight:700;color:#F59E0B'>💡 Planification du budget</span>"
            f"<span style='font-size:10px;color:{_C_MUTED};margin-left:8px'>Ajouter · Modifier · Supprimer</span></div>",
            unsafe_allow_html=True)
        if st.toggle("",key="tog_planif",label_visibility="collapsed"):
            ...
            if _action_bp=="➕ Ajouter":
                ...
                if _all_bp_cats:
                    ...
                    if _action_bp=="✏️ Modifier":
                        with st.form("form_planif_mod",clear_on_submit=False):
                            _fpm_a=st.number_input("Nouveau montant prévu (€)",value=float(_bp_prevu),step=1.0,format="%.2f",key="fpm_amt")
                            _fpm_s=st.form_submit_button("💾 Sauvegarder",use_container_width=True)
                        if _fpm_s:
                        ...
                        st.markdown(f"<div style='color:#F87171;font-size:11px;margin:4px 0'>Supprimer : {_bp_cat_k} ({_bp_prevu:.2f}€ prévu)</div>",unsafe_allow_html=True)
