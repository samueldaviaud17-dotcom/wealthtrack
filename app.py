...
with tab1:
    ...
    pat=n(v(df_p,4,1)); inv=n(v(df_p,4,3)); perf=n(v(df_p,4,5)); cagr=n(v(df_p,4,9))
    ytd=n(v(df_em,4,9)); ytdp=n(v(df_em,4,12))
    ...
    with L:
        ...
        pats=[n(v(df_em,8+i,2)) for i in range(12)]
        vars_=[n(v(df_em,8+i,3)) if str(v(df_em,8+i,3,"")).strip() not in ["—",""] else None for i in range(12)]
        ...
        if yv:
            fig2=go.Figure(go.Bar(x=xv,y=yv,
                ...
                text=[f"{'+' if x>=0 else ''}{x:,.0f}€" for x in yv],
    ...
    with R:
        ...
        vals=[n(v(df_p,16+i,2)) for i in range(7)]
        ...
        tots=[n(v(df_ea,8+i,9)) for i in range(5)]
...
with tab2:
    ...
    bi=n(v(df_b,4,1)); bs=n(v(df_b,4,3)); bpv=n(v(df_b,4,5)); bpp=n(v(df_b,4,7))
    bd=n(v(df_b,4,15)); bp=n(v(df_b,4,11))
    ...
    with L2:
        ...
        dca_cto=[n(v(df_b,9+i,2)) for i in range(12)]
        dca_pea=[n(v(df_b,9+i,3)) for i in range(12)]
        fig5=go.Figure()
    ...
    with R2:
        ...
        sol=[n(v(df_b,9+i,9)) for i in range(12)]
...
with tab3:
    ...
    with L3:
        ...
        sec("DCA Mensuel 2026","💸")
        dca_c=[n(v(df_c26,17+i,2)) for i in range(12)]
    ...
    with R3:
        ...
        pcts_c=[n(v(df_ct,19+i,13)) for i in range(5)]
...
with tab4:
    ...
    oc=n(v(df_opt,4,3)); oi=n(v(df_opt,4,6)); oroi=n(v(df_opt,4,9))
    ope=n(v(df_opt,4,12)); opd=n(v(df_opt,4,15))
    ...
    with L4:
        ...
        primes=[n(v(df_opt,8+i,9)) for i in range(12)]
        rois=[n(v(df_opt,8+i,15)) for i in range(12)]
        fig9=go.Figure()
    ...
    with R4:
        ...
        poches_data=[
            ("📈 Bourse (CTO+PEA)", n(v(df_pf,5,2)), n(v(df_pf,5,3)), C['blue']),
            ("₿ Crypto (Binance)",  n(v(df_pf,6,2)), n(v(df_pf,6,3)), C['gold']),
            ("⚙️ Options (IBK)",    n(v(df_pf,7,2)), n(v(df_pf,7,3)), C['purple']),
        ]
...
<span style="color:{C['green']};font-weight:700">+{tp:.1f}%</span></div>""",unsafe_allow_html=True)
        ...
        benchs=[
            ("🏆 Mon PF",   n(v(df_pf2,12,2)), C['gold'], True),
            ("📈 SP500",    n(v(df_pf2,13,2)), C['cyan'], False),
            ("🇫🇷 CAC 40", n(v(df_pf2,14,2)), C['cyan'], False),
            ("🌍 STOXX 600",n(v(df_pf2,15,2)), C['cyan'], False),
        ]
