...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        # Mois disponibles pour l'année sélectionnée + indicateurs perf
        _mois_dispo = {}  # {m: net_usd}
        for (_yr2, _m2), _net in _perf_par_mois.items():
            if _yr2 == _sel_yr:
                _mois_dispo[_m2] = _net

        # Construire options mois avec indicateur visuel
        _mois_opts = ["Toute l'année"]
        _mois_map  = {_mois_opts[0]: None}
        ...
        _sel_m = _mois_map.get(_sel_m_lbl)  # None = toute l'année, int = mois
