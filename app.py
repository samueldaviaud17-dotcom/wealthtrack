...
def refresh_watchlist_cours():
    ...
    st.session_state["watchlist_last_refresh"] = datetime.now().strftime("%H:%M:%S")
...
with tab6:
    ...
    # ── KPI row — basé sur la formule IFS exacte ────
    wl = st.session_state.watchlist

    def ecart_ifs(w):
        """Calcule l'écart et le statut IFS exactement comme la watchlist."""
    ...
    nb_achat      = nb_ideal + nb_sans_marge   # total opportunités d'achat
    ecarts_pos    = [ecart_ifs(w)[0] for w in wl if ecart_ifs(w)[0] > 0]
    best_ecart    = max(ecarts_pos) if ecarts_pos else 0
    avg_ecart_all = sum(ecart_ifs(w)[0] for w in wl) / len(wl) if wl else 0

    kc = st.columns(5)
    kpi_data = [
        ("ACTIONS ANALYSÉES", str(nb_total) if nb_total else "—",
         "dans la watchlist", C_VAL, "🔍"),
        ("💎 ACHAT IDÉAL",    str(nb_ideal) if nb_ideal else "—",
         f"{nb_sans_marge} achat sans marge", C['green'], "✅"),
        ("🚫 SURCOTÉES",      str(nb_surcote) if nb_surcote else "—",
         f"{nb_proche} proche · {sum(1 for w in wl if ecart_ifs(w)[1]=='wait')} attente", C['red'], "📉"),
        ("MEILLEURE OPPORT.", f"+{best_ecart:.1f}%" if best_ecart else "—",
         "écart max disponible", C['cyan'], "🎯"),
        ("ÉCART MOYEN",       f"{avg_ecart_all:+.1f}%" if wl else "—",
         "toutes actions", "#60A5FA" if avg_ecart_all >= 0 else C['red'], "📊"),
    ]
...
<div style="font-size:11px;color:{C['muted']}">Saisir les données</div>
        ...
        if any(r for r in results.values() if r and r.get("price")):
            ...
            for scen,(color,dash,width) in scen_plot.items():
                ...
                perf_r  = st.session_state.get(f"valo_perf_{scen}", 10.0)
                ms_r    = st.session_state.get(f"valo_ms_{scen}", 10.0)
                ...
                for yr in range(1, 7):
                    ...
                    pp_y   = vi_y / (1 + perf_r/100)**yr * (1 - ms_r/100)
                    prices_proj.append(pp_y)
...
<div style="font-size:10px;color:{C['muted']}">{score_detail}</div>
    ...
    if st.session_state.watchlist:
        ...
        # Auto-refresh trigger (when toggle is on and autorefresh fired)
        ...
        # ── Styled HTML table like Google Sheet ──
        # Sort by écart IFS (desc by default = meilleures opport. en haut)
        def get_ecart(w):
            c = w.get("cours", 0); p = w.get("a_payer", 0)
            return (p - c) / c * 100 if c > 0 and p > 0 else -999
        asc = st.session_state.get("wl_sort_asc", False)
        sorted_wl = sorted(st.session_state.watchlist, key=get_ecart, reverse=not asc)

        # Statut logic — copie exacte de la formule Google Sheet
        # F2 = (Prix_à_payer - Cours) / Cours  → positif si cours < prix à payer
...
<tbody>"""
        ...
        for w in sorted_wl:
            ...
            # Background color matching Google Sheet gradient
            if ecart_w > 0:
            ...
            tbl += f"""<tr style="border-bottom:1px solid {C['border']}22;transition:background .15s">
  <td style="padding:7px 12px;font-weight:600;color:{C['text']}">{w['name']}</td>
  <td style="padding:7px 12px;text-align:center;font-family:monospace;font-size:10px;color:{C_VAL}">{ticker_w if ticker_w else "—"}</td>
