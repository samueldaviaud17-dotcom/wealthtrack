...
<div style="text-align:right"><div style="font-size:12px;color:{C['muted']}">Données live · Refresh auto toutes les 5 min</div><div style="font-size:10px;color:{C['muted']};margin-top:2px" id="synchro-time">🔄 Synchro : chargement...</div></div>
...
with tab1:
    ...
    # ── Dernière synchro ────────────────────────────────
    from datetime import datetime, timezone, timedelta
    paris = datetime.now(timezone(timedelta(hours=2)))
    now = paris.strftime("%d/%m/%Y à %H:%M")
    ...
    with L:
        ...
        # Get % variations too
        varp=[n(v(df_em,13+i,4))*100 if str(v(df_em,13+i,4,"")).strip() not in ["—","","0"] else None for i in range(12)]
