  ...
  <div style="padding:0 14px 10px 14px;display:flex;justify-content:space-between;align-items:center">
    ...
    if not _earn_tickers:
        ...
        if not _earn_list:
            ...
            def _earn_src(tk):
                if tk in _ibkr_options_set:  return '⚙️ Opt.', '#818CF8'
                if tk in _ibkr_actions_set:  return '📊 Action', '#7DD3FC'
                if tk in _wl_pf_set:         return '📈 PF', '#6EE7B7'
                if tk in _wl_opt_set:         return '⚙️ WL Opt', '#C4B5FD'
                if tk in _wl_surv_set:        return '👀 Surv.', '#FCD34D'
                return '⭐ WL', '#FCD34D'
            ...
            for _wk_monday in sorted(_weeks.keys()):
                ...
                for _dow in range(5):
                    ...
                    for _e in _day_events:
                        ...
                        # Couleur carte selon urgence
                        if _dl < 0:
                            _card_border, _card_bg = '#374151', '#0F172A'
                        elif _dl == 0:
                            _card_border, _card_bg = '#818CF8', '#1E1B4B'
                        elif _dl <= 2:
                            _card_border, _card_bg = '#F97316', '#1C0E00'
                        else:
                            _card_border, _card_bg = '#334155', '#0D1117'

                        _events_html += f"""<div style="background:{_card_bg};border:1px solid {_card_border};
border-radius:6px;padding:8px 10px;margin-bottom:6px">
  <div style="font-size:14px;font-weight:800;color:#F1F5F9;letter-spacing:.03em">{_tk}</div>
  <div style="margin-top:3px">
    <span style="font-size:9px;color:{_src_c};background:{_src_c}18;
    padding:1px 5px;border-radius:3px;font-weight:600">{_src_lbl}</span>
  </div>
  {"<div style='font-size:10px;color:#64748B;margin-top:4px'>" + _est_str + "</div>" if _est_str else ""}
</div>"""
  ...
  {_events_html or _empty_cell}
                ...
                st.markdown(f"""<div style="margin-bottom:4px">{_wk_label}{_this_wk_badge}</div>
