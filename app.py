...
with tab8:
  ...
  {_best_html}
        ...
        # Bouton export image via html2canvas dans un composant HTML auto-contenu
        _mois_fn = _MOIS_FR_AN[_mois_cur-1] + '_' + str(_yr_cur)
        st.markdown(f"""
<div style='margin-top:12px;text-align:right'>
  <button onclick='exportRapport()' style='background:#21262D;color:#E6EDF3;border:1px solid #30363D;border-radius:8px;padding:8px 18px;font-size:12px;cursor:pointer;font-weight:600'>
    📸 Télécharger le rapport en image
  </button>
</div>
<script>
(function(){{
  ...
  function doExport(){{
    ...
    if(!el){{ alert('Rapport introuvable'); return; }}
    window.html2canvas(el,{{backgroundColor:'#0D1117',scale:2,useCORS:true,logging:false}}).then(function(canvas){{
      var a=document.createElement('a');
      a.download='rapport_{_mois_fn}.png';
