...
⚠️ Ce score mesure la **qualité historique**, pas la valorisation.""")
        ...
        def kpi_with_help(col, title, value, sub, color, icon, help_md):
            """Carte KPI avec bouton ? intégré dans la même cellule."""
            with col:
                c_card, c_btn = st.columns([5, 1])
                with c_card:
                    st.markdown(card(title, value, sub, color, icon), unsafe_allow_html=True)
                with c_btn:
        ...
        kpi_with_help(kpi_r[0], "PRIX À PAYER BASE",
            ...
            f"""**Formule :** VI ÷ (1+{_perf_v:.0f}%)^{_n_yr} × (1−{_ms_v:.0f}%)
...
🔴 Cours > Prix à payer → action trop chère""")
        ...
        kpi_with_help(kpi_r[1], "UPSIDE BASE",
            ...
            f"""**Formule :** (Prix à payer − Cours actuel) ÷ Cours actuel

Mesure l'écart entre le **prix à payer calculé** ({pp_base_kpi:,.0f} €) et le **cours actuel** ({cours_kpi:,.0f} €).

➕ Positif → action **sous-évaluée** par rapport à ton objectif de rendement  
➖ Négatif → action **trop chère** pour atteindre ton rendement cible""")

        kpi_with_help(kpi_r[2], "VALEUR INTRINSÈQUE",
            f"{vi_base_kpi:,.0f} €" if vi_base_kpi else "—",
            "scénario BASE", "#7DD3FC", "🔍",
            f"""**Formule :** EPS {_YR+_n_yr} projeté × PER cible {_per_v}x

C'est la **valeur estimée** de l'action dans **{_n_yr} ans** selon le scénario BASE :
- Croissance CA : **{_g_v:.0f}%/an**
- PER cible : **{_per_v}x**

Ne tient pas encore compte du rendement exigé ni de la marge de sécurité.""")

        kpi_with_help(kpi_r[3], "EPS ACTUEL",
            f"{hist_eps_kpi:.2f}" if hist_eps_kpi else "—",
            "€/$ par action", C["gold"], "📊",
            """**EPS** = Earnings Per Share = Bénéfice net ÷ Nombre d'actions

Bénéfice généré **par action** sur le dernier exercice fiscal connu (source : Yahoo Finance).

C'est le **point de départ** de toutes les projections EPS futures.""")
