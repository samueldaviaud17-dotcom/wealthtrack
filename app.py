...
<span style="font-size:12px;color:{C['muted']}">Données live · Refresh toutes les 5 min</span>
...
with tab1:
    ...
    st.markdown(f"""<div style="text-align:right;font-size:10px;color:{C['muted']};margin:4px 0 12px 0">
        🔄 Dernière synchro app : <b>{now}</b> &nbsp;·&nbsp; Refresh auto toutes les 5 min
    ...
    with L:
        ...
        # Reference line: first month value
        if yp:
            fig1.add_hline(y=yp[0], line_dash="dot", line_color=C['muted'],
                annotation_text=f"Départ: {yp[0]:,.0f} €",
        ...
        # Get % variations too
        varp=[n(v(df_em,13+i,4)) if str(v(df_em,13+i,4,"")).strip() not in ["—",""] else None for i in range(12)]
        yvp=[x for x in varp if x is not None]
        if yv:
            text_labels=[f"{'+' if x>=0 else ''}{x:,.0f}€<br><span style='font-size:9px'>{'+' if (yvp[i] if i<len(yvp) else 0)>=0 else ''}{yvp[i]*100:.1f}%</span>" if i<len(yvp) else f"{'+' if x>=0 else ''}{x:,.0f}€" for i,x in enumerate(yv)]
            fig2=go.Figure(go.Bar(x=xv,y=yv,
                marker_color=[C['green'] if x>=0 else C['red'] for x in yv],
                text=[f"{'+' if x>=0 else ''}{x:,.0f}€  {'+' if (yvp[i] if i<len(yvp) else 0)>=0 else ''}{yvp[i]*100:.1f}%" if i<len(yvp) else "" for i,x in enumerate(yv)],
        ...
        # ── Mini recap performance par poche ─────────────
        sec("Performance par enveloppe","🏆")
        poches_recap=[
            ("📈 Bourse",  n(v(df_pf,6,2)), n(v(df_pf,6,3)), C['blue']),
            ("₿ Crypto",   n(v(df_pf,7,2)), n(v(df_pf,7,3)), C['gold']),
            ("⚙️ Options", n(v(df_pf,8,2)), n(v(df_pf,8,3)), C['purple']),
        ]
        recap_cols = st.columns(3)
        for col,(nm,pi,pl,pc) in zip(recap_cols, poches_recap):
            ppv=pl-pi; pp=(pl/pi-1)*100 if pi else 0
            with col:
                st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};
                border-left:3px solid {pc};border-radius:8px;padding:12px 14px">
                <div style="font-size:11px;color:{pc};font-weight:600;margin-bottom:6px">{nm}</div>
                <div style="font-family:'Space Grotesk';font-size:18px;font-weight:700">{fmt(pl)}</div>
                <div style="display:flex;justify-content:space-between;margin-top:4px">
                    <span style="font-size:11px;color:{C['muted']}">Investi: {fmt(pi)}</span>
                    <span style="font-size:12px;font-weight:700;color:{'#3FB950' if pp>=0 else '#F85149'}">{'+' if pp>=0 else ''}{pp:.1f}%</span>
                </div>
                <div style="height:3px;background:{C['border']};border-radius:2px;margin-top:6px">
                    <div style="height:100%;width:{min(100,abs(pp)*3):.0f}%;background:{pc};border-radius:2px"></div>
                </div></div>""", unsafe_allow_html=True)

    with R:
        sec("Répartition","🍩")
        cats=["📈 Bourse","₿ Crypto","⚙️ Options","🏢 PEE","📋 PER","🛡️ AV","💵 Cash"]
        clrs=[C['blue'],C['gold'],C['purple'],C['teal'],"#059669","#6B7280","#3B82F6"]
        vals=[n(v(df_p,19+i,2)) for i in range(7)]
        lbl=[cats[i] for i,x in enumerate(vals) if x>0]
        vc=[clrs[i] for i,x in enumerate(vals) if x>0]
        vv=[x for x in vals if x>0]
        if vv:
            # Labels with € values
        ...
        cash = n(v(df_p,25,2))
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};
        border-left:3px solid #3B82F6;border-radius:8px;padding:12px 14px;margin-top:8px">
        <div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">💵 Cash disponible</div>
        <div style="font-family:'Space Grotesk';font-size:20px;font-weight:700;color:#3B82F6">{fmt(cash)}</div>
        <div style="font-size:10px;color:{C['muted']};margin-top:4px">{cash/sum(vv)*100:.1f}% du patrimoine</div>
        </div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    for i in range(15):
        ...
        if act and act not in ["Action",""] and net>0:
            div_rows.append({"Action":act,"Pays":pays,"Net (€)":f"{net:.2f} €"})
    if div_rows:
        st.dataframe(pd.DataFrame(div_rows),use_container_width=True,hide_index=True,height=280)
...
with tab4:
    df_opt=fetch("⚙️ Options 2026")
    oc=n(v(df_opt,5,0)); oi=n(v(df_opt,5,6)); oroi=n(v(df_opt,5,9))
    ...
    with R4:
        ...
        for po,pi,pl,pc in poches_data:
            ppv=pl-pi; pp=(pl/pi-1)*100 if pi else 0
        ...
        tl=sum(p[2] for p in poches_data); ti2=sum(p[1] for p in poches_data)
...
<span style="color:{C['green']};font-weight:700">+{tp:.1f}%</span></div>""",unsafe_allow_html=True)
        ...
        df_pf2=fetch("🏆 Perf. Totale")
        benchs=[
            ("🏆 Mon PF",   n(v(df_pf2,13,2)), C['gold'], True),
            ("📈 SP500",    n(v(df_pf2,14,2)), C['cyan'], False),
            ("🇫🇷 CAC 40", n(v(df_pf2,15,2)), C['cyan'], False),
            ("🌍 STOXX 600",n(v(df_pf2,16,2)), C['cyan'], False),
        ]
        # fallback si données non dispo
        if all(b[1]==0 for b in benchs):
            benchs=[("🏆 Mon PF",3.60,C['gold'],True),("📈 SP500",13.37,C['cyan'],False),
...
<span style="font-family:'Space Grotesk';font-weight:{fw};color:{bc};font-size:13px">+{bv:.2f} %</span></div>
