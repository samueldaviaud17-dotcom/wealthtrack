...
with tab1:
    ...
    with L:
        ...
        # Get % variations too
        varp_raw=[str(v(df_em,13+i,4,"")).strip() for i in range(12)]
        xv_map={xv[j]: varp_raw[j] for j in range(min(len(xv),12)) if varp_raw[j] not in ["—",""]}
        if yv:
            fig2=go.Figure(go.Bar(x=xv,y=yv,
                marker_color=[C['green'] if x>=0 else C['red'] for x in yv],
                text=[f"{'+' if x>=0 else ''}{x:,.0f}€  {xv_map.get(xv[i],'')}" for i,x in enumerate(yv)],
