    ...
    with col_form:
        ...
        if method in ["EPS / PER", "FCF / PER"]:
            ...
            for scen, emoji, color in [("BASE","📊","#7DD3FC"),("BEAR","🐻","#F85149"),("BULL","🐂","#3FB950")]:
                ...
                with s4: st.number_input("Perf. annuelle (%)", value=10.0, format="%.1f", help="Rendement annuel exigé", key=f"valo_perf_{scen}")
                with s5: st.number_input("Marge sécu. (%)", value=10.0 if scen=="BASE" else (15.0 if scen=="BEAR" else 10.0), format="%.1f", help="Décote sur prix à payer", key=f"valo_ms_{scen}")
                with s6: st.number_input("Nb années", value=5, min_value=1, max_value=10, key=f"valo_ny_{scen}")
