...
def _compute_tri_global_et_benchmarks():
    ...
    for v in _hist.get("crypto", {}).get("versements", []):
        try:
            _d = datetime.strptime(v["date"], "%Y-%m-%d")
            # Ancienne structure : champ "total"
            if "total" in v:
                _amt = float(v["total"])
            else:
                # Nouvelle structure : somme des investi_usd par token (converti en €)
                # On utilise le total € saisi (somme des versements mensuels en €)
                # Pour simplifier : on prend la somme des champs numériques non-meta
                _amt = sum(
                    d.get("investi_usd", 0) if isinstance(d, dict) else float(d)
                    for k, d in v.items()
                    if k not in ("date", "_note", "total")
                )
                # Conversion $ → € approximative (0.92)
                _amt = _amt * 0.92
            if _amt > 0 and _d < _now:
...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    with st.expander("₿ Crypto — Versement & Conversion", expanded=False):
        ...
        if st.toggle("📋 Voir l'historique des versements crypto", key="toggle_hist_crypto"):
            ...
            if _vers_c:
                _all_tks = sorted(set(k for v in _vers_c for k in v if k != "date"))
