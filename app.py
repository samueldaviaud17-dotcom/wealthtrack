...
with tab5:
    ...
    with Lp:
        ...
        poches=[
            ("📈 Bourse (CTO+PEA)", sv(df_pf,6,2), sv(df_pf,6,3), C['blue'],   _dt_perf(2022,2,1)),
            ("₿ Crypto (Binance)",  sv(df_pf,7,2), sv(df_pf,7,3), C['gold'],   _dt_perf(2022,8,1)),
            ("⚙️ Options (IBK)",
        ...
        _tbl_perf += ("<thead><tr style='background:#111827'>"
            ...
            f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>CAGR</th>"
            "</tr></thead><tbody>")

        for po,pi,pl,pc,pdate in poches:
            ppv=pl-pi; pp=(pl/pi-1)*100 if pi else 0
            _years = (_now_perf - pdate).days / 365.25
            _cagr = ((pl/pi)**(1/_years) - 1) * 100 if pi>0 and pl>0 and _years>0 else None
            _ppv_col = C['green'] if ppv>=0 else C['red']
            _pp_col  = C['green'] if pp>=0 else C['red']
            _cagr_col = C['green'] if (_cagr is not None and _cagr>=0) else C['red']
            _ppv_str = f"{'+' if ppv>=0 else '-'}{int(round(abs(ppv))):,} €".replace(',', ' ')
            _pp_str  = f"{'+' if pp>=0 else ''}{pp:.1f}%"
            _cagr_str = f"{'+' if _cagr>=0 else ''}{_cagr:.1f}%" if _cagr is not None else "—"
        ...
        st.markdown(f"<div style='font-size:10px;color:{C['muted']};padding:4px 0 0 0;font-style:italic'>"
            "CAGR calculé depuis : Bourse = févr. 2022, Crypto = août 2022, Options = déc. 2023.</div>", unsafe_allow_html=True)
