...
with tab3:
    ...
    with R3:
        ...
        # ── Pie identique à la vue d'ensemble ──────────
        combined_c = sorted(
            [(nm, cl, vl) for nm, cl, vl in zip(names_c, clrs_c, vals_c) if vl > 0],
            key=lambda x: -x[2]
        )
        lbl_c  = [x[0] for x in combined_c]
        clr_c  = [x[1] for x in combined_c]
        val_c  = [x[2] for x in combined_c]
        ...
        fig8.add_annotation(
            text=f"<b>{cd/1000:.1f}k$</b>", x=0.5, y=0.5,
            font=dict(size=16, color=C['text'], family='Space Grotesk'), showarrow=False
        )
        ...
        # ── Légende manuelle comme vue d'ensemble ────
        for nm, cl, vl in zip(lbl_c, clr_c, val_c):
            pct_ = vl / sum(val_c) * 100 if sum(val_c) else 0
            st.markdown(f"""<div style="display:flex;align-items:center;justify-content:space-between;
padding:5px 8px;border-radius:6px;margin-bottom:3px;background:{C['card']}">
<div style="display:flex;align-items:center;gap:8px">
