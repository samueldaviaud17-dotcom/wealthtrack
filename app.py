...
def fetch_analyst_data(ticker: str):
    ...
    try:
        ...
        return {
            ...
            'div_yield':    round((div_yield if div_yield > 0.20 else div_yield * 100), 2) if div_yield else None,   # en % (guard: si >20% déjà en %)
...
.mkt-card-chart svg { width: 100% !important; height: 100% !important; display: block; }
        ...
        with st.expander("📊 Indices · VIX · Fear & Greed — variation journalière", expanded=True):
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    # Ligne zéro : clampée dans les limites du SVG
                    _sy0 = max(_PAD, min(_SH - _PAD, _PAD + _DRAW * (1 - (0 - _smn) / _srng)))
                    _ssvg = (
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _vy0 = max(_VPAD, min(_VH - _VPAD, _VPAD + _VDRAW * (1 - (0 - _vmn) / _vrng)))
                    _vsvg = (
