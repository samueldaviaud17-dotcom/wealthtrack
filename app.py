...
def card(title, value, sub="", color=C['blue'], icon="", sub2=""):
    ...
    s2 = f"<div style='font-size:11px;color:{C[\'muted\']};min-height:15px'>{sub2}</div>"
...
with tab2:
    ...
    # ── Synchro time ───────────────────────────────────
    from datetime import datetime, timezone, timedelta
    _p=datetime.now(timezone(timedelta(hours=2)))
    st.markdown(f"<div style='text-align:right;font-size:10px;color:{C['muted']};margin:4px 0 8px'>🔄 Synchro : {_p.strftime('%d/%m/%Y à %H:%M')}</div>", unsafe_allow_html=True)
    ...
    with L2:
        ...
        if xc:
            sec("DCA Cumulé 2026","📈")
            figc=go.Figure(go.Scatter(x=xc,y=yc,mode='lines+markers',
    ...
    with R2:
        ...
        if div_rows:
            df_div=pd.DataFrame(div_rows).sort_values("Net (€)",ascending=False)
            st.dataframe(df_div,use_container_width=True,hide_index=True,height=260)
            total_div=sum(n(r["Net (€)"]) for r in div_rows)
            st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-left:3px solid {C['gold']};border-radius:6px;padding:10px 14px;display:flex;justify-content:space-between;margin-top:4px">
<span style="font-size:12px;color:{C['muted']}">TOTAL NET 2026</span>
<span style="font-family:'Space Grotesk';font-size:15px;font-weight:700;color:{C['gold']}">{total_div:.2f} €</span>
