            ...
            for idx, w in enumerate(sorted_e):
                ...
                with row_cols[1]:
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-family:monospace;font-size:10px;color:{list_color}'>{ticker_w or '—'}</div>"
                        f"{'<a href=\"' + _tv_url + '\" target=\"_blank\" style=\"font-size:9px;color:#6366F1;text-decoration:none;opacity:0.8\">📈 TV</a>' if ticker_w else ''}"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                with row_cols[2]:
