...
<b>✅ PRÊT À COUPER</b> : toutes les données sources sont dans <code>crypto_positions.json</code> (investi_usd, investi_eur, detenu par token). Prix live via yfinance déjà utilisé ailleurs. Calcul valeur/PV/MV/perf 100% faisable côté Python. <b>Première cible prioritaire.</b><br><br>
...
❌ Pas de données positions Bourse dans GitHub — nécessite Chantier 8b (formulaire positions Bourse).<br><br>
<b>Séquence recommandée</b> :<br>
<b>8a</b> ✅ Terminé — Saisie DCA : historique_dca.json + crypto_positions.json 100% autonomes<br>
<b>8b</b> — Couper onglet ₿ Crypto du Sheet → reconstruire depuis crypto_positions.json + yfinance<br>
<b>8c</b> — Couper onglet Performance (Crypto + Bourse) → investi/valeur depuis JSON + yfinance<br>
<b>8d</b> — Couper onglet Vue d'ensemble → patrimoine depuis JSON (nécessite 8b+8c)<br>
<b>8e</b> — Couper onglet Bourse → nécessite un snapshot initial des positions Bourse en JSON<br>
