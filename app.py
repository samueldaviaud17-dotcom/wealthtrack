...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    with st.expander("₿ Crypto — Versement & Conversion", expanded=False):
        ...
        if st.toggle("📋 Voir l'historique des versements crypto", key="toggle_hist_crypto"):
            ...
            if _vers_c:
                _all_tks = sorted(set(k for v in _vers_c for k in v if k not in ("date", "montant_eur", "frais_depot_eur")))
                _tbl_c = f"<table style='width:100%;border-collapse:collapse;font-size:11px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:3px'>Date</th>"
                for _tk in _all_tks: _tbl_c += f"<th style='text-align:right;padding:3px'>{_tk}</th>"
                _tbl_c += "<th style='text-align:right;padding:3px'>Total</th></tr></thead><tbody>"
                _totals = {t: 0.0 for t in _all_tks}
                for v in sorted(_vers_c, key=lambda x: x['date'], reverse=True):
                    _row_tot = sum(v.get(t,{}).get('investi_usd',0) if isinstance(v.get(t,0),dict) else 0 for t in _all_tks)
                    _tbl_c += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:3px'>{v['date']}</td>"
                    for _tk in _all_tks:
                        _d = v.get(_tk,{}); _a = _d.get('investi_usd',0) if isinstance(_d,dict) else 0
                        _totals[_tk] += _a
                        _tbl_c += f"<td style='text-align:right;padding:3px'>{'—' if _a==0 else f'{_a:.0f}$'}</td>"
                    _tbl_c += f"<td style='text-align:right;padding:3px;font-weight:700'>{_row_tot:.0f}$</td></tr>"
                _tbl_c += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td style='padding:3px'>TOTAL</td>"
                for _tk in _all_tks: _tbl_c += f"<td style='text-align:right;padding:3px'>{_totals[_tk]:.0f}$</td>"
                _tbl_c += f"<td style='text-align:right;padding:3px'>{sum(_totals.values()):.0f}$</td></tr></tbody></table>"
                st.markdown(_tbl_c, unsafe_allow_html=True)
            else:
                st.info("Aucun versement enregistré cette année.")
    ...
    with st.expander("💸 Retrait vers mon compte bancaire", expanded=False):
        ...
        if st.toggle("📋 Voir l'historique des retraits", key="toggle_hist_retrait"):
            ...
            if _retraits:
                _tbl_r = f"<table style='width:100%;border-collapse:collapse;font-size:12px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:4px'>Date</th><th style='text-align:left;padding:4px'>Enveloppe</th><th style='text-align:right;padding:4px'>Montant</th><th style='text-align:left;padding:4px'>Note</th></tr></thead><tbody>"
                _tot_r = 0.0
                for v in sorted(_retraits, key=lambda x: x['date'], reverse=True):
                    _tot_r += v['montant_eur']
                    _tbl_r += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:4px'>{v['date']}</td><td style='padding:4px'>{v['enveloppe']}</td><td style='text-align:right;padding:4px;color:#F59E0B'>{v['montant_eur']:.0f}€</td><td style='padding:4px;font-size:10px;color:{C['muted']}'>{v.get('note','')}</td></tr>"
                _tbl_r += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td colspan='2' style='padding:4px'>TOTAL</td><td style='text-align:right;padding:4px;color:#F59E0B'>{_tot_r:.0f}€</td><td></td></tr></tbody></table>"
                st.markdown(_tbl_r, unsafe_allow_html=True)
            else:
                st.info("Aucun retrait enregistré.")
