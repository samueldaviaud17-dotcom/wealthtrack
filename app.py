...
<div style="font-size:10px;color:{C['muted']}">{score_detail}</div>
    ...
    if st.session_state.watchlist:
        ...
        # Statut logic based on écart between cours and prix à payer
        def get_statut_info(w):
            ...
            ecart = (a_payer_w - cours_w) / cours_w * 100  # positif = cours < prix à payer = opportunité
            if ecart > 5:
...
<tbody>"""
        ...
        for w in sorted_wl:
            ...
            ecart_col = C['green'] if ecart_w >= 0 else C['red']
            ecart_bg  = "#0A1A0A" if ecart_w >= 0 else "#1A0A0A"
