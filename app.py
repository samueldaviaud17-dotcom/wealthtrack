...
with tab1:
    ...
    # Substituer la contribution Options par les données de l'app (session_state)
    ...
    _opt_sheet_val = sv(df_pf, 8, 3)  # valeur Options dans Sheet (fallback)
    _opt_sheet_inv = sv(df_pf, 8, 2)  # investi Options dans Sheet (fallback)
    # Options : utiliser directement session_state (plus fiable que Sheet)
    # On retire la contribution Options du Sheet et on met les valeurs de l'app
    _pat_sans_opt = pat - _opt_sheet_val if _opt_sheet_val > 0 else pat
    _inv_sans_opt = inv - _opt_sheet_inv if _opt_sheet_inv > 0 else inv
    if _ibkr_cr > 0:
        pat = _pat_sans_opt + _ibkr_cr
    if _ibkr_inv > 0:
        inv = _inv_sans_opt + _ibkr_inv
...
# Resync depuis fichier si la valeur session semble être 0 (rechargement app)
# On force la relecture à chaque démarrage de session
...
with tab4:
    ...
    # KPI Card 1 : Capital Réel + Capital Investi
    with _cr_col1:
...
height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column'>
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px'>🏦 CAPITAL RÉEL</div>
<div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{C['text']};flex:1'>{st.session_state['capital_reel']:.2f} €</div>
<div style='border-top:1px solid {C['border']};padding-top:6px;margin-top:4px'>
  <div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em'>💰 Capital investi</div>
  <div style='font-size:15px;font-weight:700;color:{C['blue']}'>{_cap_inv:.2f} €</div>
  <div style='font-size:10px;color:{C['muted']}'>depuis ouverture</div>
</div>
</div>""", unsafe_allow_html=True)
        with st.expander("✏️ modifier le capital réel", expanded=False):
            _new_cr = st.number_input("Valeur du compte IBKR (€)", value=float(st.session_state['capital_reel']),
...
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px'>🏆 ROI TOTAL</div>
<div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{pcol(_roi_all)};flex:1'>{_roi_all:+.2f} %</div>
<div style='font-size:11px;color:{C['muted']}'>primes nettes / {_roi_base_lbl}</div>
<div style='border-top:1px solid {C['border']};padding-top:6px;margin-top:4px'>
  <div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em'>Perf. globale</div>
  <div style='font-size:15px;font-weight:700;color:{_perf_col}'>{_perf_str}</div>
  <div style='font-size:10px;color:{C['muted']}'>(réel − investi) / investi</div>
</div>
</div>""", unsafe_allow_html=True)

        with _kpi_col5:
            st.markdown(f"""<div style='background:{C['card']};border:1px solid {C['border']};border-top:2px solid {C['gold']};border-radius:10px;padding:14px 18px;height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column'>
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px'>💰 PRIMES BRUTES</div>
<div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{C['gold']};flex:1'>{_ibkr_kpis_all['primes_brutes_eur']:+.2f} €</div>
<div style='font-size:11px;color:{C['muted']}'>Profit C/T positifs (hors roulements)</div>
<div style='border-top:1px solid {C['border']};padding-top:6px;margin-top:4px'>
  <div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em'>P&L net réalisé</div>
  <div style='font-size:15px;font-weight:700;color:{pcol(_primes_all)}'>{_primes_all:+.2f} € <span style='font-size:10px;font-weight:400;color:{C['muted']}'>≈ ${_primes_all*_fx_all:.2f}</span></div>
  <div style='font-size:10px;color:{C['muted']}'>gains − pertes toutes années</div>
</div>
</div>""", unsafe_allow_html=True)
        ...
        # Card 2 (dans _cr_col2) : Résumé de la période (remplace les 6 KPI)
        _net_pf = _total_pl - _total_frais
        with _cr_col2:
            _wr_col = C['green'] if _win_rate >= 70 else C['gold']
            st.markdown(f"""<div style='background:{C['card']};border:1px solid {C['border']};border-top:2px solid {C['blue']};border-radius:10px;padding:14px 18px;height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column'>
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px'>📊 RÉSUMÉ PÉRIODE</div>
<div style='display:flex;align-items:baseline;gap:8px;flex:1'>
  <span style='font-family:Space Grotesk;font-size:20px;font-weight:700;color:{C['purple']}'>{_nb_total} trades</span>
  <span style='font-size:12px;color:{_wr_col};font-weight:700'>{_win_rate:.0f}% win</span>
</div>
<div style='border-top:1px solid {C['border']};padding-top:6px;margin-top:4px;display:grid;grid-template-columns:1fr 1fr;gap:4px'>
  <div>
    <div style='font-size:10px;color:{C['muted']}'>Prime nette</div>
    <div style='font-size:13px;font-weight:700;color:{pcol(_total_pl)}'>{_total_pl:+.2f} €</div>
  </div>
  <div>
    <div style='font-size:10px;color:{C['muted']}'>Frais</div>
    <div style='font-size:13px;font-weight:700;color:{C['red']}'>-{_total_frais:.2f} €</div>
  </div>
  <div style='grid-column:span 2'>
    <div style='font-size:10px;color:{C['muted']}'>Net P/L−frais · ROI {_roi_trades:+.1f}%</div>
    <div style='font-size:14px;font-weight:700;color:{pcol(_net_pf)}'>{_net_pf:+.2f} €</div>
  </div>
</div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ...
        if opts_open:
            ...
            tbl_o = f"<div style='overflow-x:auto'><table style='width:100%;border-collapse:collapse;font-size:14px'>"
...
span[data-baseweb="tag"] { background-color: #111827 !important; color: #6B7280 !important; border: 1px solid #374151 !important; }
            ...
            tbl_h = f"<div style='overflow-x:auto'><table style='width:100%;border-collapse:collapse;font-size:14px'>"
