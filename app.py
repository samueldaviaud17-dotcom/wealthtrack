    ...
    if _all_depots_detail:
        ...
        if _parsed_depots:
            ...
            dernier_depot = {"date": _last_dt, "montant": _last_amt,
                             "mois": mois_f[_last_dt.month-1], "annee": _last_dt.year}
...
with tab3:
    ...
    with L3:
        ...
        for row_c in cdata:
            ...
            _pvmv_str = f"{'+' if row_c['pvmv']>=0 else ''}{row_c['pvmv']:.0f} €"
            ...
            _tbl_c += ("<tr style='border-bottom:1px solid #1F2937'>"
                ...
                f"<td style='padding:6px 10px;text-align:right'>{fmt(row_c['valeur_eur'])}</td>"
        ...
        _tot_pvmv_str = f"{'+' if cpv>=0 else ''}{cpv:.0f} €"
        ...
        _tbl_c += ("<tr style='background:#111827;font-weight:700'>"
            ...
            f"<td style='padding:7px 10px;text-align:right'>{fmt(cv)}</td>"
...
<span style="color:{_t_clr};font-weight:700">{_t_sgn}{f"{int(round(abs(tpv))):,}".replace(',', ' ')} €</span>
<span style="color:{_t_clr};font-weight:700">{'+' if tp>=0 else ''}{tp:.1f}%</span></div>""",unsafe_allow_html=True)
