...
with tab3:
    ...
    for _tk in _C3_ORDER:
        ...
        _pru_usd   = (_inv_usd / _achete) if _achete else 0.0
        _staking   = max(0.0, (_detenu - _achete) * _prix) if _prix else 0.0
        _c3_rows.append({
    ...
    with L3:
        ...
        _tbl_c += ("<thead><tr style='background:#111827'>"
            + "".join(f"<th style='padding:7px 10px;text-align:{'left' if i<2 else 'right'};font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>{h}</th>"
                      for i,h in enumerate(["Crypto","Code","Montant détenu","Investi ($)","Cours réel ($)","PRU ($)","Valeur ($)","Valeur (€)","PV/MV (€)","Perf."]))
            + "</tr></thead><tbody>")
        ...
        _tbl_c += (f"<tr style='background:#111827;font-weight:700'>"
            f"<td style='padding:7px 10px' colspan='3'>TOTAL PORTEFEUILLE</td>"
...
<span style="font-size:12px;color:{cl};font-weight:700">{vl:,.0f} $ · {pct_:.1f}%</span>
        ...
        _st_rows = [(r["nom"], _C3_COLORS.get(r["tk"], "#888"), r["staking_usd"])
                    for r in _c3_rows if r["staking_usd"] > 0.01]
...
<span style="font-family:'Space Grotesk';font-size:16px;font-weight:700;color:{C['green']}">+{_tot_st:.2f} $</span></div>""", unsafe_allow_html=True)
        elif not _st_rows:
            st.markdown(f"<div style='color:{C['muted']};font-size:13px'>Aucun gain de staking détecté (detenu ≤ achete).</div>", unsafe_allow_html=True)
