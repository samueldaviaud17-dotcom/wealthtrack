...
def fetch_analyst_data(ticker: str):
    """Récupère le consensus analystes pour un ticker via yfinance.
    Retourne dict {rec_mean, rec_label, rec_col, target_price, nb_analysts} ou None."""
    ...
    try:
        ...
        if rec_mean <= 1.5:   lbl, col = 'Strong Buy', '#22C55E'
        ...
        for list_key, list_label, list_color in SUB_LISTS:
            ...
            for idx, w in enumerate(sorted_e):
                ...
                with row_cols[7]:
                    ...
                    if _an:
                        _tp_str = f"<div style='font-size:9px;color:{C['muted']}'>Cible {_an['target_price']:.0f}$ · {_an['nb_analysts'] or '?'} anal.</div>" if _an['target_price'] else ""
                        st.markdown(
                            f"<div style='padding:4px 4px'>"
                            f"<span style='background:{_an['rec_col']}22;color:{_an['rec_col']};font-size:10px;font-weight:700;padding:2px 7px;border-radius:4px;border:1px solid {_an['rec_col']}44'>"
                            f"{_an['rec_label']}</span><br>{_tp_str}</div>",
                            unsafe_allow_html=True
                        )
                    else:
...
<b>✅ Fait</b> : Calendrier Earnings — vrai calendrier visuel 5 colonnes Lun→Ven par semaine. Fond de card teinté par source (Options IBKR / Portefeuille WL / Options WL / Surveillance). Bordure d'urgence temporelle (orange ≤2j, violet aujourd'hui). Compteur "dans X jours" dans l'en-tête de chaque semaine. Sources : options IBKR ouvertes + watchlist uniquement (actions_detenues IBKR exclues — à terme dans wl_portefeuille). NIO/ADR chinois : yfinance ne remonte pas de date, comportement normal.<br>
