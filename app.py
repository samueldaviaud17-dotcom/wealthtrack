...
with tab1:
    ...
    with R:
        ...
        vals=[n(v(df_p,20+i,2)) for i in range(7)]
        ...
        tots=[n(v(df_ea,12+i,9)) for i in range(5)]
...
with tab3:
    ...
    with R3:
        ...
        pcts_c=[n(v(df_ct,11+i,12)) for i in range(5)]
...
<span style="color:{C['green']};font-weight:700">+{tp:.1f}%</span></div>""",unsafe_allow_html=True)
        ...
        benchs=[
            ("🏆 Mon PF",   n(v(df_pf2,13,1)), C['gold'], True),
            ("📈 SP500",    n(v(df_pf2,14,1)), C['cyan'], False),
            ("🇫🇷 CAC 40", n(v(df_pf2,15,1)), C['cyan'], False),
            ("🌍 STOXX 600",n(v(df_pf2,16,1)), C['cyan'], False),
        ]
