...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    with st.expander("₿ Crypto — Versement & Conversion", expanded=False):
        ...
        def _render_tbl_vers_crypto(_vers_in, _label_tot):
            _all_tks = sorted(set(k for v in _vers_in for k in v if k not in ("date", "montant_eur", "frais_depot_eur")))
            ...
            for v in sorted(_vers_in, key=lambda x: x['date'], reverse=True):
                _row_tot = sum(v.get(t,{}).get('investi_usd',0) if isinstance(v.get(t,0),dict) else 0 for t in _all_tks)
                _tbl += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:3px'>{v['date']}</td>"
                for _tk in _all_tks:
                    _d = v.get(_tk,{}); _a = _d.get('investi_usd',0) if isinstance(_d,dict) else 0
                    _totals[_tk] += _a
            ...
            _tbl += f"<td style='text-align:right;padding:3px'>{sum(_totals.values()):.0f}$</td></tr></tbody></table>"
            return _tbl

        if st.toggle("📋 Voir l'historique des versements crypto", key="toggle_hist_crypto"):
            _vers_c = [v for v in _dca_hist["crypto"]["versements"]
                       if v["date"].startswith(str(_yr_now)) and "_note" not in v and isinstance(list(v.values())[1], dict)]
            if _vers_c:
                st.markdown(_render_tbl_vers_crypto(_vers_c, f"TOTAL {_yr_now}"), unsafe_allow_html=True)
            else:
                st.info("Aucun versement enregistré cette année.")

            _vers_c_tous = [v for v in _dca_hist["crypto"]["versements"] if "_note" not in v and isinstance(list(v.values())[1], dict)]
            ...
            for _yr_p in _yrs_c_passees:
                ...
                if st.toggle(f"▶ {_yr_p} — {len(_vers_yp)} versements", key=f"toggle_hist_crypto_{_yr_p}"):
