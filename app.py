...
# ── Synchro line + refresh button compact ──────────
_rc = st.columns([6, 1])
with _rc[0]:
    st.markdown(f"""<div style="text-align:right;font-size:10px;color:{C['muted']};margin-top:-6px">
🔄 Dernière synchro : <b>{_now}</b></div>""", unsafe_allow_html=True)
with _rc[1]:
    _btn_css = f"""<style>
div[data-testid="stButton"][id="refresh_wrap"] > button {{
...
div[data-testid="stButton"][id="refresh_wrap"] > button:hover {{
    background: linear-gradient(135deg,#1F6FEB44,#8B5CF644);
