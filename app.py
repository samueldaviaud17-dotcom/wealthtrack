...
with tab6:
    ...
    kpi_data = [
        ("💎 ACHAT IDÉAL",      str(nb_ideal)      if nb_total else "—",
         f"{nb_ideal} action(s)",      C['green'],  "💎"),
        ("🔥 SANS MARGE",       str(nb_sans_marge) if nb_total else "—",
         f"{nb_sans_marge} action(s)", "#F0883E",   "🔥"),
        ("👀 PROCHE",           str(nb_proche)     if nb_total else "—",
         f"{nb_proche} action(s)",     "#60A5FA",   "👀"),
        ("😴 ATTENDRE",         str(nb_wait_real)  if nb_total else "—",
         f"{nb_wait_real} action(s)",  C['gold'],   "😴"),
        ("🚫 SURCOTE",          str(nb_surcote)    if nb_total else "—",
         f"{nb_surcote} action(s)",    C['red'],    "🚫"),
        ("📊 ÉCART MOYEN",      f"{avg_ecart_all:+.1f}%" if wl else "—",
         f"{nb_total} analysées",
