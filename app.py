  ...
  <span style="font-size:12px;color:{_score_col};font-weight:600">{_score_lbl}</span>
            ...
            with _sc2:
                ...
                with st.popover("?", use_container_width=True):
                    st.markdown(f"""**Score de qualité fondamentale /10**

Note synthétique basée sur les données historiques Yahoo Finance ({hist_d.get('n',5)} ans).

| Critère | Valeur | Note |
|---------|--------|------|
| CAGR Chiffre d'affaires | {hist_d.get('cagr_rev',0):.1f}% | {_sc_rev:.1f}/2.5 |
| CAGR Bénéfice net | {hist_d.get('cagr_ni',0):.1f}% | {_sc_ni:.1f}/2.5 |
| Marge nette | {hist_d.get('margin',[0])[0]:.1f}% | {_sc_m:.1f}/2.5 |
| CAGR Free Cash Flow | {hist_d.get('cagr_fcf',0) or 0:.1f}% | {_sc_fcf:.1f}/2.5 |
| **TOTAL** | | **{_score:.1f}/10** |

**Grille de lecture :**
- ≥ 8 → 🟢 Excellente qualité
- ≥ 6 → 🟡 Bonne qualité  
- ≥ 4 → 🟠 Qualité correcte
- < 4 → 🔴 Qualité faible

⚠️ Ce score mesure la **qualité historique**, pas la valorisation.""")
        ...
        kpi_r = st.columns(4)
        with kpi_r[0]:
            st.markdown(card("PRIX À PAYER BASE",
            ...
            with st.popover("?", use_container_width=True):
                st.markdown(f"""**Formule :** VI ÷ (1+{_perf_v:.0f}%)^{_n_yr} × (1−{_ms_v:.0f}%)

C'est le **prix maximum** auquel tu peux acheter cette action aujourd'hui pour atteindre ton rendement cible de **{_perf_v:.0f}%/an** sur **{_n_yr} ans**, avec une marge de sécurité de **{_ms_v:.0f}%** intégrée.

🟢 Cours < Prix à payer → opportunité d'achat  
🔴 Cours > Prix à payer → action trop chère""")

        with kpi_r[1]:
            st.markdown(card("UPSIDE BASE",
                ...
                unsafe_allow_html=True)
            with st.popover("?", use_container_width=True):
                st.markdown(f"""**Formule :** (Prix à payer − Cours actuel) ÷ Cours actuel

Mesure l'écart entre le **prix à payer calculé** ({pp_base_kpi:,.0f} €) et le **cours actuel** ({cours_kpi:,.0f} €).

➕ Positif → l'action est **sous-évaluée** par rapport à ton objectif de rendement  
➖ Négatif → l'action est **trop chère** pour atteindre ton rendement cible""")

        with kpi_r[2]:
            st.markdown(card("VALEUR INTRINSÈQUE",
                f"{vi_base_kpi:,.0f} €" if vi_base_kpi else "—",
                "scénario BASE", "#7DD3FC", "🔍"),
                unsafe_allow_html=True)
            with st.popover("?", use_container_width=True):
                st.markdown(f"""**Formule :** EPS {_YR+_n_yr} projeté × PER cible {_per_v}x

C'est la **valeur estimée** de l'action dans **{_n_yr} ans** selon le scénario BASE :
- Croissance CA : **{_g_v:.0f}%/an**
- PER cible : **{_per_v}x**

Cette valeur ne tient pas encore compte de ton rendement exigé ni de la marge de sécurité.""")

        with kpi_r[3]:
            st.markdown(card("EPS ACTUEL",
                f"{hist_eps_kpi:.2f}" if hist_eps_kpi else "—",
                "€/$ par action", C["gold"], "📊"),
                unsafe_allow_html=True)
            with st.popover("?", use_container_width=True):
                st.markdown("""**EPS** = Earnings Per Share = Bénéfice net ÷ Nombre d'actions

C'est le **bénéfice généré par action** sur le dernier exercice fiscal connu (source : Yahoo Finance).

C'est le **point de départ** de toutes les projections : l'EPS est multiplié par le taux de croissance chaque année pour estimer les EPS futurs.""")
