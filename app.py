...
with tab_marche:
    ...
    st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
    sec("Indices de référence — Variation journalière", "📊", "#818CF8", "#0D0F1A")
...
.mkt-card-chart svg { width: 100% !important; height: 100% !important; display: block; }
  ...
  <div style="padding:0 14px 10px 14px;display:flex;justify-content:space-between;align-items:center">
    ...
    # Collecter tous les tickers : IBKR (options ouvertes + actions) + watchlist
    _earn_tickers = set()
    # Depuis IBKR
    ...
    for _yr_d in _ibkr_d.values():
        ...
        for _tk in _yr_d.get('actions_detenues', {}).keys():
            _earn_tickers.add(_tk)
    # Depuis watchlist
    ...
    if not _earn_tickers:
        ...
        if not _earn_list:
            ...
            _MOIS_FR = ['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc']
            _JOUR_FR = ['Lun','Mar','Mer','Jeu','Ven','Sam','Dim']

            for _wk_monday in sorted(_weeks.keys()):
                _wk_end = _wk_monday + _td(days=4)
                _wk_label = f"Semaine du {_wk_monday.day} {_MOIS_FR[_wk_monday.month-1]} → {_wk_end.day} {_MOIS_FR[_wk_end.month-1]} {_wk_end.year}"

                # Badge "cette semaine" si applicable
                _is_this_week = _wk_monday <= _today_e <= _wk_end
                _badge = ' <span style="background:#7C3AED;color:white;font-size:10px;padding:1px 7px;border-radius:10px;font-weight:700">CETTE SEMAINE</span>' if _is_this_week else ''

                with st.expander(f"📅 {_wk_label}{_badge}", expanded=_is_this_week):
                    _rows_html = ''
                    for _e in sorted(_weeks[_wk_monday], key=lambda x: x['date']):
                        _d = _e['date']
                        _days_left = (_d - _today_e).days
                        _jour_str = f"{_JOUR_FR[_d.weekday()]} {_d.day} {_MOIS_FR[_d.month-1]}"

                        # Couleur urgence
                        if _days_left <= 0:
                            _d_col, _d_bg = '#94A3B8', '#1C2333'
                            _d_label = "Aujourd'hui" if _days_left == 0 else "Passé"
                        elif _days_left <= 2:
                            _d_col, _d_bg = '#F97316', '#2D1A0A'
                            _d_label = f'Dans {_days_left}j'
                        elif _days_left <= 7:
                            _d_col, _d_bg = '#EAB308', '#2D2A0A'
                            _d_label = f'Dans {_days_left}j'
                        else:
                            _d_col, _d_bg = '#94A3B8', '#1C2333'
                            _d_label = f'Dans {_days_left}j'
                        ...
                        # EPS estimé
                        _eps_str = f"EPS est. <b>{_e['eps_est']:.2f} $</b>" if _e.get('eps_est') is not None else ''
                        _rev_str = ''
                        if _e.get('rev_est') is not None:
                            _rv = _e['rev_est']
                            _rv_fmt = f"{_rv/1e9:.1f}B $" if abs(_rv) >= 1e9 else f"{_rv/1e6:.0f}M $"
                            _rev_str = f" · Rev est. <b>{_rv_fmt}</b>"

                        # Badge source : Options IBKR / Bourse (wl_portefeuille) / Options WL / Surveillance
                        _tk_str = _e['ticker']
                        _ibkr_options = [
                        ...
                        _ibkr_actions = [
                            _tk for _yr in _ibkr_d.values()
                            for _tk in _yr.get('actions_detenues', {}).keys()
                        ]
                        _wl_pf_tks  = [w.get('ticker','') for w in st.session_state.get('wl_portefeuille', [])]
                        _wl_opt_tks = [w.get('ticker','') for w in st.session_state.get('wl_options', [])]
                        _wl_surv_tks= [w.get('ticker','') for w in st.session_state.get('wl_surveillance', [])]

                        if _tk_str in _ibkr_options:
                            _src_badge, _src_col = '⚙️ Options IBKR', '#818CF8'
                        elif _tk_str in _ibkr_actions:
                            _src_badge, _src_col = '📊 Actions IBKR', '#7DD3FC'
                        elif _tk_str in _wl_pf_tks:
                            _src_badge, _src_col = '📈 Portefeuille WL', '#6EE7B7'
                        elif _tk_str in _wl_opt_tks:
                            _src_badge, _src_col = '⚙️ Options WL', '#C4B5FD'
                        elif _tk_str in _wl_surv_tks:
                            _src_badge, _src_col = '👀 Surveillance', '#FCD34D'
                        else:
                            _src_badge, _src_col = '⭐ Watchlist', '#FCD34D'

                        _rows_html += f"""<tr style="border-bottom:1px solid #1E293B">
  <td style="padding:10px 14px;font-family:'Space Grotesk';font-size:15px;font-weight:700">{_e['ticker']}</td>
  <td style="padding:10px 14px;color:#CBD5E1;font-size:13px">{_jour_str}</td>
  <td style="padding:10px 14px">
    <span style="background:{_d_bg};color:{_d_col};font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px">{_d_label}</span>
  </td>
  <td style="padding:10px 14px;font-size:12px;color:#94A3B8">{_eps_str}{_rev_str}</td>
  <td style="padding:10px 14px">
    <span style="font-size:10px;color:{_src_col}">{_src_badge}</span>
  </td>
</tr>"""

                    st.markdown(f"""<div style="background:#0D1117;border:1px solid #1E293B;border-radius:8px;overflow:hidden">
<table style="width:100%;border-collapse:collapse">
<thead><tr style="background:#161B22;font-size:10px;text-transform:uppercase;color:#64748B">
  <th style="padding:8px 14px;text-align:left">Ticker</th>
  <th style="padding:8px 14px;text-align:left">Date</th>
  <th style="padding:8px 14px;text-align:left">Délai</th>
  <th style="padding:8px 14px;text-align:left">Estimations</th>
...
<b>✅ Fait</b> : Fear &amp; Greed Index CNN avec gauge SVG demi-cercle + 5 zones colorées.<br>
<b>✅ Fait</b> : Calendrier Earnings (yf.Ticker.calendar + earnings_dates) pour les tickers détenus (IBKR options ouvertes + actions) + watchlist. Regroupement par semaine, badge "CETTE SEMAINE", délai coloré, EPS/Rev estimés.""",
            "#6EE7B7")
