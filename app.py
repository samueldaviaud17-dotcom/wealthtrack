...
<b>✅ Fait — 4a</b> : <code>historique_dca.json</code> créé sur GitHub avec les vrais montants CTO+PEA et Crypto par mois (2022→2026). Totaux validés à &lt;0.10€ près vs Google Sheet.<br>
<b>✅ Fait — 4b</b> : <code>crypto_positions.json</code> créé sur GitHub avec les positions actuelles (investi $, acheté, détenu, frais) par token (BTC/ETH/SOL/BNB/USDC). Point de départ = Sheet du 29/06/2026.<br>
<b>✅ Fait — 4c</b> : Onglet <b>✏️ Saisie DCA</b> créé entre Watchlist et Chantier. Formulaires Bourse (CTO+PEA) et Crypto (par token avec frais) → sauvegarde automatique dans les JSON GitHub avec date exacte (Option B : multi-versements par mois possible).<br>
<b>✅ Fait — 4d</b> : Calcul XIRR hybride — lit <code>historique_dca.json</code> pour tous les flux (2022→aujourd'hui), IBKR utilise toujours les vrais dépôts parsés HTML. TRI Bourse/Crypto bien plus précis.<br>
<b>💡 À faire</b> : Chaque mois, ajouter le nouveau versement via l'onglet Saisie DCA. En fin d'année, rien à archiver — tout est déjà dans le JSON par date exacte.""",
        "#6EE7B7", "Terminé")
