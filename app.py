...
with tab_budget:
    ...
    with st.expander(f"📊 {_ML.get(_sel,'').upper()} — Suivi mensuel du budget", expanded=True):
        ...
        with _cl:
            ...
            st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:8px 0 4px'>Reste mois précédent (saisie manuelle)</div>",unsafe_allow_html=True)
            _new_reste=st.number_input("",value=float(_reste_prec),step=0.01,format="%.2f",key="reste_prec_input",label_visibility="collapsed")
            if st.button("💾 Sauvegarder",key="btn_reste_prec"):
                ...
                if _save_budget(_bud_data,f"Reste précédent {_sel}={_new_reste:.2f}€"):
                    st.success(f"✅ Reste mois précédent = {_new_reste:.2f}€"); _load_budget.clear()
                else: st.error("❌ Erreur GitHub")
        ...
        # ── GÉRER LES TRANSACTIONS (toggle dans l'expander) ──
        if st.toggle("✏️ Gérer les transactions",key="tog_gerer_tx"):
        ...
        # ── PLANIFICATION DU BUDGET (toggle dans l'expander) ──
        if st.toggle("💡 Planification du budget",key="tog_planif"):
