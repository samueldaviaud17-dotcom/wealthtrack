...
with tab_saisie:
    ...
    st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:16px'>Saisis ici tes versements DCA. Chaque entrée est sauvegardée avec sa date exacte dans GitHub.</div>", unsafe_allow_html=True)
    ...
    # SECTION 1 — BOURSE
    # ══════════════════════════════════════════════
    sec("Bourse (CTO + PEA)", "📈", "#7DD3FC", "#0A1A1C")
...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    sec("Crypto (Binance)", "₿", "#FCD34D", "#1C100A")

    # Cryptos connues = liste par défaut + celles déjà dans positions
    _cryptos_used = set(CRYPTO_CONNUS) | set(_pos_data.get("positions", {}).keys())
    _cryptos_list = sorted(_cryptos_used)

    col_cf, col_cr = st.columns([1, 1])

    with col_cf:
        with st.form("form_crypto_dca"):
            _date_c = st.date_input("Date du versement", value=datetime.now(_TZ_PARIS).date(), key="date_c")
            st.markdown(f"<div style='font-size:11px;color:{C['muted']};margin-bottom:8px'>Montants en USD ($)</div>", unsafe_allow_html=True)
            _c1, _c2 = st.columns(2)
            _inv_c = {}; _ach_c = {}; _frais_c = {}
            for i, _tk in enumerate(_cryptos_list):
                _col = _c1 if i % 2 == 0 else _c2
                with _col:
                    st.markdown(f"<div style='font-size:11px;font-weight:700;color:#FCD34D;margin-top:8px'>{_tk}</div>", unsafe_allow_html=True)
                    _inv_c[_tk]   = st.number_input(f"Investi $ ({_tk})",  min_value=0.0, step=0.01, format="%.2f", key=f"inv_{_tk}")
                    _ach_c[_tk]   = st.number_input(f"Tokens achetés ({_tk})", min_value=0.0, step=0.00000001, format="%.8f", key=f"ach_{_tk}")
                    _frais_c[_tk] = st.number_input(f"Frais $ ({_tk})",    min_value=0.0, step=0.01, format="%.2f", key=f"frais_{_tk}")
            # Autre crypto
            ...
            _autre_frais = st.number_input("Frais $ (autre)", min_value=0.0, step=0.01, format="%.2f", key="autre_frais")
        ...
        if _submit_c:
            ...
            for _tk in _cryptos_list:
                if _inv_c.get(_tk,0) > 0 or _ach_c.get(_tk,0) > 0:
                    _entry_c[_tk] = {"investi_usd": round(_inv_c[_tk],2), "achete": round(_ach_c[_tk],8), "frais_usd": round(_frais_c.get(_tk,0),2)}
                    ...
                    _pos[_tk]["frais_usd"]   = round(_pos[_tk].get("frais_usd",0) + _frais_c.get(_tk,0), 2)
                    _changed = True
            # Autre crypto
            if _autre_tk and _autre_inv > 0:
                _entry_c[_autre_tk] = {"investi_usd": round(_autre_inv,2), "achete": round(_autre_ach,8), "frais_usd": round(_autre_frais,2)}
                ...
                _pos[_autre_tk]["frais_usd"]   = round(_pos[_autre_tk].get("frais_usd",0) + _autre_frais, 2)
                _changed = True

            if _changed:
                # Sauvegarder versement dans historique_dca.json
                ...
                # Mettre à jour crypto_positions.json
                ...
                if _ok1 and _ok2: st.success(f"✅ Crypto enregistré et positions mises à jour — {_date_c}")
                else: st.error("❌ Erreur GitHub")
            else: st.warning("Saisis au moins un montant > 0")
    ...
    # ══════════════════════════════════════════════
    # ══════════════════════════════════════════════
    # SECTION 3 — RETRAITS / VENTES
    # ══════════════════════════════════════════════
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    sec("Retrait / Vente", "💸", "#F59E0B", "#1C0A00")
        ...
        if _submit_r:
            if _amt_r > 0:
                _retrait_entry = {
                    ...
                    "type": "retrait",
                    "enveloppe": _env_r,
                    "montant_eur": round(_amt_r, 2),
                    "note": _note_r or ""
                }
                if "retraits" not in _dca_hist:
                    _dca_hist["retraits"] = []
                ...
                if _ok_r:
                    st.success(f"✅ Retrait enregistré — {_date_r} · {_env_r} · {_amt_r:.0f}€{' · ' + _note_r if _note_r else ''}")
                else:
                    st.error("❌ Erreur GitHub")
            else:
                st.warning("Saisis un montant > 0")

    with col_r2:
        _retraits = _dca_hist.get("retraits", [])
        if _retraits:
            st.markdown(f"<div style='font-size:12px;font-weight:700;color:{C['muted']};margin-bottom:8px'>Historique des retraits</div>", unsafe_allow_html=True)
    ...
    # ══════════════════════════════════════════════
    # SECTION 4 — MODIFIER / SUPPRIMER
    # ══════════════════════════════════════════════
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    sec("Modifier / Supprimer un versement", "✏️", "#64748B", "#0D1117")
    with st.expander("⚠️ Correction d'un versement passé", expanded=False):
        ...
        if _vers_list:
            ...
            for v in _vers_list:
                if _edit_type == "Bourse":
                    ...
                    _tot = sum(d.get('investi_usd',0) if isinstance(d,dict) else v.get('total',0)
                               for k,d in v.items() if k not in ('date','_note','total'))
                    _vers_labels.append(f"{v['date']} — {_tot:.0f}$")
            _sel_idx = st.selectbox("Versement à modifier", range(len(_vers_labels)), format_func=lambda i: _vers_labels[i], key="edit_sel")
            ...
            if st.button("🗑️ Supprimer ce versement", key="del_vers"):
                ...
                if _ok_del: st.success("✅ Versement supprimé")
                else: st.error("❌ Erreur GitHub")
        else:
...
<b>✅ 4e</b> : Section <b>Retrait / Vente</b> dans l'onglet Saisie DCA. Date + enveloppe + montant + note → stocké dans <code>historique_dca.json</code> sous "retraits". Intégré comme flux positif dans le XIRR (argent récupéré = flux entrant). Historique des retraits affiché avec total cumulé.<br><br>
<b>🔄 Reste</b> : Les graphes, KPIs et tableaux de l'app lisent encore de nombreuses valeurs depuis le Google Sheet. Ce travail est couvert par le <b>Chantier 7</b>.""",
        "#6EE7B7", "En cours")
