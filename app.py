...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    with st.expander("₿ Versement Crypto", expanded=False):
        _cryptos_used = set(CRYPTO_CONNUS) | set(_pos_data.get("positions", {}).keys())
        _cryptos_list = sorted(_cryptos_used)
        with st.form("form_crypto_dca"):
            ...
            _inv_c = {}; _ach_c = {}
            for _tk in _cryptos_list:
                st.markdown(
                    f"<div style='display:inline-block;background:#1C1400;border:1px solid #FCD34D;color:#FCD34D;"
                    f"font-size:12px;font-weight:700;padding:3px 12px;border-radius:6px;margin:10px 0 4px 0'>{_tk}</div>",
                    unsafe_allow_html=True
                )
                _t2, _t3 = st.columns(2)
                with _t2:
                    _inv_c[_tk] = st.number_input("Investi $",      min_value=0.0, step=0.01, format="%.2f",   key=f"inv_{_tk}")
                with _t3:
                    _ach_c[_tk] = st.number_input("Tokens achetés", min_value=0.0, step=0.00000001, format="%.8f", key=f"ach_{_tk}")
            st.markdown(f"<div style='font-size:11px;font-weight:700;color:{C['muted']};margin-top:8px'>➕ Autre crypto</div>", unsafe_allow_html=True)
            _a1, _a2, _a3, _a4 = st.columns([0.7, 0.9, 1, 1])
            with _a1:
                _autre_tk  = st.text_input("Code (ex: XRP)", key="autre_tk").upper().strip()
            with _a2:
                _autre_nom = st.text_input("Nom (ex: Ripple)", key="autre_nom").strip()
            with _a3:
                _autre_inv = st.number_input("Investi $ (autre)", min_value=0.0, step=0.01, format="%.2f", key="autre_inv")
            with _a4:
                _autre_ach = st.number_input("Tokens achetés (autre)", min_value=0.0, step=0.00000001, format="%.8f", key="autre_ach")
            _submit_c = st.form_submit_button("💾 Enregistrer", use_container_width=True)
        if _submit_c:
            _entry_c = {"date": str(_date_c)}
            _pos = _pos_data.get("positions", {})
            _changed = False
            for _tk in _cryptos_list:
                if _inv_c.get(_tk,0) > 0 or _ach_c.get(_tk,0) > 0:
                    _entry_c[_tk] = {"investi_usd": round(_inv_c[_tk],2), "achete": round(_ach_c[_tk],8)}
                    if _tk not in _pos: _pos[_tk] = {"nom": _tk, "investi_usd": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                    _pos[_tk]["investi_usd"] = round(_pos[_tk]["investi_usd"] + _inv_c[_tk], 2)
                    _pos[_tk]["achete"]      = round(_pos[_tk]["achete"]      + _ach_c[_tk], 8)
                    _changed = True
            if _autre_tk and _autre_inv > 0:
                _entry_c[_autre_tk] = {"investi_usd": round(_autre_inv,2), "achete": round(_autre_ach,8)}
                if _autre_tk not in _pos: _pos[_autre_tk] = {"nom": _autre_nom or _autre_tk, "investi_usd": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                _pos[_autre_tk]["investi_usd"] = round(_pos[_autre_tk]["investi_usd"] + _autre_inv, 2)
                _pos[_autre_tk]["achete"]      = round(_pos[_autre_tk]["achete"]      + _autre_ach, 8)
                _changed = True
            if _changed:
                ...
                if _ok1 and _ok2: st.success(f"✅ Crypto enregistré — {_date_c}")
                else: st.error("❌ Erreur GitHub")
            else: st.warning("Saisis au moins un montant > 0")
    ...
    with st.expander("🔄 Conversion entre cryptos (swap)", expanded=False):
        ...
        with st.form("form_conversion_crypto"):
            ...
            _val_cv = st.number_input("Valeur $ de la conversion (au moment du swap)", min_value=0.0, step=0.01, format="%.2f", key="cv_val",
                                       help="Valeur en USD échangée — sert à transférer le coût de revient (investi_usd) du token source vers le token destination")
        ...
        if _submit_cv:
            if _tk_src == _tk_dst:
            ...
            elif _qty_src <= 0 or _qty_dst <= 0 or _val_cv <= 0:
                st.warning("Saisis une quantité source, une quantité destination et une valeur $ > 0.")
            else:
                _pos_cv = _pos_data.get("positions", {})
                if _tk_src not in _pos_cv: _pos_cv[_tk_src] = {"nom": _tk_src, "investi_usd": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                if _tk_dst not in _pos_cv: _pos_cv[_tk_dst] = {"nom": _tk_dst, "investi_usd": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                _pos_cv[_tk_src]["detenu"]      = round(_pos_cv[_tk_src].get("detenu", 0) - _qty_src, 8)
                _pos_cv[_tk_src]["investi_usd"] = round(_pos_cv[_tk_src].get("investi_usd", 0) - _val_cv, 2)
                ...
                _pos_cv[_tk_dst]["investi_usd"] = round(_pos_cv[_tk_dst].get("investi_usd", 0) + _val_cv, 2)
                ...
                _pos_data["conversions"].append({
                    ...
                    "qte_vendue": round(_qty_src, 8), "qte_achetee": round(_qty_dst, 8), "valeur_usd": round(_val_cv, 2)
                })
