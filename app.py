...
<span style="font-size:12px;color:{_clr};font-weight:700">{fmt(_val)} · {_p:.1f}%</span>
        ...
        if len(_ywf) > 1:
            ...
            _colors   = [C['blue']] + [C['green'] if d>=0 else C['red'] for d in _deltas[1:]]
...
with tab8:
  ...
  <div style='display:flex;justify-content:space-between;align-items:center'>
    ...
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
        ...
        _rapport_html = f"""<div id='rapport-mois' style='background:#0D1117;border-radius:12px;padding:24px 28px;border:1px solid #30363D;font-family:Space Grotesk,sans-serif;color:#E6EDF3'>
  <div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px'>
    <div>
      <div style='font-size:11px;color:#8B949E;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px'>SamInvest · Rapport Mensuel</div>
      <div style='font-size:30px;font-weight:700'>{_periode_display}</div>
      {"<div style='font-size:12px;color:#D29922;margin-top:4px'>⚠ Mois en cours — données partielles</div>" if _mois_partiel else ""}
    </div>
    <div style='text-align:right'>
      <div style='font-size:11px;color:#8B949E;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px'>Primes nettes</div>
      <div style='font-size:34px;font-weight:700;color:{_nc};font-family:monospace'>{_pm_net_eur:+.2f} €</div>
    </div>
  </div>
  <div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:20px'>
    <div style='background:#161B22;border-radius:8px;padding:12px 14px'>
      <div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Primes brutes</div>
      <div style='font-size:20px;font-weight:700;color:#3FB950;font-family:monospace'>{(_pm_brutes_usd-_pm_rachats_usd)/_fx_an:+.2f} €</div>
      <div style='font-size:11px;color:#8B949E;margin-top:3px'>≈ ${_pm_brutes_usd-_pm_rachats_usd:.2f}</div>
    </div>
    <div style='background:#161B22;border-radius:8px;padding:12px 14px'>
      <div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Frais</div>
      <div style='font-size:20px;font-weight:700;color:#F85149;font-family:monospace'>-{_pm_frais_eur:.2f} €</div>
    </div>
    <div style='background:#161B22;border-radius:8px;padding:12px 14px'>
      <div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Trades effectués</div>
      <div style='font-size:20px;font-weight:700;color:#58A6FF;font-family:monospace'>{_pm_nb_trades}</div>
    </div>
  </div>
  <div style='display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:4px'>
    <div>
      <div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Top tickers — {_periode_display}</div>
      {_top5_mois_html if _top5_mois_html else "<div style='color:#8B949E;font-size:12px;font-style:italic'>Aucun trade ce mois</div>"}
    </div>
    <div>
      <div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Hall of Fame — depuis le début</div>
      {_top5_all_html if _top5_all_html else "<div style='color:#8B949E;font-size:12px;font-style:italic'>Aucune donnée</div>"}
    </div>
  </div>
  {_best_html}
