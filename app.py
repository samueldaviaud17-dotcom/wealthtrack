...
\n**Différence avec le ROI Total (haut)** : le ROI Total est toujours sur toutes les années et divisé par le Capital Réel (valeur actuelle du compte).""")
        ...
        # ── Indicateur cours live + bouton refresh options ──
        _opt_refresh_cols = st.columns([3, 1])
        with _opt_refresh_cols[0]:
            if _last_opt_refresh:
                ...
                st.markdown(
                    f"<div style='font-size:11px;color:{C['gold']};padding:4px 0'>"
                    f"⏳ Récupération des cours en cours...</div>",
                    unsafe_allow_html=True)
        with _opt_refresh_cols[1]:
            if st.button("📡 Cours live", key="opt_refresh_btn", use_container_width=True,
                         help="Récupère les cours en temps réel via Yahoo Finance"):
                refresh_options_cours()
                st.rerun()
...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        # ── Sélecteur ANNÉE (boutons radio, une seule active) ──
        if 'rapport_yr_saved' not in st.session_state or st.session_state['rapport_yr_saved'] not in _avail_yrs:
            st.session_state['rapport_yr_saved'] = _avail_yrs[0]
        _sel_yr = st.session_state['rapport_yr_saved']

        st.markdown(f"<div style='font-size:11px;color:{C['muted']};margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Année</div>", unsafe_allow_html=True)
        _yr_cols = st.columns(len(_avail_yrs) + 1)
        for i, _yr2 in enumerate(_avail_yrs):
            with _yr_cols[i]:
                _is_active = (_sel_yr == _yr2)
                if st.button(f"✓ {_yr2}" if _is_active else str(_yr2),
                             key=f"rpt_yr_{_yr2}", use_container_width=True):
                    st.session_state['rapport_yr_saved'] = _yr2
                    # Reset mois quand on change d'année
                    st.session_state['rapport_m_saved'] = None
                    st.rerun()

        # ── Sélecteur MOIS (boutons colorés selon perf) ──
        # Mois disponibles pour l'année sélectionnée
        _mois_dispo = {}  # {m: net_usd}
        for (_yr3, _m3), _net in _perf_par_mois.items():
            if _yr3 == _sel_yr:
                _mois_dispo[_m3] = _net

        if 'rapport_m_saved' not in st.session_state:
            st.session_state['rapport_m_saved'] = None  # None = toute l'année
        _sel_m = st.session_state['rapport_m_saved']
        if _sel_m is not None and _sel_m not in _mois_dispo:
            _sel_m = None
            st.session_state['rapport_m_saved'] = None

        st.markdown(f"<div style='font-size:11px;color:{C['muted']};margin:10px 0 6px;text-transform:uppercase;letter-spacing:.06em'>Mois</div>", unsafe_allow_html=True)

        # Boutons mois : ALL + 12 mois
        _m_cols = st.columns(13)
        with _m_cols[0]:
            _all_active = (_sel_m is None)
            if st.button("ALL ✓" if _all_active else "ALL",
                         key="rpt_m_all", use_container_width=True):
                st.session_state['rapport_m_saved'] = None
                st.rerun()
        ...
        for _mi in range(1, 13):
            with _m_cols[_mi]:
                ...
                _is_active_m = (_sel_m == _mi)
                if _mi in _mois_dispo:
                    _v = _mois_dispo[_mi]
                    _col_m_btn = "🟢" if _v > 0 else ("🔴" if _v < 0 else "⚪")
                    _lbl_m = f"{_col_m_btn} {_mn}" if not _is_active_m else f"✓ {_mn}"
                else:
                    _lbl_m = f"· {_mn}"  # grisé = pas de trade
                if st.button(_lbl_m, key=f"rpt_m_{_mi}", use_container_width=True,
                             disabled=(_mi not in _mois_dispo)):
                    st.session_state['rapport_m_saved'] = _mi
                    st.rerun()
        ...
        # Logique brute : toutes primes encaissées y compris positions ouvertes
        # FX moyen des relevés (plus précis que le taux live)
        _fx_rpt = sum(d.get('fx', _fx_an) for d in st.session_state['ibkr_data'].values() if 0.8 < d.get('fx', 0) < 2.0)
        _fx_rpt = _fx_rpt / max(len([d for d in st.session_state['ibkr_data'].values() if 0.8 < d.get('fx',0) < 2.0]), 1)
        ...
        # Frais uniquement sur les ouvertures (IBKR ne facture pas à l'expiration)
        _pm_frais_eur   = sum(abs(_t.get('frais',0)) for _t in _trades_mois if _t.get('quantite',0) < 0)
        _pm_brutes_net_eur = (_pm_brutes_usd - _pm_rachats_usd) / _fx_rpt
        _pm_pertes_eur  = -_pm_rachats_usd / _fx_rpt
        _pm_net_eur     = _pm_brutes_net_eur - _pm_frais_eur
        _pm_nb_trades   = len(_trades_mois)

        # ── Top 5 tickers de la période — depuis transactions brutes ────
        _ticker_mois = {}
        ...
        # Convertir en EUR
        _ticker_mois_eur = {tk: v/_fx_rpt for tk, v in _ticker_mois.items()}
        _top5_mois = sorted(_ticker_mois_eur.items(), key=lambda x: -x[1])[:5]
        ...
        # ── Top 5 all-time par ticker ──────────────────────────
        _ticker_alltime = {}
        for _yd in st.session_state['ibkr_data'].values():
            for sym, val in _yd.get('synthese_realise', {}).items():
                _tk = sym.split()[0] if sym else ''
                _ticker_alltime[_tk] = _ticker_alltime.get(_tk, 0) + val
        ...
        _nc = "#3FB950" if _pm_net_eur >= 0 else "#F85149"
        _medailles = ['🥇','🥈','🥉','4.','5.']
  ...
  <span style='color:{_vc};font-family:monospace;min-width:75px;text-align:right;font-size:12px'>{_val/_fx_an:+.2f} €</span>
</div>"""
  ...
  <div style='display:flex;justify-content:space-between;align-items:center'>
    ...
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
        ...
        _rapport_html = (
            ...
            f"<div style='font-size:34px;font-weight:700;color:{_nc};font-family:monospace'>{_pm_net_eur:+.2f} €</div>"
            f"</div></div>"
            ...
            f"<div style='font-size:20px;font-weight:700;color:#3FB950;font-family:monospace'>{_pm_brutes_net_eur:+.2f} €</div>"
            f"<div style='font-size:11px;color:#8B949E;margin-top:3px'>≈ ${_pm_brutes_usd - _pm_rachats_usd:.2f}</div>"
            f"</div>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
            f"<div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Frais</div>"
            f"<div style='font-size:20px;font-weight:700;color:#F85149;font-family:monospace'>-{_pm_frais_eur:.2f} €</div>"
            f"</div>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
        ...
        try:
            ...
            def _make_rapport_png():
                ...
                nc = GRN if _pm_net_eur >= 0 else RED
                t(d,W-32,26,"PRIMES NETTES",MUT,11,anchor='rt')
                t(d,W-32,44,f"{_pm_net_eur:+.2f} EUR",nc,36,bold=True,anchor='rt')
                d.line([(32,120),(W-32,120)], fill=BORDER, width=1)

                # 3 CARTES
                cw = (W-64-32)//3
                cards = [
                    ("PRIMES BRUTES","recues - payees, hors frais",f"{(_pm_brutes_usd-_pm_rachats_usd)/_fx_an:+.2f} EUR",GRN),
                    ("FRAIS","frais payes au courtier",f"-{_pm_frais_eur:.2f} EUR",RED),
                ...
                for side, tp in enumerate(top):
                    ...
                    for i,(tk,v) in enumerate(tp):
                        ...
                        v_eur = v/_fx_an if side==0 else v
                        t(d,xb+375,ry+4,f"{v_eur:+.2f} EUR",vc,13,bold=True)
            ...
            st.markdown(f"<div style='font-size:11px;color:{C['muted']}'>Export image non disponible : {_e_png}</div>", unsafe_allow_html=True)
...
En dessous du PRU Ajusté = seuil pour vendre des calls sans risque de perte en cas d'assignation.</div>""", unsafe_allow_html=True)
        ...
        for _yr in _years_disp:
            ...
            for mois in range(1, 13):
                ...
                if v_eur != 0:
                    _cell = (f"<div style='font-size:12px;font-weight:700;color:{_col_v};margin-bottom:2px'>{v_eur:+.2f} €</div>"
                             f"<div style='font-size:9px;color:{_col_v};opacity:.65'>{v_eur*_fx_yr:+.2f} $</div>")
                else:
                    _cell = f"<div style='font-size:13px;color:{C['muted']}'>—</div>"
