...
with tab_budget:
    ...
    def _pv(section, cat):
        v = section.get(cat, {}); return _sf(v.get('prevu', v) if isinstance(v, dict) else v)

    def _pbar(pct, color):
        pc = min(pct, 100); bc = "#F87171" if pct > 100 else color
        return (f"<div style='background:#1F2937;border-radius:3px;height:7px;width:100%;margin:2px 0'>"
                f"<div style='background:{bc};width:{pc:.0f}%;height:7px;border-radius:3px'></div></div>"
    ...
    def _section_tbl(titre, color, icon, bp_sec, rbc, show_ech=False):
        ...
        if not cats:
            return (f"<div style='font-size:10px;font-weight:700;color:#F59E0B;text-transform:uppercase;padding:8px 0 4px'>{icon} {titre}</div>"
                    f"<div style='color:{_C_MUTED};font-size:11px;padding:8px 0'>Aucune donnée — utilisez 'Modifier le budget prévu' pour ajouter des catégories.</div>")
        h_extra = f"<th style='padding:4px 6px;text-align:center;font-size:9px;color:{_C_MUTED}'>ÉCH</th>" if show_ech else ""
        td_extra_empty = "<td></td>" if show_ech else ""
        ...
        for cat in cats:
            ...
            bc = "#F87171" if pct > 100 else color; tp += pr; tr += re
            ...
            tbl += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    ...
                    f"<td style='padding:4px 6px;text-align:right;font-weight:600;color:{bc}'>{re:,.0f}€</td>"
                    f"<td style='padding:4px 6px'>{_pbar(pct, color)}</td></tr>")
        tp_pct = (tr / tp * 100) if tp > 0 else 0
        tc = "#F87171" if tp_pct > 100 else color
        tbl += (f"<tr style='background:#111827;font-weight:700'><td style='padding:5px 6px'>TOTAL</td>"
                ...
                f"<td style='padding:5px 6px;text-align:right;color:{tc}'>{tr:,.0f}€</td>"
                f"<td style='padding:5px 6px'>{_pbar(tp_pct, color)}</td></tr></tbody></table>")
        return tbl
    ...
    epa_r=sum(_re.values()); crd_r=sum(_rc.values()); rest_r=rev_r-dep_r-ff_r-epa_r-crd_r
    ...
    st.markdown(
        ...
        f"<b style='font-size:18px;color:{_BUD_COLOR}'>{_ML.get(_sel,'').upper()}</b>"
        f"<span style='font-size:11px;color:{_C_MUTED};margin-left:10px'>Suivi mensuel du budget · {_sel}</span></div>",
        unsafe_allow_html=True)

    _k4 = st.columns(4)
    for _col,(_lbl,_pv2,_rv,_clr) in zip(_k4,[
        ("💶 REVENUS",       rev_p, rev_r,        "#10B981"),
        ("🛒 DÉP. & FF",    dep_p+ff_p, dep_r+ff_r,"#F87171"),
        ("💳 CRÉDITS",      0,     crd_r,          "#F59E0B"),
        ("💰 ÉPARGNE",      epa_p, epa_r,          "#818CF8"),
    ]):
        _sub = f"Prévu : {_pv2:,.0f}€" if _pv2>0 else ""
    ...
    with _g1:
        ...
        _fig_b.update_layout(barmode='group',
            title=dict(text="Résumé Entrées & Sorties", font=dict(size=11,color=_C_MUTED), x=0.5),
            legend=dict(orientation='h', y=-0.32, font=dict(size=9)), margin=dict(l=10,r=10,t=35,b=50))
    ...
    with _g2:
        ...
        if _rp:
            ...
            _fig_r.update_layout(showlegend=True,
                legend=dict(font=dict(size=8), x=0.7, y=0.5),
                title=dict(text="Répartition des Revenus", font=dict(size=11,color=_C_MUTED), x=0.3),
                margin=dict(l=0,r=80,t=35,b=10))
            st.plotly_chart(_fig_r, use_container_width=True, config={'displayModeBar':False})
        else:
            st.markdown(f"<div style='color:{_C_MUTED};font-size:11px;text-align:center;padding:60px 0'>Aucun revenu</div>", unsafe_allow_html=True)

    with _g3:
        _bp2 = [(l,v,c) for l,v,c in [("Dépenses",dep_r,"#F87171"),("Frais Fixes",ff_r,"#FBBF24"),
            ("Épargne",epa_r,"#818CF8"),("Crédits",crd_r,"#F59E0B")] if v>0]
        if _bp2:
            _fig_bp = go.Figure(go.Pie(labels=[x[0] for x in _bp2], values=[x[1] for x in _bp2],
                hole=0.6, textinfo='percent+label', textfont=dict(size=9,color='white'),
                marker=dict(colors=[x[2] for x in _bp2], line=dict(color=_C_BG,width=2))))
            _fig_bp.add_annotation(text=f"<b>{sum(v for _,v,_ in _bp2):,.0f}€</b>", x=0.5, y=0.5,
                font=dict(size=11,color=_C_TEXT), showarrow=False)
            ...
            st.markdown(f"<div style='color:{_C_MUTED};font-size:11px;text-align:center;padding:60px 0'>Aucune dépense</div>", unsafe_allow_html=True)

    # ── 3 colonnes tables (comme le Sheet) ──
    _cl, _cm, _cr = st.columns([0.9, 1.2, 1.2])

    with _cl:
        # Résumé global Entrées & Sorties
        ...
        _rc2 = "#10B981" if rest_r>=0 else "#F87171"
        _tg += (f"<tr style='background:#0D1117;font-weight:700'>"
                f"<td style='padding:5px 6px;color:{_rc2}'>TOTAL RESTANT</td>"
                f"<td style='padding:5px 6px;text-align:right;color:{_C_MUTED}'>{rest_p:,.0f}€</td>"
                f"<td style='padding:5px 6px;text-align:right;color:{_rc2}'>{rest_r:,.0f}€</td>"
                f"</tr></tbody></table>")
    ...
    # ── Formulaires dans des expanders ──
    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
    _fa, _fb_exp = st.columns(2)
    with _fa:
        with st.expander("➕ Ajouter une transaction", expanded=False):
            with st.form("form_bud_tx", clear_on_submit=True):
                ...
                _sg = _sugg_d.get(_ft,[])
                if _sg: st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:-4px 0 6px'>{' · '.join(_sg[:5])}</div>", unsafe_allow_html=True)
                _fa2 = st.number_input("Montant (€)", min_value=0.0, step=0.01, format="%.2f", key="fb_amt")
                _fd2 = st.text_input("Description", key="fb_desc")
                _fs = st.form_submit_button("💾 Ajouter", use_container_width=True)
            if _fs:
                if _fc.strip() and _fa2>0:
                    _mk = _fd.strftime("%Y-%m")
                    if _mk not in _bud_data["mois"]:
                        _bud_data["mois"][_mk] = {"nom":_ML.get(_mk,_mk),"transactions":[],
                            "budget_prevu":{"revenus":{},"depenses":{},"frais_fixes":{},"epargne":{},"credits":{}}}
                    _bud_data["mois"][_mk]["transactions"].append({"date":str(_fd),"type":_ft,
                        "categorie":_fc.strip(),"montant":round(_fa2,2),"description":_fd2.strip()})
                    _bud_data["mois"][_mk]["transactions"].sort(key=lambda x:x["date"])
                    if _save_budget(_bud_data, f"Tx {_ft} {_fc} {_fa2:.0f}€"):
                        st.success(f"✅ {_fc} — {_fa2:.2f}€"); _load_budget.clear()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie et montant requis.")

    with _fb_exp:
        with st.expander("✏️ Modifier le budget prévu", expanded=False):
            st.markdown(f"<div style='font-size:11px;color:{_C_MUTED};margin-bottom:8px'>Définir/ajuster les montants prévus pour {_ML.get(_sel,'')}</div>", unsafe_allow_html=True)
            with st.form("form_bud_prev", clear_on_submit=False):
                ...
                _fp_c = st.text_input("Catégorie (ex: Course Alimentaire)", key="fp_cat")
                _fp_a = st.number_input("Montant prévu (€)", min_value=0.0, step=1.0, format="%.0f", key="fp_amt")
                _fp_s = st.form_submit_button("✏️ Enregistrer le budget", use_container_width=True)
            if _fp_s:
                if _fp_c.strip():
                    ...
                    if _fp_t == "frais_fixes":
                        _old_e = _bud_data["mois"][_sel]["budget_prevu"][_fp_t].get(_cat_c,{})
                        _bud_data["mois"][_sel]["budget_prevu"][_fp_t][_cat_c] = {"prevu":round(_fp_a,2),"reel":0,
                            "echeance":_old_e.get("echeance",0) if isinstance(_old_e,dict) else 0}
                    else:
                        _bud_data["mois"][_sel]["budget_prevu"][_fp_t][_cat_c] = {"prevu":round(_fp_a,2),"reel":0}
                    if _save_budget(_bud_data, f"Budget {_sel} {_fp_t} {_cat_c}={_fp_a:.0f}€"):
                        st.success(f"✅ {_cat_c} → {_fp_a:.0f}€ prévu"); _load_budget.clear()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie requise.")

    # ── Transactions + Résumé annuel ──
    st.markdown("<hr style='border-color:#1F2937;margin:16px 0'>", unsafe_allow_html=True)
    _tl, _tr2 = st.columns([2, 1])
    with _tl:
        with st.expander("📋 Transactions du mois", expanded=False):
            ...
            if _txsf:
                ...
                for _h in ["Date","Type","Catégorie","Montant","Description"]:
                    _tt += f"<th style='padding:4px 8px;font-size:9px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
                _tt += "</tr></thead><tbody>"
                for _t in _txsf:
                    _tc2=_TC.get(_t['type'],"#fff"); _sc="#10B981" if _t['type']=='Revenus' else "#F87171"
                    _sg2="+" if _t['type']=='Revenus' else "-"
                    _tt += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                        f"<td style='padding:4px 8px;color:{_C_MUTED}'>{_t['date']}</td>"
                        f"<td style='padding:4px 8px'><span style='font-size:9px;background:{_tc2}22;color:{_tc2};padding:1px 5px;border-radius:3px'>{_t['type']}</span></td>"
                        f"<td style='padding:4px 8px;font-weight:500'>{_t['categorie']}</td>"
                        f"<td style='padding:4px 8px;text-align:right;font-weight:600;color:{_sc}'>{_sg2}{abs(_t['montant']):.2f}€</td>"
    ...
    with _tr2:
        with st.expander("📅 Résumé annuel 2026", expanded=False):
            _ta = (f"<table style='width:100%;border-collapse:collapse;font-size:11px'>"
                   f"<thead><tr style='background:#0D1117'>")
            for _h,_hc in [("Mois",_C_MUTED),("Rev.","#10B981"),("Dép+FF","#F87171"),("Ép.","#818CF8"),("Restant",_C_MUTED)]:
                _ta += f"<th style='padding:4px 6px;text-align:right;font-size:9px;color:{_hc};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
            _ta += "</tr></thead><tbody>"
            _tot_a=[0.0]*4
            for _mk2 in sorted(_bud_data.get("mois",{}).keys()):
                _mt2=_bud_data["mois"][_mk2].get("transactions",[])
                _mr2=sum(t['montant'] for t in _mt2 if t['type']=='Revenus')
                _md2=sum(t['montant'] for t in _mt2 if t['type'] in ['Dépenses','Frais Fixes'])
                _me2=sum(t['montant'] for t in _mt2 if t['type']=='Épargne & Investissements')
                _mrest2=_mr2-_md2-_me2
                for i,v in enumerate([_mr2,_md2,_me2,_mrest2]): _tot_a[i]+=v
                _rc3="#10B981" if _mrest2>=0 else "#F87171"
                _ta += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:4px 6px;font-weight:600'>{_ML.get(_mk2,_mk2)[:4]}</td>"
                    f"<td style='padding:4px 6px;text-align:right;color:#10B981'>{_mr2:,.0f}€</td>"
                    f"<td style='padding:4px 6px;text-align:right;color:#F87171'>{_md2:,.0f}€</td>"
                    f"<td style='padding:4px 6px;text-align:right;color:#818CF8'>{_me2:,.0f}€</td>"
                    f"<td style='padding:4px 6px;text-align:right;font-weight:700;color:{_rc3}'>{_mrest2:,.0f}€</td></tr>")
            _tr_c2="#10B981" if _tot_a[3]>=0 else "#F87171"
            _ta += (f"<tr style='background:#0D1117;font-weight:700;border-top:2px solid {_C_BORDER}'>"
                f"<td style='padding:5px 6px'>YTD</td>"
                f"<td style='padding:5px 6px;text-align:right;color:#10B981'>{_tot_a[0]:,.0f}€</td>"
                f"<td style='padding:5px 6px;text-align:right;color:#F87171'>{_tot_a[1]:,.0f}€</td>"
                f"<td style='padding:5px 6px;text-align:right;color:#818CF8'>{_tot_a[2]:,.0f}€</td>"
                f"<td style='padding:5px 6px;text-align:right;color:{_tr_c2}'>{_tot_a[3]:,.0f}€</td>"
                "</tr></tbody></table>")
