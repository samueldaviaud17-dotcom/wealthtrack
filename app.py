...
.mkt-card-chart svg { width: 100% !important; height: 100% !important; display: block; }
        ...
        with st.expander("📊 Indices · VIX · Fear & Greed — variation journalière", expanded=True):
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    # Ligne zéro : affichée seulement si 0 est dans la plage de la série
                    _sy0_raw = _PAD + _DRAW * (1 - (0 - _smn) / _srng)
                    _show_zero = _smn < 0 < _smx
                    _zero_line = (
                        f'<line x1="0" y1="{_sy0_raw:.1f}" x2="{_SW}" y2="{_sy0_raw:.1f}" '
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _vshow_zero = _vmn < 0 < _vmx
                    _vzy = _VPAD + _VDRAW * (1 - (0 - _vmn) / _vrng)
                    _vzeroline = (f'<line x1="0" y1="{_vzy:.1f}" x2="{_VW}" y2="{_vzy:.1f}" stroke="#ffffff" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.25"/>') if _vshow_zero else ""
                    _vsvg = (
