...
tab1, tab5, tab2, tab3, tab4, tab8, tab6, tab7 = st.tabs(["🏠 Vue d'ensemble", "📊 Performance", "📈 Bourse", "₿ Crypto", "⚙️ Options", "📊 IBKR Analyse", "🔍 Valorisation", "⭐ Watchlist"])
...
with tab1:
    ...
    # Substituer la contribution Options par les données HTML si disponibles
    _ibkr_cr   = st.session_state.get('capital_reel', 0.0)
    _ibkr_kpis = st.session_state.get('ibkr_kpis', {})
    _ibkr_inv  = _ibkr_kpis.get('capital_investi', 0.0)
    # Valeur Options dans le Sheet = sv(df_pf, 8, 3) = valeur live Options (Perf. Totale)
    _opt_sheet_val = sv(df_pf, 8, 3)  # valeur actuelle Options dans Sheet
    _opt_sheet_inv = sv(df_pf, 8, 2)  # capital investi Options dans Sheet
    if _ibkr_cr > 0 and _opt_sheet_val > 0:
        pat = pat - _opt_sheet_val + _ibkr_cr   # soustraire valeur Sheet, ajouter Capital Réel IBKR
    if _ibkr_inv > 0 and _opt_sheet_inv > 0:
        inv = inv - _opt_sheet_inv + _ibkr_inv  # soustraire investi Sheet, ajouter investi HTML
    ...
    if 'ibkr_data' in st.session_state and st.session_state['ibkr_data']:
        _yr_data_now = st.session_state['ibkr_data'].get(_yr_now, {})
    ...
    with _ga:
        # Objectif patrimoine avec case de saisie
        _obj_pat = st.number_input("🎯 Objectif patrimoine fin d'année (€)",
            min_value=0, value=80000, step=1000, key="obj_patrimoine",
            help="Ton objectif de patrimoine total à fin décembre")
        ...
        _fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number",
            value=_pct_obj,
            number=dict(suffix="%", font=dict(size=20, color=C['text'], family='Space Grotesk'), valueformat=".1f"),
            gauge=dict(
                axis=dict(range=[0, 120], tickfont=dict(size=8, color=C['muted']), nticks=5),
                bar=dict(color=C['green'] if _pct_obj >= 100 else (C['gold'] if _pct_obj >= _pct_yr else C['red']), thickness=0.3),
                bgcolor=C['card'], borderwidth=0,
                steps=[dict(range=[0, _pct_yr], color='rgba(31,111,235,0.12)'),
                       dict(range=[_pct_yr, 120], color='rgba(0,0,0,0)')],
                threshold=dict(line=dict(color=C['blue'], width=2), thickness=0.7, value=_pct_yr)
            )
        ))
        _fig_gauge.update_layout(**base_layout(130, False))
        _fig_gauge.update_layout(margin=dict(l=15, r=15, t=10, b=5))
        st.plotly_chart(_fig_gauge, use_container_width=True, config={'displayModeBar': False})
        st.markdown(f"<div style='text-align:center;font-size:11px;color:{C['muted']};margin-top:-12px'>Actuel <b style='color:{C['text']}'>{fmt(pat)}</b> · écart <span style='color:{_diff_col};font-weight:700'>{'+' if _diff>=0 else ''}{fmt(_diff)}</span></div>", unsafe_allow_html=True)

    with _gb:
        # Objectif DCA mensuel avec case de saisie
        _dca_mois_obj = st.number_input("💸 Objectif DCA mensuel (€)",
            min_value=0, value=600, step=50, key="obj_dca",
            help="Ton DCA cible total par mois (Bourse + Crypto + Options)")
        # Calcul DCA réalisé = Bourse (CTO+PEA) sur les mois actifs
        _pats_tmp    = [n(v(df_em,13+i,2)) for i in range(12)]
        _nb_mois     = sum(1 for x in _pats_tmp if x > 0)
        _dca_b_mois  = [n(v(df_b,11+i,2)) + n(v(df_b,11+i,3)) for i in range(12)]
        _dca_b_real  = sum(x for x in _dca_b_mois if x > 0)
        _dca_total_real = _dca_b_real
        _dca_moy_mois = _dca_total_real / max(_nb_mois, 1)
        _pct_dca = min((_dca_moy_mois / _dca_mois_obj * 100), 150) if _dca_mois_obj > 0 else 0
        _dca_col = C['green'] if _pct_dca >= 100 else (C['gold'] if _pct_dca >= 80 else C['red'])
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 12px">
  <div style="font-size:10px;color:{C['muted']};margin-bottom:4px">Moy. réalisée / mois ({_nb_mois} mois)</div>
  <div style="font-family:'Space Grotesk';font-size:22px;font-weight:700;color:{_dca_col}">{fmt(_dca_moy_mois)}<span style="font-size:11px;color:{C['muted']}"> /mois</span></div>
  <div style="font-size:10px;color:{C['muted']};margin-top:2px;margin-bottom:6px">Total YTD : {fmt(_dca_total_real)}</div>
  <div style="display:flex;justify-content:space-between;font-size:10px;color:{C['muted']};margin-bottom:3px">
    <span>vs objectif {fmt(_dca_mois_obj)}/mois</span>
    <span style="color:{_dca_col};font-weight:700">{_pct_dca:.0f}%</span>
  </div>
  <div style="background:{C['border']};border-radius:3px;height:5px">
    <div style="background:{_dca_col};height:100%;width:{min(_pct_dca,100):.0f}%;border-radius:3px"></div>
  </div>
</div>""", unsafe_allow_html=True)

    with _gc:
        # Info rapide : temps écoulé
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 12px;margin-top:28px">
  <div style="font-size:10px;color:{C['muted']};margin-bottom:4px">Année {_YR} écoulée</div>
  <div style="font-family:'Space Grotesk';font-size:22px;font-weight:700;color:{C['blue']}">{_pct_yr:.1f}%</div>
  <div style="font-size:10px;color:{C['muted']};margin-top:2px;margin-bottom:6px">Jour {_day_of_yr} / {_days_yr}</div>
  <div style="background:{C['border']};border-radius:3px;height:5px">
    ...
    with R:
        ...
        _ibkr_cap_reel = st.session_state.get('capital_reel', 0.0)
        vals=[n(v(df_p,19+i,2)) for i in range(7)]
        # Substituer la valeur Options (index 2) par Capital Réel IBKR si dispo
        if _ibkr_cap_reel > 0 and len(vals) > 2:
...
with tab3:
    ...
    with L3:
        ...
        fig7.update_layout(showlegend=True,legend=dict(orientation='h',y=-0.15,font=dict(size=11)))
...
<span style="font-size:12px;color:{cl};font-weight:700">{vl:,.0f} $ · {pct_:.1f}%</span>
        ...
        # KPI staking annualisé
        _total_st_raw = sum(n(v(df_ct,19+idx,9)) for idx in range(4))
        _nb_mois_c = sum(1 for i in range(12) if n(v(fetch("₿ Crypto"),20+i,2))>0)
        _staking_annual = (_total_st_raw / max(_nb_mois_c,1) * 12) if _total_st_raw > 0 else 0
        _staking_yield  = (_staking_annual / cd * 100) if cd > 0 else 0
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:12px 14px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center">
<div>
  <div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em">Staking annualisé (proj.)</div>
  <div style="font-family:'Space Grotesk';font-size:20px;font-weight:700;color:{C['green']}">{f'+{_staking_annual:.2f} $' if _staking_annual else '—'}</div>
</div>
<div style="text-align:right">
  <div style="font-size:10px;color:{C['muted']}">Rendement</div>
  <div style="font-size:18px;font-weight:700;color:{C['green']}">{f'{_staking_yield:.2f}%' if _staking_yield else '—'}</div>
</div>
</div>""", unsafe_allow_html=True)
...
def save_capital_reel(val):
    import json
    _CAPITAL_REEL_FILE.write_text(json.dumps({'capital_reel': val}))
...
border-top:2px solid {C['teal']};border-radius:10px;padding:14px 18px;
height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between'>
<div>
  <div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px'>🏦 CAPITAL RÉEL</div>
  <div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{C['text']}'>{st.session_state['capital_reel']:.2f} €</div>
  <div style='font-size:12px;color:{C['teal']}'>← modifiable ci-dessous</div>
</div>
</div>""", unsafe_allow_html=True)
        _new_cr = st.number_input("Capital réel (€)", value=float(st.session_state['capital_reel']),
                                  min_value=0.0, format="%.2f",
                                  label_visibility="collapsed", key="cr_input")
        if _new_cr != st.session_state['capital_reel']:
    ...
    with _cr_col2:
        st.markdown(f"""<div style='background:{C['card']};border:1px solid {C['border']};
border-top:2px solid {C['blue']};border-radius:10px;padding:14px 18px;
height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column;justify-content:flex-start'>
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px'>💰 CAPITAL INVESTI</div>
<div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{C['blue']}'>{_cap_inv:.2f} €</div>
<div style='font-size:12px;color:{C['muted']}'>depuis ouverture du compte</div>
</div>""", unsafe_allow_html=True)
    ...
    # ── Sélecteur d'années (une selectbox par année disponible + option ALL) ──
    if loaded_years:
        ...
        _yr_opts = ["—"] + [str(y) for y in loaded_years]
        _yr_sel_cols = st.columns(min(len(loaded_years) + 1, 6))

        # Bouton ALL
        with _yr_sel_cols[0]:
            _all_active = set(_prev_sel_yrs) == set(loaded_years)
            if st.button("ALL ✓" if _all_active else "ALL", key="opt_yr_all2", use_container_width=True):
                st.session_state['ibkr_active_years'] = loaded_years
                st.rerun()

        # Un bouton par année
        for i, yr in enumerate(loaded_years):
            if i + 1 < len(_yr_sel_cols):
                with _yr_sel_cols[i + 1]:
                    _active = yr in _prev_sel_yrs
                    if st.button(f"✓ {yr}" if _active else str(yr), key=f"opt_yr2_{yr}", use_container_width=True):
                        _new_sel = list(_prev_sel_yrs)
                        if _active and len(_new_sel) > 1:
                            _new_sel.remove(yr)
                        elif not _active:
                            _new_sel.append(yr)
                        st.session_state['ibkr_active_years'] = sorted(_new_sel)
                        st.rerun()

        _sel_yrs = _prev_sel_yrs
    ...
    if not st.session_state['ibkr_data']:
        ...
        with _kpi_col4:
            ...
            st.markdown(f"""<div style='background:{C['card']};border:1px solid {C['border']};border-top:2px solid {pcol(_roi_all)};border-radius:10px;padding:14px 18px;height:{_KPI_H};box-sizing:border-box;display:flex;flex-direction:column'>
<div style='font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px'>🏆 ROI TOTAL</div>
<div style='font-family:Space Grotesk;font-size:22px;font-weight:700;color:{pcol(_roi_all)};line-height:1.2;flex:1'>{_roi_all:+.2f} %</div>
<div style='font-size:12px;color:{C['muted']};min-height:17px'>primes nettes / {_roi_base_lbl}</div>
<div style='font-size:11px;color:{_perf_col};min-height:15px'>perf. globale (réel−investi) : <b>{_perf_str}</b></div>
</div>""", unsafe_allow_html=True)
            with st.popover("ⓘ aide", use_container_width=True):
                st.markdown("""**ROI Total** = Primes nettes encaissées ÷ Capital Réel (valeur actuelle du compte)
\n**Perf. globale** = (Capital Réel − Capital Investi) ÷ Capital Investi  
→ Reflète la vraie performance de ton compte en tenant compte de la valeur actuelle des actions.""")

        with _kpi_col5:
            st.markdown(
                f"""<div style='background:{C['card']};border-radius:10px;padding:14px 16px;
border:1px solid {C['gold']}44;border-top:2px solid {C['gold']};height:{_KPI_H};box-sizing:border-box;overflow:hidden'>
...
{_primes_all:+.2f} €<span style='font-size:10px;font-weight:400;color:{C['muted']}'> ≈ ${_primes_all*_fx_all:.2f}</span></div>
        ...
        with _k6:
            st.markdown(card("ROI (période)", f"{_roi_trades:+.2f} %", f"primes / capital investi", pcol(_roi_trades),"📈"), unsafe_allow_html=True)
            with st.popover("ⓘ aide", use_container_width=True):
                st.markdown("""**ROI Période** = Primes nettes de la période sélectionnée ÷ Capital Investi total (somme des dépôts)
\n→ Indique le rendement de ta stratégie options sur la période filtrée (année ou ALL), indépendamment de la valeur des actions.
\n**Différence avec le ROI Total (haut)** : le ROI Total est toujours sur toutes les années et divisé par le Capital Réel (valeur actuelle du compte).""")
        st.markdown("<br>", unsafe_allow_html=True)

        # ── Indicateur cours live (lecture seule, refresh via bouton global) ──
        if _last_opt_refresh:
            st.markdown(
                f"<div style='font-size:11px;color:{C['muted']};padding:4px 0'>"
        ...
        if opts_open:
            ...
            tbl_o += _THL("Symbole","12%") + _THL("Ticker","4%") + _THL("Type","6%") + _THC("Qté","3%") + _THL("C/P","3%") + _THL("Strike","4%") + _THC("Prix Live","5%") + _THL("Ouverture","6%") + _THL("Expiration","6%") + _THC("J. rest.","5%") + _THC("Marge","7%") + _TH("Frais","5%") + _TH("Prime enc.","7%") + _TH("Rdt","5%") + _TH("Rdt ann.","6%")
        ...
        if opts_closed:
            ...
            # Filtres statuts + tri : compact sur une ligne
            _filt_col, _sort_col = st.columns([2, 2])
            with _filt_col:
            ...
            # Tri : par date d'ouverture décroissant par défaut
            with _sort_col:
                ...
                _sort_options = {'date':'Date ouverture','expiration':'Date expiration','pl_net':'P/L','prime_nette':'Prime'}
                _cur_sort = st.session_state[_sh_key]
                _sort_label = _sort_options.get(_cur_sort, 'Date ouverture')
                _sort_dir = '↑ Croissant' if st.session_state[_sh_asc_key] else '↓ Décroissant'
                _sort_sel = st.selectbox(
                    "Tri",
                    options=list(_sort_options.keys()),
                    index=list(_sort_options.keys()).index(_cur_sort) if _cur_sort in _sort_options else 0,
                    format_func=lambda k: f"{'↑' if st.session_state[_sh_asc_key] else '↓'} {_sort_options[k]}",
                    key="hist_sort_sel",
                    label_visibility="collapsed"
                )
                if _sort_sel != st.session_state[_sh_key]:
                    st.session_state[_sh_key] = _sort_sel
                    st.rerun()
            # Bouton toggle asc/desc discret
            _asc_label = "↑ Croissant" if st.session_state.get(_sh_asc_key, False) else "↓ Décroissant"
            if st.button(_asc_label, key="hist_asc_tog", use_container_width=False):
                st.session_state[_sh_asc_key] = not st.session_state.get(_sh_asc_key, False)
                st.rerun()
            ...
            tbl_h = f"<div style='overflow-x:auto'><table style='width:100%;border-collapse:collapse;font-size:13px'>"
            tbl_h += "<thead><tr style='background:#111827'>"
            tbl_h += (_THL("Symbole","13%") + _THL("Ticker","4%") + _THL("Type","8%") + _THC("Qté","3%") + _THL("C/P","3%") + _THL("Strike","5%") +
                      _THL("Expiration","6%") + _THL("Ouverture","6%") +
                      _TH("Frais","6%") +
                      _TH("Prime obtenue","8%") + _TH("Rdt","5%") + _TH("Rdt ann.","6%") + _TH("Statut","7%"))
...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        # ── Construire liste des périodes disponibles ──────────
        # Mois disponibles (avec trades)
        _periodes_mois = set()
        _periodes_yr   = set()
        for _t in _all_trades_an:
            try:
                _dt_t = _dt_an.strptime(_t['date'], '%Y-%m-%d')
                _periodes_mois.add((_dt_t.year, _dt_t.month))
                _periodes_yr.add(_dt_t.year)
            except: pass

        _opts_mois = sorted(_periodes_mois, reverse=True)
        _opts_yr   = sorted(_periodes_yr,   reverse=True)

        # Construire les labels de la selectbox
        _periode_labels = []
        for _y, _m in _opts_mois:
            _lbl = f"{_MOIS_FR_AN[_m-1]} {_y}"
            _is_cur = (_y == _now_an.year and _m == _now_an.month)
            _periode_labels.append((_lbl + (" (en cours)" if _is_cur else ""), 'mois', _y, _m))
        for _y in _opts_yr:
            _periode_labels.append((f"Année {_y}", 'annee', _y, None))

        sec("Générateur de rapport", "📊", "#FDA4AF", "#1C0A12")

        # ── Pré-calculer les primes par (année, mois) pour les indicateurs visuels ──
        _perf_par_mois = {}
        for _t in _all_trades_an:
            try:
                _dt_t = _dt_an.strptime(_t['date'], '%Y-%m-%d')
                _key  = (_dt_t.year, _dt_t.month)
                _perf_par_mois[_key] = _perf_par_mois.get(_key, 0) + _t.get('produit', 0)
            except: pass

        _avail_yrs = sorted(set(y for y,m in _perf_par_mois.keys()), reverse=True)
        if not _avail_yrs: _avail_yrs = [_now_an.year]

        # Restaurer sélections persistantes
        _prev_yr = st.session_state.get('rapport_yr_saved', _avail_yrs[0])
        if _prev_yr not in _avail_yrs: _prev_yr = _avail_yrs[0]

        _col_yr, _col_m, _ = st.columns([1.2, 1.5, 3])
        with _col_yr:
            _sel_yr = st.selectbox("Année", _avail_yrs,
                                   index=_avail_yrs.index(_prev_yr),
                                   key="rapport_yr")
        st.session_state['rapport_yr_saved'] = _sel_yr

        # Mois disponibles pour l'année sélectionnée
        _mois_dispo = {_m3: _net for (_yr3, _m3), _net in _perf_par_mois.items() if _yr3 == _sel_yr}

        _mois_opts = ["Toute l'année"]
        _mois_map  = {"Toute l'année": None}
        for _mi in range(1, 13):
            _mn = _MOIS_FR_AN[_mi-1]
            if _mi in _mois_dispo:
                _v   = _mois_dispo[_mi]
                _ind = " ▲" if _v > 0 else (" ▼" if _v < 0 else " —")
                _lbl = f"{_mn}{_ind}"
            else:
                _lbl = f"{_mn}"
            _mois_opts.append(_lbl)
            _mois_map[_lbl] = _mi

        _prev_m_lbl = st.session_state.get('rapport_m_saved', _mois_opts[0])
        if _prev_m_lbl not in _mois_opts: _prev_m_lbl = _mois_opts[0]

        with _col_m:
            _sel_m_lbl = st.selectbox("Mois", _mois_opts,
                                      index=_mois_opts.index(_prev_m_lbl),
                                      key="rapport_m")
        st.session_state['rapport_m_saved'] = _sel_m_lbl
        _sel_m = _mois_map.get(_sel_m_lbl)

        if _sel_m is None:
            _rpt_type = 'annee'; _rpt_yr = _sel_yr; _rpt_m = None
        else:
            _rpt_type = 'mois'; _rpt_yr = _sel_yr; _rpt_m = _sel_m
        ...
        for _t in _all_trades_an:
            try:
                _dt_t = _dt_an.strptime(_t['date'], '%Y-%m-%d')
                if _rpt_type == 'mois':
                    if _dt_t.month == _rpt_m and _dt_t.year == _rpt_yr:
                        _trades_mois.append(_t)
                else:  # annee
                    if _dt_t.year == _rpt_yr:
                        _trades_mois.append(_t)
            except: pass

        # Tout en USD — pas de conversion FX pour éviter les erreurs
        _pm_brutes_usd  = sum(abs(_t['produit']) for _t in _trades_mois if _t.get('quantite',0) < 0)
        _pm_rachats_usd = sum(abs(_t['produit']) for _t in _trades_mois if _t.get('quantite',0) > 0 and _t.get('produit',0) < 0)
        # Frais en USD (comm dans les trades bruts)
        _pm_frais_usd   = sum(abs(_t.get('frais',0)) for _t in _trades_mois if _t.get('quantite',0) < 0)
        _pm_brutes_net  = _pm_brutes_usd - _pm_rachats_usd   # $ net ouvertures - rachats
        _pm_net_usd     = _pm_brutes_net - _pm_frais_usd
        # Exclure les expirations OTM (qty>0 et produit=0) — pas un acte de trading actif
        _pm_nb_trades   = sum(1 for _t in _trades_mois if not (_t.get('quantite',0) > 0 and _t.get('produit',0) == 0))

        # ── Top 5 tickers de la période — en USD direct ────
        _ticker_mois = {}
        for _t in _trades_mois:
            _tk = _t.get('ticker','')
            if not _tk: continue
            _ticker_mois[_tk] = _ticker_mois.get(_tk, 0) + _t.get('produit', 0)
        _top5_mois = sorted(_ticker_mois.items(), key=lambda x: -x[1])[:5]

        # ── Meilleur trade de la période (rendement) ───────────
        _best_trade = None; _best_rdt = -999
        for _t in _trades_mois:
            if _t.get('quantite',0) >= 0: continue
            try:
                _sk = float(str(_t.get('strike',0)).replace('$',''))
                _mg = _sk * 100
                _pr = abs(_t.get('produit', 0))
                _rdt_t = (_pr / _mg * 100) if _mg > 0 else 0
                if _rdt_t > _best_rdt:
                    _best_rdt = _rdt_t
                    _best_trade = _t
            except: pass
        ...
        # ── Top 5 all-time par ticker — depuis transactions brutes USD ────
        # Même source que le Top tickers période pour cohérence
        _ticker_alltime = {}
        for _yd in st.session_state['ibkr_data'].values():
            for _t in _yd.get('trades', []):
                _tk = _t.get('ticker', '')
                if not _tk: continue
                _ticker_alltime[_tk] = _ticker_alltime.get(_tk, 0) + _t.get('produit', 0)
        _top5_alltime = sorted(_ticker_alltime.items(), key=lambda x: -x[1])[:5]

        # Label période pour affichage
        if _rpt_type == 'mois':
            _periode_display = f"{_MOIS_FR_AN[_rpt_m-1]} {_rpt_yr}"
        else:
            _periode_display = f"Année {_rpt_yr}"

        _rpt_titre = "RAPPORT MENSUEL" if _rpt_type == 'mois' else "RAPPORT ANNUEL"

        _mois_partiel = (_rpt_type == 'mois' and _rpt_yr == _now_an.year
                         and _rpt_m == _now_an.month and _now_an.day < 28) or (
                         _rpt_type == 'annee' and _rpt_yr == _now_an.year and _now_an.month < 12)
        _nc = "#3FB950" if _pm_net_usd >= 0 else "#F85149"
        _medailles = ['🥇','🥈','🥉','4.','5.']
        _max_top = max((v for _,v in _top5_mois), default=1)
        _max_all = max((v for _,v in _top5_alltime), default=1)

        _top5_mois_html = ""
        for i, (_tk, _val) in enumerate(_top5_mois):
            _bar = max(_val/_max_top*100, 5) if _max_top > 0 else 5
            _vc  = '#3FB950' if _val >= 0 else '#F85149'
            _top5_mois_html += f"""<div style='display:flex;align-items:center;gap:10px;margin-bottom:8px'>
  <span style='font-size:15px;width:22px'>{_medailles[i]}</span>
  <span style='font-weight:700;color:#E6EDF3;min-width:55px;font-size:13px'>{_tk}</span>
  <div style='flex:1;background:#21262D;border-radius:3px;height:7px'>
    <div style='height:100%;width:{_bar:.0f}%;background:{_vc};border-radius:3px'></div></div>
  <span style='color:{_vc};font-family:monospace;min-width:75px;text-align:right;font-size:12px'>${_val:+.2f}</span>
</div>"""

        _top5_all_html = ""
        for i, (_tk, _val) in enumerate(_top5_alltime):
            _bar = max(_val/_max_all*100, 5) if _max_all > 0 else 5
            _vc  = '#3FB950' if _val >= 0 else '#F85149'
            _top5_all_html += f"""<div style='display:flex;align-items:center;gap:10px;margin-bottom:8px'>
  <span style='font-size:15px;width:22px'>{_medailles[i]}</span>
  <span style='font-weight:700;color:#E6EDF3;min-width:55px;font-size:13px'>{_tk}</span>
  <div style='flex:1;background:#21262D;border-radius:3px;height:7px'>
    <div style='height:100%;width:{_bar:.0f}%;background:{_vc};border-radius:3px'></div></div>
  <span style='color:{_vc};font-family:monospace;min-width:75px;text-align:right;font-size:12px'>${_val:+.2f}</span>
        ...
        _best_html = ""
        if _best_trade:
            _best_html = f"""<div style='border-top:1px solid #30363D;padding-top:12px;margin-top:4px'>
  <div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px'>⚡ Meilleur trade du mois (rendement)</div>
  <div style='display:flex;justify-content:space-between;align-items:center'>
    <div style='display:flex;align-items:center;gap:10px'>
      <span style='background:#21262D;color:#E6EDF3;border-radius:6px;padding:3px 10px;font-weight:700;font-size:12px'>{_best_trade.get('ticker','')}</span>
      <span style='color:#8B949E;font-size:12px'>{'CALL' if _best_trade.get('call_put','')=='C' else 'PUT'} ${_best_trade.get('strike','')} · exp. {_best_trade.get('expiration','')}</span>
    </div>
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
  </div>
</div>"""


        # Pré-calculer les blocs conditionnels pour éviter les f-string imbriquées
        _partiel_html  = "<div style='font-size:12px;color:#D29922;margin-top:4px'>⚠ Mois en cours — données partielles</div>" if _mois_partiel else ""
        _top5m_html    = _top5_mois_html if _top5_mois_html else "<div style='color:#8B949E;font-size:12px;font-style:italic'>Aucun trade ce mois</div>"
        _top5a_html    = _top5_all_html  if _top5_all_html  else "<div style='color:#8B949E;font-size:12px;font-style:italic'>Aucune donnée</div>" 

        _rapport_html = (
            f"<div id='rapport-mois' style='background:#0D1117;border-radius:12px;padding:24px 28px;"
            f"border:1px solid #30363D;font-family:Space Grotesk,sans-serif;color:#E6EDF3'>"
            f"<div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px'>"
            f"<div>"
            f"<div style='font-size:11px;color:#8B949E;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px'>SamInvest · {_rpt_titre}</div>"
            f"<div style='font-size:30px;font-weight:700'>{_periode_display}</div>"
            f"{_partiel_html}"
            f"</div>"
            f"<div style='text-align:right'>"
            f"<div style='font-size:11px;color:#8B949E;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px'>Primes nettes</div>"
            f"<div style='font-size:34px;font-weight:700;color:{_nc};font-family:monospace'>${_pm_net_usd:+.2f}</div>"
            f"</div></div>"
            f"<div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:20px'>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
            f"<div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Primes brutes</div>"
            f"<div style='font-size:20px;font-weight:700;color:#3FB950;font-family:monospace'>${_pm_brutes_net:+.2f}</div>"
            f"<div style='font-size:11px;color:#8B949E;margin-top:3px'>rachats: -${_pm_rachats_usd:.2f}</div>"
            f"</div>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
            f"<div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Frais</div>"
            f"<div style='font-size:20px;font-weight:700;color:#F85149;font-family:monospace'>-${_pm_frais_usd:.2f}</div>"
            f"</div>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
            f"<div style='font-size:10px;color:#8B949E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em'>Trades effectués</div>"
            f"<div style='font-size:20px;font-weight:700;color:#58A6FF;font-family:monospace'>{_pm_nb_trades}</div>"
            f"</div></div>"
            f"<div style='display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:4px'>"
            f"<div>"
            f"<div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Top tickers — {_periode_display}</div>"
            f"{_top5m_html}"
            f"</div>"
            f"<div>"
            f"<div style='font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Hall of Fame — depuis le début ($)</div>"
            f"{_top5a_html}"
            f"</div></div>"
            f"{_best_html}"
            f"</div>"
        ...
        st.markdown(_rapport_html, unsafe_allow_html=True)

        # Bouton export image — génération PNG côté Python avec PIL
        try:
            from PIL import Image, ImageDraw, ImageFont
            import io

            def _make_rapport_png():
                ...
                BG=(13,17,23); CARD=(22,27,34); BORDER=(48,54,61); TXT=(230,237,243)
                MUT=(139,148,158); GRN=(63,185,80); RED=(248,81,73); BLU=(88,166,255)
                GLD=(210,153,34); YLW=(210,153,34)

                def fnt(size, bold=False):
                    return ImageFont.truetype(FONT_B if bold else FONT_R, size)

                def rrect(d, x, y, w, h, r=8, fill=None, outline=None, ow=1):
                    d.rounded_rectangle([x,y,x+w,y+h], radius=r, fill=fill, outline=outline, width=ow)

                def t(d, x, y, s, col, size=13, bold=False, anchor='lt'):
                    d.text((x,y), str(s), font=fnt(size,bold), fill=col, anchor=anchor)

                def medal_badge(d, x, y, rank):
                    colors = [(255,215,0),(192,192,192),(205,127,50)]
                    fgc    = [(100,70,0),(60,60,60),(80,40,10)]
                    bg = colors[rank] if rank<3 else (60,60,80)
                    fg = fgc[rank]   if rank<3 else (180,180,200)
                    d.ellipse([x,y,x+22,y+22], fill=bg)
                    d.text((x+11,y+11), str(rank+1), font=fnt(11,True), fill=fg, anchor='mm')

                def lightning(d, x, y, size=20):
                    s = size/20
                    pts = [(x+int(8*s),y),(x+int(2*s),y+int(12*s)),(x+int(8*s),y+int(10*s)),
                           (x+int(4*s),y+int(22*s)),(x+int(14*s),y+int(8*s)),
                           (x+int(8*s),y+int(10*s)),(x+int(8*s),y)]
                    d.polygon(pts, fill=YLW)
                ...
                # HEADER
                _rpt_type_lbl = "RAPPORT MENSUEL" if _rpt_type == 'mois' else "RAPPORT ANNUEL"
                t(d,32,26,f"SAMINVEST  .  {_rpt_type_lbl}",MUT,11)
                t(d,32,46,_periode_display,TXT,36,bold=True)
                if _mois_partiel:
                    t(d,32,92,"Mois en cours - donnees partielles",YLW,12)
                nc = GRN if _pm_net_usd >= 0 else RED
                t(d,W-32,26,"PRIMES NETTES",MUT,11,anchor='rt')
                t(d,W-32,44,f"${_pm_net_usd:+.2f}",nc,36,bold=True,anchor='rt')
                d.line([(32,120),(W-32,120)], fill=BORDER, width=1)

                # 3 CARTES
                cw = (W-64-32)//3
                cards = [
                    ("PRIMES BRUTES","recues - payees, hors frais",f"${_pm_brutes_net:+.2f}",GRN),
                    ("FRAIS","frais payes au courtier",f"-${_pm_frais_usd:.2f}",RED),
                    ("TRADES EFFECTUES","",str(_pm_nb_trades),BLU),
                ]
                for i,(lbl,sub,val,col) in enumerate(cards):
                    cx = 32+i*(cw+16)
                    rrect(d,cx,130,cw,88,fill=CARD,outline=BORDER)
                    t(d,cx+14,142,lbl,MUT,10,bold=True)
                    if sub: t(d,cx+14,156,sub,MUT,9)
                    t(d,cx+14,178,val,col,22,bold=True)

                # TOP 5 + HALL OF FAME
                col_y=238
                _lbl_top = f"TOP TICKERS - {_periode_display.upper()}"
                t(d,32,col_y,_lbl_top,MUT,10,bold=True)
                x2=W//2+10
                t(d,x2,col_y,"HALL OF FAME - DEPUIS LE DEBUT",MUT,10,bold=True)
                top=[_top5_mois, _top5_alltime]
                for side, tp in enumerate(top):
                    xb = 32 if side==0 else x2
                    mv = max((v for _,v in tp), default=1)
                    for i,(tk,v) in enumerate(tp):
                        ry=col_y+22+i*46
                        vc=GRN if v>=0 else RED
                        medal_badge(d,xb,ry,i)
                        t(d,xb+30,ry+4,tk,TXT,14,bold=True)
                        bw=max(int(v/mv*280),8) if mv>0 else 8
                        rrect(d,xb+85,ry+8,280,8,r=4,fill=BORDER)
                        rrect(d,xb+85,ry+8,bw,8,r=4,fill=vc)
                        v_eur = v if side==0 else v  # déjà en $
                        t(d,xb+375,ry+4,f"${v_eur:+.2f}",vc,13,bold=True)

                # MEILLEUR TRADE
                if _best_trade:
                    d.line([(32,H-105),(W-32,H-105)], fill=BORDER, width=1)
                    lightning(d,32,H-97,18)
                    t(d,58,H-94,"MEILLEUR TRADE DE LA PERIODE EN TERMES DE RENDEMENT",MUT,10,bold=True)
                    rrect(d,32,H-72,90,28,r=6,fill=(33,38,45),outline=BORDER)
                    t(d,77,H-58,_best_trade.get('ticker',''),TXT,13,bold=True,anchor='mm')
                    bt_lbl = f"{'CALL' if _best_trade.get('call_put','')=='C' else 'PUT'} ${_best_trade.get('strike','')}  .  exp. {_best_trade.get('expiration','')}"
                    t(d,132,H-58,bt_lbl,MUT,12,anchor='lm')
                    t(d,W-32,H-58,f"{_best_rdt:.1f}%",GRN,24,bold=True,anchor='rm')

                buf = io.BytesIO()
                img.save(buf, format='PNG', dpi=(144,144))
                return buf.getvalue()

            _png_data = _make_rapport_png()
            _mois_fn  = f"rapport_{_periode_display.replace(' ','_')}.png"
            st.download_button(
                label="📸 Télécharger le rapport en image",
                data=_png_data,
                file_name=_mois_fn,
                mime="image/png",
            ...
            pass  # Export silencieux si PIL échoue
...
En dessous du PRU Ajusté = seuil pour vendre des calls sans risque de perte en cas d'assignation.</div>""", unsafe_allow_html=True)
        ...
        for _yr in _years_disp:
            ...
            _tbl_m += "</div></div>"
            with st.expander(f"{'▼' if _yr == _yr_now_exp else '▶'} {_yr}  —  Total : {_tot_m:+.2f} €  ≈  {_tot_m*_fx_yr:+.2f} $", expanded=(_yr == _yr_now_exp)):
...
<div style="font-size:13px">Calcule une valorisation dans l'onglet <b>🔍 Valorisation</b>, choisis une liste et clique sur <b>⭐ Watchlist</b></div>
        ...
        for list_key, list_label, list_color in SUB_LISTS:
            ...
            hdr_cols = st.columns([3.2, 1.0, 1.4, 1.4, 1.8, 1.5, 1.8])
            hdr_labels = ["Action", "Ticker", "PRU (€)", "Prix Actuel", "Prix Cible", "Écart ▼▲", "Verdict"]
            ...
            for idx, w in enumerate(sorted_e):
                ...
                row_cols = st.columns([3.2, 1.0, 1.4, 1.4, 1.8, 1.5, 1.8])
