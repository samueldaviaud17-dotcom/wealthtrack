...
.mkt-card-chart svg { width: 100% !important; height: 100% !important; display: block; }
        ...
        with st.expander("📊 Indices · VIX · Fear & Greed — variation journalière", expanded=True):
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _PAD = 8
                    _DRAW = _SH - 2 * _PAD
                    # Ligne zéro fixe à 78% de hauteur → alignée visuellement sur toutes les cards
                    _sy0 = _SH * 0.78
                    # Normalisation : 0 ancré à _sy0, max positif en haut, max négatif en bas
                    _smx_abs = max(abs(v) for v in _serie_pct) or 1
                    _scale = (_sy0 - _PAD) / _smx_abs  # pixels par % de variation
                    _spts = ' '.join(
                        f"{i/max(len(_serie_pct)-1,1)*_SW:.1f},{_sy0 - v*_scale:.1f}"
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _VW, _VH = 300, 180
                    _VPAD = 8
                    # Ligne zéro fixe à 78% — alignée avec les indices
                    _vzy = _VH * 0.78
                    _vmx_abs = max(abs(v) for v in _vix_pct) or 1
                    _vscale = (_vzy - _VPAD) / _vmx_abs
                    _vpts = ' '.join(
                        f"{i/max(len(_vix_pct)-1,1)*_VW:.1f},{_vzy - v*_vscale:.1f}"
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    for _v, _lbl in [(0,'0'),(25,'25'),(50,'50'),(75,'75'),(100,'100')]:
                        ...
                        _tlx, _tly = _pt(_ta, _R + 28)
                        # Décalage manuel pour 0 et 100 qui sont aux extrémités
                        _t_anchor = 'middle'
                        if _v == 0:   _t_anchor = 'start';  _tlx += 2
                        if _v == 100: _t_anchor = 'end';    _tlx -= 2
