...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    # ── FRAIS BINANCE ──
    with st.expander("💸 Frais Binance du mois", expanded=False):
        st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:12px'>Saisis ici les frais de trading Binance du mois (visibles dans ton relevé Binance). Stocké dans <code>crypto_positions.json</code> → <code>frais_historiques</code>, exploitable pour le tableau Crypto (Chantier 8).</div>", unsafe_allow_html=True)
        with st.form("form_frais_binance"):
            _fb1, _fb2, _fb3 = st.columns(3)
            with _fb1:
                _frais_annee = st.selectbox("Année", list(range(_yr_now, 2021, -1)), key="frais_annee")
            with _fb2:
                _frais_mois  = st.selectbox("Mois", list(range(1, 13)), format_func=lambda m: f"{m:02d}", key="frais_mois")
            with _fb3:
                _frais_montant = st.number_input("Montant frais (€)", min_value=0.0, step=0.01, format="%.2f", key="frais_montant")
            _submit_fb = st.form_submit_button("💾 Enregistrer les frais", use_container_width=True)
        if _submit_fb:
            if _frais_montant >= 0:
                _pos_raw_fb, _pos_sha_fb = gh_read("crypto_positions.json")
                _pos_fb = json.loads(_pos_raw_fb.decode('utf-8')) if _pos_raw_fb else {"positions": {}}
                if "frais_historiques" not in _pos_fb:
                    _pos_fb["frais_historiques"] = {"_note": "Frais Binance cumulés par année en €"}
                _yr_s = str(_frais_annee)
                _mo_s = f"{_frais_mois:02d}"
                if _yr_s not in _pos_fb["frais_historiques"] or not isinstance(_pos_fb["frais_historiques"][_yr_s], dict):
                    _pos_fb["frais_historiques"][_yr_s] = {}
                _pos_fb["frais_historiques"][_yr_s][_mo_s] = round(_frais_montant, 2)
                # Recalcule total_2026 et total_cumul
                _tot_hist = sum(v for k, v in _pos_fb["frais_historiques"].items()
                                if k not in ("_note", "total_historique", "total_2026", "total_cumul") and isinstance(v, (int, float)))
                _tot_yr = sum(v for v in _pos_fb["frais_historiques"].get(_yr_s, {}).values() if isinstance(v, (int, float)))
                _pos_fb["frais_historiques"][f"total_{_yr_s}"] = round(_tot_yr, 2)
                _all_yr_tots = sum(
                    (sum(v for v in val.values() if isinstance(v, (int, float))) if isinstance(val, dict) else float(val))
                    for k, val in _pos_fb["frais_historiques"].items()
                    if k not in ("_note", "total_historique", "total_2026", "total_cumul") and not k.startswith("total_")
                )
                _pos_fb["frais_historiques"]["total_cumul"] = round(_all_yr_tots, 2)
                _ok_fb = gh_write("crypto_positions.json",
                    json.dumps(_pos_fb, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"Frais Binance {_yr_s}-{_mo_s}: {_frais_montant:.2f}€", _pos_sha_fb)
                if _ok_fb: st.success(f"✅ Frais enregistrés — {_yr_s}-{_mo_s} · {_frais_montant:.2f}€")
                else: st.error("❌ Erreur GitHub")

        if st.toggle("📋 Voir l'historique des frais Binance", key="toggle_hist_frais"):
            _fh = _pos_data.get("frais_historiques", {})
            _yrs_frais = sorted([k for k in _fh if k not in ("_note","total_historique","total_2026","total_cumul") and not k.startswith("total_") and isinstance(_fh[k], (dict, int, float))], reverse=True)
            _tbl_f = f"<table style='width:100%;border-collapse:collapse;font-size:12px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:4px'>Année</th><th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"
            for _yk in _yrs_frais:
                _yv = _fh[_yk]
                _ytot = sum(v for v in _yv.values() if isinstance(v,(int,float))) if isinstance(_yv, dict) else float(_yv)
                _tbl_f += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:4px'>{_yk}</td><td style='text-align:right;padding:4px'>{_ytot:.2f}€</td></tr>"
            _tbl_f += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td style='padding:4px'>TOTAL CUMULÉ</td><td style='text-align:right;padding:4px'>{_fh.get('total_cumul', 0):.2f}€</td></tr></tbody></table>"
            st.markdown(_tbl_f, unsafe_allow_html=True)
...
— <b>🐛 SHA stale corrigé</b> : versements Bourse et Crypto faisaient un <code>gh_write</code> avec le sha initial de l'onglet. Si une autre opération (retrait, etc.) avait entre-temps réécrit <code>historique_dca.json</code>, le sha était obsolète → écriture GitHub silencieusement ignorée. Chaque écriture fait maintenant un <code>gh_read</code> frais juste avant.<br>
— <b>Frais Binance mensuels</b> (30/06/2026) : nouveau bloc "💸 Frais Binance du mois" dans Saisie DCA. Permet de saisir les frais de trading Binance par mois/année → mis à jour dans <code>crypto_positions.json</code> sous <code>frais_historiques</code>, totaux recalculés automatiquement. Historique par année consultable via toggle. Les frais historiques 2022→2026-06 (51.2€ cumulés) sont déjà en place depuis le point de départ du 29/06/2026. Lors du Chantier 8 (autonomie Sheet), ces données seront directement exploitables pour afficher les frais cumulés dans l'onglet Crypto.<br><br>
