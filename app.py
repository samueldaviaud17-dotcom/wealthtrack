        ...
        for list_key, list_label, list_color in SUB_LISTS:
            ...
            for idx, w in enumerate(sorted_e):
                ...
                with row_cols[7]:
                    ...
                    if _an:
                        _tp_str = (
                            f"<div style='font-size:11px;color:#E2E8F0;margin-top:3px'>"
                            f"Cible <b>{_an['target_price']:.0f}$</b>"
                            f"{' · ' + str(_an['nb_analysts']) + ' anal.' if _an['nb_analysts'] else ''}"
                            f"</div>"
                        ) if _an['target_price'] else ""
                        st.markdown(
                            f"<div style='padding:4px 4px'>"
                            f"<span style='background:{_an['rec_col']}33;color:{_an['rec_col']};font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;border:1px solid {_an['rec_col']}55'>"
                            f"{_an['rec_label']}</span>{_tp_str}</div>",
                            unsafe_allow_html=True
                        )
                    else:
...
<b>🔄 Reste</b> : Robustesse source données yfinance (fallback si calendar vide).<br>
<b>💡 À venir</b> : Dividendes — yfinance expose <code>dividendYield</code>, <code>dividendRate</code>, <code>exDividendDate</code>, <code>lastDividendDate</code>. Idée : afficher date ex-dividende + rendement en 2e ligne sous le badge Analystes dans la Watchlist. Peut aussi alimenter un mini-calendrier dividendes dans l'onglet Contexte Marché.""",
            "#EAB308")
