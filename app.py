            ...
            for idx, w in enumerate(sorted_e):
                ...
                with row_cols[1]:
                    ...
                    _tv_badge = (
                        ...
                        f'display:inline-block;margin-top:4px;font-size:10px;font-weight:700;'
                        f'color:#818CF8;background:#818CF811;border:1px solid #818CF855;'
                        f'border-radius:4px;padding:1px 6px;text-decoration:none;letter-spacing:.02em">'
                        f'📈 TV</a>'
                    ) if ticker_w else ''
                    st.markdown(
                        f"<div style='padding:6px 4px;text-align:center'>"
                        f"<div style='font-family:monospace;font-size:11px;font-weight:700;color:{list_color}'>{ticker_w or '—'}</div>"
                        f"{_tv_badge}"
                        f"</div>",
                        unsafe_allow_html=True
                    )
...
<b>✅ Fait</b> : VIX avec sparkline intégrée + badge de niveau (Calme/Normal/Prudence/Stress). Fond du graphique aligné sur la variation journalière (rouge si VIX monte, vert si baisse) comme les indices, badge de niveau garde sa couleur sémantique.<br>
