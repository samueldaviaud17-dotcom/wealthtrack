...
Alpha = TRI Mon PF − TRI Indice (pts %).</span></div>""", unsafe_allow_html=True)
    ...
    # ── Point 3 : PV/MV cumulée fin de mois ──
    with _R2:
        sec("PV/MV cumulée — fin de mois","📈","#6EE7B7","#0A1C0F")
        ...
        if _ym:
            _pvmv_col = C['green'] if _ym[-1] >= 0 else C['red']
            _r2,_g2,_b2 = int(_pvmv_col[1:3],16),int(_pvmv_col[3:5],16),int(_pvmv_col[5:7],16)
            _fig_pvmv = go.Figure(go.Scatter(
                x=_xm, y=_ym, mode='lines+markers',
                line=dict(color=_pvmv_col, width=2),
                marker=dict(color=_pvmv_col, size=6),
                fill='tozeroy', fillcolor=f"rgba({_r2},{_g2},{_b2},0.10)",
            ...
            st.plotly_chart(_fig_pvmv, use_container_width=True, config={'displayModeBar':False})
        else:
