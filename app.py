...
<div style='font-size:12px;color:{C['muted']}'>depuis ouverture du compte</div>
    ...
    # ── Sélecteur d'années à afficher (multiselect avec cases) ──
    if loaded_years:
        ...
        if not _sel_yrs: _sel_yrs = loaded_years  # fallback
        st.session_state['ibkr_active_years'] = _sel_yrs
        # Afficher les années sélectionnées
        st.markdown(
            "<div style='display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:4px'>" +
            "".join([f"<span style='background:{C['green']}22;color:{C['green']};border:1px solid {C['green']}44;border-radius:12px;padding:2px 10px;font-size:11px;font-weight:600'>{y} ✓</span>" for y in _sel_yrs]) +
            "</div>", unsafe_allow_html=True
        )
    ...
    if not st.session_state['ibkr_data']:
        ...
        with _kpi_col4:
            ...
            st.markdown(f"""<div style='background:{C['card']};border:1px solid {C['border']};border-top:2px solid {pcol(_roi_all)};border-radius:10px;padding:14px 18px;height:118px;box-sizing:border-box;display:flex;flex-direction:column'>
...
→ Reflète la vraie performance de ton compte en tenant compte de la valeur actuelle des actions.""")
        ...
        with _kpi_col5:
            st.markdown(
                f"""<div style='background:{C['card']};border-radius:12px;padding:14px 16px;
