...
with tab_budget:
    ...
    _TYPE_COLORS = {
        ...
        "Frais Fixes": "#FBBF24", "Épargne & Investissements": "#818CF8",
        "Crédits / Dettes": "#F59E0B"
    }
    _CAT_SUGGESTIONS = {
        "Revenus": ["Salaire","Remboursement","Prime","Virement entrant","Autre revenu"],
        "Dépenses": ["Course Alimentaire","Cantine Travail","Transport","Extra","Coiffeur","Osteo","Restaurant","Sport","Loisirs","Vêtements","Santé","Voyages"],
        "Frais Fixes": ["Loyer","Électricité","Internet","Téléphone mobile","Assurance habitation","Assurance Voiture","Frais Bancaire","Abonnement"],
        "Épargne & Investissements": ["Trade Republic","PEA Bourso","Crypto Binance","Interactive Broker","PEL Crédit Mutuelle","Livret A","Autre épargne"],
        "Crédits / Dettes": ["Crédit voiture","Prêt personnel","Remboursement dette"]
    }

    _now_p = datetime.now(_TZ_PARIS)
    _mois_dispo = sorted(_bud_data.get("mois", {}).keys(), reverse=True)
    _mois_cur_key = _now_p.strftime("%Y-%m")
    _mois_opts = [k for k in list(_MOIS_LABELS.keys()) if k <= _mois_cur_key]
    _mois_opts_rev = sorted(_mois_opts, reverse=True)
    _default_idx = 0

    # ── Header ──
    _bud_tx_count = sum(len(v.get("transactions", [])) for v in _bud_data.get("mois", {}).values())
    st.markdown(
        f"<div style='background:linear-gradient(90deg,{_BUD_BG} 0%,#0D1F18 100%);border-left:3px solid {_BUD_COLOR};"
        f"padding:12px 16px;border-radius:6px;margin-bottom:16px'>"
        f"<span style='font-size:18px;font-weight:700;color:{_BUD_COLOR}'>💰 Budget & Dépenses 2026</span>"
        f"<span style='font-size:12px;color:{_C_MUTED};margin-left:12px'>Données stockées sur GitHub · {_bud_tx_count} transactions</span>"
        "</div>", unsafe_allow_html=True)
    ...
    _mois_data = _bud_data.get("mois", {}).get(_sel_mois, {"transactions": [], "resume": {}})
    _txs = _mois_data.get("transactions", [])
    _res = _mois_data.get("resume", {})

    # ── KPIs mensuels ──
    _rev  = sum(t['montant'] for t in _txs if t['type'] == 'Revenus')
    _dep  = sum(t['montant'] for t in _txs if t['type'] == 'Dépenses')
    _ff   = sum(t['montant'] for t in _txs if t['type'] == 'Frais Fixes')
    _epa  = sum(t['montant'] for t in _txs if t['type'] == 'Épargne & Investissements')
    _cred = sum(t['montant'] for t in _txs if t['type'] == 'Crédits / Dettes')
    _rest = _rev - _dep - _ff - _epa - _cred

    _kpi_cols = st.columns(6)
    for _col, (_label, _val, _clr, _ic) in zip(_kpi_cols, [
        ("REVENUS",   _rev,  "#10B981","💶"),
        ("DÉPENSES",  _dep,  "#F87171","🛒"),
        ("FRAIS FIXES",_ff,  "#FBBF24","🏠"),
        ("ÉPARGNE",   _epa,  "#818CF8","💰"),
        ("CRÉDITS",   _cred, "#F59E0B","💳"),
        ("RESTANT",   _rest, "#10B981" if _rest >= 0 else "#F87171","🏦"),
    ]):
        with _col:
            st.markdown(card(_label, fmt(_val), "", _clr, _ic), unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    _Lb, _Rb = st.columns([3, 2])

    with _Lb:
        # ── Tableau des transactions ──
        sec("Transactions du mois", "📋", _BUD_COLOR, _BUD_BG)

        _type_filter = st.multiselect("Filtrer par type", _TYPES, default=_TYPES, key="bud_filter", label_visibility="collapsed")
        _txs_filtered = sorted([t for t in _txs if t['type'] in _type_filter], key=lambda x: x['date'], reverse=True)

        if _txs_filtered:
            _tbl_b = f"<div style='overflow-x:auto'><table style='width:100%;border-collapse:collapse;font-size:12px'>"
            _tbl_b += f"<thead><tr style='background:#111827'>"
            for _h in ["Date","Type","Catégorie","Montant","Description"]:
                _tbl_b += f"<th style='padding:6px 8px;text-align:left;font-size:10px;color:{_C_MUTED};text-transform:uppercase;border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
            _tbl_b += "</tr></thead><tbody>"
            for _t in _txs_filtered:
                _tc = _TYPE_COLORS.get(_t['type'], "#fff")
                _sign = "+" if _t['type'] == 'Revenus' else "-"
                _sign_col = "#10B981" if _t['type'] == 'Revenus' else "#F87171"
                _tbl_b += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:5px 8px;color:{_C_MUTED}'>{_t['date']}</td>"
                    f"<td style='padding:5px 8px'><span style='font-size:10px;background:{_tc}22;color:{_tc};padding:2px 6px;border-radius:4px;white-space:nowrap'>{_t['type']}</span></td>"
                    f"<td style='padding:5px 8px;font-weight:500'>{_t['categorie']}</td>"
                    f"<td style='padding:5px 8px;font-weight:700;color:{_sign_col};text-align:right'>{_sign}{abs(_t['montant']):.2f} €</td>"
                    f"<td style='padding:5px 8px;color:{_C_MUTED};font-size:11px'>{_t.get('description','')}</td>"
                    "</tr>")
            _tbl_b += "</tbody></table></div>"
            st.markdown(_tbl_b, unsafe_allow_html=True)
        else:
        ...
        for _mk in sorted(_bud_data.get("mois", {}).keys()):
            ...
            _mc   = sum(t['montant'] for t in _mtxs if t['type'] == 'Crédits / Dettes')
            _mrest = _mr - _md - _mf - _me - _mc
            _ann_rows.append((_MOIS_LABELS.get(_mk, _mk), _mr, _md, _mf, _me, _mc, _mrest))

        _tbl_ann = f"<table style='width:100%;border-collapse:collapse;font-size:11px'>"
        _tbl_ann += f"<thead><tr style='background:#111827'>"
        for _h in ["Mois","Revenus","Dépenses","Frais Fixes","Épargne","Crédits","Restant"]:
            _tbl_ann += f"<th style='padding:5px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
        _tbl_ann += "</tr></thead><tbody>"
        _tot = [0.0]*6
        for _nom, _r, _d, _f, _e, _c, _rest in _ann_rows:
            _rc = "#10B981" if _rest >= 0 else "#F87171"
            for i, v in enumerate([_r, _d, _f, _e, _c, _rest]): _tot[i] += v
            _tbl_ann += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                ...
                f"<td style='padding:5px 8px;text-align:right;color:#F59E0B'>{_c:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;font-weight:700;color:{_rc}'>{'+' if _rest>=0 else ''}{_rest:,.0f}€</td>"
                "</tr>")
        _tr_c = "#10B981" if _tot[5] >= 0 else "#F87171"
        _tbl_ann += (f"<tr style='background:#111827;font-weight:700;border-top:2px solid {_C_BORDER}'>"
            ...
            f"<td style='padding:6px 8px;text-align:right;color:#F59E0B'>{_tot[4]:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;font-weight:700;color:{_tr_c}'>{'+' if _tot[5]>=0 else ''}{_tot[5]:,.0f}€</td>"
            "</tr></tbody></table>")
    ...
    with _Rb:
        ...
        with st.form("form_budget_tx", clear_on_submit=True):
            ...
            _fb_cat  = st.text_input("Catégorie", key="fb_cat",
                placeholder="Ex: Course Alimentaire, Salaire...")
            _fb_cats = _CAT_SUGGESTIONS.get(_fb_type, [])
            if _fb_cats:
                _sep = " · "
                st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:-8px 0 8px 0'>Suggestions : {_sep.join(_fb_cats[:5])}</div>", unsafe_allow_html=True)
        ...
        if _fb_sub:
            if _fb_cat.strip() and _fb_amt > 0:
                _new_tx = {
                    "date": str(_fb_date),
                    "type": _fb_type,
                    "categorie": _fb_cat.strip(),
                    "montant": round(_fb_amt, 2),
                    "description": _fb_desc.strip()
                }
                _mois_key = _fb_date.strftime("%Y-%m")
                if _mois_key not in _bud_data["mois"]:
                    _bud_data["mois"][_mois_key] = {"nom": _MOIS_LABELS.get(_mois_key, _mois_key), "transactions": []}
                _bud_data["mois"][_mois_key]["transactions"].append(_new_tx)
                _bud_data["mois"][_mois_key]["transactions"].sort(key=lambda x: x["date"])
                if _save_budget(_bud_data, _bud_sha):
                    st.success(f"✅ Transaction ajoutée — {_fb_cat} {_fb_amt:.2f}€")
                    _load_budget.clear()
                else:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        sec("Répartition dépenses", "🍩", _BUD_COLOR, _BUD_BG)
        _dep_by_cat = {}
        for _t in _txs:
            if _t['type'] == 'Dépenses':
                _dep_by_cat[_t['categorie']] = _dep_by_cat.get(_t['categorie'], 0) + _t['montant']
        if _dep_by_cat:
            _dep_sorted = sorted(_dep_by_cat.items(), key=lambda x: -x[1])
            import plotly.graph_objects as _go
            _fig_dep = _go.Figure(_go.Pie(
                labels=[x[0] for x in _dep_sorted[:8]],
                values=[x[1] for x in _dep_sorted[:8]],
                hole=0.5,
                textinfo='percent', textfont=dict(size=10, color='white'),
                marker=dict(line=dict(color=_C_BG, width=2))
            ))
            _fig_dep.add_annotation(text=f"<b>{_dep:,.0f}€</b>", x=0.5, y=0.5,
                font=dict(size=14, color=_C_TEXT), showarrow=False)
            _fig_dep.update_layout(**base_layout(220, False))
            _fig_dep.update_layout(showlegend=False, margin=dict(l=0,r=0,t=10,b=0))
            st.plotly_chart(_fig_dep, use_container_width=True, config={'displayModeBar': False})

            for _cat, _amt in _dep_sorted[:8]:
                _pct_dep = _amt / _dep * 100 if _dep else 0
                st.markdown(f"<div style='display:flex;justify-content:space-between;padding:3px 0;font-size:12px'>"
                    f"<span style='color:{_C_TEXT}'>{_cat}</span>"
                    f"<span style='color:#F87171;font-weight:600'>{_amt:,.0f}€ · {_pct_dep:.0f}%</span></div>",
                    unsafe_allow_html=True)
        else:
