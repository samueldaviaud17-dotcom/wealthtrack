...
Si ça ne fonctionne pas correctement, cet onglet peut être supprimé sans impact sur le reste de l'app.</div>""",
    ...
    if not YF_AVAILABLE:
        ...
        # ── Simulation TRI pour Bourse (CTO+PEA) ──
        st.markdown("##### 2️⃣ Simulation TRI — si Bourse (CTO+PEA) avait été investie sur indices")
        _pi_bourse = sv(df_pf,6,2)
        _pl_bourse = sv(df_pf,6,3)
        ...
        for idx_name in ["SP500", "CAC 40", "STOXX 600"]:
            ...
            if df_h is None:
                _results.append({"Indice": idx_name, "TRI simulé": "—", "Valeur simulée": "—", "Note": "données indisponibles"})
                continue
            parts = 0.0
            n_ok = 0
            cfs = []
            for m in _months_b:
                px = _price_at_or_before(df_h, m)
                if px and px > 0:
                    parts += _montant_b / px
                    cfs.append((m, -_montant_b))
                    n_ok += 1
            ...
            if px_now and parts > 0 and n_ok >= 2:
                ...
                tri = xirr(cfs)
                _results.append({"Indice": idx_name,
                                  "TRI simulé": f"{tri:+.2f}%" if tri is not None else "—",
                                  "Valeur simulée": fmt(val_finale),
                                  "Note": f"{n_ok}/{len(_months_b)} mois OK (ticker {used_tk})"})
            else:
                _results.append({"Indice": idx_name, "TRI simulé": "—", "Valeur simulée": "—",
                                  "Note": f"pas assez de données ({n_ok} mois OK)"})

        # Comparaison avec le TRI réel "Mon PF" (recalculé comme dans Performance)
        _months_b_count = len(_months_b)
        _cfs_pf = [(m, -_montant_b) for m in _months_b] + [(_now_ch, _pl_bourse)]
        _tri_pf = xirr(_cfs_pf) if _pi_bourse > 0 and _pl_bourse > 0 else None
        _results.insert(0, {"Indice": "🏆 Mon PF (Bourse réel)",
                             "TRI simulé": f"{_tri_pf:+.2f}%" if _tri_pf is not None else "—",
                             "Valeur simulée": fmt(_pl_bourse),
                             "Note": f"{_months_b_count} mois DCA depuis {_start_bourse.strftime('%m/%Y')}"})
...
Méthode : DCA simulé le 4 de chaque mois, montant = capital investi total / nb de mois, prix indice =
dernier cours connu à cette date (ou avant). Si un indice affiche "—", le ticker yfinance n'est pas
accessible depuis cet environnement — à vérifier en prod (Streamlit Cloud).</div>""", unsafe_allow_html=True)
