    ...
    with col_res:
        ...
        if method in ["EPS / PER", "FCF / PER"]:
            ...
            for scen in ["BASE","BEAR","BULL"]:
                ...
                ms  = 10.0
                # EPS estimated: hist * (1+g)^5 for base metric approach
                nb  = st.session_state.get("valo_nb", 0)
                rc  = st.session_state.get("valo_rachat", -1.5)
                # Project revenue growth → apply margin → get earnings → /shares
                if hist > 0:
                    eps_proj = hist * (1 + g/100)**5
                    price = eps_proj * per
                    # Apply performance discount
                    price_pay = price / (1+ms/100)
                    results[scen] = {"price": price, "price_pay": price_pay, "per": per}
                else:
                    results[scen] = None
        else:
        ...
        for col, scen in zip(s_cols, ["BASE","BEAR","BULL"]):
            ...
            with col:
                if r and r.get("price"):
                    ...
                    # upside vs cours actuel (prix à payer vs cours)
                    ...
                    # Store for conviction
                    r["upside_pay"] = upside_pay
                    r["upside_val"] = upside_val
                    st.markdown(f"""<div style="background:{bg};border:1px solid {color}33;border-radius:8px;padding:14px;margin-bottom:4px">
<div style="font-size:10px;font-weight:700;color:{color};text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">{label}</div>
<div style="font-size:9px;color:{C['muted']};margin-bottom:3px">💰 Prix à payer aujourd'hui</div>
<div style="font-family:'Space Grotesk';font-size:22px;font-weight:700;color:{color};margin-bottom:4px">{pp:,.0f} €</div>
<div style="font-size:10px;color:{C['muted']}">Valeur intrinsèque 2031 : <span style="color:{C['text']};font-weight:600">{p:,.0f} €</span></div>
<div style="font-size:12px;font-weight:600;color:{up_color};margin-top:6px">{up_arrow} {upside_pay:+.1f}% vs cours actuel</div>
