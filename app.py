...
Alpha = TRI Mon PF − TRI Indice (pts %).</span></div>""", unsafe_allow_html=True)
    ...
    _fig_contrib.update_layout(**base_layout(160, False))
    _fig_contrib.add_vline(x=0, line_color=C['border'], line_width=1)
    _fig_contrib.update_xaxes(tickformat='.0f', ticksuffix=' €')
    st.plotly_chart(_fig_contrib, use_container_width=True, config={'displayModeBar': False})

    # ── PV/MV cumulée dans le temps — placeholder (chantier 5) ──
    st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
    sec("PV/MV dans le temps","📈","#6EE7B7","#0A1C0F")
...
⏳ <b style='color:{C['text']}'>En construction (Chantier 5 — snapshots GitHub)</b><br>
Ce graphique affichera la PV/MV totale (<b style='color:{C["green"]}'>{fmt(pvmv_t)}</b> aujourd'hui)
à la fin de chaque mois, sans lien avec le Google Sheet.
Chaque mois, la valeur de <code>pvmv_t</code> sera automatiquement sauvegardée sur GitHub
et ajoutée à la courbe. Disponible une fois le chantier 5 implémenté.</div>""",
    unsafe_allow_html=True)
