...
with tab_budget:
    ...
    _re=_rbc("Épargne & Investissements"); _rc=_rbc("Crédits / Dettes")
    ...
    rev_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("revenus",{}).values())
    dep_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("depenses",{}).values())
    ff_p =sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("frais_fixes",{}).values())
    epa_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("epargne",{}).values())
    cred_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("credits",{}).values())
    ...
    with st.expander(f"📊 {_ML.get(_sel,'').upper()} — Suivi mensuel du budget", expanded=True):
        ...
        st.markdown(
            ...
            f"<b style='font-size:16px;color:{_BUD_COLOR}'>{_ML.get(_sel,'').upper()}</b>"
        ...
        with _cl:
            ...
            # Saisie reste mois précédent (au-dessus du tableau)
            ...
            with _c_rp1:
                _new_reste=st.number_input("",value=float(_reste_prec),step=0.01,format="%.2f",key="reste_prec_input",label_visibility="collapsed")
            with _c_rp2:
                if st.button("💾",key="btn_reste_prec",help="Sauvegarder"):
                    if _sel not in _bud_data["mois"]:
                        _bud_data["mois"][_sel]={"nom":_ML.get(_sel,""),"transactions":[],"budget_prevu":{}}
                    _bud_data["mois"][_sel]["reste_mois_precedent"]=round(_new_reste,2)
                    if _save_budget(_bud_data,f"Reste précédent {_sel}={_new_reste:.2f}€"):
                        st.success(f"✅ {_new_reste:.2f}€"); _load_budget.clear(); st.rerun()
                    else: st.error("❌")

            # Résumé Entrées & Sorties avec reste mois précédent
            ...
            _tg=(f"<div style='font-size:10px;font-weight:700;color:{_C_MUTED};text-transform:uppercase;padding:12px 0 4px'>📊 RÉSUMÉ ENTRÉES & SORTIES</div>"
                 ...
                 f"<th style='padding:4px 6px;text-align:right;font-size:9px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>PRÉVU</th>"
                 f"<th style='padding:4px 6px;text-align:right;font-size:9px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉEL</th></tr></thead><tbody>"
                 f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                 f"<td style='padding:4px 6px;color:{_rpc};font-size:11px'>+  Reste mois précédent</td>"
                 f"<td style='padding:4px 6px;text-align:right;color:{_C_MUTED}'>{_reste_prec:,.2f}€</td>"
                 f"<td style='padding:4px 6px;text-align:right;color:{_rpc}'>{_reste_prec:,.2f}€</td></tr>")
            for _lbl2,_pv3,_rv3,_clr3 in [
            ...
            # Montant restant à dépenser = Total Restant Réel
        ...
        # ── TRANSACTIONS (toggle dans l'expander) ──
    ...
    # End of main expander (transactions toggle ends here)

    # ════════════ GÉRER LES TRANSACTIONS ════════════
    with st.expander("✏️ Gérer les transactions", expanded=False):

        def _get_cats(type_name):
            _tmap={"Revenus":"revenus","Dépenses":"depenses","Frais Fixes":"frais_fixes",
                   "Épargne & Investissements":"epargne","Crédits / Dettes":"credits"}
            _sk=_tmap.get(type_name,"")
            _cats=list(_bp.get(_sk,{}).keys())
            for _t in _txs:
                if _t['type']==type_name and _t['categorie'] not in _cats:
                    _cats.append(_t['categorie'])
            return _cats if _cats else []

        _action_tx=st.radio("",["➕ Créer","✏️ Modifier","🗑️ Supprimer"],horizontal=True,key="tx_action",label_visibility="collapsed")
        ...
        if _action_tx=="➕ Créer":
            ...
            with _cA:
                ...
                _cats_c=_get_cats(_ft)
                if _cats_c:
                    _fc=st.selectbox("Catégorie",_cats_c,key="fb_cat_c")
                else:
                    _fc=st.text_input("Catégorie (nouvelle)",key="fb_cat_c_txt")
            with _cB:
            ...
            if st.button("💾 Ajouter la transaction",use_container_width=True,key="btn_add_tx"):
                _fc_val=_fc if _cats_c else st.session_state.get("fb_cat_c_txt","")
                if _fc_val and _fa3>0:
                    _mk2=_fd.strftime("%Y-%m")
                    _raw_fc, _ = gh_read(_BUD_PATH)
                    _data_fc = json.loads(_raw_fc.decode('utf-8')) if _raw_fc else {"mois":{}}
                    if _mk2 not in _data_fc.get("mois",{}):
                        _data_fc.setdefault("mois",{})[_mk2]={"nom":_ML.get(_mk2,_mk2),"transactions":[],"budget_prevu":{}}
                    _data_fc["mois"][_mk2]["transactions"].append({"date":str(_fd),"type":_ft,
                        "categorie":_fc_val.strip(),"montant":round(_fa3,2),"description":_fd3.strip()})
                    _data_fc["mois"][_mk2]["transactions"].sort(key=lambda x:x["date"])
                    if _save_budget(_data_fc,f"Tx {_ft} {_fc_val} {_fa3:.0f}€"):
                        st.success(f"✅ {_fc_val} — {_fa3:.2f}€"); _load_budget.clear(); st.rerun()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie et montant requis.")

        elif _action_tx=="✏️ Modifier":
            if _txs:
                _tx_lbl=[f"{i} | {t['date']} · {t['type']} · {t['categorie']} · {t['montant']:.2f}€" for i,t in enumerate(_txs)]
                _tx_sel=st.selectbox("Transaction à modifier",_tx_lbl,key="tx_mod_sel")
                _tx_idx=int(_tx_sel.split(" | ")[0]); _tx_obj=_txs[_tx_idx]
                _cA2,_cB2=st.columns(2)
                with _cA2:
                    _fm_date=st.date_input("Date",value=datetime.strptime(_tx_obj['date'],"%Y-%m-%d").date(),key="fm_date")
                    _fm_type=st.selectbox("Type",_TYPES,index=_TYPES.index(_tx_obj['type']) if _tx_obj['type'] in _TYPES else 0,key="fm_type")
                    _cats_m=_get_cats(_fm_type)
                ...
                with _cB2:
                    _fm_amt=st.number_input("Montant (€)",value=float(_tx_obj['montant']),step=0.01,format="%.2f",key="fm_amt")
                    _fm_desc=st.text_input("Description",value=_tx_obj.get('description',''),key="fm_desc")
                if st.button("💾 Sauvegarder la modification",use_container_width=True,key="btn_mod_tx"):
                    _raw_fresh, _ = gh_read(_BUD_PATH)
                    _data_fresh = json.loads(_raw_fresh.decode('utf-8')) if _raw_fresh else {}
                    if _sel in _data_fresh.get("mois",{}) and _tx_idx < len(_data_fresh["mois"][_sel].get("transactions",[])):
                        _data_fresh["mois"][_sel]["transactions"][_tx_idx]={
                            "date":str(_fm_date),"type":_fm_type,
                            "categorie":_fm_cat.strip(),"montant":round(_fm_amt,2),"description":_fm_desc.strip()}
                        _data_fresh["mois"][_sel]["transactions"].sort(key=lambda x:x["date"])
                        if _save_budget(_data_fresh,f"Modif tx {_fm_cat}"):
                            st.success("✅ Transaction modifiée"); _load_budget.clear(); st.rerun()
                        else: st.error("❌ Erreur GitHub")
                    else: st.error("❌ Index invalide, rechargez la page")
            else: st.info("Aucune transaction à modifier.")

        else:  # Supprimer
            if _txs:
                _tx_lbl2=[f"{i} | {t['date']} · {t['type']} · {t['categorie']} · {t['montant']:.2f}€" for i,t in enumerate(_txs)]
                ...
                if st.button("🗑️ Confirmer la suppression",key="btn_del_tx"):
                    ...
                    _raw_fresh2, _ = gh_read(_BUD_PATH)
                    _data_fresh2 = json.loads(_raw_fresh2.decode('utf-8')) if _raw_fresh2 else {}
                    if _sel in _data_fresh2.get("mois",{}) and _idx_del < len(_data_fresh2["mois"][_sel].get("transactions",[])):
                        _data_fresh2["mois"][_sel]["transactions"].pop(_idx_del)
                        if _save_budget(_data_fresh2,f"Suppression tx idx={_idx_del}"):
                            st.success("✅ Transaction supprimée"); _load_budget.clear(); st.rerun()
                        else: st.error("❌ Erreur GitHub")
                    else: st.error("❌ Index invalide, rechargez la page")
            else: st.info("Aucune transaction à supprimer.")

    # ════════════ PLANIFICATION DU BUDGET ════════════
    with st.expander("💡 Planification du budget", expanded=False):
        _action_bp=st.radio("",["➕ Ajouter","✏️ Modifier","🗑️ Supprimer"],horizontal=True,key="bp_action",label_visibility="collapsed")
        ...
        if _action_bp=="➕ Ajouter":
            ...
            with _cP1:
                _fp_t=st.selectbox("Section",list(_SEC_MAP.keys()),format_func=lambda x:_SEC_MAP[x],key="fp_type_a")
            with _cP2:
                _fp_a=st.number_input("Montant prévu (€)",min_value=0.0,step=1.0,format="%.2f",key="fp_amt_a")
            ...
            if st.button("✏️ Enregistrer",use_container_width=True,key="btn_planif_add"):
                if _fp_c.strip():
                    ...
                    if _save_budget(_bud_data,f"Budget {_sel} {_fp_t} {_cc2}={_fp_a:.0f}€"):
                        st.success(f"✅ {_cc2} → {_fp_a:.2f}€"); _load_budget.clear(); st.rerun()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie requise.")

        elif _action_bp in ["✏️ Modifier","🗑️ Supprimer"]:
            _all_bp_cats=[(s,c) for s,sd in _bp.items() for c in sd]
            if _all_bp_cats:
                _bp_lbls=[f"{_SEC_MAP.get(s,s)} · {c}" for s,c in _all_bp_cats]
                _bp_sel=st.selectbox("Catégorie de budget",_bp_lbls,key="bp_sel")
                _bp_idx=_bp_lbls.index(_bp_sel); _bp_sec_k,_bp_cat_k=_all_bp_cats[_bp_idx]
                _bp_obj=_bp.get(_bp_sec_k,{}).get(_bp_cat_k,{})
                _bp_prevu=_sf(_bp_obj.get('prevu',_bp_obj) if isinstance(_bp_obj,dict) else _bp_obj)

                if _action_bp=="✏️ Modifier":
                    _fpm_a=st.number_input("Nouveau montant prévu (€)",value=float(_bp_prevu),step=1.0,format="%.2f",key="fpm_amt")
                    if st.button("💾 Sauvegarder",use_container_width=True,key="btn_mod_bp"):
                        if _bp_sec_k=="frais_fixes":
                            _oe2=_bud_data["mois"][_sel]["budget_prevu"][_bp_sec_k].get(_bp_cat_k,{})
                            _bud_data["mois"][_sel]["budget_prevu"][_bp_sec_k][_bp_cat_k]={"prevu":round(_fpm_a,2),"reel":0,
                                "echeance":_oe2.get("echeance",0) if isinstance(_oe2,dict) else 0}
                        else:
                            _bud_data["mois"][_sel]["budget_prevu"][_bp_sec_k][_bp_cat_k]={"prevu":round(_fpm_a,2),"reel":0}
                        if _save_budget(_bud_data,f"Modif budget {_bp_cat_k}={_fpm_a:.0f}€"):
                            st.success(f"✅ {_bp_cat_k} → {_fpm_a:.2f}€"); _load_budget.clear(); st.rerun()
                        else: st.error("❌ Erreur GitHub")
                else:
                    st.markdown(f"<div style='color:#F87171;font-size:11px;margin:4px 0'>Supprimer : <b>{_bp_cat_k}</b> ({_bp_prevu:.2f}€ prévu)</div>",unsafe_allow_html=True)
                    if st.button("🗑️ Confirmer la suppression",key="btn_del_bp"):
                        del _bud_data["mois"][_sel]["budget_prevu"][_bp_sec_k][_bp_cat_k]
                        if _save_budget(_bud_data,f"Suppression budget {_bp_cat_k}"):
                            st.success(f"✅ {_bp_cat_k} supprimé"); _load_budget.clear()
                        else: st.error("❌ Erreur GitHub")
            else: st.info("Aucune planification à modifier/supprimer.")
    ...
    with st.expander("📅 RÉSUMÉ ANNUEL 2026", expanded=False):
        ...
        for _mk3 in sorted(_bud_data.get("mois",{}).keys()):
            ...
            _mrp=_sf(_bud_data["mois"][_mk3].get("reste_mois_precedent",0))
            ...
            _mrest3=_mrp+_mr3-_md3-_mf3-_me3-_mc3
