        ...
        if _action_tx=="➕ Créer":
            ...
            if st.button("💾 Ajouter la transaction",use_container_width=True,key="btn_add_tx"):
                ...
                if _fc_val and _fa3>0:
                    ...
                    if _mk2 not in _bud_data["mois"]:
                        _bud_data["mois"][_mk2]={"nom":_ML.get(_mk2,_mk2),"transactions":[],"budget_prevu":{}}
                    ...
                    if _save_budget(_bud_data,f"Tx {_ft} {_fc_val} {_fa3:.0f}€"):
            ...
            if _txs:
                ...
                if st.button("💾 Sauvegarder la modification",use_container_width=True,key="btn_mod_tx"):
                    _bud_data["mois"][_sel]["transactions"][_tx_idx]={"date":str(_fm_date),"type":_fm_type,
                        "categorie":_fm_cat.strip(),"montant":round(_fm_amt,2),"description":_fm_desc.strip()}
                    _bud_data["mois"][_sel]["transactions"].sort(key=lambda x:x["date"])
                    if _save_budget(_bud_data,f"Modif tx {_fm_cat}"):
