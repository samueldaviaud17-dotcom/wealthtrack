    ...
    padding-bottom: 0 !important;
        ...
        with st.expander("📊 Indices de référence — variation journalière", expanded=True):
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _SW, _SH = 300, 160
                    _smn, _smx = min(_serie_pct), max(_serie_pct)
                    _srng = (_smx - _smn) if _smx != _smn else 1
                    _spts = ' '.join(
                        f"{i/max(len(_serie_pct)-1,1)*_SW:.1f},{_SH-(v-_smn)/_srng*(_SH-4):.1f}"
                        for i,v in enumerate(_serie_pct)
                    )
                    _spoly = f"0,{_SH} {_spts} {_SW},{_SH}"
                    # Calcul position Y de la ligne zéro
                    _sy0 = _SH - (0 - _smn) / _srng * (_SH - 4)
                    _sy0 = max(2, min(_SH - 2, _sy0))
                    _ssvg = (
                    ...
                    st.markdown(
                        f'<div style="background:{C["card"]};border:1px solid {C["border"]};'
                        f'border-radius:8px;overflow:hidden;min-height:220px;display:flex;flex-direction:column">'
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _vy0 = _VH - (0 - _vmn) / _vrng * _VH
                    _vy0 = max(2, min(_VH - 2, _vy0))
                    _vsvg = (
