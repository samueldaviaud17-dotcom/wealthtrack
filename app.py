...
tab1, tab5, tab2, tab3, tab4, tab6 = st.tabs(["🏠 Vue d'ensemble", "📊 Performance", "📈 Bourse", "₿ Crypto", "⚙️ Options", "🔍 Valorisation"])
...
with tab6:
    ...
    nb_total = len(wl)
    nb_buy   = sum(1 for w in wl if w.get("statut")=="buy")
    nb_avoid = sum(1 for w in wl if w.get("statut")=="avoid")
    nb_wait  = sum(1 for w in wl if w.get("statut")=="wait")
    upsides  = [w["upside"] for w in wl if w.get("statut")=="buy" and w.get("upside")]
    avg_up   = sum(upsides)/len(upsides) if upsides else 0

    kc = st.columns(5)
    for col,(ti,va,su,co,ic) in zip(kc,[
        ("ACTIONS ANALYSÉES", str(nb_total) if nb_total else "—", "watchlist", C_VAL, "🔍"),
        ("SOUS PRIX CIBLE",   str(nb_buy)   if nb_buy   else "—", "opportunités", C['green'], "✅"),
        ("SURÉVALUÉES",       str(nb_avoid) if nb_avoid else "—", "watchlist",    C['red'],   "🔴"),
        ("EN ATTENTE",        str(nb_wait)  if nb_wait  else "—", "zone neutre",  C['gold'],  "⏳"),
        ("UPSIDE MOYEN",      f"{avg_up:+.1f}%" if avg_up else "—", "actions sous cible", C['cyan'], "🎯"),
    ]):
        with col: st.markdown(card(ti,va,su,co,ic), unsafe_allow_html=True)
    ...
    with col_form:
        ...
        if ticker_input and len(ticker_input) >= 2:
            if st.button("🔄 Récupérer cours + données historiques", key="valo_fetch", use_container_width=True):
                if YF_AVAILABLE:
                    with st.spinner("Récupération des données Yahoo Finance..."):
                        try:
                            ...
                            nb_sh = info.get("sharesOutstanding",0) or 0

                            # FX conversion
                            try:
                                fx = yf.Ticker("EURUSD=X").info.get("regularMarketPrice",1.16) if cur=="USD" else 1.0
                            except: fx = 1.16
                            ...
                            try:
                                ...
                                years_inc = [str(c.year) for c in inc.columns] if inc is not None and not inc.empty else []
                                ...
                                if rev_row is not None:
                                    rev_vals = [float(rev_row.iloc[i])/1e9 if rev_row.iloc[i] else 0 for i in range(min(5,len(rev_row)))]
                                    ni_vals  = [float(ni_row.iloc[i])/1e9 if ni_row is not None and ni_row.iloc[i] else 0 for i in range(min(5,len(rev_row)))]
                                    fcf_vals = [float(fcf_row.iloc[i])/1e9 if fcf_row is not None and i<len(fcf_row) and fcf_row.iloc[i] else 0 for i in range(min(5,len(rev_row)))]
                                    eps_vals = [float(eps_row.iloc[i]) if eps_row is not None and i<len(eps_row) and eps_row.iloc[i] else 0 for i in range(min(5,len(rev_row)))]
                                    margin_vals = [ni_vals[i]/rev_vals[i]*100 if rev_vals[i]!=0 else 0 for i in range(len(rev_vals))]
                                    fcf_margin  = [fcf_vals[i]/rev_vals[i]*100 if rev_vals[i]!=0 else 0 for i in range(len(rev_vals))]

                                    def cagr(vals):
                                        v = [x for x in vals if x!=0]
                                        if len(v)<2: return 0
                                        return ((abs(v[0])/abs(v[-1]))**(1/(len(v)-1))-1)*100

                                    hist_data = {
                                        "years":    years_inc[:len(rev_vals)],
                                        "revenue":  rev_vals,
                                        "ni":       ni_vals,
                                        "margin":   margin_vals,
                                        "eps":      eps_vals,
                                        "fcf":      fcf_vals,
                                        "fcf_m":    fcf_margin,
                                        "cagr_rev": cagr(rev_vals),
                                        "cagr_ni":  cagr(ni_vals),
                                        "cagr_eps": cagr(eps_vals),
                                        "cagr_fcf": cagr(fcf_vals),
                                        "nb_shares": nb_sh/1e6,
                                    }
                            ...
                            st.error(f"Ticker introuvable. Vérifie le format (ex: MC.PA, ADBE)")
                else:
            ...
            if hist_data and hist_data.get("years"):
                ...
                hdr += f"<span style='text-align:right;color:{GO}'>CAGR</span></div>"
                ...
                for metric, vals in rows.items():
                    ...
                    if cagr_v is not None:
                        ...
                        row_html += f"<span style='text-align:right;color:{cclr};font-weight:700;font-family:monospace'>{cagr_v:+.1f}%</span>"
                    else:
        ...
        c1,c2 = st.columns(2)
        with c1:
            nb_actions = st.number_input("Nb actions (M)", min_value=0.0, value=0.0, format="%.1f", key="valo_nb")
        with c2:
            rachat = st.number_input("Rachat annuel (%)", value=-1.5, format="%.1f", key="valo_rachat")
        ...
        if method in ["EPS / PER", "FCF / PER"]:
            ...
            for scen, emoji, color in [("BASE","📊","#7DD3FC"),("BEAR","🐻","#F85149"),("BULL","🐂","#3FB950")]:
                st.markdown(f"<div style='font-size:10px;font-weight:700;color:{color};text-transform:uppercase;letter-spacing:.08em;margin:8px 0 5px'>{emoji} Scénario {scen}</div>", unsafe_allow_html=True)
                s1,s2,s3 = st.columns(3)
                with s1: st.number_input(growth_label, value=9.0 if scen=="BASE" else (5.0 if scen=="BEAR" else 13.0), format="%.1f", help=growth_help, key=f"valo_g_{scen}")
                with s2: st.number_input(margin_label, value=28.5 if scen=="BASE" else (24.0 if scen=="BEAR" else 32.0), format="%.1f", help=margin_help, key=f"valo_m_{scen}")
                with s3: st.number_input(mult_label, value=25 if scen=="BASE" else (18 if scen=="BEAR" else 32), help=mult_help, key=f"valo_per_{scen}")
                s4,s5 = st.columns(2)
                with s4: st.number_input("Perf. annuelle (%)", value=10.0, format="%.1f", help="Rendement annuel exigé sur la période", key=f"valo_perf_{scen}")
                with s5: st.number_input("Marge de sécurité (%)", value=10.0 if scen=="BASE" else (15.0 if scen=="BEAR" else 10.0), format="%.1f", help="Décote appliquée au prix à payer", key=f"valo_ms_{scen}")
            ...
            for scen, emoji, color in [("BASE","📊","#7DD3FC"),("BEAR","🐻","#F85149"),("BULL","🐂","#3FB950")]:
                st.markdown(f"<div style='font-size:10px;font-weight:700;color:{color};margin:8px 0 5px'>{emoji} {scen}</div>", unsafe_allow_html=True)
                s1,s2 = st.columns(2)
                with s1: st.number_input("Croissance FCF (%)", value=9.0 if scen=="BASE" else (4.0 if scen=="BEAR" else 14.0), format="%.1f",
                    help="Taux de croissance annuel du FCF pour ce scénario", key=f"valo_gfcf_{scen}")
                with s2: st.number_input("Marge de sécurité (%)", value=10.0 if scen=="BASE" else (15.0 if scen=="BEAR" else 10.0), format="%.1f",
                    help="Décote appliquée à la valeur intrinsèque pour le prix à payer", key=f"valo_ms_{scen}")

        st.markdown("---")

        # Common params
        st.markdown(f"<div style='font-size:9px;color:{C_VAL};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px'>⚙️ Paramètres communs</div>", unsafe_allow_html=True)
        cp1, cp2 = st.columns(2)
        with cp1: st.number_input("Nb années projection", value=5, min_value=1, max_value=10, key="valo_nyears")
        with cp2: st.number_input("Rachat actions/an (%)", value=-1.5, format="%.1f", key="valo_rachat2", help="Taux de rachat annuel — réduit le nb d'actions")
    ...
    with col_res:
        ...
        n_years = int(st.session_state.get("valo_nyears", 5))
        rachat  = st.session_state.get("valo_rachat2", st.session_state.get("valo_rachat", -1.5))
        ...
        if method in ["EPS / PER", "FCF / PER"]:
            ...
            for scen in ["BASE","BEAR","BULL"]:
                ...
                if rev_last > 0 and nb_act > 0 and m > 0:
                    ...
                    # Fallback: project EPS directly if no revenue data
                    eps_final = hist_eps * (1 + g/100)**n_years * (1 + m/100 - 0.285) if m>0 else hist_eps*(1+g/100)**n_years
                else:
                    results[scen] = None; continue

                price_target = eps_final * per
                # Prix à payer today = Price_target / (1+perf%)^n_years
                price_pay_before_ms = price_target / (1 + perf/100)**n_years if perf > 0 else price_target
                price_pay = price_pay_before_ms * (1 - ms/100)
                results[scen] = {
                    "price": price_target,
                    "price_pay": price_pay,
                    "price_perf": price_pay_before_ms,
                    "eps_final": eps_final,
                    "per": per,
                    "n_years": n_years,
                }
...
<div style="font-family:'Space Grotesk';font-size:20px;font-weight:700;color:{color};margin-bottom:3px">{pp:,.0f} €</div>
...
<div style="font-size:11px;color:{C['muted']}">Saisir les données</div>
        ...
        if any(r for r in results.values() if r and r.get("price")):
            sec("Projection prix 2026–2031", "📈", C_VAL, C_VAL_DIM)
            years = ["2026","2027","2028","2029","2030","2031"]
            fig_v = go.Figure()
            scen_plot = {
                "BASE": ("#7DD3FC", "solid", 2.5),
                "BEAR": ("#F85149", "dot",   1.5),
                "BULL": ("#3FB950", "dot",   1.5),
            }
            for scen,(color,dash,width) in scen_plot.items():
                r = results.get(scen)
                if not r or not r.get("price"): continue
                hist = st.session_state.get("valo_hist", 0)
                g = st.session_state.get(f"valo_g_{scen}", 9)
                per = st.session_state.get(f"valo_per_{scen}", 25)
                prices_proj = []
                eps = hist
                for yr in range(6):
                    eps_y = hist * (1+g/100)**yr
                    prices_proj.append(eps_y * per)
                fig_v.add_trace(go.Scatter(
                    x=years, y=prices_proj, name=f"Prix {scen}",
                    line=dict(color=color, width=width, dash=dash),
                    mode='lines+markers', marker=dict(size=5),
                    hovertemplate=f"<b>%{{x}}</b><br>{scen}: %{{y:,.0f}} €<extra></extra>"
                ))
            # Current price horizontal line
            if cours_v > 0:
                fig_v.add_hline(y=cours_v, line_dash="dot", line_color=C['gold'],
                    annotation_text=f"Cours: {cours_v:,.0f}€",
                    annotation_font=dict(size=9, color=C['gold']),
                    annotation_position="bottom right")
            fig_v.update_layout(**base_layout(220, True))
            fig_v.update_layout(legend=dict(orientation='h', y=-0.15, font=dict(size=10)))
            fig_v.update_yaxes(tickformat='.0f', ticksuffix=' €')
...
<div style="font-size:10px;color:{C['muted']}">{score_detail}</div>
    ...
    if st.session_state.watchlist:
        sec("Watchlist — Prix cibles", "⭐", C_VAL, C_VAL_DIM)
        wl_data = []
        for w in sorted(st.session_state.watchlist, key=lambda x: -x.get("upside",0)):
            statut = w.get("statut","wait")
            badge = "✅ Sous cible" if statut=="buy" else ("🔴 Surévalué" if statut=="avoid" else "⏳ Zone neutre")
            badge_color = C['green'] if statut=="buy" else (C['red'] if statut=="avoid" else C['gold'])
            upside = w.get("upside",0)
            marge_s = ((w['a_payer'] - w['cours']) / w['cours'] * 100) if w.get('cours',0) > 0 else 0
            wl_data.append({
                "Société": w["name"],
                "Cours (€)": f"{w['cours']:,.0f} €",
                "Cible BASE (€)": f"{w['cible']:,.0f} €",
                "Prix à payer (€)": f"{w['a_payer']:,.0f} €",
                "Upside": f"{upside:+.1f}%",
                "Marge / Prix cible": f"{marge_s:+.1f}%",
                "Statut": badge,
                "Méthode": w.get("method","—"),
            })
        df_wl = pd.DataFrame(wl_data)
        st.dataframe(df_wl, use_container_width=True, hide_index=True, height=min(300, len(wl_data)*38+40))
        ...
        with del_cols[0]:
            names_wl = [w["name"] for w in st.session_state.watchlist]
        ...
        with del_cols[1]:
            if st.button("🗑️ Supprimer", key="valo_del_btn", use_container_width=True):
                st.session_state.watchlist = [w for w in st.session_state.watchlist if w["name"] != to_delete]
                st.rerun()
        ...
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:24px;text-align:center;color:{C['muted']};margin-top:8px">
⭐ Ta watchlist est vide — calcule une valorisation et clique sur "Watchlist" pour l'ajouter
</div>""", unsafe_allow_html=True)
