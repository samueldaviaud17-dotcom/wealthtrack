...
def fetch_market_history(ticker, period, interval):
    ...
    try:
        if period == "1d":
            # Récupère 5j daily pour avoir la clôture veille (référence Yahoo Finance)
            h_day = yf.Ticker(ticker).history(period="5d", interval="1d")
            if h_day is None or len(h_day) < 2: return None
            closes_day = h_day['Close'].dropna().tolist()
            prix_veille = closes_day[-2]  # clôture J-1 = référence Yahoo Finance

            # Série intraday pour sparkline
            _intr_interval = "5m" if ticker != "BTC-USD" else "15m"
            h_intr = yf.Ticker(ticker).history(period="1d", interval=_intr_interval)
            ...
            return {"prix": prix_actuel, "var_pct": var_pct, "serie": serie}
...
tab1, tab5, tab2, tab3, tab4, tab8, tab6, tab7, tab9 = st.tabs(["🏠 Vue d'ensemble", "📊 Performance", "📈 Bourse", "₿ Crypto", "⚙️ Options", "📊 IBKR Analyse", "🔍 Valorisation", "⭐ Watchlist", "🚧 Chantier"])
...
with tab1:
    ...
    # Options : UNIQUEMENT depuis JSON (capital_reel.json) — ZERO lien Sheet
    _ibkr_cr  = load_capital_reel()
    st.session_state['capital_reel'] = _ibkr_cr
    ...
    # ── Marchés (indices + BTC) — façon Google Finance ──
    if YF_AVAILABLE:
        st.markdown("<div style='margin-top:18px'></div>", unsafe_allow_html=True)
        _tf_cols = st.columns([1,1,1,1,1,7])
        if 'mkt_timeframe' not in st.session_state:
            st.session_state['mkt_timeframe'] = "1J"
        for _tcol, _tf in zip(_tf_cols, MARKET_TIMEFRAMES.keys()):
            with _tcol:
                if st.button(_tf, key=f"tf_{_tf}", use_container_width=True,
                              type="primary" if st.session_state['mkt_timeframe']==_tf else "secondary"):
                    st.session_state['mkt_timeframe'] = _tf
                    st.rerun()

        _tf_sel = st.session_state['mkt_timeframe']
        _tf_cfg = MARKET_TIMEFRAMES[_tf_sel]
        _mkt_cols = st.columns(len(MARKET_TICKERS))
        for _mcol, (_mname, _mtk, _micon) in zip(_mkt_cols, MARKET_TICKERS):
            _snap = fetch_market_history(_mtk, _tf_cfg["period"], _tf_cfg["interval"])
            with _mcol:
                if _snap is None:
...
<div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                ...
                st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px 4px 14px">
<div style="font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em">{_micon} {_mname}</div>
<div style="display:flex;align-items:baseline;gap:8px;margin-top:2px">
<span style="font-family:'Space Grotesk';font-size:18px;font-weight:700">{_prix_str}</span>
<span style="font-size:12px;font-weight:700;color:{_vcol}">{'+' if _snap['var_pct']>=0 else ''}{_snap['var_pct']:.2f}%</span>
</div></div>""", unsafe_allow_html=True)
                _r,_g,_b = int(_vcol[1:3],16), int(_vcol[3:5],16), int(_vcol[5:7],16)
                _fig_spark = go.Figure(go.Scatter(y=_snap['serie'], mode='lines',
                    line=dict(color=_vcol, width=2), fill='tozeroy', fillcolor=f"rgba({_r},{_g},{_b},0.12)"))
                _fig_spark.update_layout(height=40, margin=dict(l=0,r=0,t=0,b=0),
                    showlegend=False, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                    xaxis=dict(visible=False), yaxis=dict(visible=False))
                st.plotly_chart(_fig_spark, use_container_width=True, config={'displayModeBar':False}, key=f"spark_{_mname}_{_tf_sel}")
  ...
  <div style="background:{C['border']};border-radius:3px;height:5px">
    ...
    with R:
        ...
        # Capital réel : toujours lire depuis JSON (source de vérité)
        _ibkr_cap_reel = load_capital_reel()
        st.session_state['capital_reel'] = _ibkr_cap_reel
...
Benchmarks" ci-contre).""")
    ...
    with Rp:
        ...
        _benchs=[("🏆 Mon PF (Global)", _tg['tri_global'], C['gold'], True)]
        for _idx_name in ["SP500","CAC 40","STOXX 600"]:
            _b = _tg['benchmarks'].get(_idx_name, {})
            _benchs.append((_b.get('display',_idx_name), _b.get('tri'), C['cyan'], False))

        _valid_vals = [b[1] for b in _benchs if b[1] is not None]
        mx = max(abs(v) for v in _valid_vals) or 1 if _valid_vals else 1
        for bn,bv,bc,bold in _benchs:
...
<span style="font-size:12px;color:{C['muted']}">—</span></div>
<div style="height:6px;background:{C['border']};border-radius:3px"></div></div>""",unsafe_allow_html=True)
                continue
            _bar_col = bc if bv>=0 else C['red']
            st.markdown(f"""<div style="margin-bottom:14px">
<div style="display:flex;justify-content:space-between;margin-bottom:5px">
<span style="font-size:13px;font-weight:{fw};color:{fc}">{bn}</span>
<span style="font-family:'Space Grotesk';font-weight:{fw};color:{_bar_col};font-size:14px">{'+' if bv>=0 else ''}{bv:.2f} %</span></div>
<div style="height:6px;background:{C['border']};border-radius:3px">
<div style="height:100%;width:{abs(bv)/mx*100:.0f}%;background:{_bar_col};border-radius:3px"></div></div></div>""",unsafe_allow_html=True)

        # Alpha vs benchmarks
        mon_pf = _benchs[0][1]
        st.markdown("<div style='margin-top:16px'>",unsafe_allow_html=True)
        sec("Alpha vs Benchmarks","📐","#6EE7B7","#0A1C0F")
        for bn,bv,bc,bold in _benchs[1:]:
            if bv is None or mon_pf is None:
                st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid {C['border']}">
<span style="font-size:12px;color:{C['muted']}">{bn}</span>
<span style="font-size:12px;color:{C['muted']}">—</span></div>""",unsafe_allow_html=True)
                continue
            alpha = mon_pf - bv
...
TRI (XIRR) calculé sur les versements réels : Bourse/Crypto = DCA le 4 de chaque mois depuis
févr. 2022 / août 2022, Options = dépôts IBKR réels. Benchmarks simulés avec les mêmes flux
investis dans chaque indice (SP500 TR, CAC40 GR, STOXX 600). Alpha = TRI Mon PF − TRI Indice (pts %).</span></div>""",unsafe_allow_html=True)
...
with tab7:
    ...
    if "wl_portefeuille" not in st.session_state:
        _wl_gh = load_watchlist_gh()
        st.session_state.wl_portefeuille  = _wl_gh.get("wl_portefeuille", [])
        st.session_state.wl_options       = _wl_gh.get("wl_options", [])
        st.session_state.wl_surveillance  = _wl_gh.get("wl_surveillance", [])
        st.session_state.watchlist = (st.session_state.wl_portefeuille +
                                       st.session_state.wl_options +
                                       st.session_state.wl_surveillance)
...
with tab9:
    ...
    st.markdown(f"<div style='font-size:12px;color:{C['muted']};padding:4px 0 16px 0'>Mémo partagé entre sessions — état des chantiers en cours et à venir.</div>", unsafe_allow_html=True)
...
<div style='font-size:12px;color:{C['text']};line-height:1.7'>{contenu}</div>
    ...
    # ── Priorité ──
    st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin:8px 0 10px 2px'>🔴 Priorité</div>", unsafe_allow_html=True)
    _chantier_card("Export / Backup JSON", "💾",
        """Bouton "exporter mes données" (<code>capital_reel.json</code>, <code>depots_detail</code>,
historique IBKR...) téléchargeable depuis l'app + fonction d'import pour restaurer en cas de perte
serveur. Filet de sécurité, peu coûteux à implémenter.""",
        "#F87171", "À faire en premier")

    # ── Chantiers validés, à planifier ──
    st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin:18px 0 10px 2px'>📋 Chantiers validés — à planifier</div>", unsafe_allow_html=True)

    _chantier_card("Alertes visuelles in-app", "🔔",
        """Bandeau/badge si position options proche ITM, DCA mensuel manqué, etc. Visible à la
prochaine ouverture (usage mobile fréquent confirmé). Pas d'email — nécessiterait un cron externe,
hors scope actuel.""",
        C['cyan'])

    _chantier_card("Historique TRI dans le temps", "📈",
        """Snapshot périodique du TRI Global (granularité à définir ensemble — mensuel probable,
affichage possiblement trimestriel/semestriel pour lisser le bruit).""",
        C['cyan'])

    _chantier_card('Onglet "Fiscalité" dédié', "🧾",
        """Nécessite de tracker l'historique des ventes Bourse/Crypto (date, qté, prix vente, PRU
au moment de la vente, PV/MV réalisée — pas suivi aujourd'hui, contrairement aux Options où
l'historique existe déjà). Calcul PFU/flat tax.""",
        C['purple'], "Gros morceau")

    _chantier_card("Chantier Visuel — Vue d'ensemble", "🎨",
        """Cards "Objectif patrimoine fin d'année" + jauge + "Objectif DCA mensuel" + barre "Année
écoulée" : revoir la mise en page (alignement, tailles, cohérence). Pas encore détaillé précisément
— à affiner ensemble.""",
        C['gold'])

    _chantier_card("Mobile / responsive", "📱",
        """Usage mobile fréquent confirmé → tester les tableaux HTML (<code>overflow-x:auto</code>)
en conditions réelles, ajuster font-size/padding si besoin.""",
        C['blue'])

    _chantier_card("Rebalancing assistant", "⚖️",
        """À creuser ensemble, pas prioritaire actuellement.""",
        C['muted'])

    _chantier_card("Calendrier earnings (positions Bourse/Options)", "📅",
        """Afficher les prochaines dates de résultats pour les tickers détenus
(<code>yf.Ticker.calendar</code>) — utile pour anticiper la volatilité avant de vendre un call
(stratégie de la roue).""",
        C['cyan'])

    _chantier_card("VIX / Fear & Greed Index", "🌡️",
        """Petite card "contexte de marché global" — pertinent pour la roue : vendre des puts
quand la peur est élevée = primes plus grasses. Source possible : yfinance (^VIX) pour le VIX ;
Fear & Greed nécessite une source externe (CNN/Alternative.me) à explorer.""",
        C['gold'])

    _chantier_card("News sur positions détenues", "📰",
        """Source envisagée : <code>yf.Ticker.news</code> (gratuit, pas de clé API, déjà dans
l'écosystème) — retourne titre + lien + source par ticker. Limite : qualité/dispo variable selon
le ticker (parfois vide pour les petites caps type MARA/NIO), flux brut sans filtrage.
Intégration envisagée : section repliable par ticker dans l'onglet Bourse (pas un onglet dédié
au départ — à voir si ça grossit).""",
        C['blue'])

    _chantier_card("Convertisseur EUR/USD live", "💱",
        """<code>eurusd_live</code> est déjà en session_state — à afficher en petit widget dans
l'onglet Options. Optimisation de confort, basse priorité — à traiter après les autres sujets.""",
        C['muted'])

    # ── Chantier de fond ──
    st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin:18px 0 10px 2px'>🗺️ Chantier de fond</div>", unsafe_allow_html=True)

    _chantier_card("Autonomie totale vs Google Sheet", "🗺️",
        """Objectif à moyen terme : remplacer le Sheet par une saisie directe dans l'app (dividendes,
PEE, AV, mouvements, valeurs Bourse/Crypto...), avec archivage JSON des valeurs Sheet actuelles
comme "point zéro", et formulaires Streamlit structurés (pas de saisie en langage naturel — trop
risqué pour du financier). Une fois en place, les retraits deviennent un non-sujet : ils seront
captés nativement comme <code>depots_detail</code> l'est déjà pour IBKR. Question ouverte à
trancher avant de se lancer : le stockage persistant (Streamlit Cloud = filesystem éphémère → JSON
committé via API GitHub, ou SQLite + service externe).
<br><br><b>Dépendances Sheet restantes (Performance / Vue d'ensemble)</b> :
<code>sv(df_pf,6,2/3)</code> Bourse et <code>sv(df_pf,7,2/3)</code> Crypto (investi + valeur
actuelle) — utilisées par le TRI Global et les KPI Performance. Options déjà 100% IBKR/JSON.""",
        "#A78BFA")
    ...
    _chantier_card("Retraits non gérés (TRI Global)", "⚠️",
        """Aucun retrait effectué à ce jour → calcul correct pour l'instant. Si un retrait a lieu un
jour, il faudra : (1) l'ajouter comme cashflow positif dans le calcul TRI global, et (2) simuler une
vente de parts au même montant/date dans chaque indice simulé. Le moteur XIRR gère déjà nativement
les flux positifs intermédiaires — seule la collecte de la donnée "retrait" manque.""",
        "#F59E0B")
