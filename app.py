...
_rbtn = st.columns([7, 1, 1, 1])
with _rbtn[1]:
    # ── Restaurer depuis backup JSON ──
    _restore_file = st.file_uploader("📂", type="json", label_visibility="collapsed",
                                      key="restore_backup_upload")
    if _restore_file is not None and not st.session_state.get("_restore_done"):
        try:
            _bk = json.loads(_restore_file.read())
            if "capital_reel" in _bk:
                _cr_val = float(_bk["capital_reel"])
                st.session_state["capital_reel"] = _cr_val
                save_capital_reel(_cr_val)
            if "watchlist" in _bk:
                st.session_state["wl_portefeuille"] = _bk["watchlist"].get("wl_portefeuille", [])
                st.session_state["wl_options"]      = _bk["watchlist"].get("wl_options", [])
                st.session_state["wl_surveillance"] = _bk["watchlist"].get("wl_surveillance", [])
            ...
            st.toast("✅ Backup restauré avec succès !", icon="💾")
            st.rerun()
        except Exception as _e:
            st.toast(f"❌ Erreur restauration : {_e}", icon="⚠️")
    elif _restore_file is None:
...
# Resync depuis GitHub au démarrage de session (mais pas juste après une restauration)
if st.session_state.get('_cr_loaded') is None and not st.session_state.get('_restore_done'):
    st.session_state['capital_reel'] = load_capital_reel()
...
<div style='font-size:12px;color:{C['text']};line-height:1.7'>{contenu}</div>
    ...
    _chantier_card("1 — Export / Backup JSON", "💾",
        """Bouton "exporter mes données" (capital_reel.json, depots_detail, historique IBKR, watchlist...)
téléchargeable depuis l'app + fonction d'import pour restaurer en cas de perte serveur.
Filet de sécurité ultime — à faire en premier car débloque l'historique TRI (point 5).""",
        "#F87171", "🔴 En cours")
...
<b>7e ⚠️</b> — Mettre à jour le backup JSON (bouton 💾) pour inclure toutes les nouvelles données
saisies manuellement (PEE, AV, dividendes, versements...) afin qu'elles soient exportables et
réimportables via le bouton Restaurer. À faire impérativement avant de couper définitivement le Sheet.<br><br>
