...
_YR = datetime.now().year  # année courante — utilisée dans tous les titres
...
# ── Init watchlist session state (doit être avant tous les onglets) ──────
for _wl_k in ["wl_portefeuille", "wl_options", "wl_surveillance"]:
    if _wl_k not in st.session_state:
        st.session_state[_wl_k] = []
    ...
    with col_res:
        ...
        if hist_d and hist_d.get("cagr_rev") is not None:
            ...
            _score_detail = (
                ...
                f"CAGR Bénéfice net : {hist_d.get('cagr_ni',0):.1f}% → {_sc_ni:.1f}/2.5\n"
                f"Marge nette : {hist_d.get('margin',[0])[0]:.1f}% → {_sc_m:.1f}/2.5\n"
                f"CAGR FCF : {hist_d.get('cagr_fcf',0) or 0:.1f}% → {_sc_fcf:.1f}/2.5\n\n"
                f"Score = somme des 4 composantes sur 10\n"
                f"≥8 Excellente · ≥6 Bonne · ≥4 Correcte · <4 Faible\n"
                f"Basé sur les données historiques Yahoo Finance"
            )
...
cursor:help" title="{_score_detail}">
<div>
  <div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em">
    Score de qualité <span style="opacity:0.6">ⓘ</span>
  </div>
...
<div style="display:flex;align-items:baseline;gap:6px">
  ...
  <span style="font-size:12px;color:{_score_col};font-weight:600">{_score_lbl}</span>
        ...
        def kpi_card_tooltip(title, value, sub, color, icon, tooltip):
            return f"""<div style="background:{C['card']};border:1px solid {C['border']};border-top:2px solid {color};
border-radius:10px;padding:14px 18px;height:118px;box-sizing:border-box;display:flex;flex-direction:column;
position:relative;cursor:help" title="{tooltip}">
<div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">
  {icon} {title}
  <span style="font-size:10px;color:{C['muted']};margin-left:4px;opacity:0.6">ⓘ</span>
</div>
<div style="font-family:'Space Grotesk';font-size:22px;font-weight:700;color:{C['text']};line-height:1.2;flex:1">{value}</div>
<div style="font-size:12px;color:{color};min-height:17px">{sub}</div>
</div>"""
        ...
        _tt_pp = (f"Prix à payer = VI ÷ (1+{_perf_v:.0f}%)^{_n_yr} × (1−{_ms_v:.0f}%) = {pp_base_kpi:,.0f}€\n"
                  f"C'est le prix maximum auquel tu peux acheter aujourd'hui pour obtenir ton rendement cible "
                  f"de {_perf_v:.0f}%/an, avec une marge de sécurité de {_ms_v:.0f}% intégrée.")
        _tt_up = (f"Upside = (Prix à payer − Cours actuel) ÷ Cours actuel\n"
                  f"= ({pp_base_kpi:,.0f}€ − {cours_kpi:,.0f}€) ÷ {cours_kpi:,.0f}€\n"
                  f"Positif = action sous-évaluée vs ton prix cible. Négatif = action trop chère.")
        _tt_vi = (f"Valeur intrinsèque = EPS projeté {_YR+_n_yr} × PER cible {_per_v}x\n"
                  f"C'est la valeur estimée de l'action dans {_n_yr} ans selon le scénario BASE "
                  f"(croissance CA {_g_v:.0f}%/an, PER {_per_v}x).")
        _tt_eps= (f"EPS (Earnings Per Share) = Bénéfice net ÷ Nombre d'actions\n"
                  f"C'est le bénéfice généré par action sur le dernier exercice connu. "
                  f"C'est la base de tous les calculs de projection.")

        kpi_r = st.columns(4)
        _kpi_data = [
            ("PRIX À PAYER BASE", f"{pp_base_kpi:,.0f} €" if pp_base_kpi else "—",
             "avec marge de sécurité",
             C["green"] if upside_kpi and upside_kpi > 0 else C["red"] if upside_kpi is not None else C_VAL,
             "💰", _tt_pp),
            ("UPSIDE BASE", f"{upside_kpi:+.1f}%" if upside_kpi is not None else "—",
             "vs cours actuel",
             C["green"] if upside_kpi and upside_kpi > 0 else C["red"] if upside_kpi is not None else C["muted"],
             "📈", _tt_up),
            ("VALEUR INTRINSÈQUE", f"{vi_base_kpi:,.0f} €" if vi_base_kpi else "—",
             "scénario BASE", "#7DD3FC", "🔍", _tt_vi),
            ("EPS ACTUEL", f"{hist_eps_kpi:.2f}" if hist_eps_kpi else "—",
             "€/$ par action", C["gold"], "📊", _tt_eps),
        ]
        for col, (ti, va, su, co, ic, tt) in zip(kpi_r, _kpi_data):
            with col: st.markdown(kpi_card_tooltip(ti, va, su, co, ic, tt), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
...
<tr style="background:#1C2333;color:{C['muted']};font-size:10px;text-transform:uppercase;letter-spacing:.06em">
  <th style="padding:8px 12px;text-align:left;border-bottom:1px solid {C['border']}">Scénario</th>"""
        ...
        # Comparaison multi-méthodes (EPS/PER, FCF/PER, DCF)
        if method == "EPS / PER" and results.get("BASE") and results["BASE"].get("price"):
            _r_b = results["BASE"]
            _r_bear = results.get("BEAR") or {}
            _r_bull = results.get("BULL") or {}
            sec("Synthèse des valorisations", "📊", C_VAL, C_VAL_DIM)
            _comp_tbl = f"""<div style="overflow-x:auto;margin-bottom:8px">
<table style="width:100%;border-collapse:collapse;font-size:12px;table-layout:fixed">
<thead><tr style="background:#1C2333;color:{C['muted']};font-size:10px;text-transform:uppercase">
  <th style="padding:7px 12px;text-align:left;width:30%;border-bottom:1px solid {C['border']}">Scénario</th>
  <th style="padding:7px 12px;text-align:right;width:23%;border-bottom:1px solid {C['border']}">Valeur intrinsèque</th>
  <th style="padding:7px 12px;text-align:right;width:23%;border-bottom:1px solid {C['border']}">Prix avec perf</th>
  <th style="padding:7px 12px;text-align:right;width:24%;border-bottom:1px solid {C['border']}">Prix à payer</th>
</tr></thead><tbody>"""
            for _scen, _color, _icon in [("BASE","#7DD3FC","🏠"),("BEAR","#F85149","🐻"),("BULL","#3FB950","🐂")]:
                _r = results.get(_scen)
                if _r and _r.get("price"):
                    _vi   = _r.get("price", 0)
                    _pp   = _r.get("price_pay", 0)
                    _pperf= _r.get("price_perf", 0)
                    _up   = ((_pp - cours_v) / cours_v * 100) if cours_v > 0 else 0
                    _upcl = C['green'] if _up > 0 else C['red']
                    _comp_tbl += f"""<tr style="border-bottom:1px solid {C['border']}22">
  <td style="padding:7px 12px;color:{_color};font-weight:700">{_icon} {_scen.capitalize()}</td>
  <td style="padding:7px 12px;text-align:right;color:{C['text']}">{_vi:,.0f} €</td>
  <td style="padding:7px 12px;text-align:right;color:{C['text']}">{_pperf:,.0f} €</td>
  <td style="padding:7px 12px;text-align:right;font-weight:700">
    <span style="color:{_color}">{_pp:,.0f} €</span>
    <span style="font-size:10px;color:{_upcl};margin-left:4px">{_up:+.1f}%</span>
  </td>
</tr>"""
            _comp_tbl += "</tbody></table></div>"
            st.markdown(_comp_tbl, unsafe_allow_html=True)
