...
with tab9:
    ...
    st.markdown(f"<div style='font-size:12px;color:{C['muted']};padding:4px 0 16px 0'>Mémo partagé entre sessions — état des chantiers en cours et à venir. Séquence validée le 28/06/2026 · Dernière mise à jour : 30/06/2026.</div>", unsafe_allow_html=True)
...
<b>tab3 ₿ Crypto</b> → <code>fetch("₿ Crypto Total")</code> + <code>fetch("₿ Crypto")</code> (KPIs, tableau positions, DCA chart)<br>
<b>✅ 8b FAIT (30/06/2026)</b> : Onglet ₿ Crypto 100% autonome du Google Sheet.<br>
— KPIs (Valeur PF €/$, Total Investi, PV/MV, EUR/$) → calculés depuis <code>crypto_positions.json</code> + yfinance<br>
— Tableau portefeuille enrichi (Montant détenu, Investi $, Cours réel $, PRU $, Valeur $/€, PV/MV €, Perf.) → 0 Sheet<br>
— Répartition (donut) → calculée depuis valeurs live yfinance<br>
— Gains Staking → calculés comme (detenu − achete) × prix_live<br>
— DCA Mensuel → lu depuis <code>historique_dca.json</code> (0 Sheet)<br>
— Évolution mensuelle supprimée (nécessitait snapshots Sheet non migrés → Chantier 10)<br>
— Prix cachés 5 min (<code>@st.cache_data(ttl=300)</code>), fallback fx depuis session_state<br><br>
<b>tab1 🏠 Vue d'ensemble</b> → <code>fetch("🏠 Patrimoine")</code> + 4 autres sheets (pat, inv, perf, divs_ytd, évolution mensuelle/annuelle).<br>
⏳ Dépend de Bourse + Crypto résolus avant.<br><br>
<b>tab5 📊 Performance</b> → <code>fetch("🏆 Perf. Totale")</code> pour investi/valeur Bourse et Crypto (pi_b, pl_b, pi_c, pl_c).<br>
⏳ Partiel : XIRR déjà autonome, mais valeur live Bourse manque encore dans GitHub.<br><br>
<b>tab2 📈 Bourse</b> → <code>fetch("📊 Bourse")</code> — positions détenues, cours, PV/MV.<br>
❌ Pas de données positions Bourse dans GitHub — nécessite snapshot initial des positions Bourse.<br><br>
<b>Séquence recommandée</b> :<br>
<b>8a</b> ✅ Terminé — Saisie DCA : historique_dca.json + crypto_positions.json 100% autonomes<br>
<b>8b</b> ✅ Terminé — Onglet ₿ Crypto coupé du Sheet<br>
<b>8c</b> — Couper onglet Performance (Crypto ligne + Bourse ligne) → valeurs live depuis JSON + yfinance<br>
<b>8d</b> — Couper onglet Vue d'ensemble → patrimoine depuis JSON (nécessite 8b+8c)<br>
<b>8e</b> — Couper onglet Bourse → nécessite snapshot initial positions Bourse en JSON<br>
<b>8f</b> — Suppression totale dépendance gspread/Sheet + bouton "📂 Restaurer" backup JSON""",
        "#A78BFA", "En cours")
