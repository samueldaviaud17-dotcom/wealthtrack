...
def fetch_analyst_data(ticker: str):
    ...
    try:
        ...
        return {
            ...
            'div_yield':    round(div_yield * 100, 2) if div_yield else None,   # en %
        ...
        if not _div_list:
            ...
            for _wk_mon in sorted(_div_weeks.keys()):
                ...
                if _is_tw_d:
                    ...
                    _delay_d = f"<span style='font-size:11px;color:#64748B;margin-left:10px'>— dans {_days_to} jour{{'s' if _days_to>1 else ''}}</span>"
                else:
                ...
                for _dow_d in range(5):
                    ...
                    for _dv in _by_day_d[_dow_d]:
                        ...
                        _yld_html = f"<div style='font-size:11px;color:#E2E8F0;margin-top:4px;font-weight:500'>{_dv['yield_pct']:.2f}%/an</div>" if _dv.get('yield_pct') else ""
                        _rate_html = f"<div style='font-size:10px;color:#94A3B8'>{_dv['rate']:.2f}$/an</div>" if _dv.get('rate') else ""
                        _evts_d += (
