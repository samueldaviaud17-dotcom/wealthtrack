...
def _compute_tri_global_et_benchmarks():
    ...
    # ── Flux Bourse & Crypto depuis historique_dca.json (2022-2025) + Sheet 2026 ──
    ...
    for yr, data in _hist.get("bourse", {}).items():
        if yr.startswith("_"): continue
        for i, mois in enumerate(data["mois"]):
            try:
                _d = datetime.strptime(mois + "-04", "%Y-%m-%d")
                _amt = float(data["cto"][i]) + float(data["pea"][i])
                if _amt > 0 and _d < _now:
                    _bourse_cfs.append((_d, _amt))
            except Exception: continue

    for yr, data in _hist.get("crypto", {}).items():
        if yr.startswith("_"): continue
        for i, mois in enumerate(data["mois"]):
            try:
                _d = datetime.strptime(mois + "-04", "%Y-%m-%d")
                _amt = float(data["montant"][i])
                if _amt > 0 and _d < _now:
                    _crypto_cfs.append((_d, _amt))
            except Exception: continue

    # Année en cours 2026 — lu depuis historique_dca.json comme les autres années
    # (les mois futurs sont filtrés par _d < _now plus bas)
