...
with tab1:
    ...
    if YF_AVAILABLE:
        with st.expander("📊 Indices de référence — variation journalière", expanded=True):
...
Alpha = TRI Mon PF − TRI Indice (pts %).</span></div>""", unsafe_allow_html=True)
    ...
    # ── Section 2 : nouveaux graphiques Performance ─────────────
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    _L2, _R2 = st.columns([1, 1])

    # ── Point 2 : Contribution par enveloppe ──
    with _L2:
        sec("Contribution à la PV/MV globale","📊","#6EE7B7","#0A1C0F")
        _contribs = [
            ("Bourse (CTO+PEA)", _pl_b - _pi_b, C['blue']),
            ("Options (IBK)",    _pl_o - _pi_o, C['purple']),
            ("Crypto (Binance)", _pl_c - _pi_c, C['gold']),
        ]
        _contribs.sort(key=lambda x: abs(x[1]), reverse=True)
        _fig_contrib = go.Figure(go.Bar(
            y=[c[0] for c in _contribs],
            x=[c[1] for c in _contribs],
            orientation='h',
            marker_color=[c[2] for c in _contribs],
            text=[f"{'+' if c[1]>=0 else ''}{c[1]:,.0f} €".replace(',', ' ') for c in _contribs],
            textposition='outside',
            textfont=dict(color=C['text'], size=11),
            hovertemplate='<b>%{y}</b><br>%{x:+,.0f} €<extra></extra>'))
        _fig_contrib.update_layout(**base_layout(180, False))
        _fig_contrib.add_vline(x=0, line_color=C['border'], line_width=1)
        _fig_contrib.update_xaxes(tickformat='.0f', ticksuffix=' €')
        st.plotly_chart(_fig_contrib, use_container_width=True,
            config={'displayModeBar': False})

    # ── Point 3 : Évolution mensuelle PV/MV ──
    with _R2:
        sec("Évolution mensuelle PV/MV","📈","#6EE7B7","#0A1C0F")
        _df_em_p = fetch("📅 Évol. Mensuelle")
        _pvmv_m  = [sv(_df_em_p,13+i,9) for i in range(12)]
        _xm = [mois_s[i] for i,v in enumerate(_pvmv_m) if v != 0]
        _ym = [v for v in _pvmv_m if v != 0]
        if _ym:
            _fig_pvmv = go.Figure(go.Bar(
                x=_xm, y=_ym,
                marker_color=[C['green'] if v>=0 else C['red'] for v in _ym],
                text=[f"{'+' if v>=0 else ''}{v:,.0f}€".replace(',', ' ') for v in _ym],
                textposition='outside', textfont=dict(color=C['text'], size=10),
                hovertemplate='<b>%{x}</b><br>%{y:+,.0f} €<extra></extra>'))
            _fig_pvmv.add_hline(y=0, line_color=C['border'], line_width=1)
            _fig_pvmv.update_layout(**base_layout(180))
            _fig_pvmv.update_yaxes(tickformat='.0f', ticksuffix=' €')
            st.plotly_chart(_fig_pvmv, use_container_width=True,
                config={'displayModeBar': False})
        else:
            st.markdown(f"<div style='color:{C["muted"]};font-size:12px;padding:20px'>Pas de données disponibles.</div>", unsafe_allow_html=True)

    # ── Point 4 : Tableau historique annuel ──
    st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
    sec("Historique annuel","📋","#6EE7B7","#0A1C0F")
    _yrs_h  = [str(_YR-4), str(_YR-3), str(_YR-2), str(_YR-1), str(_YR)]
    _pat_h  = [sv(df_ea,11+i,9)  for i in range(5)]
    _inv_h  = [sv(df_ea,11+i,3)  for i in range(5)]
    _pvmv_h = [sv(df_ea,11+i,6)  for i in range(5)]
    _perf_h = [sv(df_ea,11+i,12) for i in range(5)]
    _tbl_hist = "<div style='overflow-x:auto'><table style='width:100%;border-collapse:collapse;font-size:13px'>"
    _tbl_hist += (f"<thead><tr style='background:#111827'>"
        f"<th style='padding:6px 10px;text-align:left;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>Année</th>"
        f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>Investi</th>"
        f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>Patrimoine</th>"
        f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>PV/MV</th>"
        f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>Perf. %</th>"
        "</tr></thead><tbody>")
    for i, yr in enumerate(_yrs_h):
        if _pat_h[i] == 0: continue
        _pv = _pvmv_h[i]; _pf = _perf_h[i]
        _pv_col = C['green'] if _pv >= 0 else C['red']
        _pf_col = C['green'] if _pf >= 0 else C['red']
        _is_cur = yr == str(_YR)
        _bg = "background:#1a1f2e;" if _is_cur else ""
        _fw = "font-weight:700;" if _is_cur else ""
        _tbl_hist += (f"<tr style='border-bottom:1px solid #1F2937;{_bg}'>"
            f"<td style='padding:7px 10px;{_fw}'>{yr}{'  ← en cours' if _is_cur else ''}</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{C["muted"]}'>{fmt(_inv_h[i])}</td>"
            f"<td style='padding:7px 10px;text-align:right;{_fw}'>{fmt(_pat_h[i])}</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{_pv_col};font-weight:600'>{'+' if _pv>=0 else ''}{fmt(_pv)}</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{_pf_col};font-weight:700'>{'+' if _pf>=0 else ''}{_pf:.1f}%</td>"
            "</tr>")
    _tbl_hist += "</tbody></table></div>"
    st.markdown(_tbl_hist, unsafe_allow_html=True)

    # ── Point 5 : Courbe comparative Mon PF vs SP500 ──
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    sec("Évolution 100€ investis — Mon PF vs Indices","📉","#6EE7B7","#0A1C0F")
    _tg_bk  = st.session_state.get('tri_global', {})
    _bench_sp = _tg_bk.get('benchmarks', {}).get('SP500', {})
    _bench_cac = _tg_bk.get('benchmarks', {}).get('CAC 40', {})
    _bench_st  = _tg_bk.get('benchmarks', {}).get('STOXX 600', {})

    if YF_AVAILABLE:
        @st.cache_data(ttl=3600, show_spinner=False)
        def _fetch_normed(tickers_try, start_str, end_str):
            """Retourne série normalisée à 100 depuis start."""
            for tk in tickers_try:
                try:
                    h = yf.Ticker(tk).history(start=start_str, end=end_str, interval="1mo")
                    if h is not None and len(h) >= 3:
                        closes = h['Close'].dropna()
                        base = closes.iloc[0]
                        normed = (closes / base * 100).tolist()
                        dates  = [str(d)[:7] for d in closes.index]
                        return dates, normed
                except Exception:
                    continue
            return None, None

        _start_str  = "2022-02-01"
        _end_str    = datetime.now(_TZ_PARIS).strftime("%Y-%m-%d")

        # PF normalisé depuis évolution patrimoine annuelle (approximation mensuelle via df_em)
        _df_em_c = fetch("📅 Évol. Mensuelle")
        _pats_all = []
        _dates_all = []
        for yr_off in range(4, -1, -1):
            yr_label = str(_YR - yr_off)
            for m_idx in range(12):
                _val = sv(_df_em_c, 13+m_idx, 2) if yr_off == 0 else 0
                # Approximation : on utilise l'évolution annuelle
        _pf_dates, _pf_vals = None, None
        _investi_base = _pi_b + _pi_c + _pi_o
        if _investi_base > 0:
            # Courbe PF = évolution du ratio valeur/investi * 100
            _pf_dates_y = [str(_YR-4), str(_YR-3), str(_YR-2), str(_YR-1), str(_YR)]
            _pf_vals_y  = [sv(df_ea,11+i,9) for i in range(5)]
            _valid_pf = [(d, v) for d, v in zip(_pf_dates_y, _pf_vals_y) if v > 0]
            if _valid_pf:
                _pf_dates = [x[0] for x in _valid_pf]
                _base_pf = _valid_pf[0][1]
                _pf_vals = [v / _base_pf * 100 for _, v in _valid_pf]

        _fig_comp = go.Figure()
        _sp_dates, _sp_vals = _fetch_normed(["^SP500TR","^GSPC"], _start_str, _end_str)
        _cac_dates, _cac_vals = _fetch_normed(["PX1GR.PA","^FCHI"], _start_str, _end_str)
        _stx_dates, _stx_vals = _fetch_normed(["^STOXX","^SXXP"], _start_str, _end_str)

        if _pf_vals:
            _fig_comp.add_trace(go.Scatter(x=_pf_dates, y=_pf_vals, mode='lines+markers',
                name='Mon PF', line=dict(color=C['gold'], width=3),
                hovertemplate='<b>Mon PF</b><br>%{x}<br>%{y:.1f}<extra></extra>'))
        if _sp_vals:
            _fig_comp.add_trace(go.Scatter(x=_sp_dates, y=_sp_vals, mode='lines',
                name=_bench_sp.get('display','SP500 TR'), line=dict(color=C['cyan'], width=2, dash='dot'),
                hovertemplate='<b>SP500 TR</b><br>%{x}<br>%{y:.1f}<extra></extra>'))
        if _cac_vals:
            _fig_comp.add_trace(go.Scatter(x=_cac_dates, y=_cac_vals, mode='lines',
                name=_bench_cac.get('display','CAC40'), line=dict(color='#A78BFA', width=2, dash='dash'),
                hovertemplate='<b>CAC40</b><br>%{x}<br>%{y:.1f}<extra></extra>'))
        if _stx_vals:
            _fig_comp.add_trace(go.Scatter(x=_stx_dates, y=_stx_vals, mode='lines',
                name=_bench_st.get('display','STOXX 600'), line=dict(color='#6B7280', width=2, dash='longdash'),
                hovertemplate='<b>STOXX 600</b><br>%{x}<br>%{y:.1f}<extra></extra>'))

        _fig_comp.add_hline(y=100, line_color=C['border'], line_width=1, line_dash='dot')
        _fig_comp.update_layout(**base_layout(300))
        _fig_comp.update_layout(legend=dict(orientation='h', y=-0.15, font=dict(size=11)))
        _fig_comp.update_yaxes(ticksuffix='', title_text='Base 100 (fév. 2022)')
        st.plotly_chart(_fig_comp, use_container_width=True,
            config={'displayModeBar':True,'scrollZoom':True,'displaylogo':False})
        st.markdown(f"<div style='font-size:10px;color:{C['muted']};font-style:italic'>Base 100 = valeur en février 2022. Mon PF = approximation depuis l'évolution annuelle du patrimoine.</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='color:{C['muted']};font-size:12px;padding:12px'>yfinance non disponible — graphique comparatif indisponible.</div>", unsafe_allow_html=True)

    # ── Point 1 : TRI dans le temps (placeholder — nécessite stockage snapshots) ──
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    sec("Évolution du TRI dans le temps","⚡","#6EE7B7","#0A1C0F")
...
⏳ <b style='color:{C['text']}'>En cours de construction (Chantier 5)</b><br>
Ce graphique nécessite un snapshot mensuel du TRI Global stocké sur GitHub.
Chaque mois, un point sera ajouté automatiquement pour tracer la courbe d'évolution.
Disponible une fois le chantier 5 implémenté.</div>""", unsafe_allow_html=True)
...
GITHUB_REPO, GITHUB_BRANCH.""", "#6EE7B7")
    ...
    _chantier_card("3 — VIX + Calendrier Earnings", "🌡️📅",
        """VIX (^VIX via yfinance) en card "contexte de marché" dans Vue d'ensemble. Calendrier
earnings pour les tickers détenus (yf.Ticker.calendar) dans l'onglet Options. Même session.""",
        C['cyan'])
...
(cron externe hors scope). À faire après stabilisation visuelle de tous les onglets.""",
    ...
    _chantier_card("5 — Historique TRI dans le temps", "📈",
        """Placeholder visible dans l'onglet Performance (section "Évolution du TRI dans le temps").
Nécessite un snapshot mensuel du TRI Global stocké sur GitHub — un point ajouté chaque mois.
<b>Prérequis</b> : mécanisme d'écriture mensuelle automatique sur GitHub (ou déclenchement
manuel au 1er du mois). À implémenter.""",
        C['gold'], "🔶 Placeholder en place")
...
Bourse. Qualité variable selon les tickers (parfois vide pour petites caps type MARA/NIO).""",
        C['blue'])
