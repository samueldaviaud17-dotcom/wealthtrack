...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        _pm_rachats_usd = sum(abs(_t['produit']) for _t in _trades_mois if _t.get('quantite',0) > 0)
        _pm_frais_eur   = sum(abs(_t.get('frais',0)) for _t in _trades_mois)
        _pm_net_eur     = (_pm_brutes_usd - _pm_rachats_usd) / _fx_an - _pm_frais_eur
        _pm_nb_trades   = len(_trades_mois)

        # ── Top 5 tickers de la période ────────────────────────
        _ticker_mois = {}
        for _t in _trades_mois:
            _tk = _t.get('ticker','')
            if not _tk: continue
            _prod = _t.get('produit', 0)
            _sign = 1 if _t.get('quantite',0) < 0 else -1
            _ticker_mois[_tk] = _ticker_mois.get(_tk, 0) + _sign * _prod
  ...
  <div style='display:flex;justify-content:space-between;align-items:center'>
    ...
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
        ...
        _pm_brutes_net = (_pm_brutes_usd - _pm_rachats_usd) / _fx_an
        _pm_brutes_usd_net = _pm_brutes_usd - _pm_rachats_usd
        ...
        _rapport_html = (
            ...
            f"<div style='font-size:11px;color:#8B949E;margin-top:3px'>≈ ${_pm_brutes_usd_net:.2f}</div>"
            f"</div>"
            f"<div style='background:#161B22;border-radius:8px;padding:12px 14px'>"
