...
with tab1:
    ...
    _opt_sheet_val = sv(df_p, 21, 20)  # valeur Options IBK dans le Sheet (ligne 21, col T = index 20)
    _opt_sheet_inv = sv(df_pf, 8, 2)  # capital investi Options Sheet (Perf. Totale ligne 8 col 2)
    if _ibkr_cr > 0:
        pat = pat - _opt_sheet_val + _ibkr_cr   # remplacer valeur Options Sheet par Capital Réel
    if _ibkr_inv > 0:
        inv = inv - _opt_sheet_inv + _ibkr_inv  # remplacer investi Options Sheet par capital investi HTML
