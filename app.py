...
with tab1:
    ...
    # Options : UNIQUEMENT depuis session_state (capital_reel.json) — ZERO lien Sheet
...
with tab4:
    ...
    with _cr_col1:
        ...
        # Composant HTML autonome avec crayon dans le coin et popup au centre
        _cr_component_html = f"""
<div style="font-family:'Space Grotesk',sans-serif;position:relative;width:100%">
  <div id="cr-card" style="position:relative;background:#161B22;border:1px solid #30363D;
    border-top:2px solid #1D9E75;border-radius:10px;padding:14px 18px;height:118px;
    box-sizing:border-box;display:flex;flex-direction:column">
    <div style="font-size:10px;color:#8B949E;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">
      🏦 CAPITAL RÉEL
    </div>
    <div id="cr-display" style="font-size:22px;font-weight:700;color:#E6EDF3;line-height:1.2;flex:1">
      {_cr_val:,.2f} €
    </div>
    <div style="font-size:12px;color:#1D9E75">← modifiable</div>
    <button onclick="openModal()" title="Modifier"
      style="position:absolute;top:10px;right:10px;background:transparent;border:none;
      color:#8B949E;font-size:18px;cursor:pointer;padding:4px 6px;border-radius:6px;
      line-height:1;transition:color .15s,background .15s"
      onmouseover="this.style.color='#E6EDF3';this.style.background='#21262D'"
  ...
  <!-- Overlay modal -->
  <div id="cr-overlay" onclick="closeModal(event)"
    ...
    <div onclick="event.stopPropagation()"
      ...
      padding:24px;width:300px;box-shadow:0 8px 32px rgba(0,0,0,.6)">
      <div style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:4px">
        ✏️ Modifier le capital réel
      </div>
      <div style="font-size:12px;color:#8B949E;margin-bottom:16px">
        Valeur actuelle : <b style="color:#E6EDF3">{_cr_val:,.2f} €</b>
      </div>
      <input id="cr-input" type="number" step="0.01" value="{_cr_val:.2f}"
        style="width:100%;box-sizing:border-box;background:#0D1117;border:1px solid #30363D;
        border-radius:8px;padding:8px 12px;color:#E6EDF3;font-size:15px;margin-bottom:12px;
        font-family:'Space Grotesk',sans-serif"
        onkeydown="if(event.key==='Enter')saveVal()"/>
      <div style="display:flex;gap:8px">
        <button onclick="saveVal()"
          style="flex:1;background:#1D9E75;border:none;border-radius:8px;color:white;
          padding:9px 0;cursor:pointer;font-size:13px;font-weight:600;font-family:'Space Grotesk',sans-serif">
          ✓ Enregistrer
        </button>
        <button onclick="closeModal()"
          style="flex:1;background:transparent;border:1px solid #30363D;border-radius:8px;
          color:#8B949E;padding:9px 0;cursor:pointer;font-size:13px;font-family:'Space Grotesk',sans-serif">
          Annuler
        </button>
      </div>
    </div>
...
<script>
function openModal() {{
  var o = document.getElementById('cr-overlay');
  o.style.display = 'flex';
  setTimeout(function(){{ document.getElementById('cr-input').focus(); }}, 50);
}}
function closeModal(e) {{
  if (!e || e.target === document.getElementById('cr-overlay'))
    document.getElementById('cr-overlay').style.display = 'none';
}}
function saveVal() {{
  var v = parseFloat(document.getElementById('cr-input').value);
  if (isNaN(v) || v < 0) return;
  // Formatter FR
  var fmt = v.toLocaleString('fr-FR', {{minimumFractionDigits:2, maximumFractionDigits:2}}) + ' €';
  document.getElementById('cr-display').textContent = fmt;
  document.getElementById('cr-overlay').style.display = 'none';
  // Envoyer la valeur à Streamlit via query param
...
"""
        ...
        _new_val = _st_components.html(_cr_component_html, height=135, scrolling=False)

        # Si une nouvelle valeur est retournée par le composant
        if _new_val is not None and float(_new_val) != _cr_val:
            st.session_state['capital_reel'] = float(_new_val)
            save_capital_reel(float(_new_val))
            st.rerun()
