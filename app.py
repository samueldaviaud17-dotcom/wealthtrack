...
with tab1:
    ...
    # Objectif patrimoine annuel (col 7 de Patrimoine ligne 5)
    _obj_pat = n(v(df_p, 5, 7)) if df_p.shape[1] > 7 else 0
    _pct_obj = (pat / _obj_pat * 100) if _obj_pat > 0 else 0

    # DCA mensuel objectif et réalisé
    _dca_obj  = n(v(df_em, 5, 6)) if df_em.shape[1] > 6 else 0   # col 6 = objectif DCA
    _dca_real = n(v(df_em, 5, 3)) if df_em.shape[1] > 3 else 0   # col 3 = réalisé YTD

    _g1, _g2 = st.columns(2)
    with _g1:
        # Gauge progression annuelle
        _fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=_pct_obj if _obj_pat > 0 else _pct_yr,
            title=dict(text="Progression annuelle" if _obj_pat > 0 else f"Année {_YR} écoulée", font=dict(size=12, color=C['muted'])),
            delta=dict(reference=_pct_yr, valueformat=".1f", suffix="%",
                       increasing=dict(color=C['green']), decreasing=dict(color=C['red'])) if _obj_pat > 0 else None,
            number=dict(suffix="%", font=dict(size=22, color=C['text'], family='Space Grotesk'), valueformat=".1f"),
            gauge=dict(
                axis=dict(range=[0, 100], tickfont=dict(size=9, color=C['muted'])),
                bar=dict(color=C['green'] if (_pct_obj if _obj_pat>0 else _pct_yr) >= _pct_yr else C['gold'], thickness=0.25),
                bgcolor=C['card'],
                borderwidth=0,
                steps=[
                    dict(range=[0, _pct_yr], color='rgba(31,111,235,0.1)'),
                    dict(range=[_pct_yr, 100], color='rgba(0,0,0,0)')
                ],
                threshold=dict(line=dict(color=C['blue'], width=2), thickness=0.8, value=_pct_yr)
            )
        ))
        _fig_gauge.update_layout(**base_layout(180, False))
        _fig_gauge.update_layout(margin=dict(l=20, r=20, t=30, b=10))
        st.plotly_chart(_fig_gauge, use_container_width=True, config={'displayModeBar': False})
        if _obj_pat > 0:
            _diff = pat - _obj_pat
            _col = C['green'] if _diff >= 0 else C['red']
            st.markdown(f"<div style='text-align:center;font-size:11px;color:{C['muted']};margin-top:-8px'>Objectif : <b style='color:{C['text']}'>{fmt(_obj_pat)}</b> · écart : <span style='color:{_col};font-weight:700'>{'+' if _diff>=0 else ''}{fmt(_diff)}</span></div>", unsafe_allow_html=True)

    with _g2:
        # KPI DCA mensuel objectif vs réalisé
        _dca_mois_obj = 600  # ton DCA cible mensuel (350+120+100+30 env.)
        # Calcul du DCA moyen mensuel réalisé sur les mois écoulés
        ...
        _dca_c_real  = sum(n(v(fetch("₿ Crypto"),20+i,2)) for i in range(12) if n(v(fetch("₿ Crypto"),20+i,2)) > 0) if True else 0
        _dca_total_real = _dca_b_real + primes_ytd
        _pct_dca = min((_dca_total_real / (_dca_mois_obj * max(_nb_mois,1))) * 100, 150) if _dca_mois_obj > 0 else 0

        st.markdown(f"""
<div style="background:{C['card']};border:1px solid {C['border']};border-radius:10px;padding:16px;height:160px;display:flex;flex-direction:column;justify-content:space-between">
  <div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em">DCA Mensuel moyen réalisé</div>
  <div>
    <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{C['text']}">{fmt(_dca_total_real / max(_nb_mois,1))}<span style="font-size:13px;color:{C['muted']}"> / mois</span></div>
    <div style="font-size:11px;color:{C['muted']};margin-top:2px">sur {_nb_mois} mois · total {fmt(_dca_total_real)}</div>
  </div>
  <div>
    <div style="display:flex;justify-content:space-between;font-size:10px;color:{C['muted']};margin-bottom:4px">
      <span>Objectif ~{_dca_mois_obj}€/mois</span>
      <span style="color:{C['green'] if _pct_dca>=100 else C['gold']};font-weight:700">{_pct_dca:.0f}%</span>
    </div>
    <div style="background:{C['border']};border-radius:4px;height:6px;overflow:hidden">
      <div style="background:{C['green'] if _pct_dca>=100 else C['gold']};height:100%;width:{min(_pct_dca,100):.0f}%;border-radius:4px;transition:width .3s"></div>
    </div>
  </div>
...
with tab6:
    ...
    with col_res:
        ...
        for col, (ti, va, su, co, ic) in zip(kpi_r, kpi_valo):
            with col: st.markdown(card(ti, va, su, co, ic), unsafe_allow_html=True)
        # Score qualité synthétique sur 10
        if hist_d and hist_d.get("cagr_rev") is not None:
            _sc_rev  = min(hist_d.get("cagr_rev",0) or 0, 30) / 30 * 2.5    # CAGR CA → /2.5
            _sc_ni   = min(hist_d.get("cagr_ni",0) or 0, 30) / 30 * 2.5     # CAGR BN → /2.5
            _sc_m    = min(hist_d.get("margin",[0])[0] or 0, 40) / 40 * 2.5  # Marge nette → /2.5
            _sc_fcf  = min(hist_d.get("cagr_fcf",0) or 0, 30) / 30 * 2.5    # CAGR FCF → /2.5
            ...
            st.markdown(f"""<div style="background:{C['card']};border:1px solid {_score_col}44;border-radius:10px;padding:12px 16px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between">
<div>
  <div style="font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.06em">Score de qualité</div>
  <div style="font-size:9px;color:{C['muted']};margin-top:2px">CA · BN · Marge · FCF</div>
</div>
