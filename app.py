...
with tab_budget:
    ...
    def _save_budget(data, sha, msg="Budget update"):
    ...
    _MOIS_LABELS = {
        "2026-01":"Janvier","2026-02":"Février","2026-03":"Mars",
        "2026-04":"Avril","2026-05":"Mai","2026-06":"Juin",
        "2026-07":"Juillet","2026-08":"Août","2026-09":"Septembre",
        "2026-10":"Octobre","2026-11":"Novembre","2026-12":"Décembre"
    }
    _TYPES = ["Revenus","Dépenses","Frais Fixes","Épargne & Investissements","Crédits / Dettes"]
    _TYPE_COLORS = {
        "Revenus":"#10B981","Dépenses":"#F87171",
        "Frais Fixes":"#FBBF24","Épargne & Investissements":"#818CF8","Crédits / Dettes":"#F59E0B"
    }
    _CAT_SUGGESTIONS = {
        "Revenus": ["Salaire","Remboursement","Prime","Virement entrant","Autre revenu"],
        "Dépenses": ["Course Alimentaire","Cantine Travail","Transport","Extra","Coiffeur","Osteo","Restaurant","Sport","Loisirs","Voyages"],
        "Frais Fixes": ["Loyer","Électricité","Internet","Téléphone mobile","Assurance habitation","Assurance Voiture","Frais Bancaire"],
        "Épargne & Investissements": ["Trade Republic","PEA Bourso","Crypto Binance","Interactive Broker","PEL Crédit Mutuelle","Livret A"],
        "Crédits / Dettes": ["Crédit voiture","Prêt personnel","Remboursement dette"]
    }

    _now_p = datetime.now(_TZ_PARIS)
    _mois_cur_key = _now_p.strftime("%Y-%m")
    _mois_opts_rev = sorted([k for k in list(_MOIS_LABELS.keys()) if k <= _mois_cur_key], reverse=True)

    # ── Sélecteur + header ──
    _sel_mois = st.selectbox("Mois affiché", _mois_opts_rev,
        format_func=lambda k: _MOIS_LABELS.get(k, k), key="budget_mois_sel")
    _mois_data = _bud_data.get("mois", {}).get(_sel_mois, {"transactions": [], "budget_prevu": {}, "resume_global": {}})
    _txs   = _mois_data.get("transactions", [])
    _bprevu = _mois_data.get("budget_prevu", {})
    _rg    = _mois_data.get("resume_global", {})

    # Calculs réel depuis transactions
    def _reel_par_cat(type_filter):
        d = {}
        for t in _txs:
            if t['type'] == type_filter:
                d[t['categorie']] = d.get(t['categorie'], 0) + t['montant']
        return d

    _rev_reel  = sum(t['montant'] for t in _txs if t['type'] == 'Revenus')
    _dep_reel  = sum(t['montant'] for t in _txs if t['type'] == 'Dépenses')
    _ff_reel   = sum(t['montant'] for t in _txs if t['type'] == 'Frais Fixes')
    _epa_reel  = sum(t['montant'] for t in _txs if t['type'] == 'Épargne & Investissements')
    _cred_reel = sum(t['montant'] for t in _txs if t['type'] == 'Crédits / Dettes')
    _rest_reel = _rev_reel - _dep_reel - _ff_reel - _epa_reel - _cred_reel

    _rev_prevu  = sum(_bprevu.get("revenus",{}).values())
    _dep_prevu  = sum(_bprevu.get("depenses",{}).values())
    _ff_prevu   = sum(v["prevu"] if isinstance(v, dict) else v for v in _bprevu.get("frais_fixes",{}).values())
    _epa_prevu  = sum(_bprevu.get("epargne",{}).values())
    _rest_prevu = _rev_prevu - _dep_prevu - _ff_prevu - _epa_prevu

    # ── KPIs ──
    _kc = st.columns(6)
    for _col, (_label, _prevu, _reel, _clr, _ic) in zip(_kc, [
        ("REVENUS",   _rev_prevu,  _rev_reel,  "#10B981","💶"),
        ("DÉPENSES",  _dep_prevu,  _dep_reel,  "#F87171","🛒"),
        ("FRAIS FIXES",_ff_prevu,  _ff_reel,   "#FBBF24","🏠"),
        ("ÉPARGNE",   _epa_prevu,  _epa_reel,  "#818CF8","💰"),
        ("CRÉDITS",   0,           _cred_reel, "#F59E0B","💳"),
        ("RESTANT",   _rest_prevu, _rest_reel, "#10B981" if _rest_reel >= 0 else "#F87171","🏦"),
    ]):
        with _col:
            _sub = f"Prévu : {_prevu:,.0f}€" if _prevu else ""
            st.markdown(card(_label, f"{_reel:,.0f} €", _sub, _clr, _ic), unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Helper : table PRÉVU vs RÉEL avec barre ──
    def _progress_bar(pct, color):
        pct_c = min(pct, 100)
        over = pct > 100
        bar_c = "#F87171" if over else color
        return (f"<div style='background:#1F2937;border-radius:4px;height:6px;width:100%;margin-top:3px'>"
                f"<div style='background:{bar_c};width:{pct_c:.0f}%;height:6px;border-radius:4px'></div></div>")

    def _table_prevu_reel(titre, color, categories_prevu, categories_reel, icon=""):
        rows = []
        all_cats = list(categories_prevu.keys())
        reel_cats = set(categories_reel.keys())
        extra = [c for c in reel_cats if c not in all_cats]
        for c in extra: all_cats.append(c)
        for cat in all_cats:
            prevu = categories_prevu.get(cat, 0)
            if isinstance(prevu, dict): prevu = prevu.get("prevu", 0)
            reel  = categories_reel.get(cat, 0)
            pct   = (reel / prevu * 100) if prevu > 0 else (100 if reel > 0 else 0)
            rows.append((cat, prevu, reel, pct))
        if not rows: return ""
        tbl = (f"<table style='width:100%;border-collapse:collapse;font-size:12px'>"
               ...
               f"<th style='padding:6px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>PRÉVU</th>"
               f"<th style='padding:6px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉEL</th>"
               f"<th style='padding:6px 8px;text-align:left;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER};width:120px'>PROGRESSION</th>"
               "</tr></thead><tbody>")
        tot_p = tot_r = 0
        for cat, prevu, reel, pct in rows:
            ...
            tbl += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:5px 8px;font-weight:500'>{cat}</td>"
                    f"<td style='padding:5px 8px;text-align:right;color:{_C_MUTED}'>{prevu:,.0f}€</td>"
                    f"<td style='padding:5px 8px;text-align:right;font-weight:600;color:{bar_c}'>{reel:,.0f}€</td>"
                    f"<td style='padding:5px 8px'>{_progress_bar(pct, color)}"
                    f"<span style='font-size:9px;color:{bar_c}'>{pct_lbl}</span></td>"
                    "</tr>")
        tot_pct = (tot_r / tot_p * 100) if tot_p > 0 else 0
        tot_c = "#F87171" if tot_pct > 100 else color
        tbl += (f"<tr style='background:#111827;font-weight:700'>"
                ...
                f"<td style='padding:6px 8px'>{_progress_bar(tot_pct, color)}"
                f"<span style='font-size:9px;color:{tot_c}'>{tot_pct:.0f}%</span></td>"
                "</tr></tbody></table>")
        return tbl

    # ── Layout : 3 colonnes de tables + formulaire ──
    _col_budget1, _col_budget2 = st.columns([2, 1])

    with _col_budget1:
        # Revenus
        st.markdown(_table_prevu_reel("REVENUS", "#10B981",
            _bprevu.get("revenus", {}), _reel_par_cat("Revenus"), "💶"), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        # Dépenses
        st.markdown(_table_prevu_reel("DÉPENSES VARIABLES", "#F87171",
            _bprevu.get("depenses", {}), _reel_par_cat("Dépenses"), "🛒"), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        # Frais Fixes
        _ff_prevu_simple = {k: (v["prevu"] if isinstance(v, dict) else v) for k, v in _bprevu.get("frais_fixes", {}).items()}
        st.markdown(_table_prevu_reel("FRAIS FIXES", "#FBBF24",
            _ff_prevu_simple, _reel_par_cat("Frais Fixes"), "🏠"), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        # Épargne
        st.markdown(_table_prevu_reel("ÉPARGNE & INVESTISSEMENTS", "#818CF8",
            _bprevu.get("epargne", {}), _reel_par_cat("Épargne & Investissements"), "💰"), unsafe_allow_html=True)

    with _col_budget2:
        # ── Graphique répartition budget ──
        sec("Répartition du budget", "🍩", _BUD_COLOR, _BUD_BG)
        _budget_totals = [
            ("Dépenses", _dep_reel, "#F87171"),
            ("Frais Fixes", _ff_reel, "#FBBF24"),
            ("Épargne", _epa_reel, "#818CF8"),
            ("Crédits", _cred_reel, "#F59E0B"),
        ]
        _bt_vals = [x[1] for x in _budget_totals if x[1] > 0]
        _bt_lbls = [x[0] for x in _budget_totals if x[1] > 0]
        _bt_clrs = [x[2] for x in _budget_totals if x[1] > 0]
        if _bt_vals:
            _fig_bud = go.Figure(go.Pie(labels=_bt_lbls, values=_bt_vals, hole=0.55,
                marker=dict(colors=_bt_clrs, line=dict(color=_C_BG, width=2)),
                textinfo='percent+label', textfont=dict(size=10, color='white')))
            _total_dep_all = sum(_bt_vals)
            _fig_bud.add_annotation(text=f"<b>{_total_dep_all:,.0f}€</b>", x=0.5, y=0.5,
                font=dict(size=13, color=_C_TEXT), showarrow=False)
            _fig_bud.update_layout(**base_layout(230, False))
            _fig_bud.update_layout(showlegend=False, margin=dict(l=0,r=0,t=10,b=0))
            st.plotly_chart(_fig_bud, use_container_width=True, config={'displayModeBar':False})

        # ── Résumé global Prévu vs Réel ──
        sec("Résumé Prévu vs Réel", "📊", _BUD_COLOR, _BUD_BG)
        _resume_rows = [
            ("+ Total Revenus", _rev_prevu, _rev_reel, "#10B981"),
            ("− Dépenses",      _dep_prevu, _dep_reel, "#F87171"),
            ("− Frais Fixes",   _ff_prevu,  _ff_reel,  "#FBBF24"),
            ("− Épargne",       _epa_prevu, _epa_reel, "#818CF8"),
            ("= RESTANT",       _rest_prevu, _rest_reel, "#10B981" if _rest_reel >= 0 else "#F87171"),
        ]
        _tbl_res = f"<table style='width:100%;border-collapse:collapse;font-size:12px'>"
        _tbl_res += f"<thead><tr style='background:#111827'><th style='padding:5px 8px;text-align:left;font-size:10px;color:{_C_MUTED}'>CATÉGORIE</th><th style='text-align:right;padding:5px 8px;font-size:10px;color:{_C_MUTED}'>PRÉVU</th><th style='text-align:right;padding:5px 8px;font-size:10px;color:{_C_MUTED}'>RÉEL</th></tr></thead><tbody>"
        for _lbl, _pv, _rv, _rc in _resume_rows:
            _bold = "font-weight:700;border-top:2px solid #374151" if "RESTANT" in _lbl else ""
            _tbl_res += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                f"<td style='padding:5px 8px;color:{_C_TEXT};{_bold}'>{_lbl}</td>"
                f"<td style='padding:5px 8px;text-align:right;color:{_C_MUTED}'>{_pv:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;font-weight:600;color:{_rc};{_bold}'>{_rv:,.0f}€</td>"
                "</tr>")
        _tbl_res += "</tbody></table>"
        st.markdown(_tbl_res, unsafe_allow_html=True)
        ...
        with st.form("form_budget_tx", clear_on_submit=True):
            ...
            _sep = " · "
            _fb_cats = _CAT_SUGGESTIONS.get(_fb_type, [])
            if _fb_cats:
                st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:-8px 0 8px 0'>{_sep.join(_fb_cats[:5])}</div>", unsafe_allow_html=True)
            _fb_amt  = st.number_input("Montant (€)", min_value=0.0, step=0.01, format="%.2f", key="fb_amt")
            _fb_desc = st.text_input("Description (optionnel)", key="fb_desc")
            _fb_sub  = st.form_submit_button("💾 Ajouter", use_container_width=True)

        if _fb_sub:
            if _fb_cat.strip() and _fb_amt > 0:
                _new_tx = {"date": str(_fb_date), "type": _fb_type, "categorie": _fb_cat.strip(),
                           "montant": round(_fb_amt, 2), "description": _fb_desc.strip()}
                _mk = _fb_date.strftime("%Y-%m")
                if _mk not in _bud_data["mois"]:
                    _bud_data["mois"][_mk] = {"nom": _MOIS_LABELS.get(_mk, _mk), "transactions": [], "budget_prevu": {}}
                _bud_data["mois"][_mk]["transactions"].append(_new_tx)
                _bud_data["mois"][_mk]["transactions"].sort(key=lambda x: x["date"])
                if _save_budget(_bud_data, _bud_sha):
                    st.success(f"✅ {_fb_cat} — {_fb_amt:.2f}€")
                    _load_budget.clear()
                else:
                    st.error("❌ Erreur GitHub")
            else:
                st.warning("Catégorie et montant requis.")

    # ── Résumé annuel + Transactions ──
    st.markdown("<hr style='border-color:#1F2937;margin:24px 0'>", unsafe_allow_html=True)
    _ann_col1, _ann_col2 = st.columns([1, 1])

    with _ann_col1:
        sec("Résumé annuel 2026", "📅", _BUD_COLOR, _BUD_BG)
        _ann_rows = []
        for _mk in sorted(_bud_data.get("mois", {}).keys()):
            _mtxs = _bud_data["mois"][_mk].get("transactions", [])
            _mr   = sum(t['montant'] for t in _mtxs if t['type'] == 'Revenus')
            _md   = sum(t['montant'] for t in _mtxs if t['type'] == 'Dépenses')
            _mf   = sum(t['montant'] for t in _mtxs if t['type'] == 'Frais Fixes')
            _me   = sum(t['montant'] for t in _mtxs if t['type'] == 'Épargne & Investissements')
            _mrest = _mr - _md - _mf - _me
            _ann_rows.append((_MOIS_LABELS.get(_mk, _mk), _mr, _md, _mf, _me, _mrest))
        _tbl_ann = f"<table style='width:100%;border-collapse:collapse;font-size:11px'>"
        _tbl_ann += f"<thead><tr style='background:#111827'>"
        for _h, _hc in [("Mois",_C_MUTED),("Revenus","#10B981"),("Dépenses","#F87171"),("FF","#FBBF24"),("Épargne","#818CF8"),("Restant",_C_MUTED)]:
            _tbl_ann += f"<th style='padding:5px 8px;text-align:right;font-size:10px;color:{_hc};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
        _tbl_ann += "</tr></thead><tbody>"
        _tot = [0.0]*5
        for _nom, _r, _d, _f, _e, _rest in _ann_rows:
            for i, v in enumerate([_r, _d, _f, _e, _rest]): _tot[i] += v
            _rc = "#10B981" if _rest >= 0 else "#F87171"
            _tbl_ann += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                f"<td style='padding:5px 8px;font-weight:600'>{_nom}</td>"
                f"<td style='padding:5px 8px;text-align:right;color:#10B981'>{_r:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;color:#F87171'>{_d:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;color:#FBBF24'>{_f:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;color:#818CF8'>{_e:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;font-weight:700;color:{_rc}'>{_rest:,.0f}€</td>"
                "</tr>")
        _tr_c = "#10B981" if _tot[4] >= 0 else "#F87171"
        _tbl_ann += (f"<tr style='background:#111827;font-weight:700;border-top:2px solid {_C_BORDER}'>"
            f"<td style='padding:6px 8px'>TOTAL YTD</td>"
            f"<td style='padding:6px 8px;text-align:right;color:#10B981'>{_tot[0]:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;color:#F87171'>{_tot[1]:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;color:#FBBF24'>{_tot[2]:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;color:#818CF8'>{_tot[3]:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;font-weight:700;color:{_tr_c}'>{_tot[4]:,.0f}€</td>"
            "</tr></tbody></table>")
        st.markdown(_tbl_ann, unsafe_allow_html=True)

    with _ann_col2:
        sec("Transactions du mois", "📋", _BUD_COLOR, _BUD_BG)
        _type_filter = st.multiselect("Filtrer", _TYPES, default=_TYPES, key="bud_filter", label_visibility="collapsed")
        _txs_f = sorted([t for t in _txs if t['type'] in _type_filter], key=lambda x: x['date'], reverse=True)
        if _txs_f:
            _tbl_tx = f"<table style='width:100%;border-collapse:collapse;font-size:11px'>"
            _tbl_tx += f"<thead><tr style='background:#111827'>"
            for _h in ["Date","Catégorie","Montant"]:
                _tbl_tx += f"<th style='padding:4px 6px;text-align:left;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
            _tbl_tx += "</tr></thead><tbody>"
            for _t in _txs_f[:40]:
                _tc = _TYPE_COLORS.get(_t['type'], "#fff")
                _sc = "#10B981" if _t['type'] == 'Revenus' else "#F87171"
                _sign = "+" if _t['type'] == 'Revenus' else "-"
                _tbl_tx += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:4px 6px;color:{_C_MUTED};font-size:10px'>{_t['date'][5:]}</td>"
                    f"<td style='padding:4px 6px'><span style='font-size:9px;background:{_tc}22;color:{_tc};padding:1px 4px;border-radius:3px'>{_t['categorie']}</span>"
                    + (f"<span style='font-size:9px;color:{_C_MUTED};margin-left:4px'>{_t['description'][:20]}</span>" if _t.get('description') else "")
                    + f"</td><td style='padding:4px 6px;text-align:right;font-weight:600;color:{_sc}'>{_sign}{abs(_t['montant']):.0f}€</td></tr>")
            _tbl_tx += "</tbody></table>"
            st.markdown(_tbl_tx, unsafe_allow_html=True)
        else:
            st.info("Aucune transaction pour ce mois.")
