...
Si ça ne fonctionne pas correctement, cet onglet peut être supprimé sans impact sur le reste de l'app.</div>""",
    ...
    if not YF_AVAILABLE:
        ...
        for idx_name, tk_list in _tickers_idx.items():
            ...
            if df_h is not None:
                _debug_rows.append({"Indice": idx_name, "Ticker utilisé": used_tk,
                                     "Lignes": len(df_h),
                                     "Première date": df_h.index[0].strftime("%Y-%m-%d"),
                                     "Dernière date": df_h.index[-1].strftime("%Y-%m-%d"),
                                     "Statut": "✅ OK"})
            else:
                _debug_rows.append({"Indice": idx_name, "Ticker utilisé": "—",
        ...
        for idx_name in ["SP500", "CAC 40", "STOXX 600"]:
            ...
            if df_h is None:
                _results.append({"Portefeuille / Indice": idx_name, "TRI simulé": "—", "Valeur": "—",
            ...
            if px_now and parts > 0 and n_ok >= 2:
                ...
                tri = xirr(cfs)
                _results.append({"Portefeuille / Indice": idx_name,
