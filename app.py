...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        # FX : utiliser le taux du fichier de l'année sélectionnée
        # Si indisponible, fallback sur le taux live
        _fx_yr_data = st.session_state['ibkr_data'].get(_rpt_yr, {})
        _fx_rpt = _fx_yr_data.get('fx', _fx_an)
        if not (0.8 < _fx_rpt < 2.0): _fx_rpt = _fx_an
