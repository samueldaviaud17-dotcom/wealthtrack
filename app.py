                ...
                if YF_AVAILABLE:
                    with st.spinner(f"📡 Récupération {ticker_input.upper()}..."):
                        try:
                            ...
                            hist_data = {}
                            try:
                                inc = tk.financials   # annual income statement (cols = dates récentes en premier)
                                ...
                                if rev_row is not None:
                                    # ── Toujours exactement 5 ans, années dynamiques ──
                                    N = min(5, len(rev_row))  # 5 ans max, ou moins si dispo
