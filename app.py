...
with _hcols[1]:
    if st.button("🔄 Rafraîchir", key="global_refresh_btn", use_container_width=True,
        help="Rafraîchit les données Google Sheet, les cours live et l'heure de synchro"):
...
<div style="font-size:13px">Calcule une valorisation dans l'onglet <b>🔍 Valorisation</b>, choisis une liste et clique sur <b>⭐ Watchlist</b></div>
        ...
        # ── Barre contrôles compacte (1 seule ligne) ─
        last_refresh = st.session_state.get("watchlist_last_refresh", "—")
        upd_count    = st.session_state.get("watchlist_updated_count", 0)
        refresh_txt  = f"🕐 {last_refresh}" + (f" · {upd_count} MAJ" if last_refresh != "—" else "")

        ctrl = st.columns([4, 1, 1])
        with ctrl[0]:
            txt = f"🕐 Cours MAJ : **{last_refresh}**" + (f" · {upd_count} ticker(s)" if last_refresh != "—" else "")
            st.markdown(f"<div style='font-size:11px;color:{C['muted']};padding:4px 0'>{txt}</div>", unsafe_allow_html=True)
        with ctrl[1]:
