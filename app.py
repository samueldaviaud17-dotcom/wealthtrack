...
def card(title, value, sub="", color=C['blue'], icon="", sub2=""):
    sub_html = f"<div style='font-size:12px;color:{color};margin-top:4px'>{sub}</div>" if sub else ""
    sub2_html = f"<div style='font-size:11px;color:{C['muted']};margin-top:2px'>{sub2}</div>" if sub2 else ""
    return f"""<div style="background:{C['card']};border:1px solid {C['border']};border-top:2px solid {color};
border-radius:10px;padding:14px 18px;min-height:90px;display:flex;flex-direction:column;justify-content:space-between">
...
with tab1:
    ...
    with R:
        ...
        clrs=[C['blue'],C['gold'],C['purple'],C['teal'],"#A855F7","#6B7280","#F59E0B"]
