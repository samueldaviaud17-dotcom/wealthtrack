...
# ── DEBUG temporaire GitHub ──
_gh_debug = f"Token: {'OK longueur='+str(len(_GH_TOKEN)) if _GH_TOKEN else 'VIDE'} | Repo: {_GH_REPO} | Branch: {_GH_BRANCH}"
...
with tab9:
    ...
    import requests as _req_debug
    _r_debug = _req_debug.get(f"{_GH_API}/repos/{_GH_REPO}", headers=_gh_headers(), timeout=10)
    st.info(f"🔧 Debug GitHub : {_gh_debug} | API status: {_r_debug.status_code} | msg: {_r_debug.json().get('message','OK') if _r_debug.status_code != 200 else '✅ repo OK'}")
...
(cron externe hors scope). À faire après stabilisation visuelle de tous les onglets.""",
    ...
    with st.expander("✅ 5 — Snapshots mensuels PV/MV & TRI", expanded=False):
        _chantier_card("Snapshots mensuels PV/MV & TRI", "📸",
            """Bouton "📸 Snapshot" en haut de page — enregistre la PV/MV totale et le TRI Global
du mois en cours dans <code>ibkr_data/snapshots.json</code> sur GitHub (un point par mois,
écrasement si déjà fait ce mois-ci). Le graphique "PV/MV & TRI dans le temps" (onglet
Performance) affiche la courbe cumulée + le point live du jour. 100% app — zéro lien Sheet.""",
            "#6EE7B7")
