...
with tab1:
    ...
    # Options : remplacer la contribution Sheet par la valeur app (delta uniquement)
    ...
    _opt_sheet_val = sv(df_pf, 8, 3)  # ce que le Sheet pense que vaut IBKR
    _opt_sheet_inv = sv(df_pf, 8, 2)  # ce que le Sheet pense qu'on a investi IBKR
    # Appliquer le delta : retirer valeur Sheet, mettre valeur app
    if _ibkr_cr > 0:
        pat = pat - _opt_sheet_val + _ibkr_cr
    if _ibkr_inv > 0:
        inv = inv - _opt_sheet_inv + _ibkr_inv
...
with tab4:
    ...
    with _cr_col1:
        ...
        # CSS pour simuler la card avec le crayon intégré dans le coin
        st.markdown(f"""
<div style='position:relative;background:{C['card']};border:1px solid {C['border']};
border-top:2px solid {C['teal']};border-radius:10px;padding:14px 18px 10px;
height:118px;box-sizing:border-box;display:flex;flex-direction:column'>
  ...
  color:{C['text']};line-height:1.2;flex:1'>{_cr_val:.2f} €</div>
  <div style='font-size:12px;color:{C['teal']}'>← modifiable</div>
</div>""", unsafe_allow_html=True)
        # Popover crayon positionné en bas de la card via CSS négatif
        st.markdown("""<style>
div[data-testid="stPopover"] > button {
    position:relative; margin-top:-108px; margin-left:auto;
    display:block; background:transparent; border:none;
    color:#8B949E; font-size:14px; cursor:pointer; padding:4px 8px;
    float:right; z-index:10;
}
div[data-testid="stPopover"] > button:hover { color:#E6EDF3; }
</style>""", unsafe_allow_html=True)
        with st.popover("✏️", use_container_width=False):
            st.markdown(f"**Modifier le capital réel**")
            st.markdown(f"Valeur actuelle : **{_cr_val:.2f} €**")
            _new_cr = st.number_input(
                "Nouveau capital IBKR (€)",
                value=float(_cr_val),
