...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    # ══════════════════════════════════════════════
    # SECTION 3 — MODIFIER / SUPPRIMER
    # ══════════════════════════════════════════════
    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
    sec("Modifier / Supprimer un versement", "✏️", "#F59E0B", "#1C0A00")
...
(cron externe hors scope). À faire après stabilisation visuelle de tous les onglets.""",
    ...
    _chantier_card("4 — Historique DCA & TRI précis", "📊",
        """Amélioration de la précision du calcul XIRR Bourse/Crypto en remplaçant le lissage uniforme par les vrais flux mensuels.<br><br>
<b>5a</b> — Créer <code>historique_dca.json</code> sur GitHub avec les montants estimés 2022→2025 par enveloppe (Bourse CTO+PEA, Crypto) — meilleure approximation disponible pour le passé.<br>
<b>5b</b> — Créer <code>dca_YYYY.json</code> sur GitHub pour l'année en cours avec saisie mois par mois des montants réels (CTO, PEA, BTC, ETH, SOL, BNB séparément).<br>
<b>5c</b> — Nouvel onglet ou section dédiée pour la saisie des DCA mensuels de l'année en cours (Bourse CTO+PEA, Crypto). Stockage GitHub automatique à la validation.<br>
<b>5d</b> — Calcul XIRR hybride : historique JSON (2022-2025) + DCA réels année en cours + dépôts IBKR réels (déjà en place). TRI plus précis sur la partie récente.<br>
<b>5e</b> — En fin d'année : archivage automatique de <code>dca_YYYY.json</code> dans <code>historique_dca.json</code> pour capitaliser l'historique. Cycle perpétuel sans dépendance Sheet.<br><br>
<b>Note</b> : IBKR déjà parfait (dépôts réels parsés depuis HTML). Seuls Bourse et Crypto nécessitent cette amélioration. Compatible avec la future coupure Google Sheet (Chantier 7).""",
        "#6EE7B7", "2-3 sessions")
