...
with tab_budget:
    ...
    _SEC_MAP = {"depenses":"Dépenses","frais_fixes":"Frais Fixes",
                "epargne":"Épargne & Investissements","revenus":"Revenus","credits":"Crédits / Dettes"}
    _SEC_KEY = {"Revenus":"revenus","Dépenses":"depenses","Frais Fixes":"frais_fixes",
                "Épargne & Investissements":"epargne","Crédits / Dettes":"credits"}

    # ── Chargement centralisé (1 seul gh_read par run, SHA conservé) ──
    ...
    def _budget_write(data, msg):
        """Écrit et efface le cache. Utilise une lecture SHA fraîche."""
        _, sha = gh_read(_BUD_PATH)
        ok = gh_write(_BUD_PATH, json.dumps(data, ensure_ascii=False, indent=2).encode('utf-8'), msg, sha)
        if ok:
            _load_budget.clear()
        return ok

    def _budget_modify(fn, msg):
        """Pattern générique: lit frais, applique fn(data), écrit."""
        raw, sha = gh_read(_BUD_PATH)
        data = json.loads(raw.decode('utf-8')) if raw else {"annee":2026,"mois":{}}
        fn(data)
        ok = gh_write(_BUD_PATH, json.dumps(data, ensure_ascii=False, indent=2).encode('utf-8'), msg, sha)
        if ok:
            _load_budget.clear()
    ...
    def _get_prevu(v):
        return _sf(v.get('prevu', v) if isinstance(v, dict) else v)
    ...
    def _kpi_bud(label, reel, prevu, color, icon):
        ...
        bc = "#F87171" if pct > 100 else color; pc = min(pct, 100)
    ...
    def _sec_tbl(titre, color, icon, bp_sec, rbc, show_ech=False):
        ...
        tp = tr = 0
        for cat in cats:
            v = bp_sec.get(cat, {}); pr = _get_prevu(v)
    ...
    def _get_cats(type_name, bp, txs):
        sk = _SEC_KEY.get(type_name, "")
        cats = list(bp.get(sk, {}).keys())
        for t in txs:
            if t['type'] == type_name and t['categorie'] not in cats:
                cats.append(t['categorie'])
        return cats
    ...
    # ── Sélecteur mois ──
    _sel = st.selectbox("", _mois_opts, format_func=lambda k: _ML.get(k, k),
                        key="bud_mois", label_visibility="collapsed")
    _md = _bud_data.get("mois", {}).get(_sel, {"transactions": [], "budget_prevu": {}})
    _txs = _md.get("transactions", [])
    _bp  = _md.get("budget_prevu", {})
    _reste_prec = _sf(_md.get("reste_mois_precedent", 0))

    # ── Calculs réel (une seule passe sur _txs) ──
    _rr = {}; _rd = {}; _rf = {}; _re = {}; _rc = {}
    for _t in _txs:
        _typ = _t['type']; _cat = _t['categorie']; _amt = _t['montant']
        if _typ == "Revenus":                     _rr[_cat] = _rr.get(_cat, 0) + _amt
        elif _typ == "Dépenses":                  _rd[_cat] = _rd.get(_cat, 0) + _amt
        elif _typ == "Frais Fixes":               _rf[_cat] = _rf.get(_cat, 0) + _amt
        elif _typ == "Épargne & Investissements": _re[_cat] = _re.get(_cat, 0) + _amt
        elif _typ == "Crédits / Dettes":          _rc[_cat] = _rc.get(_cat, 0) + _amt

    rev_r = sum(_rr.values()); dep_r = sum(_rd.values()); ff_r = sum(_rf.values())
    epa_r = sum(_re.values()); crd_r = sum(_rc.values())

    # ── Calculs prévu ──
    rev_p  = sum(_get_prevu(v) for v in _bp.get("revenus", {}).values())
    dep_p  = sum(_get_prevu(v) for v in _bp.get("depenses", {}).values())
    ff_p   = sum(_get_prevu(v) for v in _bp.get("frais_fixes", {}).values())
    epa_p  = sum(_get_prevu(v) for v in _bp.get("epargne", {}).values())
    cred_p = sum(_get_prevu(v) for v in _bp.get("credits", {}).values())
    rest_p = _reste_prec + rev_p - dep_p - ff_p - epa_p - cred_p
    rest_r = _reste_prec + rev_r - dep_r - ff_r - epa_r - crd_r

    # ════════════ EXPANDER SUIVI MENSUEL ════════════
    with st.expander(f"📊 {_ML.get(_sel,'').upper()} — Suivi mensuel du budget", expanded=True):
        st.markdown(
            f"<div style='background:linear-gradient(90deg,#0A1C12,#0D1F18);"
            f"border-left:3px solid #10B981;padding:8px 14px;border-radius:6px;margin-bottom:10px'>"
            f"<b style='font-size:16px;color:#10B981'>{_ML.get(_sel,'').upper()}</b>"
            f"<span style='font-size:11px;color:{_C_MUTED};margin-left:10px'>Suivi mensuel · {_sel}</span></div>",
            unsafe_allow_html=True)

        # ── 5 KPIs ──
        _k5 = st.columns(5)
        for _col, (_lbl, _pv, _rv, _clr, _ic) in zip(_k5, [
            ("REVENUS",    rev_p,  rev_r,  "#10B981", "💶"),
            ("DÉPENSES",   dep_p,  dep_r,  "#F87171", "🛒"),
            ("FRAIS FIXES",ff_p,   ff_r,   "#FBBF24", "🏠"),
            ("CRÉDITS",    cred_p, crd_r,  "#F59E0B", "💳"),
            ("ÉPARGNE",    epa_p,  epa_r,  "#818CF8", "💰"),
        ]):
            with _col: st.markdown(_kpi_bud(_lbl, _rv, _pv, _clr, _ic), unsafe_allow_html=True)
        ...
        with _g1:
            _fig_b = go.Figure()
            _xb = ["Revenus","Dépenses","Frais Fixes","Épargne"]
            _fig_b.add_trace(go.Bar(name='Prévu', x=_xb, y=[rev_p,dep_p,ff_p,epa_p],
                marker_color='rgba(99,102,241,0.5)', text=[f"{v:,.0f}" for v in [rev_p,dep_p,ff_p,epa_p]],
                textposition='outside', textfont=dict(size=8)))
            _fig_b.add_trace(go.Bar(name='Réel', x=_xb, y=[rev_r,dep_r,ff_r,epa_r],
                marker_color='rgba(16,185,129,0.85)', text=[f"{v:,.0f}" for v in [rev_r,dep_r,ff_r,epa_r]],
                textposition='outside', textfont=dict(size=8)))
        ...
        with _g2:
            ...
            if _rp2:
                ...
                _fr2 = go.Figure(go.Pie(labels=[x[0] for x in _rp2], values=[x[1] for x in _rp2],
                ...
                _fr2.add_annotation(text=f"<b>{rev_r:,.0f}€</b>", x=0.5, y=0.5,
                    font=dict(size=11, color=_C_TEXT), showarrow=False)
                _fr2.update_layout(**base_layout(185, False))
                _fr2.update_layout(showlegend=True,
        ...
        with _g3:
            ...
            if _bp4:
                _fbp = go.Figure(go.Pie(labels=[x[0] for x in _bp4], values=[x[1] for x in _bp4],
                ...
                _fbp.add_annotation(text=f"<b>{sum(v for _,v,_ in _bp4):,.0f}€</b>",
                    x=0.5, y=0.5, font=dict(size=11, color=_C_TEXT), showarrow=False)
                _fbp.update_layout(**base_layout(185, False))
                _fbp.update_layout(showlegend=False,
        ...
        with _cl:
            ...
            # Saisie reste mois précédent
            st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:12px 0 4px;font-weight:600'>📅 Reste du mois précédent</div>",unsafe_allow_html=True)
            _c_rp1, _c_rp2 = st.columns([3, 1])
            with _c_rp1:
                _new_reste = st.number_input("", value=float(_reste_prec), step=0.01, format="%.2f",
                                              key="reste_prec_input", label_visibility="collapsed")
            with _c_rp2:
                if st.button("💾", key="btn_reste_prec", help="Sauvegarder"):
                    def _fn_reste(d):
                        d.setdefault("mois",{}).setdefault(_sel,{})["reste_mois_precedent"] = round(_new_reste, 2)
                    if _budget_modify(_fn_reste, f"Reste préc. {_sel}={_new_reste:.2f}€"):
                        st.success(f"✅ {_new_reste:.2f}€"); st.rerun()
                    else: st.error("❌")

            # Résumé Entrées & Sorties
            _rc3 = "#10B981" if rest_r >= 0 else "#F87171"
            _rpc = "#10B981" if _reste_prec >= 0 else "#F87171"
            _rows_es = [
                ("+  Reste mois préc.", _reste_prec, _reste_prec, _rpc),
                ("+  Total Revenus",    rev_p,  rev_r,  "#10B981"),
                ("−  Dépenses",         dep_p,  dep_r,  "#F87171"),
                ("−  Frais Fixes",      ff_p,   ff_r,   "#FBBF24"),
                ("−  Crédits",          cred_p, crd_r,  "#F59E0B"),
                ("−  Épargne & Inv.",   epa_p,  epa_r,  "#818CF8"),
            ]
            _tg = (f"<div style='font-size:10px;font-weight:700;color:{_C_MUTED};text-transform:uppercase;padding:12px 0 4px'>📊 RÉSUMÉ ENTRÉES & SORTIES</div>"
                   ...
                   f"<th style='padding:4px 6px;text-align:right;font-size:9px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉEL</th>"
                   "</tr></thead><tbody>"
                   + "".join(f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                              f"<td style='padding:4px 6px;color:{clr};font-size:11px'>{lbl}</td>"
                              f"<td style='padding:4px 6px;text-align:right;color:{_C_MUTED}'>{pv:,.2f}€</td>"
                              f"<td style='padding:4px 6px;text-align:right;color:{clr}'>{rv:,.2f}€</td></tr>"
                              for lbl, pv, rv, clr in _rows_es)
                   + f"<tr style='background:#0D1117;font-weight:700'>"
                     ...
                     f"<td style='padding:5px 6px;text-align:right;color:{_rc3}'>{rest_r:,.2f}€</td>"
                     f"</tr></tbody></table>")
            st.markdown(_tg, unsafe_allow_html=True)

            # Montant restant à dépenser
            _rdc = "#10B981" if rest_r >= 0 else "#F87171"
            st.markdown(
                f"<div style='background:#0A1C12;border:1px solid #10B98133;border-radius:6px;padding:10px 12px;margin:10px 0'>"
                f"<div style='font-size:9px;color:{_C_MUTED};text-transform:uppercase'>Montant restant à dépenser</div>"
                f"<div style='font-size:22px;font-weight:800;color:{_rdc};font-family:Space Grotesk'>{rest_r:,.2f} €</div>"
                f"<div style='font-size:10px;color:{_C_MUTED}'>{'✅ Budget respecté' if rest_r >= 0 else f'⚠️ Dépassé de {abs(rest_r):,.2f}€'}</div></div>",
                unsafe_allow_html=True)
        ...
        if st.toggle("📋 Transactions du mois", key="tog_tx"):
            ...
            if _txsf:
                _tt = (f"<table style='width:100%;border-collapse:collapse;font-size:11px'>"
                       f"<thead><tr style='background:#0D1117'>"
                       + "".join(f"<th style='padding:4px 10px;font-size:9px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{h}</th>"
                                 for h in ["Date","Type","Catégorie","Montant","Description"])
                       + "</tr></thead><tbody>")
                for _t3 in _txsf:
                    _tc3 = _TC.get(_t3['type'], "#fff")
                    _sc3 = "#10B981" if _t3['type'] == 'Revenus' else "#F87171"
                    _sg3 = "+" if _t3['type'] == 'Revenus' else "-"
                    _tt += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                            f"<td style='padding:4px 10px;color:{_C_MUTED}'>{_t3['date']}</td>"
                            f"<td style='padding:4px 10px'><span style='font-size:9px;background:{_tc3}22;color:{_tc3};padding:1px 6px;border-radius:3px'>{_t3['type']}</span></td>"
                            f"<td style='padding:4px 10px;font-weight:500'>{_t3['categorie']}</td>"
                            f"<td style='padding:4px 10px;text-align:right;font-weight:600;color:{_sc3}'>{_sg3}{abs(_t3['montant']):.2f}€</td>"
                            f"<td style='padding:4px 10px;color:{_C_MUTED};font-size:10px'>{_t3.get('description','')}</td></tr>")
                _tt += "</tbody></table>"
    ...
    with st.expander("✏️ Gérer les transactions", expanded=False):
        _action_tx = st.radio("", ["➕ Créer","✏️ Modifier","🗑️ Supprimer"],
                              horizontal=True, key="tx_action", label_visibility="collapsed")

        if _action_tx == "➕ Créer":
            _cA, _cB = st.columns(2)
            with _cA:
                _fd   = st.date_input("Date", value=_now_p.date(), key="fb_date_c")
                _ft   = st.selectbox("Type", _TYPES, key="fb_type_c")
                _cats_c = _get_cats(_ft, _bp, _txs)
                if _cats_c:
                    _fc = st.selectbox("Catégorie", _cats_c, key="fb_cat_sel_c")
                else:
                    _fc = st.text_input("Catégorie (nouvelle)", key="fb_cat_txt_c")
            with _cB:
                _fa3 = st.number_input("Montant (€)", min_value=0.0, step=0.01, format="%.2f", key="fb_amt_c")
                _fd3 = st.text_input("Description", key="fb_desc_c")
            if st.button("💾 Ajouter la transaction", use_container_width=True, key="btn_add_tx"):
                _fc_val = _fc if _fc else ""
                if _fc_val.strip() and _fa3 > 0:
                    _mk2 = _fd.strftime("%Y-%m")
                    def _fn_create(d):
                        d.setdefault("mois",{}).setdefault(_mk2,{"nom":_ML.get(_mk2,_mk2),"transactions":[],"budget_prevu":{}})
                        d["mois"][_mk2]["transactions"].append({"date":str(_fd),"type":_ft,
                            "categorie":_fc_val.strip(),"montant":round(_fa3,2),"description":_fd3.strip()})
                        d["mois"][_mk2]["transactions"].sort(key=lambda x:x["date"])
                    if _budget_modify(_fn_create, f"Tx {_ft} {_fc_val} {_fa3:.0f}€"):
                        st.success(f"✅ {_fc_val} — {_fa3:.2f}€"); st.rerun()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie et montant requis.")

        elif _action_tx == "✏️ Modifier":
            if _txs:
                _tx_lbl = [f"{i} | {t['date']} · {t['type']} · {t['categorie']} · {t['montant']:.2f}€"
                           for i, t in enumerate(_txs)]
                _tx_sel  = st.selectbox("Transaction à modifier", _tx_lbl, key="tx_mod_sel")
                _tx_idx  = int(_tx_sel.split(" | ")[0])
                _tx_obj  = _txs[_tx_idx]
                _cA2, _cB2 = st.columns(2)
                with _cA2:
                    _fm_date = st.date_input("Date",
                        value=datetime.strptime(_tx_obj['date'],"%Y-%m-%d").date(), key="fm_date")
                    _fm_type = st.selectbox("Type", _TYPES,
                        index=_TYPES.index(_tx_obj['type']) if _tx_obj['type'] in _TYPES else 0,
                        key="fm_type")
                    _cats_m  = _get_cats(_fm_type, _bp, _txs)
                    _cur_cat = _tx_obj['categorie']
                    if _cats_m:
                        _def_idx = _cats_m.index(_cur_cat) if _cur_cat in _cats_m else 0
                        _fm_cat  = st.selectbox("Catégorie", _cats_m, index=_def_idx, key="fm_cat_sel")
                    else:
                        _fm_cat  = st.text_input("Catégorie", value=_cur_cat, key="fm_cat_txt")
                with _cB2:
                    _fm_amt  = st.number_input("Montant (€)", value=float(_tx_obj['montant']),
                                               step=0.01, format="%.2f", key="fm_amt")
                    _fm_desc = st.text_input("Description",
                                             value=_tx_obj.get('description',''), key="fm_desc")
                if st.button("💾 Sauvegarder la modification", use_container_width=True, key="btn_mod_tx"):
                    def _fn_mod(d):
                        txlist = d.get("mois",{}).get(_sel,{}).get("transactions",[])
                        if _tx_idx < len(txlist):
                            txlist[_tx_idx] = {"date":str(_fm_date),"type":_fm_type,
                                "categorie":_fm_cat.strip(),"montant":round(_fm_amt,2),
                                "description":_fm_desc.strip()}
                            txlist.sort(key=lambda x: x["date"])
                    if _budget_modify(_fn_mod, f"Modif tx idx={_tx_idx} {_fm_cat}"):
                        st.success("✅ Transaction modifiée"); st.rerun()
                    else: st.error("❌ Erreur GitHub")
            else:
                st.info("Aucune transaction à modifier.")

        else:  # Supprimer
            if _txs:
                _tx_lbl2 = [f"{i} | {t['date']} · {t['type']} · {t['categorie']} · {t['montant']:.2f}€"
                            for i, t in enumerate(_txs)]
                _tx_del  = st.selectbox("Transaction à supprimer", _tx_lbl2, key="tx_del_sel")
                if st.button("🗑️ Confirmer la suppression", key="btn_del_tx"):
                    _idx_del = int(_tx_del.split(" | ")[0])
                    def _fn_del(d):
                        txlist = d.get("mois",{}).get(_sel,{}).get("transactions",[])
                        if _idx_del < len(txlist):
                            txlist.pop(_idx_del)
                    if _budget_modify(_fn_del, f"Suppression tx idx={_idx_del}"):
                        st.success("✅ Transaction supprimée"); st.rerun()
                    else: st.error("❌ Erreur GitHub")
            else:
                st.info("Aucune transaction à supprimer.")

    # ════════════ PLANIFICATION DU BUDGET ════════════
    with st.expander("💡 Planification du budget", expanded=False):
        _action_bp = st.radio("", ["➕ Ajouter","✏️ Modifier","🗑️ Supprimer"],
                              horizontal=True, key="bp_action", label_visibility="collapsed")

        if _action_bp == "➕ Ajouter":
            ...
            with _cP1:
                _fp_t = st.selectbox("Section", list(_SEC_MAP.keys()),
                                     format_func=lambda x: _SEC_MAP[x], key="fp_type_a")
            with _cP2:
                _fp_a = st.number_input("Montant prévu (€)", min_value=0.0, step=1.0,
                                        format="%.2f", key="fp_amt_a")
            _fp_c = st.text_input("Nouvelle catégorie", key="fp_cat_a")
            if st.button("✏️ Enregistrer", use_container_width=True, key="btn_planif_add"):
                if _fp_c.strip():
                    _cc2 = _fp_c.strip()
                    def _fn_add_bp(d):
                        bp_s = d.setdefault("mois",{}).setdefault(_sel,{}).setdefault("budget_prevu",{}).setdefault(_fp_t,{})
                        if _fp_t == "frais_fixes":
                            bp_s[_cc2] = {"prevu":round(_fp_a,2),"reel":0,
                                "echeance":bp_s.get(_cc2,{}).get("echeance",0) if isinstance(bp_s.get(_cc2,{}),dict) else 0}
                        else:
                            bp_s[_cc2] = {"prevu":round(_fp_a,2),"reel":0}
                    if _budget_modify(_fn_add_bp, f"Budget {_sel} {_fp_t} {_cc2}={_fp_a:.0f}€"):
                        st.success(f"✅ {_cc2} → {_fp_a:.2f}€"); st.rerun()
                    else: st.error("❌ Erreur GitHub")
                else: st.warning("Catégorie requise.")

        else:
            _all_bp = [(s, c) for s, sd in _bp.items() for c in sd]
            if _all_bp:
                _bp_lbls = [f"{_SEC_MAP.get(s,s)} · {c}" for s, c in _all_bp]
                _bp_sel  = st.selectbox("Catégorie de budget", _bp_lbls, key="bp_sel")
                _bp_idx  = _bp_lbls.index(_bp_sel)
                _bp_sec_k, _bp_cat_k = _all_bp[_bp_idx]
                _bp_obj  = _bp.get(_bp_sec_k, {}).get(_bp_cat_k, {})
                _bp_prev = _get_prevu(_bp_obj)

                if _action_bp == "✏️ Modifier":
                    _fpm_a = st.number_input("Nouveau montant prévu (€)", value=float(_bp_prev),
                                             step=1.0, format="%.2f", key="fpm_amt")
                    if st.button("💾 Sauvegarder", use_container_width=True, key="btn_mod_bp"):
                        def _fn_mod_bp(d):
                            bp_s = d.get("mois",{}).get(_sel,{}).get("budget_prevu",{}).get(_bp_sec_k,{})
                            if _bp_sec_k == "frais_fixes":
                                bp_s[_bp_cat_k] = {"prevu":round(_fpm_a,2),"reel":0,
                                    "echeance":bp_s.get(_bp_cat_k,{}).get("echeance",0) if isinstance(bp_s.get(_bp_cat_k,{}),dict) else 0}
                            else:
                                bp_s[_bp_cat_k] = {"prevu":round(_fpm_a,2),"reel":0}
                        if _budget_modify(_fn_mod_bp, f"Modif budget {_bp_cat_k}={_fpm_a:.0f}€"):
                            st.success(f"✅ {_bp_cat_k} → {_fpm_a:.2f}€"); st.rerun()
                        else: st.error("❌ Erreur GitHub")

                else:  # Supprimer
                    st.markdown(f"<div style='color:#F87171;font-size:11px'>Supprimer : <b>{_bp_cat_k}</b> ({_bp_prev:.2f}€ prévu)</div>",unsafe_allow_html=True)
                    if st.button("🗑️ Confirmer la suppression", key="btn_del_bp"):
                        def _fn_del_bp(d):
                            bp_s = d.get("mois",{}).get(_sel,{}).get("budget_prevu",{}).get(_bp_sec_k,{})
                            bp_s.pop(_bp_cat_k, None)
                        if _budget_modify(_fn_del_bp, f"Suppression budget {_bp_cat_k}"):
                            st.success(f"✅ {_bp_cat_k} supprimé"); st.rerun()
                        else: st.error("❌ Erreur GitHub")
            else:
                st.info("Aucune planification à modifier/supprimer.")
    ...
    with st.expander("📅 RÉSUMÉ ANNUEL 2026", expanded=False):
        st.markdown(
            ...
            unsafe_allow_html=True)
        _hdrs = [("Mois",_C_MUTED),("Revenus","#10B981"),("Dépenses","#F87171"),
        ...
        _ta = (f"<table style='width:100%;border-collapse:collapse;font-size:12px'>"
               f"<thead><tr style='background:#0D1117'>"
               + "".join(f"<th style='padding:6px 10px;text-align:right;font-size:10px;color:{hc};border-bottom:1px solid {_C_BORDER}'>{h}</th>" for h,hc in _hdrs)
               + "</tr></thead><tbody>")
        _tot_a = [0.0] * 6
        for _mk3 in sorted(_bud_data.get("mois", {}).keys()):
            _mt3  = _bud_data["mois"][_mk3].get("transactions", [])
            _mrp3 = _sf(_bud_data["mois"][_mk3].get("reste_mois_precedent", 0))
            _vals = [0.0] * 6
            for _t3 in _mt3:
                _tp3 = _t3['type']; _am3 = _t3['montant']
                if _tp3 == "Revenus":                     _vals[0] += _am3
                elif _tp3 == "Dépenses":                  _vals[1] += _am3
                elif _tp3 == "Frais Fixes":               _vals[2] += _am3
                elif _tp3 == "Épargne & Investissements": _vals[3] += _am3
                elif _tp3 == "Crédits / Dettes":          _vals[4] += _am3
            _mrest3 = _mrp3 + _vals[0] - _vals[1] - _vals[2] - _vals[3] - _vals[4]
            _vals[5] = _mrest3
            for i, v in enumerate(_vals): _tot_a[i] += v
            _rc4 = "#10B981" if _mrest3 >= 0 else "#F87171"
            _clrs4 = ["#10B981","#F87171","#FBBF24","#818CF8","#F59E0B",_rc4]
            _ta += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:5px 10px;font-weight:600'>{_ML.get(_mk3,_mk3)}</td>"
                    + "".join(f"<td style='padding:5px 10px;text-align:right;color:{c}'>{v:,.2f}€</td>"
                              for v, c in zip(_vals, _clrs4))
                    + "</tr>")
        _tr_c3 = "#10B981" if _tot_a[5] >= 0 else "#F87171"
        _tot_clrs = ["#10B981","#F87171","#FBBF24","#818CF8","#F59E0B",_tr_c3]
        _ta += (f"<tr style='background:#0D1117;font-weight:700;border-top:2px solid {_C_BORDER}'>"
                f"<td style='padding:6px 10px'>TOTAL YTD</td>"
                + "".join(f"<td style='padding:6px 10px;text-align:right;color:{c}'>{v:,.2f}€</td>"
                          for v, c in zip(_tot_a, _tot_clrs))
                + "</tr></tbody></table>")
