...
Benchmarks" ci-contre).""")
    ...
    with Rp:
        ...
        _valid = [b[1] for b in _benchs_data if b[1] is not None]
        _mx = max(abs(v) for v in _valid) or 1 if _valid else 1
        for _bname, _btri in _benchs_data:
            ...
            _bar_w = int(abs(_btri) / _mx * 60)
            _tbl_bench += (f"<tr style='border-bottom:1px solid #1F2937'>"
                f"<td style='padding:7px 10px'>"
                f"<div>{_bname}</div>"
                f"<div style='height:3px;background:{C['border']};border-radius:2px;margin-top:3px'>"
                f"<div style='height:100%;width:{_bar_w}%;background:{_tri_col};border-radius:2px'></div></div>"
                f"</td>"
