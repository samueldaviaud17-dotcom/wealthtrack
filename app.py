...
with tab8:
    if not st.session_state.get('ibkr_data'):
        ...
        _periode_options = [p[0] for p in _periode_labels]
        # Restaurer la sélection précédente si elle existe encore dans la liste
        _prev_sel = st.session_state.get('rapport_periode_saved', _periode_options[0] if _periode_options else '')
        _default_idx = _periode_options.index(_prev_sel) if _prev_sel in _periode_options else 0

        _sel_col, _ = st.columns([2, 4])
        with _sel_col:
        ...
        # Sauvegarder le choix manuellement pour résister au Rafraîchir
        st.session_state['rapport_periode_saved'] = _sel_lbl
        _sel_periode = next((p for p in _periode_labels if p[0] == _sel_lbl), _periode_labels[0] if _periode_labels else None)

        if _sel_periode:
            _rpt_type = _sel_periode[1]  # 'mois' ou 'annee'
            _rpt_yr   = _sel_periode[2]
            _rpt_m    = _sel_periode[3]
        else:
            _rpt_type = 'mois'; _rpt_yr = _now_an.year; _rpt_m = _now_an.month
        ...
        _mois_partiel = (_rpt_type == 'mois' and _rpt_yr == _now_an.year
                         and _rpt_m == _now_an.month and _now_an.day < 28)
  ...
  <div style='display:flex;justify-content:space-between;align-items:center'>
    ...
    <span style='font-size:18px;font-weight:700;color:#3FB950;font-family:monospace'>{_best_rdt:.1f}%</span>
        ...
        _rapport_html = (
            ...
            f"<div>"
            f"<div style='font-size:11px;color:#8B949E;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px'>SamInvest · Rapport Mensuel</div>"
