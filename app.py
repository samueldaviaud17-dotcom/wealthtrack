...
def save_snapshot(pvmv, tri_global, crypto_val_eur=None):
    """Ajoute ou met à jour le snapshot du mois en cours (timezone Paris)."""
    data = load_snapshots()
    snaps = data.get("snapshots", [])
    _now_paris = datetime.now(_TZ_PARIS)
    mois_key = _now_paris.strftime("%Y-%m")
    snaps = [s for s in snaps if s.get("date","")[:7] != mois_key]
    entry = {"date": _now_paris.strftime("%Y-%m-%d"), "pvmv": pvmv, "tri": tri_global}
    if crypto_val_eur is not None:
        entry["crypto_val_eur"] = round(crypto_val_eur, 2)
    snaps.append(entry)
...
# Shortcuts globaux pour éviter les guillemets imbriqués dans les f-strings
_C_BG = C['bg']; _C_CARD = C['card']; _C_BORDER = C['border']
_C_TEXT = C['text']; _C_MUTED = C['muted']; _C_GREEN = C['green']
_C_RED = C['red']; _C_GOLD = C['gold']; _C_BLUE = C['blue']
...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_analyst_data(ticker: str, _v=2):
    ...
    try:
        ...
        # Dividendes
        div_yield_raw = info.get('dividendYield', None) or info.get('trailingAnnualDividendYield', None)
        # Normalisation : yfinance retourne parfois déjà en % (ex: 0.98 pour ~1%), parfois en ratio (0.0083)
        # Heuristique : si > 0.5 → déjà en %, sinon × 100. Plafond 30% pour filtrer les aberrations.
        if div_yield_raw:
            div_yield = div_yield_raw if div_yield_raw > 0.5 else div_yield_raw * 100
            div_yield = min(div_yield, 30.0)  # plafond 30%/an
        else:
            div_yield = None
        ...
        return {
            ...
            'div_yield':    round(div_yield, 2) if div_yield else None,   # en % normalisé
...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_dividend_calendar(tickers: tuple, _v=3):
    """Retourne liste de dicts {ticker, ex_date, yield_pct, rate} triés par ex_date.
    Fait son propre appel yfinance indépendant (ne passe pas par fetch_analyst_data)."""
    if not YF_AVAILABLE:
        return []
    from datetime import date as _date
    results = []
    ...
    for tk_str in tickers:
        ...
        try:
            ...
            _ex_ts = info.get('exDividendDate', None)
            if not _ex_ts:
                continue
            ex_date = _date.fromtimestamp(int(_ex_ts))
            if ex_date < today:
                continue
            # Rendement : yfinance retourne parfois ratio (0.0083) parfois % (0.98)
            _raw = info.get('dividendYield') or info.get('trailingAnnualDividendYield')
            if _raw:
                yield_pct = round(min(_raw if _raw > 0.5 else _raw * 100, 30.0), 2)
            else:
                yield_pct = None
            _rate = info.get('dividendRate') or info.get('trailingAnnualDividendRate')
            results.append({
                'ticker':    tk_str,
                'ex_date':   ex_date,
                'yield_pct': yield_pct,
                'rate':      round(float(_rate), 2) if _rate else None,
            })
        except Exception:
            continue
...
with _rbtn[2]:
    ...
    _mois_actuel = datetime.now(_TZ_PARIS).strftime("%Y-%m")
    ...
    if st.button(f"{'✅' if _deja_snap else '📸'} Snapshot", key="snapshot_btn",
        ...
        _pvmv_snap = st.session_state.get('pvmv_t',
                     st.session_state.get('tri_global', {}).get('valeur_actuelle_totale', 0) -
                     st.session_state.get('tri_global', {}).get('valeur_investie_totale', 0))
        _tri_snap  = st.session_state.get('tri_global', {}).get('tri_global')
        _crypto_snap = st.session_state.get('crypto_val_eur', None)
        if save_snapshot(_pvmv_snap, _tri_snap, _crypto_snap):
...
def _compute_tri_global_et_benchmarks():
    ...
    # ── Flux Bourse & Crypto depuis historique_dca.json ──
    _hist_raw, _hist_sha = gh_read("historique_dca.json")
    _hist = json.loads(_hist_raw.decode('utf-8')) if _hist_raw else {}

    _bourse_cfs = []
    _crypto_cfs = []

    # Nouvelle structure : liste de versements avec date exacte
    for v in _hist.get("bourse", {}).get("versements", []):
        try:
            _d = datetime.strptime(v["date"], "%Y-%m-%d")
            _amt = float(v.get("cto", 0)) + float(v.get("pea", 0))
            if _amt > 0 and _d < _now:
                _bourse_cfs.append((_d, _amt))
        except Exception: continue

    for v in _hist.get("crypto", {}).get("versements", []):
        try:
            _d = datetime.strptime(v["date"], "%Y-%m-%d")
            if "montant_eur" in v:
                # Nouveau format (USDC) : montant € exact saisi par l'utilisateur
                _amt = float(v["montant_eur"])
            elif "total" in v:
                # Ancienne structure : champ "total" en $, conversion approximative
                _amt = float(v["total"]) * 0.92
            else:
                # Structure intermédiaire : somme des investi_usd par token, conversion approximative
                _amt = sum(
                    d.get("investi_usd", 0) if isinstance(d, dict) else 0
                    for k, d in v.items()
                    if k not in ("date", "_note", "total", "montant_eur", "frais_depot_eur")
                )
                _amt = _amt * 0.92
            if _amt > 0 and _d < _now:
                _crypto_cfs.append((_d, _amt))
        except Exception: continue

    _bourse_cfs.sort(key=lambda x: x[0])
    _crypto_cfs.sort(key=lambda x: x[0])
    ...
    # Flux retraits = flux positifs dans le XIRR (argent récupéré)
    _retraits_cfs = []
    for v in _hist.get("retraits", []):
        try:
            _d = datetime.strptime(v["date"], "%Y-%m-%d")
            _amt = float(v.get("montant_eur", 0))
            if _amt > 0 and _d < _now:
                _retraits_cfs.append((_d, _amt))  # positif = flux entrant
        except Exception:
            continue
    ...
    # XIRR : sorties (versements négatifs) + retraits positifs + valeur finale positive
    _cfs_pf = [(d, -m) for d, m in _all_cfs_in] + [(d, m) for d, m in _retraits_cfs] + [(_now, _val_actuelle)]
...
tab1, tab5, tab_marche, tab2, tab3, tab4, tab8, tab6, tab7, tab_saisie, tab_budget, tab9 = st.tabs(["🏠 Vue d'ensemble", "📊 Performance", "🌍 Contexte Marché", "📈 Bourse", "₿ Crypto", "⚙️ Options", "📊 IBKR Analyse", "🔍 Valorisation", "⭐ Watchlist", "✏️ Saisie DCA", "💰 Budget", "🚧 Chantier"])
...
# ══════════════════════════════════════════════════════
with tab3:
    # ── Chargement positions depuis GitHub (plus de Google Sheet) ──
    @st.cache_data(ttl=300, show_spinner=False)
    def _fetch_crypto_prices():
        """Récupère prix live + FX pour tous les tokens crypto."""
        _tickers = {"BTC":"BTC-USD","ETH":"ETH-USD","SOL":"SOL-USD","BNB":"BNB-USD","USDC":"USDC-USD"}
        _prices = {}
        for _tk, _sym in _tickers.items():
            try:
                _fi = yf.Ticker(_sym).fast_info
                _prices[_tk] = getattr(_fi, 'last_price', None) or 0.0
            except Exception:
                _prices[_tk] = 0.0
        try:
            _fx = yf.Ticker("EURUSD=X").fast_info.last_price or 1.0
            if not (0.8 < _fx < 1.8): _fx = 1.0
        except Exception:
            _fx = st.session_state.get('eurusd_live', 1.0) or 1.0
        return _prices, _fx

    _c3_raw, _ = gh_read("crypto_positions.json")
    try:
        _c3_data = json.loads(_c3_raw.decode('utf-8')) if _c3_raw else {"positions": {}}
    except Exception:
        _c3_data = {"positions": {}}
    _c3_pos = _c3_data.get("positions", {})

    _c3_prices, _c3_fx = _fetch_crypto_prices()

    # Noms affichage
    _C3_NAMES = {"BTC":"Bitcoin","ETH":"Ethereum","SOL":"Solana","BNB":"BinanceCoin","USDC":"USDC"}
    _C3_COLORS = {"BTC":"#F7931A","ETH":"#627EEA","SOL":"#9945FF","BNB":"#F3BA2F","USDC":"#2775CA"}
    _C3_ORDER  = ["BTC","SOL","ETH","BNB","USDC"]

    # Calcul des valeurs par token
    _c3_rows = []
    for _tk in _C3_ORDER:
        _p = _c3_pos.get(_tk, {})
        _detenu    = _p.get("detenu", 0.0)
        _achete    = _p.get("achete", 0.0)
        _inv_usd   = _p.get("investi_usd", 0.0)
        _prix      = _c3_prices.get(_tk, 0.0)
        _val_usd   = _detenu * _prix
        _val_eur   = _val_usd / _c3_fx if _c3_fx else 0.0
        _pru_usd   = (_inv_usd / _detenu) if _detenu else 0.0
        _staking   = (_detenu - _achete) * _prix if _prix else 0.0
        _c3_rows.append({
            "tk": _tk, "nom": _C3_NAMES.get(_tk, _tk),
            "detenu": _detenu, "achete": _achete,
            "inv_usd": _inv_usd,
            "prix": _prix, "pru_usd": _pru_usd,
            "val_usd": _val_usd, "val_eur": _val_eur,
            "staking_usd": _staking,
        })

    # Total investi EUR : base snapshot Sheet (au 29/06/2026) + nouveaux versements € après cette date
    _c3_eur_base      = _c3_data.get("investi_eur_base", 0.0)
    _c3_eur_base_date = _c3_data.get("investi_eur_base_date", "2026-06-29")
    _c3_dca_raw_kpi, _ = gh_read("historique_dca.json")
    try:
        _c3_dca_kpi = json.loads(_c3_dca_raw_kpi.decode('utf-8')) if _c3_dca_raw_kpi else {}
    except Exception:
        _c3_dca_kpi = {}
    _c3_new_eur = sum(
        float(v.get("montant_eur", 0))
        for v in _c3_dca_kpi.get("crypto", {}).get("versements", [])
        if v.get("date", "") > _c3_eur_base_date and v.get("montant_eur", 0) > 0
    )
    _c3_tot_inv_eur = _c3_eur_base + _c3_new_eur

    # Totaux valeur/PV/MV
    _c3_tot_val_usd = sum(r["val_usd"] for r in _c3_rows)
    _c3_tot_val_eur = sum(r["val_eur"] for r in _c3_rows)
    _c3_tot_pvmv    = _c3_tot_val_eur - _c3_tot_inv_eur
    _c3_tot_perf    = (_c3_tot_pvmv / _c3_tot_inv_eur * 100) if _c3_tot_inv_eur else 0.0

    # Total frais cumulés depuis frais_historiques
    _c3_frais_hist  = _c3_data.get("frais_historiques", {})
    _c3_frais_total = _c3_frais_hist.get("total_cumul", 0.0)

    # Stocker pour le snapshot et l'onglet Performance
    st.session_state['crypto_val_eur'] = _c3_tot_val_eur
    st.session_state['crypto_inv_eur'] = _c3_tot_inv_eur

    # ── KPIs ──
    _c3_pvmv_brut = _c3_tot_val_eur - _c3_tot_inv_eur
    _c3_perf_brut = (_c3_pvmv_brut / _c3_tot_inv_eur * 100) if _c3_tot_inv_eur else 0.0
    cols = st.columns(5)
    for col, (ti, va, su, co, ic) in zip(cols, [
        ("VALEUR PF",      fmt(_c3_tot_val_eur),            f"{_c3_tot_val_usd:,.2f} $",      C['gold'],          "💶"),
        ("TOTAL INVESTI",  fmt(_c3_tot_inv_eur),             "",                               C['blue'],          "💰"),
        ("PV / MV",        fmt(_c3_pvmv_brut),               pct(_c3_perf_brut),               pcol(_c3_perf_brut),"📈"),
        ("FRAIS CUMULÉS",  f"-{_c3_frais_total:.2f} €",      "depuis 2022 (non déduits du PV/MV)", C['red'],      "💸"),
        ("COURS EUR/$",    f"{_c3_fx:.4f}",                   "",                              C['muted'],         "💱"),
    ]):
    ...
    with L3:
        ...
        _tbl_c += ("<thead><tr style='background:#111827'>"
            + "".join(f"<th style='padding:7px 10px;text-align:{'left' if i<2 else 'right'};font-size:10px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid {C['border']}'>{h}</th>"
                      for i,h in enumerate(["Crypto","Code","Montant détenu","Montant acheté","Investi ($)","Cours réel ($)","PRU ($)","Valeur ($)","Valeur (€)","PV/MV (€)","Perf."]))
            + "</tr></thead><tbody>")

        for r in _c3_rows:
            # PV/MV par token : val_eur - (inv_usd/fx) — meilleure approximation disponible par token
            _inv_eur_tok = r["inv_usd"] / _c3_fx if _c3_fx else 0.0
            _pvmv_tok    = r["val_eur"] - _inv_eur_tok
            _perf_tok    = (_pvmv_tok / _inv_eur_tok * 100) if _inv_eur_tok else 0.0
            _pc = C['green'] if _pvmv_tok >= 0 else C['red']
            _tk_color = _C3_COLORS.get(r["tk"], "#fff")
            _tbl_c += (f"<tr style='border-bottom:1px solid #1F2937'>"
                f"<td style='padding:6px 10px;font-weight:600;color:{_tk_color}'>{r['nom']}</td>"
                f"<td style='padding:6px 10px;color:{C['muted']}'>{r['tk']}</td>"
                f"<td style='padding:6px 10px;text-align:right'>{r['detenu']:.8f}</td>"
                f"<td style='padding:6px 10px;text-align:right;color:{C['muted']}'>{r['achete']:.8f}</td>"
                f"<td style='padding:6px 10px;text-align:right'>{r['inv_usd']:,.2f} $</td>"
                f"<td style='padding:6px 10px;text-align:right'>{r['prix']:,.2f} $</td>"
                f"<td style='padding:6px 10px;text-align:right'>{r['pru_usd']:,.2f} $</td>"
                f"<td style='padding:6px 10px;text-align:right'>{r['val_usd']:,.2f} $</td>"
                f"<td style='padding:6px 10px;text-align:right'>{fmt2(r['val_eur'])}</td>"
                f"<td style='padding:6px 10px;text-align:right;color:{_pc};font-weight:600'>{'+' if _pvmv_tok>=0 else ''}{_pvmv_tok:,.2f} €</td>"
                f"<td style='padding:6px 10px;text-align:right;color:{_pc};font-weight:600'>{'+' if _perf_tok>=0 else ''}{_perf_tok:.1f} %</td>"
                "</tr>")

        _pc_tot = C['green'] if _c3_tot_pvmv >= 0 else C['red']
        _tbl_c += (f"<tr style='background:#111827;font-weight:700'>"
            f"<td style='padding:7px 10px' colspan='4'>TOTAL PORTEFEUILLE</td>"
            f"<td style='padding:7px 10px;text-align:right'>{sum(r['inv_usd'] for r in _c3_rows):,.2f} $</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{C['muted']}'>—</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{C['muted']}'>—</td>"
            f"<td style='padding:7px 10px;text-align:right'>{_c3_tot_val_usd:,.2f} $</td>"
            f"<td style='padding:7px 10px;text-align:right'>{fmt2(_c3_tot_val_eur)}</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{_pc_tot}'>{'+' if _c3_tot_pvmv>=0 else ''}{_c3_tot_pvmv:,.2f} €</td>"
            f"<td style='padding:7px 10px;text-align:right;color:{_pc_tot}'>{'+' if _c3_tot_perf>=0 else ''}{_c3_tot_perf:.1f} %</td>"
            "</tr>")
        _tbl_c += "</tbody></table></div>"
        st.markdown(_tbl_c, unsafe_allow_html=True)

        # ── DCA Mensuel depuis historique_dca.json ──
        _c3_dca_raw, _ = gh_read("historique_dca.json")
        try:
            _c3_dca_hist = json.loads(_c3_dca_raw.decode('utf-8')) if _c3_dca_raw else {}
        except Exception:
            _c3_dca_hist = {}

        sec(f"DCA Mensuel {_YR}", "💸", "#FCD34D", "#1C100A")
        _dca_c_par_mois = [0.0] * 12
        for _vv in _c3_dca_hist.get("crypto", {}).get("versements", []):
            try:
                _vdate = datetime.strptime(_vv["date"], "%Y-%m-%d")
                if _vdate.year == _YR:
                    _mo_idx = _vdate.month - 1
                    _amt_c = _vv.get("montant_eur", 0)
                    if _amt_c == 0:
                        # anciens formats : total en $ * fx approx, ou somme tokens en $
                        _amt_c = _vv.get("total", 0) * 0.92
                        if _amt_c == 0:
                            _amt_c = sum(
                                d.get("investi_usd", 0) if isinstance(d, dict) else (d if isinstance(d, (int, float)) else 0)
                                for k, d in _vv.items() if k not in ("date", "_note", "total", "montant_eur", "frais_depot_eur")
                            ) / _c3_fx
                    _dca_c_par_mois[_mo_idx] += _amt_c
            except Exception:
                continue

        fig7 = go.Figure(go.Bar(
            x=mois_s, y=_dca_c_par_mois,
            marker_color=['#FCD34D' if x > 0 else C['border'] for x in _dca_c_par_mois],
            text=[f"{x:.0f}€" if x > 0 else "" for x in _dca_c_par_mois],
            textposition='outside', textfont=dict(color=C['text'], size=10)
        ))
        _dca_actif_c = [x for x in _dca_c_par_mois if x > 0]
        if _dca_actif_c:
            _moy_c = sum(_dca_actif_c) / len(_dca_actif_c)
            fig7.add_trace(go.Scatter(
                name=f'Moy. {_moy_c:.0f}€/mois', x=mois_s, y=[_moy_c] * 12,
                mode='lines', line=dict(color='#F59E0B', width=2, dash='dot'),
                hovertemplate=f'Moyenne : {_moy_c:.0f} €/mois<extra></extra>'
            ))
            fig7.add_annotation(x=mois_s[-1], y=_moy_c, text=f'<b>{_moy_c:.0f} €/mois</b>',
                showarrow=False, yshift=12, xanchor='right', font=dict(size=11, color='#F59E0B'))
        ...
        # ── Évolution mensuelle du portefeuille crypto (depuis snapshots) ──
        _c3_snaps = load_snapshots().get("snapshots", [])
        _c3_snaps_crypto = [(s["date"], s["crypto_val_eur"]) for s in _c3_snaps if "crypto_val_eur" in s]
        _MOIS_FR_C = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"]
        def _fmt_c3_lbl(d):
            try: y,m = d[:7].split("-"); return f"{_MOIS_FR_C[int(m)-1]} {y}"
            except: return d
        # Ajoute le mois courant dynamiquement si pas encore snapshoté
        _c3_cur_lbl = _fmt_c3_lbl(datetime.now(_TZ_PARIS).strftime("%Y-%m-%d"))
        if not any(_fmt_c3_lbl(d) == _c3_cur_lbl for d, _ in _c3_snaps_crypto):
            _c3_snaps_crypto.append((datetime.now(_TZ_PARIS).strftime("%Y-%m-%d"), _c3_tot_val_eur))
        if _c3_snaps_crypto:
            sec(f"Évolution du portefeuille crypto", "📈", "#FCD34D", "#1C100A")
            _xc = [_fmt_c3_lbl(d) for d, _ in _c3_snaps_crypto]
            _yc = [v for _, v in _c3_snaps_crypto]
            _c3_colors_bar = ['#FCD34D' if i < len(_xc)-1 else '#F59E0B' for i in range(len(_xc))]
            _fig_c_ev = go.Figure(go.Bar(
                x=_xc, y=_yc,
                marker_color=_c3_colors_bar,
                marker_line=dict(color=C['bg'], width=1),
                text=[f"{y/1000:.1f}k€" for y in _yc],
                textposition='outside', textfont=dict(color=C['text'], size=9),
                hovertemplate='<b>%{x}</b><br>%{y:,.0f} €<extra></extra>'
            ))
            if len(_yc) > 1:
                _evol_c = (_yc[-1] / _yc[0] - 1) * 100
                _fig_c_ev.add_annotation(
                    x=_xc[-1], y=_yc[-1],
                    text=f"{'+' if _evol_c >= 0 else ''}{_evol_c:.1f}%",
            ...
            st.plotly_chart(_fig_c_ev, use_container_width=True, config={'displayModeBar': False})
            _n_saved = len([d for d, _ in _c3_snaps_crypto if _fmt_c3_lbl(d) != _c3_cur_lbl or any(_fmt_c3_lbl(s["date"]) == _c3_cur_lbl for s in _c3_snaps if "crypto_val_eur" in s)])
            st.markdown(f"<div style='font-size:10px;color:{C['muted']};font-style:italic'>Point courant ({_c3_cur_lbl}) dynamique · 📸 Snapshot en haut pour figer la valeur du mois</div>", unsafe_allow_html=True)

    with R3:
        sec("Répartition", "🍩", "#FCD34D", "#1C100A")
        _c3_rep = [(r["nom"], _C3_COLORS.get(r["tk"], "#888"), r["val_usd"]) for r in _c3_rows if r["val_usd"] > 0]
        _c3_rep_sorted = sorted(_c3_rep, key=lambda x: -x[2])
        _all_lbl_c = [x[0] for x in _c3_rep_sorted]
        _all_clr_c = [x[1] for x in _c3_rep_sorted]
        _all_val_c = [x[2] for x in _c3_rep_sorted]

        sel_c = st.multiselect("", _all_lbl_c, default=_all_lbl_c, key="crypto_rep_filter",
            help="Cliquez pour afficher/masquer une crypto")
        lbl_c = [l for l in _all_lbl_c if l in sel_c]
        clr_c = [_all_clr_c[_all_lbl_c.index(l)] for l in lbl_c]
        val_c = [_all_val_c[_all_lbl_c.index(l)] for l in lbl_c]
        ...
        if val_c:
            ...
            fig8.add_annotation(text=f"<b>{total_c/1000:.1f}k$</b>", x=0.5, y=0.5,
                font=dict(size=16, color=C['text'], family='Space Grotesk'), showarrow=False)
        ...
        for nm, cl, vl in zip(_all_lbl_c, _all_clr_c, _all_val_c):
            active = nm in sel_c
            pct_ = vl / sum(_all_val_c) * 100 if sum(_all_val_c) else 0
            op = "1" if active else "0.3"
            st.markdown(f"""<div style="display:flex;align-items:center;justify-content:space-between;
padding:5px 8px;border-radius:6px;margin-bottom:3px;background:{C['card']};opacity:{op}">
<div style="display:flex;align-items:center;gap:8px">
...
<span style="font-size:12px;color:{cl};font-weight:700">{vl:,.0f} $ · {pct_:.1f}%</span>
        ...
        _st_rows = [(r["nom"], _C3_COLORS.get(r["tk"], "#888"), r["staking_usd"])
                    for r in _c3_rows if abs(r["staking_usd"]) > 0.001 and r["tk"] != "USDC"]
        for nm, clr, g in _st_rows:
            _gcol = C['green'] if g >= 0 else C['red']
            _sign = '+' if g >= 0 else ''
...
<span style="font-family:'Space Grotesk';font-weight:700;color:{_gcol}">{_sign}{g:.2f} $</span></div>""", unsafe_allow_html=True)
        _tot_st = sum(x[2] for x in _st_rows)
        if _st_rows:
            _tot_col = C['green'] if _tot_st >= 0 else C['red']
            _tot_sign = '+' if _tot_st >= 0 else ''
            st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:10px 0">
<span style="color:{C['muted']};font-size:12px">TOTAL STAKING NET</span>
<span style="font-family:'Space Grotesk';font-size:16px;font-weight:700;color:{_tot_col}">{_tot_sign}{_tot_st:.2f} $</span></div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"<div style='color:{C['muted']};font-size:13px'>Aucun gain/perte de staking détecté.</div>", unsafe_allow_html=True)
...
with tab8:
    if not st.session_state.get('ibkr_data'):
        st.markdown(f"<div style='text-align:center;padding:60px;color:{_C_MUTED};font-size:13px'>📂 Chargez vos relevés IBKR dans l'onglet Options pour accéder à l'analyse.</div>", unsafe_allow_html=True)
    else:
...
En dessous du PRU Ajusté = seuil pour vendre des calls sans risque de perte en cas d'assignation.</div>"""
        ...
        for _yr in _years_disp:
            ...
            for mois in range(1, 13):
                ...
                _border_top = f'2px solid {_col_v}' if v_eur != 0 else f'2px solid {_C_BORDER}'
                if v_eur != 0:
...
with tab5:
    ...
    # Placeholder KPI — sera rempli après correction des valeurs crypto
    ...
    # ── Valeurs Crypto depuis session_state (calculées dans tab3, 100% GitHub+yfinance) ──
    _perf_crypto_val_eur = st.session_state.get('crypto_val_eur', None)
    _perf_crypto_inv_eur = st.session_state.get('crypto_inv_eur', None)
    # Si tab3 pas encore rendu ce run, on relit directement le JSON
    if _perf_crypto_val_eur is None or _perf_crypto_inv_eur is None:
        try:
            _pc3_raw, _ = gh_read("crypto_positions.json")
            _pc3 = json.loads(_pc3_raw.decode('utf-8')) if _pc3_raw else {}
            _pc3_pos = _pc3.get("positions", {})
            _pc3_prices, _pc3_fx = _fetch_crypto_prices() if '_fetch_crypto_prices' in dir() else ({}, 1.14)
            _perf_crypto_val_eur = sum(
                _pc3_pos.get(tk, {}).get("detenu", 0) * _pc3_prices.get(tk, 0) / _pc3_fx
                for tk in ["BTC","ETH","SOL","BNB","USDC"]
            ) if _pc3_prices else 0.0
            _perf_crypto_inv_eur = _pc3.get("investi_eur_base", 0.0)
        except Exception:
            _perf_crypto_val_eur = 0.0
            _perf_crypto_inv_eur = _pi_c  # fallback Sheet
    _pi_c_real = _perf_crypto_inv_eur if _perf_crypto_inv_eur else _pi_c
    _pl_c_real = _perf_crypto_val_eur if _perf_crypto_val_eur else _pl_c

    # Mise à jour KPI avec vraies valeurs crypto
    _pi_c  = _pi_c_real
    _pl_c  = _pl_c_real
    inv_t  = _pi_b + _pi_c + _pi_o
    live_t = _pl_b + _pl_c + _pl_o
    ...
    poches=[
            ...
            ("₿ Crypto (Binance)",  _pi_c_real, _pl_c_real,        C['gold'],   _dt_perf(2022,8,4)),
            ("⚙️ Options (IBK)",
    ...
    # ── KPIs mis à jour avec vraies valeurs crypto ──
    st.session_state['pvmv_t'] = pvmv_t  # ← utilisé par le bouton Snapshot pour cohérence avec le KPI
    _tri_str = pct(_tri_g) if _tri_g is not None else '—'
    _kpi_placeholder.markdown(
        f'<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;margin-bottom:0px">'
        + card('TOTAL INVESTI', fmt(inv_t), '', C['blue'], '💰')
        + card('VALEUR ACTUELLE', fmt(live_t), '', C['cyan'], '📈')
        + card('PV / MV TOTAL', fmt(pvmv_t), pct(pp_t), pcol(pp_t), '💹')
        + card('TRI ANNUALISÉ', _tri_str, 'global, XIRR', C['purple'], '⚡')
        + '</div>',
        unsafe_allow_html=True
    )
    ...
    with Lp:
        ...
        _fn1 = f"<div style='font-size:10px;color:{C['muted']};padding:1px 2px 0 2px;margin-top:-2px;font-style:italic'>TRI (XIRR) calculé sur les versements réels : Bourse/Crypto = DCA le 4 de chaque mois depuis févr. 2022 / août 2022, Options = dépôts IBKR réels depuis déc. 2023.</div>"
        st.markdown(_tbl_perf + _fn1, unsafe_allow_html=True)
    ...
    with Rp:
        ...
        _fn2 = f'<div style="font-size:10px;color:{_C_MUTED};padding:1px 2px 0 2px;margin-top:-2px;font-style:italic">TRI (XIRR) : Bourse/Crypto = DCA le 4 de chaque mois depuis févr. 2022 / août 2022, Options = dépôts IBKR réels. Benchmarks simulés avec les mêmes flux (SP500 TR, CAC40 GR, STOXX 600). Alpha = TRI Mon PF − TRI Indice (pts %).</div>'
        st.markdown(_tbl_bench + _fn2, unsafe_allow_html=True)
    ...
    if _snaps:
        ...
        _mois_cur = datetime.now(_TZ_PARIS).strftime("%Y-%m")
        _MOIS_FR = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"]
...
(PV/MV actuelle : <b style='color:{_C_GREEN}'>{fmt(pvmv_t)}</b>, TRI : <b style='color:{C["purple"]}'>{pct(_tri_g) if _tri_g else "—"}</b>).
    ...
    with col_form:
        ...
        if ticker_input and len(ticker_input) >= 2:
            ...
            if hist_data and hist_data.get("years"):
                ...
                st.markdown(f"<div style='font-size:9px;color:{_C_MUTED};margin-bottom:8px;font-style:italic'>⚠️ Données Yahoo Finance · Vérifier avant utilisation · Colonnes = années les plus récentes en premier</div>", unsafe_allow_html=True)
  ...
  <span style="font-size:12px;color:{_score_col};font-weight:600">{_score_lbl}</span>
        ...
        _kpi_data_h = [
            ("💰 PRIX À PAYER BASE",
             ...
             _C_GREEN if upside_kpi and upside_kpi > 0 else _C_RED if upside_kpi is not None else C_VAL,
            ...
            ("📈 UPSIDE BASE",
             ...
             "vs cours actuel",
             _C_GREEN if upside_kpi and upside_kpi > 0 else _C_RED if upside_kpi is not None else _C_MUTED,
        ...
        for col, scen in zip(s_cols, ["BASE","BEAR","BULL"]):
            ...
            with col:
                if r and r.get("price"):
                    ...
                    formula_line = f"<div style='font-size:8px;color:{_C_MUTED};font-family:monospace;margin-top:4px;padding-top:4px;border-top:1px solid {_C_BORDER}'>VI={p:,.0f}€ ÷ (1+{perf_v:.0f}%)^{ny} × (1−{ms_v:.0f}%) = {pp:,.0f}€</div>"
  ...
  <th style="padding:8px 12px;text-align:left;border-bottom:1px solid {C['border']}">Scénario</th>"""
                ...
                for scen, icon, color, eps_proj, cagr_eps in proj_rows:
                    ...
                    for i, eps_v in enumerate(eps_proj):
                        ...
                        sub = f"<div style='font-size:9px;color:{_C_MUTED};font-family:monospace;margin-top:1px'>${eps_usd:.1f}</div>" if is_usd else ""
...
with tab7:
    ...
    def get_statut_info(w):
        ...
        if ecart > 0:
            return "buy",   _C_GREEN, "💎 ACHAT IDÉAL 💎",      _C_GREEN
        elif ecart >= -10.01:
            ...
            return "avoid", _C_RED,   "🚫 SURCOTE 🚫",            _C_RED
    ...
    if not wl:
        st.markdown(f'''<div style="background:{_C_CARD};border:1px solid {C_WL}44;border-radius:12px;padding:40px;text-align:center;color:{_C_MUTED};margin-top:16px">
...
<div style="font-size:13px">Calcule une valorisation dans l'onglet <b>🔍 Valorisation</b>, choisis une liste et clique sur <b>⭐ Watchlist</b></div>
        ...
        kc = st.columns(6)
        kpi_data = [
            ("💎 ACHAT IDÉAL",  str(nb_ideal)      if nb_total else "—", f"{nb_ideal} action(s)",      _C_GREEN,  "💎"),
            ...
            ("🚫 SURCOTE",      str(nb_surcote)    if nb_total else "—", f"{nb_surcote} action(s)",    _C_RED,    "🚫"),
            ("📊 ÉCART MOYEN",  f"{avg_ecart_all:+.1f}%" if wl else "—", f"{nb_total} analysées",
             _C_GREEN if avg_ecart_all >= 0 else _C_RED, "📊"),
        ]
        ...
        _ths = "font-size:10px;color:{m};text-transform:uppercase;letter-spacing:.06em;border-bottom:1px solid {b};padding:8px 12px;white-space:nowrap".format(m=_C_MUTED, b=_C_BORDER)
        ...
        for list_key, list_label, list_color in SUB_LISTS:
            ...
            for idx, w in enumerate(sorted_e):
                ...
                else:                  ecart_col=_C_RED;  ecart_bg="#2A0D0D"
                ...
                _ct = _C_TEXT; _cm = _C_MUTED; _ccyan = C["cyan"]
                with row_cols[0]:
                    st.markdown(f"<div style='padding:8px 4px;font-weight:600;font-size:12px;color:{_ct}'>{w['name']}</div>", unsafe_allow_html=True)
                with row_cols[1]:
                    _tv_url = f"https://www.tradingview.com/chart/?symbol={ticker_w}" if ticker_w else "#"
                    _ticker_html = (
                        f'<a href="{_tv_url}" target="_blank" style="'
                        f'font-family:monospace;font-size:11px;font-weight:700;color:{list_color};'
                        f'text-decoration:none;border-bottom:1px dashed {list_color}88;'
                        f'padding-bottom:1px" title="Ouvrir sur TradingView">'
                        f'{ticker_w} 📈</a>'
                    ) if ticker_w else '<span style="color:#475569">—</span>'
                    st.markdown(
                        f"<div style='padding:8px 4px;text-align:center'>{_ticker_html}</div>",
                        unsafe_allow_html=True
                    )
                with row_cols[2]:
                    # PRU éditable
...
.mkt-card-chart { flex: 1 !important; overflow: hidden; }
.mkt-card-chart svg { width: 100% !important; height: auto !important; display: block; }
</style>""", unsafe_allow_html=True)
        with st.expander("📊 Indices · VIX · Fear & Greed — variation journalière", expanded=True):
            ...
            <div style="font-size:12px;color:{C['muted']};margin-top:6px">⚠️ Données indisponibles</div>
            <div style="font-size:10px;color:{C['muted']};opacity:0.6;margin-top:2px">yfinance inaccessible · Rafraîchir</div></div>""", unsafe_allow_html=True)
                        continue
                    ...
                    _PAD = 8
                    _DRAW = _SH - 2 * _PAD
                    # Normalisation : 0 toujours visible, ligne zéro à 15% du bas minimum
                    _smn_real = min(_serie_pct)
                    _smx_real = max(_serie_pct)
                    # Forcer une marge sous/dessus le zéro pour qu'il soit visible
                    _margin = max(abs(_smx_real), abs(_smn_real)) * 0.15
                    _smn = min(_smn_real, -_margin)
                    _smx = max(_smx_real,  _margin)
                    ...
                    _zero_line = f'<line x1="0" y1="{_sy0:.1f}" x2="{_SW}" y2="{_sy0:.1f}" stroke="#ffffff" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.25"/>'
                    _ssvg = (
                        ...
                        f' overflow="hidden">'
                        f'<defs><clipPath id="cp"><rect x="0" y="0" width="{_SW}" height="{_SH}"/></clipPath></defs>'
                    ...
                    st.markdown(
                        f'<div class="mkt-card" style="background:{_C_CARD};border:1px solid {_C_BORDER}">'
                        f'<div class="mkt-card-body">'
                        f'<div style="font-size:11px;color:{_C_MUTED};text-transform:uppercase;letter-spacing:.05em">{_micon} {_mname}</div>'
            ...
            with _mkt_cols[-2]:
                ...
                if _vix_snap is None:
                    st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px">
        <div style="font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em">😱 VIX — Peur</div>
        <div style="font-size:12px;color:{C['muted']};margin-top:6px">⚠️ Données indisponibles</div>
        <div style="font-size:10px;color:{C['muted']};opacity:0.6;margin-top:2px">yfinance inaccessible · Rafraîchir</div></div>""", unsafe_allow_html=True)
                else:
                    ...
                    _VPAD = 8
                    _VDRAW = _VH - 2 * _VPAD
                    _vmn_real = min(_vix_pct)
                    _vmx_real = max(_vix_pct)
                    _vmargin = max(abs(_vmx_real), abs(_vmn_real)) * 0.15
                    _vmn = min(_vmn_real, -_vmargin)
                    _vmx = max(_vmx_real,  _vmargin)
                    _vrng = (_vmx - _vmn) if _vmx != _vmn else 1
                    _vzy = _VPAD + _VDRAW * (1 - (0 - _vmn) / _vrng)
                    _vpts = ' '.join(
                        f"{i/max(len(_vix_pct)-1,1)*_VW:.1f},{_VPAD + _VDRAW*(1-(v-_vmn)/_vrng):.1f}"
                        for i,v in enumerate(_vix_pct)
                    )
                    _vpoly = f"0,{_VH} {_vpts} {_VW},{_VH}"
                    _vzeroline = f'<line x1="0" y1="{_vzy:.1f}" x2="{_VW}" y2="{_vzy:.1f}" stroke="#ffffff" stroke-width="0.8" stroke-dasharray="4,4" opacity="0.25"/>'
                    _vsvg = (
                        f'<svg viewBox="0 0 {_VW} {_VH}" xmlns="http://www.w3.org/2000/svg" ' +
                        f'style="width:100%;display:block;border-radius:0 0 6px 6px">' +
                        f'<polygon points="{_vpoly}" fill="{_vix_var_col}" opacity="0.12"/>' +
                        _vzeroline +
                        f'<polyline points="{_vpts}" fill="none" stroke="{_vix_var_col}" ' +
                        f'stroke-width="1.5" stroke-linejoin="round"/></svg>'
                    )
                    st.markdown(
                        f'<div class="mkt-card" style="background:{_C_CARD};border:1px solid {_C_BORDER}">' +
                        f'<div class="mkt-card-body">' +
                        f'<div style="font-size:11px;color:{_C_MUTED};text-transform:uppercase;letter-spacing:.05em">😱 VIX — Peur</div>' +
                        f'<div style="display:flex;align-items:baseline;gap:8px;margin-top:2px">' +
                        ...
                        _vsvg +
                        f'</div>' +
                        f'<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 10px 6px 10px">' +
                        f'<div style="padding:2px 10px;border-radius:4px;background:{_vix_col}22;border:1px solid {_vix_col}55">' +
                        f'<span style="font-size:11px;font-weight:700;color:{_vix_col}">{_vix_label}</span></div>' +
                        f'<span style="font-size:8px;color:{_C_MUTED}">&lt;15 Calme &middot; &lt;20 Normal &middot; &lt;30 Prudence &middot; &lt;40 Stress</span>' +
                        f'</div></div>',
                        unsafe_allow_html=True
                    )
            ...
            <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    # Ticks + labels repères 25,50,75 (0 et 100 redondants avec Ext.Fear/Greed)
                    _ticks_svg = ''
                    for _v, _lbl in [(0,''),(25,'25'),(50,'50'),(75,'75'),(100,'')]:
                        _ta = 180 - _v * 1.8
                        _tx1, _ty1 = _pt(_ta, _R + 4)
                        _tx2, _ty2 = _pt(_ta, _R + 14)
                        _tlx, _tly = _pt(_ta, _R + 26)
                        _ticks_svg += f'<line x1="{_tx1:.1f}" y1="{_ty1:.1f}" x2="{_tx2:.1f}" y2="{_ty2:.1f}" stroke="#888" stroke-width="1.5"/>'
                        if _lbl:
                            _ticks_svg += f'<text x="{_tlx:.1f}" y="{_tly:.1f}" text-anchor="middle" dominant-baseline="middle" font-size="11" font-weight="600" fill="#AAAAAA">{_lbl}</text>'
  ...
  <div style="padding:0 14px 10px 14px;display:flex;justify-content:space-between;align-items:center">
    ...
    with st.expander("📅 Calendrier Earnings", expanded=True):
        ...
        if not _earn_tickers:
            ...
            # Badge sources — vert si date connue dans yfinance, orange sinon
            _earn_tickers_with_date = {e['ticker'] for e in _earn_list}
            _src_html = ' '.join([
                f'<span style="background:{"#14532D" if t in _earn_tickers_with_date else "#1C1007"};'
                f'border:1px solid {"#22C55E" if t in _earn_tickers_with_date else "#F97316"};'
                f'border-radius:4px;padding:2px 8px;font-size:11px;'
                f'color:{"#86EFAC" if t in _earn_tickers_with_date else "#FDBA74"}">{t}</span>'
            ...
            if not _earn_list:
                st.markdown(f"""<div style="background:{C['card']};border:1px solid #F59E0B44;border-radius:8px;padding:12px 16px;color:#FCD34D;font-size:13px">
⚠️ Aucune date d'earnings trouvée — yfinance ne dispose pas de calendrier pour ces tickers, ou les données sont temporairement indisponibles.
</div>""", unsafe_allow_html=True)
            else:
    ...
    with st.expander("💰 Calendrier Dividendes", expanded=True):
        ...
        if not _div_tickers:
            ...
            _div_tickers_with_date = {d['ticker'] for d in _div_list}
            _div_src_html = ' '.join([
                f'<span style="background:{"#14532D" if t in _div_tickers_with_date else "#1C1007"};'
                f'border:1px solid {"#22C55E" if t in _div_tickers_with_date else "#F97316"};'
                f'border-radius:4px;padding:2px 8px;font-size:11px;'
                f'color:{"#86EFAC" if t in _div_tickers_with_date else "#FDBA74"}">{t}</span>'
            ...
            if not _div_list:
                st.markdown(f"""<div style="background:{C['card']};border:1px solid #818CF844;border-radius:8px;padding:12px 16px;color:#A5B4FC;font-size:13px">
ℹ️ Aucune date ex-dividende à venir — ces actions ne versent pas de dividende, ou la date n'est pas encore publiée sur yfinance.
</div>""", unsafe_allow_html=True)
            else:
...
with tab_saisie:
    sec("Saisie DCA mensuel", "✏️", "#818CF8", "#0D0F1A")
    st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:16px'>Ouvre la section correspondant à ton opération.</div>", unsafe_allow_html=True)

    _dca_raw, _dca_sha = gh_read("historique_dca.json")
    _pos_raw, _pos_sha = gh_read("crypto_positions.json")
    try:
        _dca_hist = json.loads(_dca_raw.decode('utf-8')) if _dca_raw else {"bourse": {"versements": []}, "crypto": {"versements": []}}
    except Exception as _e_dca:
        st.error(f"❌ Impossible de lire historique_dca.json — JSON invalide : {_e_dca}")
        _dca_hist = {"bourse": {"versements": []}, "crypto": {"versements": []}}
    try:
        _pos_data = json.loads(_pos_raw.decode('utf-8')) if _pos_raw else {"positions": {}}
    except Exception as _e_pos:
        st.error(f"❌ Impossible de lire crypto_positions.json — JSON invalide : {_e_pos}. Vérifie le fichier sur GitHub et restaure-le si nécessaire.")
        _pos_data = {"positions": {}}
    CRYPTO_CONNUS = ["BTC", "ETH", "SOL", "BNB", "USDC"]
    _yr_now = datetime.now(_TZ_PARIS).year

    # ── Helpers de rendu (définis au niveau de l'onglet pour être accessibles partout) ──
    def _entry_amount_crypto(v):
        if 'total' in v:
            return float(v['total'])
        return sum(
            d.get('investi_usd', 0) if isinstance(d, dict) else (d if isinstance(d, (int, float)) else 0)
            for k, d in v.items() if k not in ('date', '_note', 'total', 'montant_eur', 'frais_depot_eur')
        )

    def _render_tbl_vers_crypto(_vers_in, _label_tot):
        _all_tks = sorted(set(k for v in _vers_in for k in v if k not in ("date", "_note", "total", "montant_eur", "frais_depot_eur")))
        _tbl = f"<table style='width:100%;border-collapse:collapse;font-size:11px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:3px'>Date</th>"
        for _tk in _all_tks: _tbl += f"<th style='text-align:right;padding:3px'>{_tk}</th>"
        _tbl += "<th style='text-align:right;padding:3px'>Total</th></tr></thead><tbody>"
        _totals = {t: 0.0 for t in _all_tks}
        _tot_general = 0.0
        for v in sorted(_vers_in, key=lambda x: x['date'], reverse=True):
            _row_tot = _entry_amount_crypto(v)
            _tot_general += _row_tot
            _tbl += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:3px'>{v['date']}</td>"
            for _tk in _all_tks:
                _d = v.get(_tk)
                _a = _d.get('investi_usd', 0) if isinstance(_d, dict) else (_d if isinstance(_d, (int, float)) else 0)
                _totals[_tk] += _a
                _tbl += f"<td style='text-align:right;padding:3px'>{'—' if _a==0 else f'{_a:.0f}$'}</td>"
            _tbl += f"<td style='text-align:right;padding:3px;font-weight:700'>{_row_tot:.0f}$</td></tr>"
        _tbl += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td style='padding:3px'>{_label_tot}</td>"
        for _tk in _all_tks: _tbl += f"<td style='text-align:right;padding:3px'>{_totals[_tk]:.0f}$</td>"
        _tbl += f"<td style='text-align:right;padding:3px'>{_tot_general:.0f}$</td></tr></tbody></table>"
        return _tbl

    def _render_tbl_conv(_conv_in, _label_tot):
        _tbl = f"<table style='width:100%;border-collapse:collapse;font-size:11px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:3px'>Date</th><th style='text-align:left;padding:3px'>De</th><th style='text-align:left;padding:3px'>Vers</th><th style='text-align:right;padding:3px'>Qté vendue</th><th style='text-align:right;padding:3px'>Qté achetée</th><th style='text-align:right;padding:3px'>$ vendu</th><th style='text-align:right;padding:3px'>$ acheté</th></tr></thead><tbody>"
        _tot_src = _tot_dst = 0.0
        for _cv in sorted(_conv_in, key=lambda x: x['date'], reverse=True):
            _v_src = _cv.get('valeur_usd_vendue', _cv.get('valeur_usd', 0))
            _v_dst = _cv.get('valeur_usd_achetee', _cv.get('valeur_usd', 0))
            _tot_src += _v_src; _tot_dst += _v_dst
            _tbl += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:3px'>{_cv['date']}</td><td style='padding:3px'>{_cv['de']}</td><td style='padding:3px'>{_cv['vers']}</td><td style='text-align:right;padding:3px'>{_cv['qte_vendue']:.8f}</td><td style='text-align:right;padding:3px'>{_cv['qte_achetee']:.8f}</td><td style='text-align:right;padding:3px'>{_v_src:.2f}$</td><td style='text-align:right;padding:3px'>{_v_dst:.2f}$</td></tr>"
        _tbl += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td colspan='5' style='padding:3px'>{_label_tot}</td><td style='text-align:right;padding:3px'>{_tot_src:.2f}$</td><td style='text-align:right;padding:3px'>{_tot_dst:.2f}$</td></tr></tbody></table>"
        return _tbl

    def _render_tbl_retrait(_ret_in, _label_tot):
        _tbl = f"<table style='width:100%;border-collapse:collapse;font-size:12px'><thead><tr style='color:{C['muted']}'><th style='text-align:left;padding:4px'>Date</th><th style='text-align:left;padding:4px'>Enveloppe</th><th style='text-align:right;padding:4px'>Montant</th><th style='text-align:left;padding:4px'>Note</th></tr></thead><tbody>"
        _tot = 0.0
        for v in sorted(_ret_in, key=lambda x: x['date'], reverse=True):
            _tot += v['montant_eur']
            _tbl += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:4px'>{v['date']}</td><td style='padding:4px'>{v['enveloppe']}</td><td style='text-align:right;padding:4px;color:#F59E0B'>{v['montant_eur']:.0f}€</td><td style='padding:4px;font-size:10px;color:{C['muted']}'>{v.get('note','')}</td></tr>"
        _tbl += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td colspan='2' style='padding:4px'>{_label_tot}</td><td style='text-align:right;padding:4px;color:#F59E0B'>{_tot:.0f}€</td><td></td></tr></tbody></table>"
        return _tbl

    # ── BOURSE ──
    with st.expander("📈 Versement Bourse (CTO + PEA)", expanded=False):
        with st.form("form_bourse_dca"):
            _hb1, _hb2, _hb3 = st.columns(3)
            with _hb1:
                _date_b = st.date_input("Date du versement", value=datetime.now(_TZ_PARIS).date(), key="date_b")
            with _hb2:
                _cto_b  = st.number_input("Montant CTO (€)", min_value=0.0, step=1.0, format="%.2f")
            with _hb3:
                _pea_b  = st.number_input("Montant PEA (€)", min_value=0.0, step=1.0, format="%.2f")
            _submit_b = st.form_submit_button("💾 Enregistrer", use_container_width=True)
        if _submit_b:
            if _cto_b > 0 or _pea_b > 0:
                _dca_hist["bourse"]["versements"].append({"date": str(_date_b), "cto": round(_cto_b,2), "pea": round(_pea_b,2)})
                _dca_hist["bourse"]["versements"].sort(key=lambda x: x["date"])
                _dca_raw_b, _dca_sha_b = gh_read("historique_dca.json")
                _ok_b = gh_write("historique_dca.json",
                    json.dumps(_dca_hist, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"DCA Bourse {_date_b}: CTO={_cto_b}€ PEA={_pea_b}€", _dca_sha_b)
                if _ok_b: st.success(f"✅ Bourse enregistré — {_date_b} · CTO {_cto_b:.0f}€ + PEA {_pea_b:.0f}€")
                else: st.error("❌ Erreur GitHub")
            else: st.warning("Saisis au moins un montant > 0")

        st.markdown("<div style='margin-top:14px'></div>", unsafe_allow_html=True)
        if st.toggle("📋 Voir l'historique des versements Bourse", key="toggle_hist_bourse"):
            _vers_b = [v for v in _dca_hist["bourse"]["versements"] if v["date"].startswith(str(_yr_now))]
            if _vers_b:
                _tbl_b = f"""<table style='width:100%;border-collapse:collapse;font-size:12px'>
<thead><tr style='color:{_C_MUTED}'><th style='text-align:left;padding:4px'>Date</th>
<th style='text-align:right;padding:4px'>CTO</th><th style='text-align:right;padding:4px'>PEA</th>
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
                _tot_cto = _tot_pea = 0.0
                for v in sorted(_vers_b, key=lambda x: x['date'], reverse=True):
                    _tc = v.get('cto',0); _tp = v.get('pea',0); _tot_cto += _tc; _tot_pea += _tp
                    _tbl_b += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:4px'>{v['date']}</td><td style='text-align:right;padding:4px'>{_tc:.2f}€</td><td style='text-align:right;padding:4px'>{_tp:.2f}€</td><td style='text-align:right;padding:4px;font-weight:700'>{_tc+_tp:.2f}€</td></tr>"
                _tbl_b += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td style='padding:4px'>TOTAL {_yr_now}</td><td style='text-align:right;padding:4px'>{_tot_cto:.2f}€</td><td style='text-align:right;padding:4px'>{_tot_pea:.2f}€</td><td style='text-align:right;padding:4px'>{_tot_cto+_tot_pea:.2f}€</td></tr></tbody></table>"
                st.markdown(_tbl_b, unsafe_allow_html=True)
            else:
                st.info("Aucun versement enregistré cette année.")

            _yrs_b_passees = sorted(set(v["date"][:4] for v in _dca_hist["bourse"]["versements"] if v["date"][:4] != str(_yr_now)), reverse=True)
            for _yr_p in _yrs_b_passees:
                _vers_yp = [v for v in _dca_hist["bourse"]["versements"] if v["date"].startswith(_yr_p)]
                _tot_yp = sum(v.get('cto',0) + v.get('pea',0) for v in _vers_yp)
                if st.toggle(f"▶ {_yr_p} — {len(_vers_yp)} versements, {_tot_yp:.0f}€", key=f"toggle_hist_bourse_{_yr_p}"):
                    _tbl_yp = f"""<table style='width:100%;border-collapse:collapse;font-size:12px'>
<thead><tr style='color:{_C_MUTED}'><th style='text-align:left;padding:4px'>Date</th>
<th style='text-align:right;padding:4px'>CTO</th><th style='text-align:right;padding:4px'>PEA</th>
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
                    _tc_yp = _tp_yp = 0.0
                    for v in sorted(_vers_yp, key=lambda x: x['date'], reverse=True):
                        _tc = v.get('cto',0); _tp = v.get('pea',0); _tc_yp += _tc; _tp_yp += _tp
                        _tbl_yp += f"<tr style='border-top:1px solid {C['border']}'><td style='padding:4px'>{v['date']}</td><td style='text-align:right;padding:4px'>{_tc:.2f}€</td><td style='text-align:right;padding:4px'>{_tp:.2f}€</td><td style='text-align:right;padding:4px;font-weight:700'>{_tc+_tp:.2f}€</td></tr>"
                    _tbl_yp += f"<tr style='border-top:2px solid {C['border']};font-weight:700'><td style='padding:4px'>TOTAL {_yr_p}</td><td style='text-align:right;padding:4px'>{_tc_yp:.2f}€</td><td style='text-align:right;padding:4px'>{_tp_yp:.2f}€</td><td style='text-align:right;padding:4px'>{_tc_yp+_tp_yp:.2f}€</td></tr></tbody></table>"
                    st.markdown(_tbl_yp, unsafe_allow_html=True)

    # ── CRYPTO (Versement + Conversion dans un seul onglet, formulaires distincts) ──
    with st.expander("₿ Crypto — Versement & Conversion", expanded=False):
        st.markdown(f"<div style='font-size:11px;font-weight:700;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px'>💶 Versement (virement bancaire)</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:12px'>Le virement bancaire arrive sur Binance et se convertit automatiquement en <b>USDC</b>. Pour acheter BTC/ETH/SOL/BNB (ou autre) ensuite, utilise le formulaire <b>🔄 Conversion</b> juste en dessous.</div>", unsafe_allow_html=True)
        with st.form("form_crypto_dca"):
            _hc1, _hc2 = st.columns(2)
            with _hc1:
                _date_c = st.date_input("Date du versement", value=datetime.now(_TZ_PARIS).date(), key="date_c")
            with _hc2:
                _frais_depot = st.number_input("Frais de dépôt bancaire (€)", min_value=0.0, step=0.01, format="%.2f", key="frais_depot",
                                               help="Frais du virement de ta banque vers Binance — unique pour tout le versement")
            st.markdown("<hr style='border-color:#1E293B;margin:8px 0'>", unsafe_allow_html=True)
            _hc3, _hc4 = st.columns(2)
            with _hc3:
                _montant_eur = st.number_input("Montant versé (€)", min_value=0.0, step=0.01, format="%.2f", key="montant_eur",
                                                help="Le montant viré depuis ta banque, avant conversion en USDC")
            with _hc4:
                _montant_usdc = st.number_input("Montant reçu en USDC ($)", min_value=0.0, step=0.01, format="%.2f", key="montant_usdc",
                                                 help="Le montant en $ effectivement crédité en USDC sur Binance après conversion")
            _submit_c = st.form_submit_button("💾 Enregistrer le versement", use_container_width=True)
        if _submit_c:
            if _montant_usdc <= 0:
                st.warning("Saisis un montant USDC > 0")
            else:
                _entry_c = {"date": str(_date_c), "USDC": {"investi_usd": round(_montant_usdc,2), "achete": round(_montant_usdc,2)}}
                if _montant_eur > 0: _entry_c["montant_eur"] = round(_montant_eur, 2)
                if _frais_depot > 0: _entry_c["frais_depot_eur"] = round(_frais_depot, 2)
                _dca_hist["crypto"]["versements"].append(_entry_c)
                _dca_hist["crypto"]["versements"].sort(key=lambda x: x["date"])
                _dca_raw_c, _dca_sha_c = gh_read("historique_dca.json")
                _ok1 = gh_write("historique_dca.json",
                    json.dumps(_dca_hist, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"DCA Crypto {_date_c}", _dca_sha_c)
                _pos = _pos_data.get("positions", {})
                if "USDC" not in _pos: _pos["USDC"] = {"nom": "USDC", "investi_usd": 0, "investi_eur": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                _pos["USDC"]["investi_usd"] = round(_pos["USDC"]["investi_usd"] + _montant_usdc, 2)
                _pos["USDC"]["investi_eur"] = round(_pos["USDC"].get("investi_eur", 0) + _montant_eur, 2)
                _pos["USDC"]["achete"]      = round(_pos["USDC"]["achete"]      + _montant_usdc, 2)
                _pos["USDC"]["detenu"]      = round(_pos["USDC"].get("detenu", 0) + _montant_usdc, 2)
                _pos_data["positions"] = _pos
                _pos_data["_last_update"] = str(_date_c)
                _pos_raw2, _pos_sha2 = gh_read("crypto_positions.json")
                _ok2 = gh_write("crypto_positions.json",
                    json.dumps(_pos_data, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"Positions crypto update {_date_c}", _pos_sha2)
                if _ok1 and _ok2: st.success(f"✅ {_montant_usdc:.2f}$ USDC enregistré — {_date_c}")
                else: st.error("❌ Erreur GitHub")

        st.markdown("<hr style='border-color:#1E293B;margin:20px 0 14px 0'>", unsafe_allow_html=True)
        st.markdown(f"<div style='font-size:11px;font-weight:700;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px'>🔄 Conversion (swap entre tokens)</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:12px'>Pour un swap interne (ex: USDC → BTC) — aucun argent ne rentre depuis ta banque, donc ce n'est <b>pas</b> un versement et ça n'impacte pas le XIRR. Ça déplace simplement la valeur d'un token vers un autre dans tes positions.</div>", unsafe_allow_html=True)
        _cryptos_conv = sorted(set(CRYPTO_CONNUS) | set(_pos_data.get("positions", {}).keys()))
        with st.form("form_conversion_crypto"):
            _date_cv = st.date_input("Date de la conversion", value=datetime.now(_TZ_PARIS).date(), key="date_cv")
            _cv1, _cv2 = st.columns(2)
            with _cv1:
                _tk_src = st.selectbox("Token source (vendu)", _cryptos_conv, key="cv_tk_src")
                _qty_src = st.number_input(f"Quantité vendue", min_value=0.0, step=0.00000001, format="%.8f", key="cv_qty_src")
            with _cv2:
                _tk_dst = st.selectbox("Token destination (acheté)", _cryptos_conv, key="cv_tk_dst", index=min(1, len(_cryptos_conv)-1))
                _qty_dst = st.number_input(f"Quantité achetée", min_value=0.0, step=0.00000001, format="%.8f", key="cv_qty_dst")
            _cv3, _cv4 = st.columns(2)
            with _cv3:
                _val_src = st.number_input("Valeur $ vendue (source)", min_value=0.0, step=0.01, format="%.2f", key="cv_val_src",
                                            help="Valeur en USD du montant source vendu")
            with _cv4:
                _val_dst = st.number_input("Valeur $ achetée (destination)", min_value=0.0, step=0.01, format="%.2f", key="cv_val_dst",
                                            help="Valeur en USD effectivement reçue côté destination (peut différer légèrement à cause des frais de swap)")
            _submit_cv = st.form_submit_button("🔄 Enregistrer la conversion", use_container_width=True)
        if _submit_cv:
            if _tk_src == _tk_dst:
                st.warning("Le token source et destination doivent être différents.")
            elif _qty_src <= 0 or _qty_dst <= 0 or _val_src <= 0 or _val_dst <= 0:
                st.warning("Saisis une quantité source, une quantité destination et des valeurs $ > 0 pour les deux côtés.")
            else:
                _pos_cv = _pos_data.get("positions", {})
                if _tk_src not in _pos_cv: _pos_cv[_tk_src] = {"nom": _tk_src, "investi_usd": 0, "investi_eur": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                if _tk_dst not in _pos_cv: _pos_cv[_tk_dst] = {"nom": _tk_dst, "investi_usd": 0, "investi_eur": 0, "achete": 0, "detenu": 0, "frais_usd": 0}
                # part proportionnelle du coût de revient EUR du token source qui part avec ce swap
                _src_inv_usd_avant = _pos_cv[_tk_src].get("investi_usd", 0)
                _src_inv_eur_avant = _pos_cv[_tk_src].get("investi_eur", 0)
                _eur_transfere = round(_src_inv_eur_avant * (_val_src / _src_inv_usd_avant), 2) if _src_inv_usd_avant > 0 else 0.0
                _pos_cv[_tk_src]["detenu"]      = round(_pos_cv[_tk_src].get("detenu", 0) - _qty_src, 8)
                _pos_cv[_tk_src]["investi_usd"] = round(_pos_cv[_tk_src].get("investi_usd", 0) - _val_src, 2)
                _pos_cv[_tk_src]["investi_eur"] = round(_pos_cv[_tk_src].get("investi_eur", 0) - _eur_transfere, 2)
                _pos_cv[_tk_dst]["detenu"]      = round(_pos_cv[_tk_dst].get("detenu", 0) + _qty_dst, 8)
                _pos_cv[_tk_dst]["achete"]      = round(_pos_cv[_tk_dst].get("achete", 0) + _qty_dst, 8)
                _pos_cv[_tk_dst]["investi_usd"] = round(_pos_cv[_tk_dst].get("investi_usd", 0) + _val_dst, 2)
                _pos_cv[_tk_dst]["investi_eur"] = round(_pos_cv[_tk_dst].get("investi_eur", 0) + _eur_transfere, 2)
                _pos_data["positions"] = _pos_cv
                _pos_data["_last_update"] = str(_date_cv)
                if "conversions" not in _pos_data: _pos_data["conversions"] = []
                _pos_data["conversions"].append({
                    "date": str(_date_cv), "de": _tk_src, "vers": _tk_dst,
                    "qte_vendue": round(_qty_src, 8), "qte_achetee": round(_qty_dst, 8),
                    "valeur_usd_vendue": round(_val_src, 2), "valeur_usd_achetee": round(_val_dst, 2)
                })
                _pos_raw_cv, _pos_sha_cv = gh_read("crypto_positions.json")
                _ok_cv = gh_write("crypto_positions.json",
                    json.dumps(_pos_data, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"Conversion {_tk_src}→{_tk_dst} {_date_cv}", _pos_sha_cv)
                if _ok_cv: st.success(f"✅ Conversion enregistrée — {_qty_src:.8f} {_tk_src} → {_qty_dst:.8f} {_tk_dst}")
                else: st.error("❌ Erreur GitHub")

        st.markdown("<hr style='border-color:#1E293B;margin:20px 0 14px 0'>", unsafe_allow_html=True)

        if st.toggle("📋 Voir l'historique des versements crypto", key="toggle_hist_crypto"):
            _vers_c = [v for v in _dca_hist["crypto"]["versements"] if v["date"].startswith(str(_yr_now))]
            if _vers_c:
                st.markdown(_render_tbl_vers_crypto(_vers_c, f"TOTAL {_yr_now}"), unsafe_allow_html=True)
            else:
                st.info("Aucun versement enregistré cette année.")

            _vers_c_tous = _dca_hist["crypto"]["versements"]
            _yrs_c_passees = sorted(set(v["date"][:4] for v in _vers_c_tous if v["date"][:4] != str(_yr_now)), reverse=True)
            for _yr_p in _yrs_c_passees:
                _vers_yp = [v for v in _vers_c_tous if v["date"].startswith(_yr_p)]
                _tot_yp = sum(_entry_amount_crypto(v) for v in _vers_yp)
                if st.toggle(f"▶ {_yr_p} — {len(_vers_yp)} versements, {_tot_yp:.0f}$", key=f"toggle_hist_crypto_{_yr_p}"):
                    st.markdown(_render_tbl_vers_crypto(_vers_yp, f"TOTAL {_yr_p}"), unsafe_allow_html=True)

        if _pos_data.get("conversions"):
            if st.toggle("📋 Voir l'historique des conversions", key="toggle_hist_conv"):
                _conv_yr = [c for c in _pos_data["conversions"] if c["date"].startswith(str(_yr_now))]
                if _conv_yr:
                    st.markdown(_render_tbl_conv(_conv_yr, f"TOTAL {_yr_now}"), unsafe_allow_html=True)
                else:
                    st.info("Aucune conversion enregistrée cette année.")

                _yrs_cv_passees = sorted(set(c["date"][:4] for c in _pos_data["conversions"] if c["date"][:4] != str(_yr_now)), reverse=True)
                for _yr_p in _yrs_cv_passees:
                    _conv_yp = [c for c in _pos_data["conversions"] if c["date"].startswith(_yr_p)]
                    if st.toggle(f"▶ {_yr_p} — {len(_conv_yp)} conversions", key=f"toggle_hist_conv_{_yr_p}"):
                        st.markdown(_render_tbl_conv(_conv_yp, f"TOTAL {_yr_p}"), unsafe_allow_html=True)

    # ── RETRAIT ──
    with st.expander("💸 Retrait vers mon compte bancaire", expanded=False):
        st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:12px'>Un retrait est un flux positif dans le XIRR (argent récupéré). Ex : vente d'actions pour acheter une voiture.</div>", unsafe_allow_html=True)
        with st.form("form_retrait"):
            _hr1, _hr2, _hr3 = st.columns(3)
            with _hr1:
                _date_r = st.date_input("Date du retrait", value=datetime.now(_TZ_PARIS).date(), key="date_r")
            with _hr2:
                _env_r  = st.selectbox("Enveloppe", ["CTO", "PEA", "Crypto (total €)"], key="env_r")
            with _hr3:
                _amt_r  = st.number_input("Montant retiré (€)", min_value=0.0, step=1.0, format="%.2f", key="amt_r")
            _note_r   = st.text_input("Note (optionnel, ex: achat voiture)", key="note_r")
            _submit_r = st.form_submit_button("💾 Enregistrer retrait", use_container_width=True)
        if _submit_r:
            if _amt_r > 0:
                _retrait_entry = {"date": str(_date_r), "type": "retrait", "enveloppe": _env_r, "montant_eur": round(_amt_r, 2), "note": _note_r or ""}
                if "retraits" not in _dca_hist: _dca_hist["retraits"] = []
                _dca_hist["retraits"].append(_retrait_entry)
                _dca_hist["retraits"].sort(key=lambda x: x["date"])
                _dca_raw_r, _dca_sha_r = gh_read("historique_dca.json")
                _ok_r = gh_write("historique_dca.json",
                    json.dumps(_dca_hist, ensure_ascii=False, indent=2).encode('utf-8'),
                    f"Retrait {_env_r} {_date_r}: {_amt_r:.0f}€", _dca_sha_r)
                if _ok_r: st.success(f"✅ Retrait enregistré — {_date_r} · {_env_r} · {_amt_r:.0f}€{' · ' + _note_r if _note_r else ''}")
                else: st.error("❌ Erreur GitHub")
            else: st.warning("Saisis un montant > 0")

        st.markdown("<div style='margin-top:14px'></div>", unsafe_allow_html=True)

        if st.toggle("📋 Voir l'historique des retraits", key="toggle_hist_retrait"):
            _retraits = _dca_hist.get("retraits", [])
            _retraits_yr = [v for v in _retraits if v["date"].startswith(str(_yr_now))]
            if _retraits_yr:
                st.markdown(_render_tbl_retrait(_retraits_yr, f"TOTAL {_yr_now}"), unsafe_allow_html=True)
            else:
                st.info("Aucun retrait enregistré cette année.")

            _yrs_r_passees = sorted(set(v["date"][:4] for v in _retraits if v["date"][:4] != str(_yr_now)), reverse=True)
            for _yr_p in _yrs_r_passees:
                _ret_yp = [v for v in _retraits if v["date"].startswith(_yr_p)]
                if st.toggle(f"▶ {_yr_p} — {len(_ret_yp)} retraits", key=f"toggle_hist_retrait_{_yr_p}"):
                    st.markdown(_render_tbl_retrait(_ret_yp, f"TOTAL {_yr_p}"), unsafe_allow_html=True)

    # ── MODIFIER / SUPPRIMER ──
    with st.expander("✏️ Modifier / Supprimer un versement", expanded=False):
        _edit_type = st.radio("Type d'opération", ["Bourse", "Crypto", "Conversion (swap)"], horizontal=True, key="edit_type")

        if _edit_type in ("Bourse", "Crypto"):
            _vers_list = _dca_hist["bourse"]["versements"] if _edit_type == "Bourse" else _dca_hist["crypto"]["versements"]
            if _vers_list:
                _vers_labels = []
                for v in _vers_list:
                    if _edit_type == "Bourse":
                        _vers_labels.append(f"{v['date']} — CTO {v.get('cto',0):.0f}€ + PEA {v.get('pea',0):.0f}€")
                    else:
                        _tot = v.get('total', 0)
                        for k, d in v.items():
                            if k in ('date', '_note', 'total', 'montant_eur', 'frais_depot_eur'):
                                continue
                            if isinstance(d, dict):   _tot += d.get('investi_usd', 0)
                            elif isinstance(d, (int, float)): _tot += d
                        _vers_labels.append(f"{v['date']} — {_tot:.0f}$")
                _sel_idx = st.selectbox("Versement à modifier", range(len(_vers_labels)), format_func=lambda i: _vers_labels[i], key=f"edit_sel_{_edit_type}")
                st.json(_vers_list[_sel_idx])
                if st.button("🗑️ Supprimer ce versement", key="del_vers"):
                    _deleted_entry = _vers_list[_sel_idx]
                    _vers_list.pop(_sel_idx)
                    _dca_raw3, _dca_sha3 = gh_read("historique_dca.json")
                    _ok_del = gh_write("historique_dca.json",
                        json.dumps(_dca_hist, ensure_ascii=False, indent=2).encode('utf-8'),
                        f"Suppression versement", _dca_sha3)
                    _ok_pos = True
                    if _edit_type == "Crypto":
                        _has_detail = any(isinstance(d, dict) for k, d in _deleted_entry.items() if k not in ('date', '_note', 'total'))
                        if _has_detail:
                            _pos_raw3, _pos_sha3 = gh_read("crypto_positions.json")
                            _pos_data3 = json.loads(_pos_raw3.decode('utf-8')) if _pos_raw3 else {"positions": {}}
                            _pos3 = _pos_data3.get("positions", {})
                            for _tk, _d in _deleted_entry.items():
                                if _tk in ('date', '_note', 'total') or not isinstance(_d, dict):
                                    continue
                                if _tk in _pos3:
                                    _pos3[_tk]["investi_usd"] = round(_pos3[_tk].get("investi_usd", 0) - _d.get("investi_usd", 0), 2)
                                    _pos3[_tk]["achete"]      = round(_pos3[_tk].get("achete", 0)      - _d.get("achete", 0), 8)
                                    _pos3[_tk]["detenu"]      = round(_pos3[_tk].get("detenu", 0)      - _d.get("achete", 0), 8)
                            if "montant_eur" in _deleted_entry and "USDC" in _pos3:
                                _pos3["USDC"]["investi_eur"] = round(_pos3["USDC"].get("investi_eur", 0) - _deleted_entry["montant_eur"], 2)
                            _pos_data3["positions"] = _pos3
                            _ok_pos = gh_write("crypto_positions.json",
                                json.dumps(_pos_data3, ensure_ascii=False, indent=2).encode('utf-8'),
                                f"Correction positions après suppression versement", _pos_sha3)
                        else:
                            st.warning("⚠️ Ce versement n'a pas de détail par token (ancien format) — les positions crypto_positions.json n'ont pas pu être corrigées automatiquement.")
                    if _ok_del and _ok_pos: st.success("✅ Versement supprimé" + (" et positions crypto corrigées" if _edit_type == "Crypto" else ""))
                    else: st.error("❌ Erreur GitHub")
            else:
                st.info("Aucun versement enregistré.")

        else:  # Conversion (swap)
            _convs = _pos_data.get("conversions", [])
            if _convs:
                _conv_labels = [
                    f"{c['date']} — {c['de']} → {c['vers']} · {c.get('valeur_usd_vendue', c.get('valeur_usd',0)):.0f}$ vendu / {c.get('valeur_usd_achetee', c.get('valeur_usd',0)):.0f}$ acheté"
                    for c in _convs
                ]
                _sel_cv = st.selectbox("Conversion à supprimer", range(len(_conv_labels)), format_func=lambda i: _conv_labels[i], key="edit_sel_conv")
                st.json(_convs[_sel_cv])
                if st.button("🗑️ Annuler cette conversion", key="del_conv"):
                    _del_cv = _convs[_sel_cv]
                    _pos_raw_dc, _pos_sha_dc = gh_read("crypto_positions.json")
                    _pos_dc = json.loads(_pos_raw_dc.decode('utf-8')) if _pos_raw_dc else {"positions": {}, "conversions": []}
                    _p = _pos_dc.get("positions", {})
                    _tk_s = _del_cv["de"]; _tk_d = _del_cv["vers"]
                    _v_src = _del_cv.get("valeur_usd_vendue", _del_cv.get("valeur_usd", 0))
                    _v_dst = _del_cv.get("valeur_usd_achetee", _del_cv.get("valeur_usd", 0))
                    _qty_s = _del_cv.get("qte_vendue", 0); _qty_d = _del_cv.get("qte_achetee", 0)
                    # Calcul du € transféré (même logique proportionnelle que lors de la conversion)
                    _dst_inv_usd = _p.get(_tk_d, {}).get("investi_usd", 0)
                    _dst_inv_eur = _p.get(_tk_d, {}).get("investi_eur", 0)
                    _eur_rev = round(_dst_inv_eur * (_v_dst / _dst_inv_usd), 2) if _dst_inv_usd > 0 else 0.0
                    # Annulation côté destination
                    if _tk_d in _p:
                        _p[_tk_d]["detenu"]      = round(_p[_tk_d].get("detenu", 0)      - _qty_d, 8)
                        _p[_tk_d]["achete"]      = round(_p[_tk_d].get("achete", 0)      - _qty_d, 8)
                        _p[_tk_d]["investi_usd"] = round(_p[_tk_d].get("investi_usd", 0) - _v_dst, 2)
                        _p[_tk_d]["investi_eur"] = round(_p[_tk_d].get("investi_eur", 0) - _eur_rev, 2)
                    # Annulation côté source
                    if _tk_s in _p:
                        _p[_tk_s]["detenu"]      = round(_p[_tk_s].get("detenu", 0)      + _qty_s, 8)
                        _p[_tk_s]["investi_usd"] = round(_p[_tk_s].get("investi_usd", 0) + _v_src, 2)
                        _p[_tk_s]["investi_eur"] = round(_p[_tk_s].get("investi_eur", 0) + _eur_rev, 2)
                    # Suppression de la conversion de la liste
                    _pos_dc["conversions"] = [c for i, c in enumerate(_pos_dc.get("conversions", [])) if i != _sel_cv]
                    _pos_dc["positions"] = _p
                    _ok_cv_del = gh_write("crypto_positions.json",
                        json.dumps(_pos_dc, ensure_ascii=False, indent=2).encode('utf-8'),
                        f"Annulation conversion {_tk_s}→{_tk_d} {_del_cv['date']}", _pos_sha_dc)
                    if _ok_cv_del: st.success(f"✅ Conversion annulée et positions corrigées — {_tk_s}→{_tk_d}")
                    else: st.error("❌ Erreur GitHub")
            else:
...
with tab_budget:
    ...
    _MOIS_LABELS = {"2026-01":"Janvier","2026-02":"Février","2026-03":"Mars","2026-04":"Avril",
    ...
    _TYPE_COLORS = {"Revenus":"#10B981","Dépenses":"#F87171","Frais Fixes":"#FBBF24",
        "Épargne & Investissements":"#818CF8","Crédits / Dettes":"#F59E0B"}
    _now_p = datetime.now(_TZ_PARIS)
    _mois_cur_key = _now_p.strftime("%Y-%m")
    _mois_opts_rev = sorted([k for k in _MOIS_LABELS if k <= _mois_cur_key], reverse=True)

    def _sf(v):
        try: return float(v) if v is not None else 0.0
        except: return 0.0

    def _pbar(pct, color, height=6):
        pct_c = min(pct, 100); bar_c = "#F87171" if pct > 100 else color
        return (f"<div style='background:#1F2937;border-radius:3px;height:{height}px;width:100%'>"
                f"<div style='background:{bar_c};width:{pct_c:.0f}%;height:{height}px;border-radius:3px'></div></div>")

    def _build_section_table(titre, color, icon, bp_section, reel_by_cat, show_echeance=False):
        all_cats = list(bp_section.keys())
        for c in reel_by_cat:
            if c not in all_cats: all_cats.append(c)
        if not all_cats: return f"<div style='color:{_C_MUTED};font-size:12px;padding:8px'>Aucune donnée</div>"
        tbl = (f"<table style='width:100%;border-collapse:collapse;font-size:12px'>"
               f"<thead><tr style='background:#0D1117'>"
               f"<th style='padding:6px 8px;text-align:left;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{icon} {titre}</th>")
        if show_echeance:
            tbl += f"<th style='padding:6px 8px;text-align:center;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>ÉCH.</th>"
        tbl += (f"<th style='padding:6px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>PRÉVU</th>"
                f"<th style='padding:6px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉEL</th>"
                f"<th style='padding:6px 8px;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER};min-width:100px'>PROGRESSION</th>"
                "</tr></thead><tbody>")
        tot_p = tot_r = 0
        for cat in all_cats:
            v = bp_section.get(cat, {})
            prevu = _sf(v.get('prevu', v) if isinstance(v, dict) else v)
            reel  = reel_by_cat.get(cat, 0)
            pct   = (reel / prevu * 100) if prevu > 0 else (100 if reel > 0 else 0)
            bar_c = "#F87171" if pct > 100 else color
            pct_lbl = f"{pct:.0f}%" if prevu > 0 else "—"
            ech = v.get('echeance', 0) if isinstance(v, dict) else 0
            tot_p += prevu; tot_r += reel
            tbl += f"<tr style='border-bottom:1px solid {_C_BORDER}'><td style='padding:5px 8px;font-weight:500'>{cat}</td>"
            if show_echeance:
                tbl += f"<td style='padding:5px 8px;text-align:center;color:{_C_MUTED};font-size:10px'>{ech if ech else '—'}</td>"
            tbl += (f"<td style='padding:5px 8px;text-align:right;color:{_C_MUTED}'>{prevu:,.0f}€</td>"
                    f"<td style='padding:5px 8px;text-align:right;font-weight:600;color:{bar_c}'>{reel:,.0f}€</td>"
                    f"<td style='padding:5px 8px'>{_pbar(pct, color)}"
                    f"<span style='font-size:9px;color:{bar_c}'>{pct_lbl}</span></td></tr>")
        tot_pct = (tot_r / tot_p * 100) if tot_p > 0 else 0
        tot_c = "#F87171" if tot_pct > 100 else color
        tbl += (f"<tr style='background:#111827;font-weight:700'><td style='padding:6px 8px'>TOTAL</td>"
                + (f"<td></td>" if show_echeance else "")
                + f"<td style='padding:6px 8px;text-align:right'>{tot_p:,.0f}€</td>"
                f"<td style='padding:6px 8px;text-align:right;color:{tot_c}'>{tot_r:,.0f}€</td>"
                f"<td style='padding:6px 8px'>{_pbar(tot_pct, color)}"
                f"<span style='font-size:9px;color:{tot_c};font-weight:700'>{tot_pct:.0f}%</span></td></tr></tbody></table>")
        return tbl

    _sel_mois = st.selectbox("Mois affiché", _mois_opts_rev,
        format_func=lambda k: _MOIS_LABELS.get(k, k), key="budget_mois_sel")
    _md = _bud_data.get("mois", {}).get(_sel_mois, {"transactions":[],"budget_prevu":{}})
    _txs = _md.get("transactions", [])
    _bp  = _md.get("budget_prevu", {})

    def _rbc(type_filter):
        d = {}
        for t in _txs:
            if t['type'] == type_filter:
                d[t['categorie']] = d.get(t['categorie'], 0.0) + t['montant']
        return d

    _rbc_rev=_rbc("Revenus"); _rbc_dep=_rbc("Dépenses"); _rbc_ff=_rbc("Frais Fixes")
    _rbc_epa=_rbc("Épargne & Investissements"); _rbc_cred=_rbc("Crédits / Dettes")
    _rev_r=sum(_rbc_rev.values()); _dep_r=sum(_rbc_dep.values())
    _ff_r=sum(_rbc_ff.values()); _epa_r=sum(_rbc_epa.values()); _cred_r=sum(_rbc_cred.values())
    _rest_r=_rev_r-_dep_r-_ff_r-_epa_r-_cred_r
    _rev_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("revenus",{}).values())
    _dep_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("depenses",{}).values())
    _ff_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("frais_fixes",{}).values())
    _epa_p=sum(_sf(v.get('prevu',v) if isinstance(v,dict) else v) for v in _bp.get("epargne",{}).values())
    _rest_p=_rev_p-_dep_p-_ff_p-_epa_p

    st.markdown(
        f"<div style='background:linear-gradient(90deg,{_BUD_BG},#0D1F18);border-left:3px solid {_BUD_COLOR};"
        f"padding:10px 16px;border-radius:6px;margin-bottom:12px'>"
        f"<span style='font-size:20px;font-weight:800;color:{_BUD_COLOR}'>{_MOIS_LABELS.get(_sel_mois,'').upper()}</span>"
        f"<span style='font-size:13px;color:{_C_MUTED};margin-left:12px'>Suivi mensuel du budget</span></div>",
        unsafe_allow_html=True)

    _k4 = st.columns(4)
    for _col, (_lbl, _pv, _rv, _clr) in zip(_k4, [
        ("REVENUS", _rev_p, _rev_r, "#10B981"),
        ("DÉPENSES & FF", _dep_p+_ff_p, _dep_r+_ff_r, "#F87171"),
        ("CRÉDITS", 0, _cred_r, "#F59E0B"),
        ("ÉPARGNE & INVEST.", _epa_p, _epa_r, "#818CF8"),
    ]):
        _sub = f"Prévu : {_pv:,.0f}€" if _pv > 0 else ""
        with _col: st.markdown(card(_lbl, f"{_rv:,.2f} €", _sub, _clr, ""), unsafe_allow_html=True)

    _g1, _g2, _g3 = st.columns(3)
    with _g1:
        _fig_bar = go.Figure()
        _cats_bar = ["Revenus","Dépenses","Frais Fixes","Épargne"]
        _pvs_bar  = [_rev_p, _dep_p, _ff_p, _epa_p]
        _rvs_bar  = [_rev_r, _dep_r, _ff_r, _epa_r]
        _fig_bar.add_trace(go.Bar(name='Prévu', x=_cats_bar, y=_pvs_bar,
            marker_color='rgba(99,102,241,0.6)', text=[f"{v:,.0f}" for v in _pvs_bar],
            textposition='outside', textfont=dict(size=9)))
        _fig_bar.add_trace(go.Bar(name='Réel', x=_cats_bar, y=_rvs_bar,
            marker_color='rgba(16,185,129,0.8)', text=[f"{v:,.0f}" for v in _rvs_bar],
            textposition='outside', textfont=dict(size=9)))
        _fig_bar.update_layout(**base_layout(180))
        _fig_bar.update_layout(barmode='group',
            title=dict(text="Entrées & Sorties", font=dict(size=11, color=_C_MUTED), x=0.5),
            legend=dict(orientation='h', y=-0.35, font=dict(size=9)))
        _fig_bar.update_yaxes(ticksuffix='€', tickfont=dict(size=9))
        _fig_bar.update_xaxes(tickfont=dict(size=9))
        st.plotly_chart(_fig_bar, use_container_width=True, config={'displayModeBar':False})

    with _g2:
        _rev_pie = [(c, v) for c, v in _rbc_rev.items() if v > 0]
        if _rev_pie:
            _clrs_rev = ['#10B981','#34D399','#6EE7B7','#A7F3D0','#D1FAE5']
            _fig_rev = go.Figure(go.Pie(
                labels=[x[0] for x in _rev_pie], values=[x[1] for x in _rev_pie], hole=0.55,
                textinfo='percent', textfont=dict(size=9, color='white'),
                marker=dict(colors=_clrs_rev[:len(_rev_pie)], line=dict(color=_C_BG, width=2))))
            _fig_rev.add_annotation(text=f"<b>{_rev_r:,.0f}€</b>", x=0.5, y=0.5,
                font=dict(size=12, color=_C_TEXT), showarrow=False)
            _fig_rev.update_layout(**base_layout(180, False))
            _fig_rev.update_layout(showlegend=True,
                legend=dict(font=dict(size=8), orientation='v', x=1.0, y=0.5),
                title=dict(text="Répartition Revenus", font=dict(size=11, color=_C_MUTED), x=0.3),
    ...
    with _g3:
        _bp_vals = [(l,v,c) for l,v,c in [("Dépenses",_dep_r,"#F87171"),("Frais Fixes",_ff_r,"#FBBF24"),
            ("Épargne",_epa_r,"#818CF8"),("Crédits",_cred_r,"#F59E0B")] if v > 0]
        if _bp_vals:
            _fig_bud = go.Figure(go.Pie(
                labels=[x[0] for x in _bp_vals], values=[x[1] for x in _bp_vals], hole=0.55,
                textinfo='percent+label', textfont=dict(size=9, color='white'),
                marker=dict(colors=[x[2] for x in _bp_vals], line=dict(color=_C_BG, width=2))))
            _fig_bud.add_annotation(text=f"<b>{sum(v for _,v,_ in _bp_vals):,.0f}€</b>",
                x=0.5, y=0.5, font=dict(size=12, color=_C_TEXT), showarrow=False)
            _fig_bud.update_layout(**base_layout(180, False))
            _fig_bud.update_layout(showlegend=False,
                title=dict(text="Répartition Budget", font=dict(size=11, color=_C_MUTED), x=0.5),
                margin=dict(l=0, r=0, t=30, b=0))
            st.plotly_chart(_fig_bud, use_container_width=True, config={'displayModeBar':False})

    _cl, _cm, _cr = st.columns([1, 1.3, 1.3])

    with _cl:
        _tbl_ent = (f"<table style='width:100%;border-collapse:collapse;font-size:12px'>"
            f"<thead><tr style='background:#0D1117'>"
            f"<th style='padding:5px 8px;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉSUMÉ ENTRÉES & SORTIES</th>"
            f"<th style='padding:5px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>PRÉVU</th>"
            f"<th style='padding:5px 8px;text-align:right;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>RÉEL</th></tr></thead><tbody>")
        for _lbl, _pv, _rv, _clr in [
            ("+ Total Revenus", _rev_p, _rev_r, "#10B981"),
            ("− Dépenses",      _dep_p, _dep_r, "#F87171"),
            ("− Frais Fixes",   _ff_p,  _ff_r,  "#FBBF24"),
            ("− Crédits",       0,      _cred_r,"#F59E0B"),
            ("− Épargne",       _epa_p, _epa_r, "#818CF8"),
        ]:
            _tbl_ent += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                f"<td style='padding:5px 8px;color:{_clr}'>{_lbl}</td>"
                f"<td style='padding:5px 8px;text-align:right;color:{_C_MUTED}'>{_pv:,.0f}€</td>"
                f"<td style='padding:5px 8px;text-align:right;color:{_clr}'>{_rv:,.0f}€</td></tr>")
        _rc = "#10B981" if _rest_r >= 0 else "#F87171"
        _tbl_ent += (f"<tr style='background:#0D1117;font-weight:700'>"
            f"<td style='padding:6px 8px;color:{_rc}'>= TOTAL RESTANT</td>"
            f"<td style='padding:6px 8px;text-align:right;color:{_C_MUTED}'>{_rest_p:,.0f}€</td>"
            f"<td style='padding:6px 8px;text-align:right;color:{_rc}'>{_rest_r:,.0f}€</td></tr></tbody></table>")
        st.markdown(_tbl_ent, unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        _budget_dep_tot = _dep_p + _ff_p
        _restant_dep = _budget_dep_tot - (_dep_r + _ff_r)
        _restant_col = "#10B981" if _restant_dep >= 0 else "#F87171"
        st.markdown(
            f"<div style='background:{_BUD_BG};border:1px solid {_BUD_COLOR}44;border-radius:8px;padding:12px 14px;margin-bottom:12px'>"
            f"<div style='font-size:10px;color:{_C_MUTED};text-transform:uppercase'>Montant restant à dépenser</div>"
            f"<div style='font-size:24px;font-weight:800;color:{_restant_col};font-family:Space Grotesk'>{_restant_dep:,.2f} €</div>"
            f"<div style='font-size:10px;color:{_C_MUTED}'>{'✅ Budget respecté' if _restant_dep >= 0 else f'⚠️ Dépassé de {abs(_restant_dep):,.0f}€'}</div></div>",
            unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(_build_section_table("RÉSUMÉ DES REVENUS","#10B981","💶",
            _bp.get("revenus",{}), _rbc_rev), unsafe_allow_html=True)

    with _cm:
        st.markdown(_build_section_table("RÉSUMÉ DES DÉPENSES","#F87171","🛒",
            _bp.get("depenses",{}), _rbc_dep), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(_build_section_table("RÉSUMÉ ÉPARGNE & INVESTISSEMENTS","#818CF8","💰",
            _bp.get("epargne",{}), _rbc_epa), unsafe_allow_html=True)

    with _cr:
        st.markdown(_build_section_table("RÉSUMÉ DES FRAIS FIXES","#FBBF24","🏠",
            _bp.get("frais_fixes",{}), _rbc_ff, show_echeance=True), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(_build_section_table("RÉSUMÉ CRÉDITS / DETTES","#F59E0B","💳",
            _bp.get("credits",{}), _rbc_cred), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        sec("Ajouter une transaction", "➕", _BUD_COLOR, _BUD_BG)
        with st.form("form_budget_tx", clear_on_submit=True):
            _fb_date = st.date_input("Date", value=_now_p.date(), key="fb_date")
            _fb_type = st.selectbox("Type", _TYPES, key="fb_type")
            _fb_cat  = st.text_input("Catégorie", key="fb_cat", placeholder="Ex: Course Alimentaire...")
            _sep = " · "
            _fb_cats_d = {"Revenus":["Salaire","Remboursement","Prime"],
                "Dépenses":["Course Alimentaire","Cantine Travail","Transport","Extra","Restaurant","Sport","Coiffeur","Osteo"],
            ...
            _sugg = _fb_cats_d.get(_fb_type, [])
            if _sugg:
                st.markdown(f"<div style='font-size:10px;color:{_C_MUTED};margin:-6px 0 6px'>{_sep.join(_sugg[:5])}</div>", unsafe_allow_html=True)
            _fb_amt  = st.number_input("Montant (€)", min_value=0.0, step=0.01, format="%.2f", key="fb_amt")
            _fb_desc = st.text_input("Description", key="fb_desc")
            _fb_sub  = st.form_submit_button("💾 Ajouter", use_container_width=True)
        if _fb_sub:
            if _fb_cat.strip() and _fb_amt > 0:
                _mk = _fb_date.strftime("%Y-%m")
                if _mk not in _bud_data["mois"]:
                    _bud_data["mois"][_mk] = {"nom":_MOIS_LABELS.get(_mk,_mk),"transactions":[],
                        "budget_prevu":{"revenus":{},"depenses":{},"frais_fixes":{},"epargne":{},"credits":{}}}
                _bud_data["mois"][_mk]["transactions"].append({"date":str(_fb_date),"type":_fb_type,
                    "categorie":_fb_cat.strip(),"montant":round(_fb_amt,2),"description":_fb_desc.strip()})
                _bud_data["mois"][_mk]["transactions"].sort(key=lambda x: x["date"])
                if _save_budget(_bud_data, f"Tx {_fb_type} {_fb_cat} {_fb_amt:.0f}€"):
                    st.success(f"✅ {_fb_cat} — {_fb_amt:.2f}€"); _load_budget.clear()
                else: st.error("❌ Erreur GitHub")
            else: st.warning("Catégorie et montant requis.")

        st.markdown("<br>", unsafe_allow_html=True)
        sec("Modifier le budget prévu", "✏️", "#F59E0B", "#1C1400")
        st.markdown(f"<div style='font-size:11px;color:{_C_MUTED};margin-bottom:8px'>Définir ou ajuster les montants prévus pour {_MOIS_LABELS.get(_sel_mois,'')}</div>", unsafe_allow_html=True)
        with st.form("form_budget_prevu", clear_on_submit=False):
            _fp_type = st.selectbox("Section", ["depenses","frais_fixes","epargne","revenus","credits"],
            ...
            _fp_cat  = st.text_input("Catégorie", key="fp_cat")
            _fp_amt  = st.number_input("Montant prévu (€)", min_value=0.0, step=1.0, format="%.2f", key="fp_amt")
            _fp_sub  = st.form_submit_button("✏️ Enregistrer", use_container_width=True)
        if _fp_sub:
            if _fp_cat.strip():
                if _sel_mois not in _bud_data["mois"]:
                    _bud_data["mois"][_sel_mois] = {"nom":_MOIS_LABELS.get(_sel_mois,""),"transactions":[],
                        "budget_prevu":{"revenus":{},"depenses":{},"frais_fixes":{},"epargne":{},"credits":{}}}
                if "budget_prevu" not in _bud_data["mois"][_sel_mois]:
                    _bud_data["mois"][_sel_mois]["budget_prevu"] = {"revenus":{},"depenses":{},"frais_fixes":{},"epargne":{},"credits":{}}
                _sec = _bud_data["mois"][_sel_mois]["budget_prevu"].setdefault(_fp_type, {})
                _cat_c = _fp_cat.strip()
                if _fp_type == "frais_fixes":
                    _old_ech = _sec.get(_cat_c,{}).get("echeance",0) if isinstance(_sec.get(_cat_c,{}),dict) else 0
                    _sec[_cat_c] = {"prevu": round(_fp_amt,2), "reel":0, "echeance": _old_ech}
                else:
                    _sec[_cat_c] = {"prevu": round(_fp_amt,2), "reel": 0}
                if _save_budget(_bud_data, f"Budget {_sel_mois} {_fp_type} {_cat_c}={_fp_amt:.0f}€"):
                    st.success(f"✅ {_cat_c} → {_fp_amt:.0f}€"); _load_budget.clear()
                else: st.error("❌ Erreur GitHub")

    st.markdown("<hr style='border-color:#1F2937;margin:20px 0'>", unsafe_allow_html=True)
    _lt1, _lt2 = st.columns([2, 1])
    with _lt1:
        sec("Transactions du mois", "📋", _BUD_COLOR, _BUD_BG)
        _tf = st.multiselect("Filtrer", _TYPES, default=_TYPES, key="bud_filter", label_visibility="collapsed")
        _txsf = sorted([t for t in _txs if t['type'] in _tf], key=lambda x: x['date'], reverse=True)
        if _txsf:
            _tbl_tx = f"<table style='width:100%;border-collapse:collapse;font-size:11px'><thead><tr style='background:#0D1117'>"
            for _h in ["Date","Type","Catégorie","Montant","Description"]:
                _tbl_tx += f"<th style='padding:4px 8px;font-size:10px;color:{_C_MUTED};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
            _tbl_tx += "</tr></thead><tbody>"
            for _t in _txsf:
                _tc = _TYPE_COLORS.get(_t['type'], "#fff"); _sc = "#10B981" if _t['type'] == 'Revenus' else "#F87171"
                _sg = "+" if _t['type'] == 'Revenus' else "-"
                _tbl_tx += (f"<tr style='border-bottom:1px solid {_C_BORDER}'>"
                    f"<td style='padding:4px 8px;color:{_C_MUTED}'>{_t['date']}</td>"
                    f"<td style='padding:4px 8px'><span style='font-size:9px;background:{_tc}22;color:{_tc};padding:1px 5px;border-radius:3px'>{_t['type']}</span></td>"
                    f"<td style='padding:4px 8px;font-weight:500'>{_t['categorie']}</td>"
                    f"<td style='padding:4px 8px;text-align:right;font-weight:600;color:{_sc}'>{_sg}{abs(_t['montant']):.2f}€</td>"
                    f"<td style='padding:4px 8px;color:{_C_MUTED};font-size:10px'>{_t.get('description','')}</td></tr>")
            _tbl_tx += "</tbody></table>"
            st.markdown(_tbl_tx, unsafe_allow_html=True)
        else: st.info("Aucune transaction.")
    with _lt2:
        sec("Résumé annuel", "📅", _BUD_COLOR, _BUD_BG)
        _ta = f"<table style='width:100%;border-collapse:collapse;font-size:11px'><thead><tr style='background:#0D1117'>"
        for _h,_hc in [("Mois",_C_MUTED),("Rev.","#10B981"),("Dép+FF","#F87171"),("Ép.","#818CF8"),("Restant",_C_MUTED)]:
            _ta += f"<th style='padding:4px 6px;text-align:right;font-size:10px;color:{_hc};border-bottom:1px solid {_C_BORDER}'>{_h}</th>"
        _ta += "</tr></thead><tbody>"
        _tot = [0.0]*4
        for _mk in sorted(_bud_data.get("mois",{}).keys()):
            _mt = _bud_data["mois"][_mk].get("transactions",[])
            _mr=sum(t['montant'] for t in _mt if t['type']=='Revenus')
            _md=sum(t['montant'] for t in _mt if t['type']=='Dépenses')
            _mf=sum(t['montant'] for t in _mt if t['type']=='Frais Fixes')
            _me=sum(t['montant'] for t in _mt if t['type']=='Épargne & Investissements')
            _mrest=_mr-_md-_mf-_me
            for i,v in enumerate([_mr,_md+_mf,_me,_mrest]): _tot[i]+=v
            _rc="#10B981" if _mrest>=0 else "#F87171"
            _ta += (f"<tr style='border-bottom:1px solid {_C_BORDER}'><td style='padding:4px 6px;font-weight:600'>{_MOIS_LABELS.get(_mk,_mk)[:4]}</td>"
                f"<td style='padding:4px 6px;text-align:right;color:#10B981'>{_mr:,.0f}€</td>"
                f"<td style='padding:4px 6px;text-align:right;color:#F87171'>{_md+_mf:,.0f}€</td>"
                f"<td style='padding:4px 6px;text-align:right;color:#818CF8'>{_me:,.0f}€</td>"
                f"<td style='padding:4px 6px;text-align:right;font-weight:700;color:{_rc}'>{_mrest:,.0f}€</td></tr>")
        _tr_c="#10B981" if _tot[3]>=0 else "#F87171"
        _ta += (f"<tr style='background:#0D1117;font-weight:700;border-top:2px solid {_C_BORDER}'><td style='padding:5px 6px'>TOTAL</td>"
            f"<td style='padding:5px 6px;text-align:right;color:#10B981'>{_tot[0]:,.0f}€</td>"
            f"<td style='padding:5px 6px;text-align:right;color:#F87171'>{_tot[1]:,.0f}€</td>"
            f"<td style='padding:5px 6px;text-align:right;color:#818CF8'>{_tot[2]:,.0f}€</td>"
            f"<td style='padding:5px 6px;text-align:right;color:{_tr_c}'>{_tot[3]:,.0f}€</td></tr></tbody></table>")
        st.markdown(_ta, unsafe_allow_html=True)

with tab9:
    sec("🚧 Chantier — Roadmap & dette technique", "🚧", "#F59E0B", "#1C1400")
    # ── DEBUG GitHub (temporaire) ──
    st.markdown(f"<div style='font-size:12px;color:{C['muted']};padding:4px 0 16px 0'>Mémo partagé entre sessions — état des chantiers en cours et à venir. Séquence validée le 28/06/2026 · Dernière mise à jour : 01/07/2026 (grosse session — onglet Crypto 100% autonome du Sheet).</div>", unsafe_allow_html=True)
...
GITHUB_REPO, GITHUB_BRANCH.""", "#6EE7B7")
    ...
    with st.expander("✅ 3 — Contexte Marché (terminé)", expanded=False):
...
<b>✅ Fait</b> : VIX avec sparkline intégrée + badge de niveau (Calme/Normal/Prudence/Stress). Fond du graphique aligné sur la variation journalière. Badge niveau + légende déplacés sous le graphe (comme Fear & Greed).<br>
...
<b>✅ Fait</b> : Lien TradingView — ticker cliquable dans la Watchlist (3 listes), ouvre directement le chart TV dans un nouvel onglet avec le compte connecté du navigateur. Badge discret 📈 inline sur le ticker.<br>
...
<b>✅ Fait</b> : Robustesse yfinance — messages explicites si données indisponibles sur toutes les cards (indices, VIX) et calendriers (Earnings, Dividendes). Try/except partout, pas d'erreur silencieuse.""",
            "#EAB308")

    with st.expander("✅ 3b — Chantier Visuel Intermédiaire (terminé)", expanded=False):
        _chantier_card("Ajustements visuels Performance", "🎨",
            """Corrections d'espacement sur l'onglet Performance.<br>
<b>✅ Fait</b> : 4 KPIs rendus en un seul bloc HTML (grid CSS) — suppression de l'espace Streamlit entre KPIs et tableaux.<br>
<b>✅ Fait</b> : Footnotes Performance et TRI fusionnés dans le HTML de leurs tableaux respectifs — collés directement dessous, même style.<br>
<b>✅ Fait</b> : Suppression du bouton "aide" qui forçait un bloc Streamlit séparé entre KPIs et tableaux.""",
            "#818CF8")

    _chantier_card("5 — Améliorations Visuelles", "🎨",
        """Chantier dédié aux améliorations visuelles et UX sur l'ensemble des onglets.<br>
Contenu à définir au fil des sessions — regroupement des petits fix visuels inter-chantiers.""",
        "#818CF8")

    _chantier_card("6 — Alertes visuelles in-app", "🔔",
...
(cron externe hors scope). À faire après stabilisation visuelle de tous les onglets.""",
    ...
    _chantier_card("4 — Historique DCA & TRI précis", "📊",
        """Amélioration de la précision du calcul XIRR Bourse/Crypto.<br><br>
<b>✅ 4a</b> : <code>historique_dca.json</code> sur GitHub — vrais montants CTO+PEA par mois (2022→2026). Validé à &lt;0.10€ près vs Sheet.<br>
<b>✅ 4b</b> : <code>crypto_positions.json</code> sur GitHub — positions actuelles par token (BTC/ETH/SOL/BNB/USDC). Point de départ = Sheet 29/06/2026.<br>
<b>✅ 4c</b> : Onglet <b>✏️ Saisie DCA</b> — formulaires Bourse (CTO+PEA) et Crypto (par token + frais) → sauvegarde GitHub avec date exacte. Multi-versements par mois possible.<br>
<b>✅ 4d</b> : Calcul XIRR hybride — flux réels depuis <code>historique_dca.json</code> pour Bourse/Crypto. TRI = +10.60% (vs +19% aberrant avant). Benchmarks recalculés avec les mêmes flux.<br><br>
<b>✅ 4e</b> : Section <b>💸 Retrait vers mon compte bancaire</b> dans l'onglet Saisie DCA. Date + enveloppe + montant € + note → stocké dans <code>historique_dca.json</code> sous "retraits". Intégré comme flux positif dans le XIRR. Les ventes d'actifs seront gérées dans un onglet dédié ultérieurement.<br><br>
<b>✅ 4f</b> (30/06/2026) : Refonte onglet <b>Modifier / Supprimer un versement</b> + <b>Versement Crypto</b>.<br>
— Fix selectbox "Versement à modifier" qui gardait l'affichage de l'ancienne enveloppe au changement Bourse/Crypto (clé dépendante de l'enveloppe).<br>
— Fix calcul du montant total affiché pour les versements Crypto (ancien format <code>total</code> ET nouveau format par token retournaient 0$).<br>
— Suppression d'un versement Crypto : corrige désormais aussi automatiquement <code>crypto_positions.json</code> (investi_usd + quantités) pour rester synchronisé avec l'historique. Avertissement si l'ancien format (sans détail par token) empêche la correction auto.<br>
— Formulaire <b>Versement Crypto</b> : titre simplifié, layout horizontal par token (badge + 2 champs côte à côte), historique dans un sous-menu repliable (<code>st.toggle</code>, les expanders imbriqués n'étant pas supportés par Streamlit).<br>
— Fix bug latent <code>_autre_frais</code> non défini (plantait l'enregistrement d'une crypto hors liste si rempli).<br>
— Nouveau bloc <b>🔄 Conversion entre cryptos (swap)</b> : gère les conversions internes (ex: USDC → BTC) sans flux externe — déplace investi_usd/detenu entre tokens dans <code>crypto_positions.json</code> sans toucher <code>historique_dca.json</code> (donc sans impact XIRR). Historique des conversions consultable.<br><br>
<b>✅ 4g</b> (30/06/2026, suite) : Refonte du flux Crypto pour coller au fonctionnement réel (virement → USDC auto sur Binance → achat via swap).<br>
— <b>Versement Crypto</b> simplifié : ne gère plus que l'arrivée en USDC (Date, Frais dépôt €, Montant versé €, Montant reçu USDC $). Les achats BTC/ETH/SOL/BNB passent désormais exclusivement par le bloc Conversion.<br>
— Ajout du suivi <code>investi_eur</code> par position (en plus de <code>investi_usd</code>) — alimenté par le Montant € du versement, et transféré proportionnellement entre tokens lors d'une conversion (coût de revient € cohérent après plusieurs swaps en cascade).<br>
— Bloc Conversion enrichi : <code>valeur_usd_vendue</code> / <code>valeur_usd_achetee</code> distincts (au lieu d'une valeur unique) pour capter un écart dû aux frais de swap.<br>
— Versement Crypto et Conversion fusionnés dans un seul expander "₿ Crypto — Versement & Conversion" (2 formulaires distincts, 2 boutons), avec les 2 historiques (versements + conversions) regroupés en bas via <code>st.toggle</code>.<br>
— Formulaire <b>Versement Bourse</b> passé en layout horizontal (Date/CTO/PEA sur une ligne) + historique déplacé dans un <code>st.toggle</code> repliable, même pattern que Crypto.<br>
— Formulaire <b>Retrait</b> également passé en layout horizontal + historique dans un <code>st.toggle</code>, pour une cohérence totale entre les 4 sections de l'onglet (Bourse/Crypto/Conversion/Retrait).<br>
— Vérifié : toutes les dates (versement, conversion, retrait) sont déjà pré-remplies à la date du jour par défaut (modifiable via le calendrier) — aucun changement nécessaire.<br>
— Tableaux d'historique (Bourse, Crypto, Conversions, Retraits) passés en <b>vue par année</b> : l'année en cours reste affichée en clair, les années précédentes se replient chacune dans leur propre <code>st.toggle</code> ("▶ 2025 — 12 versements") à ouvrir au besoin — évite un tableau géant et illisible au fil des années.<br>
— <b>🐛 Bug corrigé</b> : le tableau "Historique des versements crypto" affichait "Aucun versement" même quand il y en avait, car le filtre exigeait le nouveau format par token en dict (USDC uniquement) — excluant de fait tout l'historique 2022→juin 2026 (formats <code>total</code> et tokens en nombre simple). Calcul du montant unifié sur les 3 formats existants, total par année toujours affiché même sans détail par token pour les anciennes entrées.<br>
— <b>🐛 Bug XIRR corrigé</b> : le calcul du flux € crypto (Vue d'ensemble / Performance) sommait par erreur <code>montant_eur</code> et <code>frais_depot_eur</code> comme s'il s'agissait de tokens supplémentaires, doublant artificiellement certains montants. Utilise maintenant le <code>montant_eur</code> exact saisi quand disponible (plus précis que l'ancienne approximation $→€ à 0.92). Même correction appliquée au calcul d'affichage dans Modifier/Supprimer et au tableau Historique des versements (colonnes fictives "montant_eur"/"frais_depot_eur" exclues).<br>
— Suppression d'une <b>Conversion (swap)</b> ajoutée dans "Modifier / Supprimer" comme 3e option (en plus de Bourse/Crypto). Annule la conversion dans les deux sens : restitue <code>detenu</code>/<code>investi_usd</code>/<code>investi_eur</code> au token source, et les soustrait au token destination. Retire aussi l'entrée de la liste <code>conversions[]</code> dans <code>crypto_positions.json</code>.<br>
— <b>🐛 Crash onglet Saisie DCA corrigé</b> : un JSON invalide dans <code>crypto_positions.json</code> (corrompu par une écriture test) plantait tout l'onglet avec une <code>JSONDecodeError</code>. Ajout de try/except sur les deux parsings JSON au chargement — l'onglet continue à fonctionner en mode dégradé avec un message d'erreur explicite au lieu d'un crash complet.<br>
— <b>🔧 Passe clean finale (30/06/2026)</b> : toutes les fonctions helper de rendu (<code>_entry_amount_crypto</code>, <code>_render_tbl_vers_crypto</code>, <code>_render_tbl_conv</code>, <code>_render_tbl_retrait</code>) remontées au niveau de l'onglet (au lieu d'être définies dans des blocs imbriqués). Élimine les risques de <code>NameError</code> si une fonction était appelée hors de son bloc de définition. Vérifié : aucune clé Streamlit dupliquée, syntaxe Python valide.<br>
— <b>🐛 SHA stale corrigé</b> : versements Bourse et Crypto faisaient un <code>gh_write</code> avec le sha initial de l'onglet. Si une autre opération avait entre-temps réécrit <code>historique_dca.json</code>, le sha était obsolète → écriture GitHub silencieusement ignorée. Chaque écriture fait maintenant un <code>gh_read</code> frais juste avant.<br>
— Note : les frais de virement bancaire (frais_depot_eur) sont tracés par versement dans <code>historique_dca.json</code>. Les frais de swap sont implicitement capturés par l'écart entre "Valeur $ vendue" et "Valeur $ achetée" dans le formulaire Conversion. Les frais historiques Binance 2022→2026-06 (51.2€) sont en dur dans <code>crypto_positions.json → frais_historiques</code> — pas de saisie mensuelle nécessaire (Binance ne prélève pas de frais de trading dans le cadre de l'utilisation de Samuel).<br><br>
<b>🔄 Reste</b> : Les graphes, KPIs et tableaux lisent encore des valeurs depuis le Google Sheet. Ce travail est couvert par le <b>Chantier 8</b> — Autonomie totale vs Google Sheet.""",
        "#6EE7B7", "En cours")

    _chantier_card("7 — News sur positions détenues", "📰",
...
(parfois vide pour petites caps type MARA/NIO).""",
    ...
    _chantier_card("8 — 🗺️ GRAND CHANTIER — Autonomie totale vs Google Sheet", "🗺️",
        """<b>Audit des dépendances Sheet (30/06/2026)</b> — carte complète des liens restants.<br><br>

<b>✅ 8a FAIT</b> — Saisie DCA : <code>historique_dca.json</code> + <code>crypto_positions.json</code> 100% autonomes (versements, conversions, retraits, suppressions, historiques par année).<br><br>

<b>✅ 8b FAIT (30/06/2026 → 01/07/2026)</b> — Onglet <b>₿ Crypto 100% autonome</b> du Google Sheet. Première coupure majeure.<br>
— KPIs (Valeur PF €/$, Total Investi, PV/MV brut, Frais cumulés, EUR/$) → calculés depuis <code>crypto_positions.json</code> + yfinance<br>
— Tableau portefeuille complet : Montant détenu, Montant acheté, Investi ($), Cours réel ($), PRU ($), Valeur $/€, PV/MV €, Perf. → 0 Sheet<br>
— PRU = <code>investi_usd / detenu</code> (coût de revient par token actuellement détenu) ✅<br>
— Total Investi € = <code>investi_eur_base</code> (snapshot Sheet 6 205€ au 29/06) + nouveaux versements post-date → stable, plus sensible au taux live<br>
— Répartition (donut) + légende → calculées depuis valeurs live yfinance<br>
— Gains/Pertes Staking → calculés comme (detenu − achete) × prix_live, positif ET négatif (ex: BNB = frais payés en BNB)<br>
— DCA Mensuel → lu depuis <code>historique_dca.json</code> avec <code>montant_eur</code> exact (0 Sheet)<br>
— Évolution mensuelle du PF → alimentée par <code>ibkr_data/snapshots.json</code> + point dynamique courant (0 Sheet)<br>
— Valeurs crypto stockées dans <code>session_state</code> → accessibles par les autres onglets sans re-lecture GitHub<br><br>

<b>✅ 8b-bis FAIT</b> — Ligne Crypto dans <b>onglet Performance</b> coupée du Sheet.<br>
— Investi/Valeur/PV/MV/Perf Crypto dans le tableau Performance → depuis <code>session_state['crypto_val_eur']</code> / <code>crypto_inv_eur</code> (ou fallback gh_read si tab3 pas encore rendu)<br>
— KPIs Performance totaux (Total Investi, Valeur Actuelle, PV/MV) recalculés avec les vraies valeurs Crypto<br>
— TRI Crypto individuel (+6.0%) → calculé depuis flux réels <code>historique_dca.json</code><br>
— 🐛 <b>Bug Snapshot timezone corrigé</b> : <code>datetime.now()</code> UTC → <code>datetime.now(_TZ_PARIS)</code> partout (save_snapshot, mois_actuel, mois_cur dans graphe). Plus d'écrasement nocturne du mois précédent.<br>
— Snapshot enrichi avec <code>crypto_val_eur</code> pour alimenter le graphe Évolution crypto.<br><br>

<b>⏳ 8c — À faire</b> : Couper ligne Bourse dans Performance → nécessite positions Bourse en JSON.<br>
<b>⏳ 8d — À faire</b> : Couper Vue d'ensemble → dépend de 8c (patrimoine total sans Sheet).<br>
<b>⏳ 8e — À faire</b> : Couper onglet Bourse → snapshot initial des positions CTO+PEA en JSON.<br>
<b>⏳ 8f — À faire</b> : Suppression totale gspread + bouton "📂 Restaurer" JSON.<br><br>

<i>Restant Sheet : tab1 Vue d'ensemble (patrimoine, évolution mensuelle/annuelle), tab2 Bourse (positions), tab5 Performance (ligne Bourse). Options/IBKR : déjà 100% autonomes depuis le début.</i>""",
        "#A78BFA", "En cours")

    _chantier_card("9 — Onglet Fiscalité dédié", "🧾",
...
<i>Priorité : après Chantier 8 terminé (ou en parallèle si Sheet encore défaillant).</i>""",
    ...
    _chantier_card("Tableau Portefeuille Crypto depuis JSON", "₿",
        """✅ FAIT (01/07/2026) — Tableau reconstruit entièrement depuis crypto_positions.json + yfinance.
Fait partie du Chantier 8b terminé.""",
        C['green'])
...
intermédiaires — seule la collecte de la donnée "retrait" manque (sera traitée en 7b).""",
    ...
    _chantier_card("🐛 Snapshot PV/MV — décalage entre KPI et graphe — ✅ CORRIGÉ", "📸",
        """Corrigé le 01/07/2026 : pvmv_t stocké dans session_state depuis l'onglet Performance
après correction crypto (même valeur que le KPI PV/MV affiché). Le bouton Snapshot utilise
maintenant cette valeur au lieu de tri_global (calculé au chargement de page). Le graphe
reflétera désormais exactement le KPI visible à l'écran au moment du clic.""",
        C['green'])
