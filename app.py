...
with tab1:
    ...
    # ── Marchés (indices + BTC) — variation journalière ──
    if YF_AVAILABLE:
        st.markdown("<div style='margin-top:18px'></div>", unsafe_allow_html=True)
        st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin-bottom:8px'>📊 Indices de référence — variation journalière</div>", unsafe_allow_html=True)
...
<span style="font-size:12px;font-weight:700;color:{_vcol}">{'+' if _snap['var_pct']>=0 else ''}{_snap['var_pct']:.2f}%</span>
                ...
                st.plotly_chart(_fig_spark, use_container_width=True, config={'displayModeBar':False}, key=f"spark_{_mname}")
    ...
    # ── Objectifs + progression — 4 cards compactes ──────────────────────
    _o1, _o2, _o3, _o4 = st.columns(4)

    # Card 1 — Objectif patrimoine (saisie persistée GitHub)
    with _o1:
        _obj_pat_new = st.number_input("🎯 Objectif patrimoine fin d'année (€)",
            min_value=0, value=st.session_state.get('obj_patrimoine', 80000),
            step=1000, key="obj_patrimoine_input",
            help="Ton objectif de patrimoine total à fin décembre")
        if _obj_pat_new != st.session_state.get('obj_patrimoine', 80000):
        ...
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px">
  <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{_bar_col}">{_pct_obj:.1f}%</div>
  <div style="font-size:10px;color:{C['muted']};margin:2px 0 6px">Actuel <b style="color:{C['text']}">{fmt(pat)}</b> · écart <span style="color:{_diff_col};font-weight:700">{'+' if _diff>=0 else ''}{fmt(_diff)}</span></div>
  <div style="background:{C['border']};border-radius:3px;height:5px;margin-bottom:4px">
  ...
  <div style="font-size:10px;color:{C['muted']}">objectif {fmt(_obj_pat)}</div>
    ...
    # Card 2 — Objectif DCA mensuel (saisie persistée GitHub)
    with _o2:
        _obj_dca_new = st.number_input("💸 Objectif DCA mensuel (€)",
            min_value=0, value=st.session_state.get('obj_dca', 600),
            step=50, key="obj_dca_input",
            help="Ton DCA cible total par mois (Bourse + Crypto + Options)")
        if _obj_dca_new != st.session_state.get('obj_dca', 600):
        ...
        # Crypto (depuis feuille "₿ Crypto")
        try:
            _df_c_dca = fetch("₿ Crypto")
            _dca_c_mois = [n(v(_df_c_dca,20+i,2)) for i in range(12)]
            _dca_c_real = sum(x for x in _dca_c_mois if x > 0)
        except Exception:
            _dca_c_real = 0.0

        # Options (dernier dépôt IBKR = mois en cours → sinon 0)
        _dca_o_real = 0.0
        if 'ibkr_data' in st.session_state:
            _yr_data_now = st.session_state['ibkr_data'].get(_YR, {})
            _depots_yr = _yr_data_now.get('depots_detail', [])
            if _depots_yr:
                # Trouver le dernier dépôt par date
                _now_dt = datetime.now(_TZ_PARIS).replace(tzinfo=None)
                _parsed_deps = []
                for _lbl, _amt in _depots_yr:
                    for _f in ("%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d-%m-%Y"):
                        try:
                            _parsed_deps.append((datetime.strptime(_lbl[:10], _f), _amt))
                            break
                        except ValueError:
                            continue
                if _parsed_deps:
                    _last_dep_dt, _last_dep_amt = max(_parsed_deps, key=lambda x: x[0])
                    if _last_dep_dt.year == _now_dt.year and _last_dep_dt.month == _now_dt.month:
                        _dca_o_real = _last_dep_amt

        # DCA mois en cours par enveloppe
        _now_mois = datetime.now(_TZ_PARIS).replace(tzinfo=None)
        _mois_idx = _now_mois.month - 1  # 0-indexed
        ...
        if 'ibkr_data' in st.session_state:
            ...
            if _depots_yr:
                ...
                if _parsed_deps:
                    _last_dep_dt, _last_dep_amt = max(_parsed_deps, key=lambda x: x[0])
                    if _last_dep_dt.year == _now_mois.year and _last_dep_dt.month == _now_mois.month:
                        _dca_o_real = _last_dep_amt

        _dca_ce_mois = _dca_b_ce_mois + _dca_c_ce_mois + _dca_o_real

        # Aussi calcul moyenne YTD (pour barre de progression)
        _pats_tmp    = [n(v(df_em,13+i,2)) for i in range(12)]
        _nb_mois     = sum(1 for x in _pats_tmp if x > 0)
        _dca_b_real  = sum(n(v(df_b,11+i,2)) + n(v(df_b,11+i,3)) for i in range(12) if n(v(df_b,11+i,2)) + n(v(df_b,11+i,3)) > 0)
        _dca_moy_mois = (_dca_b_real) / max(_nb_mois, 1)
        ...
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px;height:108px;box-sizing:border-box">
  <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{_dca_col}">{fmt(_dca_ce_mois)}<span style="font-size:11px;color:{C['muted']}"> {_mois_nom}</span></div>
  <div style="font-size:9px;color:{C['muted']};margin:4px 0 6px">
    Bourse <b>{fmt(_dca_b_ce_mois)}</b> · Crypto <b>{fmt(_dca_c_ce_mois)}</b> · Options <b>{fmt(_dca_o_real)}</b>
  </div>
  <div style="background:{C['border']};border-radius:3px;height:5px;margin-bottom:4px">
    <div style="background:{_dca_col};height:100%;width:{min(_pct_dca,100):.0f}%;border-radius:3px"></div>
  </div>
  <div style="font-size:10px;color:{C['muted']}"><span style="color:{_dca_col};font-weight:700">{_pct_dca:.0f}%</span> · objectif {fmt(_dca_mois_obj)}/mois</div>
</div>""", unsafe_allow_html=True)

    # Card 3 — Année écoulée
    with _o3:
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px;margin-top:28px;height:108px;box-sizing:border-box">
  <div style="font-size:10px;color:{C['muted']};margin-bottom:2px">Année {_YR} écoulée</div>
  <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{C['blue']}">{_pct_yr:.1f}%</div>
  <div style="font-size:10px;color:{C['muted']};margin:2px 0 6px">Jour {_day_of_yr} / {_days_yr}</div>
  <div style="background:{C['border']};border-radius:3px;height:5px">
    ...
    # Card 4 — Trajectoire objectif
    with _o4:
        _obj_expected = _obj_pat * (_pct_yr / 100)
        _avance = pat - _obj_expected
        _avance_col = C['green'] if _avance >= 0 else C['red']
        _avance_label = "En avance" if _avance >= 0 else "En retard"
        st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:10px 14px;margin-top:28px;height:108px;box-sizing:border-box">
  <div style="font-size:10px;color:{C['muted']};margin-bottom:2px">Trajectoire objectif</div>
  <div style="font-family:'Space Grotesk';font-size:24px;font-weight:700;color:{_avance_col}">{'+' if _avance>=0 else ''}{fmt(_avance)}</div>
  <div style="font-size:10px;color:{C['muted']};margin:2px 0 6px"><span style="color:{_avance_col};font-weight:700">{_avance_label}</span> · attendu {fmt(_obj_expected)}</div>
  <div style="background:{C['border']};border-radius:3px;height:5px">
    <div style="background:{_avance_col};height:100%;width:{min(abs(_avance)/_obj_pat*100,100) if _obj_pat else 0:.0f}%;border-radius:3px"></div>
  </div>
</div>""", unsafe_allow_html=True)
    ...
    with R:
        ...
        _ibkr_cap_reel = st.session_state.get('capital_reel', 0.0) or load_capital_reel()
...
<div style='font-size:12px;color:{C['text']};line-height:1.7'>{contenu}</div>
    ...
    # ── Séquence validée ──
    st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin:8px 0 10px 2px'>📋 Séquence de chantiers validée</div>", unsafe_allow_html=True)

    _chantier_card("1 — Export / Backup JSON", "💾",
        """✅ <b>Fait</b> — Bouton "💾 Backup" en haut à droite, génère un fichier
<code>saminvest_backup_YYYYMMDD.json</code> téléchargeable localement (capital_reel, watchlist,
ibkr_kpis, années IBKR).<br><br>
<b>Pourquoi pas de bouton "Restaurer" pour l'instant</b> : GitHub persiste déjà automatiquement
toutes les données (HTML IBKR, capital_reel.json, watchlist.json) — elles sont restaurées au
démarrage sans action manuelle. Le backup JSON local reste utile comme filet ultime (perte du repo,
suppression accidentelle).<br><br>
Le bouton "📂 Restaurer" sera ajouté lors du <b>chantier 7e</b>, quand le backup contiendra
toutes les données manuelles (PEE, AV, dividendes...) qui n'auront plus de source Sheet de secours.
À ce moment-là il aura une vraie valeur de récupération.""",
        "#6EE7B7", "✅ Fait")

    _chantier_card("2 — Chantier Visuel — Vue d'ensemble", "🎨",
        """Revoir la mise en page des cards "Objectif patrimoine", jauge, "Objectif DCA mensuel",
barre "Année écoulée". Rapide, impact quotidien immédiat (usage mobile fréquent).""",
        C['gold'])

    _chantier_card("3 — VIX + Calendrier Earnings", "🌡️📅",
        """VIX (^VIX via yfinance) en card "contexte de marché" dans Vue d'ensemble — pertinent pour
la roue (primes plus grasses quand peur élevée). Calendrier earnings pour les tickers détenus
(yf.Ticker.calendar) dans l'onglet Options. Les deux utilisent yfinance → même session.""",
        C['cyan'])

    _chantier_card("4 — Alertes visuelles in-app", "🔔",
        """Bandeau/badge si position options proche ITM, DCA mensuel manqué, etc.
À faire après stabilisation de la Vue d'ensemble (point 2). Pas d'email — nécessiterait
un cron externe, hors scope.""",
        C['cyan'])

    _chantier_card("5 — Historique TRI dans le temps", "📈",
        """Snapshot mensuel du TRI Global stocké sur GitHub (nécessite point 1 — backup JSON).
Graphique d'évolution trimestriel/semestriel pour voir la tendance dans le temps.""",
        C['cyan'])

    _chantier_card("6 — News sur positions détenues", "📰",
        """Section repliable par ticker dans l'onglet Bourse via yf.Ticker.news (gratuit, pas de clé API).
Limite : qualité variable selon les tickers (parfois vide pour petites caps type MARA/NIO).""",
        C['blue'])

    _chantier_card("7 — 🗺️ GRAND CHANTIER — Autonomie totale vs Google Sheet", "🗺️",
        """Le plus structurant — à démarrer après stabilisation des points 1-6.<br><br>
<b>7a</b> — Archivage snapshot du Sheet actuel (valeurs Bourse/Crypto/PEE/AV en "point zéro" JSON)<br>
<b>7b</b> — Formulaires de saisie manuelle : dividendes, valeurs PEE/AV, versements Bourse/Crypto,
retraits — même pattern que depots_detail IBKR, persisté sur GitHub<br>
<b>7c</b> — Migration KPI Performance/Vue d'ensemble vers JSON locaux (couper sv(df_pf,...))<br>
<b>7d</b> — Suppression totale dépendance gspread/Google Sheet<br>
<b>7e ⚠️</b> — Ajouter le bouton "📂 Restaurer" (import backup JSON) + mettre à jour le backup
pour inclure toutes les nouvelles données manuelles (PEE, AV, dividendes, versements...).
À faire impérativement avant de couper définitivement le Sheet — c'est à ce moment que la
restauration a une vraie valeur (plus de fallback Sheet en cas de perte).<br><br>
<b>Dépendances Sheet restantes actuellement</b> : sv(df_pf,6,2/3) Bourse et sv(df_pf,7,2/3) Crypto
(investi + valeur actuelle). Options déjà 100% IBKR/JSON. Stockage : JSON committés via API
GitHub (infrastructure déjà en place depuis la session persistance).""",
        "#A78BFA", "Plusieurs sessions")

    _chantier_card("8 — Onglet Fiscalité dédié", "🧾",
        """Dépend de 7b obligatoirement — il faut tracker les ventes manuellement dans l'app avant
de calculer la fiscalité. Historique ventes Bourse/Crypto (date, qté, prix vente, PRU, PV/MV)
+ calcul PFU/flat tax. Les Options ont déjà leur historique.""",
        C['purple'], "Après point 7")

    _chantier_card("9 — Retraits dans le TRI", "↩️",
        """Dépend de 7b — les retraits seront saisis via formulaires (point 7b), le moteur XIRR
les gère déjà nativement. Aucun retrait effectué à ce jour → calcul actuel correct.""",
        C['purple'], "Après point 7")

    # ── Au fil de l'eau ──
    st.markdown(f"<div style='font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.08em;font-weight:700;margin:18px 0 10px 2px'>🔵 Au fil de l'eau (basse priorité)</div>", unsafe_allow_html=True)

    _chantier_card("Mobile / responsive", "📱",
        """Usage mobile fréquent confirmé → tester tableaux HTML en conditions réelles,
ajuster font-size/padding si besoin.""",
        C['muted'])
    ...
    _chantier_card("Convertisseur EUR/USD live", "💱",
        """eurusd_live est déjà en session_state — à afficher en widget dans l'onglet Options.
Basse priorité, après les autres sujets.""",
        C['muted'])
