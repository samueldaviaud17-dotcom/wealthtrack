...
# ══════════════════════════════════════════════════════
# TAB 9 — CHANTIER (expérimental, TRI vs DCA simulé sur indices)
# ══════════════════════════════════════════════════════
with tab9:
    sec("🚧 Chantier — TRI comparatif (DCA simulé sur indices)", "🚧", "#F59E0B", "#1C1400")
    st.markdown(f"""<div style='font-size:12px;color:{C['muted']};padding:8px 0 16px 0'>
Onglet expérimental — objectif : comparer le TRI réel de "Mon PF" au TRI qu'on aurait obtenu
en investissant <b>les mêmes montants, aux mêmes dates (DCA le 4 de chaque mois)</b>, mais sur
un indice (SP500 / CAC40 / STOXX600) au lieu de tes choix actifs.<br>
Si ça ne fonctionne pas correctement, cet onglet peut être supprimé sans impact sur le reste de l'app.</div>""",
    unsafe_allow_html=True)

    with st.expander("📋 Roadmap / Résumé du chantier (vision commune)", expanded=False):
        st.markdown(f"""<div style='font-size:12px;color:{C['text']};line-height:1.7'>

<b>✅ Validé — fonctionnement actuel</b><br>
Le TRI (XIRR) "Mon PF (Global)" combine tous les flux réels : DCA Bourse (53 mois depuis 02/2022),
DCA Crypto (47 mois depuis 08/2022, montant constant = capital investi / nb de mois, versé le 4 de
chaque mois), et dépôts IBKR réels (Options, dates exactes via <code>depots_detail</code>).
Comparé au TRI simulé si ces mêmes flux avaient été investis dans <b>SP500 TR</b> (^SP500TR),
<b>CAC40 GR</b> (PX1GR.PA, fallback ^FCHI si indispo), <b>STOXX 600</b> (^STOXX).

<br><br><b>⚠️ Limite connue — retraits non gérés</b><br>
Aucun retrait effectué à ce jour → calcul correct pour l'instant. Si un retrait a lieu un jour, il
faudra : (1) l'ajouter comme cashflow positif dans <code>_cfs_pf_global</code>, et (2) simuler une
...
<br><br><b>🗺️ Chantier de fond — autonomie totale vs Google Sheet</b><br>
Objectif à moyen terme : remplacer le Sheet par une saisie directe dans l'app (dividendes, PEE, AV,
mouvements...), avec archivage JSON des valeurs Sheet actuelles comme "point zéro", et formulaires
Streamlit structurés (pas de saisie en langage naturel — trop risqué pour du financier). Une fois
en place, les retraits deviennent un non-sujet : ils seront captés nativement comme
<code>depots_detail</code> l'est déjà pour IBKR. Question ouverte à trancher avant de se lancer :
le stockage persistant (Streamlit Cloud = filesystem éphémère → JSON committé via API GitHub, ou
SQLite + service externe).

<br><br><b>🔀 3 migrations identifiées pour la bascule Chantier → app principale</b><br>
1. <b>"CAGR vs Benchmarks" (Performance, lignes Sheet 13-16)</b> : à supprimer entièrement, remplacé
par le tableau TRI Global de cet onglet. ✅ Aucune autre dépendance trouvée sur ces lignes.<br>
2. <b>"CAGR ANNUALISÉ" (KPI cards, Vue d'ensemble L930 + Performance L2386, source <code>sv(df_p,5,9)</code>)</b> :
conceptuellement identique à notre TRI "Mon PF (Global)" — candidat à remplacer aussi, mais 2 cards
distinctes à mettre à jour. Décision séparée à prendre.<br>
3. <b>Mutualisation du code</b> : déplacer <code>xirr</code>, <code>_dca_months</code>,
<code>_price_at_or_before</code>, <code>_fetch_index_history</code> vers le haut du fichier
(zone des helpers globaux) pour réutilisation dans Performance/Vue d'ensemble.

</div>""", unsafe_allow_html=True)

    if not YF_AVAILABLE:
        st.error("⚠️ yfinance n'est pas disponible dans cet environnement — cet onglet ne peut pas fonctionner.")
    else:
        from datetime import datetime as _dt_ch

        # ── Config : dates de départ par enveloppe (mêmes que Performance) ──
        _start_bourse  = _dt_ch(2022, 2, 4)
        _start_crypto  = _dt_ch(2022, 8, 4)
        _now_ch        = _dt_ch.now(_TZ_PARIS).replace(tzinfo=None)
        ...
        _idx_display_names = {"SP500": "SP500 TR", "CAC 40": "CAC40 GR", "STOXX 600": "STOXX 600"}

        def _dca_months(start_date, today):
            months = []
            d = start_date
            while d <= today:
                months.append(d)
                d = _dt_ch(d.year + (1 if d.month == 12 else 0),
                           1 if d.month == 12 else d.month + 1, 4)
            return months

        @st.cache_data(ttl=3600, show_spinner=False)
        def _fetch_index_history(tickers_try, start_str, end_str):
            """Essaie plusieurs tickers jusqu'à en trouver un avec des données. Retourne (ticker_utilisé, DataFrame) ou (None, None)."""
        ...
        def _price_at_or_before(df_h, target_date):
            ...
            if len(sub) == 0:
                sub = df_h  # fallback : premier dispo
                return float(sub['Close'].iloc[0])
            return float(sub['Close'].iloc[-1])

        # ── Récupération des données ──
        st.markdown("##### 1️⃣ Vérification des données indices")
        _end_str = (_now_ch + __import__('datetime').timedelta(days=1)).strftime("%Y-%m-%d")
        _idx_data = {}
        _debug_rows = []
        for idx_name, tk_list in _tickers_idx.items():
            start_str = _start_bourse.strftime("%Y-%m-%d") if idx_name != "Crypto" else _start_crypto.strftime("%Y-%m-%d")
            used_tk, df_h = _fetch_index_history(tk_list, start_str, _end_str)
            _idx_data[idx_name] = (used_tk, df_h)
            if df_h is not None:
                _disp_name = _idx_display_names[idx_name]
                if idx_name == "CAC 40" and used_tk != "PX1GR.PA":
                    _disp_name = "CAC40"  # fallback prix (sans dividendes)
                if idx_name == "SP500" and used_tk == "^GSPC":
                    _disp_name = "SP500"  # fallback prix (sans dividendes)
                _idx_display_names[idx_name] = _disp_name
                _debug_rows.append({"Indice": _disp_name, "Ticker utilisé": used_tk,
                                     "Lignes": len(df_h),
                                     "Première date": df_h.index[0].strftime("%Y-%m-%d"),
                                     "Dernière date": df_h.index[-1].strftime("%Y-%m-%d"),
                                     "Statut": "✅ OK"})
            else:
                _debug_rows.append({"Indice": _idx_display_names[idx_name], "Ticker utilisé": "—",
                                     "Lignes": 0, "Première date": "—", "Dernière date": "—",
                                     "Statut": f"❌ Échec (testés: {', '.join(tk_list)})"})
        st.dataframe(pd.DataFrame(_debug_rows), use_container_width=True, hide_index=True)

        # ── Simulation TRI GLOBALE (Bourse + Crypto + Options combinés) ──
        st.markdown("##### 2️⃣ Simulation TRI — Portefeuille GLOBAL (Bourse+Crypto+Options) vs indices")

        _pi_bourse = sv(df_pf,6,2); _pl_bourse = sv(df_pf,6,3)
        _pi_crypto = sv(df_pf,7,2); _pl_crypto = sv(df_pf,7,3)
        _pi_options = (st.session_state['ibkr_kpis'].get('capital_investi', sv(df_pf,8,2))
                       if 'ibkr_kpis' in st.session_state else sv(df_pf,8,2))
        _pl_options = st.session_state.get('capital_reel', 0.0) or sv(df_pf,8,3)

        _months_b = _dca_months(_start_bourse, _now_ch)
        _months_c = _dca_months(_start_crypto, _now_ch)
        _montant_b = _pi_bourse / len(_months_b) if _months_b else 0
        _montant_c = _pi_crypto / len(_months_c) if _months_c else 0

        # Flux Options = vrais dépôts IBKR (mêmes que dans Performance)
        _options_cfs_dates = []
        if 'ibkr_data' in st.session_state:
            for _yd in st.session_state['ibkr_data'].values():
                for _lbl, _amt in _yd.get('depots_detail', []):
                    for _f in ("%Y-%m-%d","%d/%m/%Y","%m/%d/%Y","%d-%m-%Y"):
                        try:
                            _options_cfs_dates.append((_dt_ch.strptime(_lbl[:10], _f), _amt)); break
                        except ValueError: continue
            _options_cfs_dates.sort(key=lambda x: x[0])

        # Flux combinés (toutes enveloppes, montants positifs = versements)
        _all_cfs_in = ([(m, _montant_b) for m in _months_b]
                      + [(m, _montant_c) for m in _months_c]
                      + _options_cfs_dates)
        _all_cfs_in.sort(key=lambda x: x[0])

        _val_actuelle_totale = _pl_bourse + _pl_crypto + _pl_options
        _val_investie_totale = _pi_bourse + _pi_crypto + _pi_options

        # TRI réel "Mon PF" global
        _cfs_pf_global = [(d, -m) for d, m in _all_cfs_in] + [(_now_ch, _val_actuelle_totale)]
        _tri_pf_global = xirr(_cfs_pf_global) if _val_investie_totale > 0 and _val_actuelle_totale > 0 else None

        _results = [{"Portefeuille / Indice": "🏆 Mon PF (Global)",
                      "TRI simulé": f"{_tri_pf_global:+.2f}%" if _tri_pf_global is not None else "—",
                      "Valeur": fmt(_val_actuelle_totale),
                      "Note": f"{len(_all_cfs_in)} flux (Bourse+Crypto DCA + dépôts IBKR réels)"}]

        for idx_name in ["SP500", "CAC 40", "STOXX 600"]:
            used_tk, df_h = _idx_data[idx_name]
            if df_h is None:
                _results.append({"Portefeuille / Indice": _idx_display_names[idx_name], "TRI simulé": "—", "Valeur": "—",
            ...
            for d, montant in _all_cfs_in:
                px = _price_at_or_before(df_h, d)
                if px and px > 0:
                    parts += montant / px
                    cfs.append((d, -montant))
                    n_ok += 1
            px_now = _price_at_or_before(df_h, _now_ch)
            if px_now and parts > 0 and n_ok >= 2:
                val_finale = parts * px_now
                cfs.append((_now_ch, val_finale))
                tri = xirr(cfs)
                _results.append({"Portefeuille / Indice": _idx_display_names[idx_name],
                                  "TRI simulé": f"{tri:+.2f}%" if tri is not None else "—",
                                  "Valeur": fmt(val_finale),
        ...
        st.markdown(f"""<div style='font-size:10px;color:{C['muted']};padding:8px 0;font-style:italic'>
Méthode : tous les flux réels (DCA Bourse depuis 02/2022, DCA Crypto depuis 08/2022, dépôts IBKR réels)
sont simulés comme investis dans l'indice à leur date respective, prix = dernier cours connu à cette
date (ou avant). Valeur finale = parts accumulées × cours actuel de l'indice.
Si un indice affiche "—", le ticker yfinance n'est pas accessible depuis cet environnement — à vérifier
en prod (Streamlit Cloud).</div>""", unsafe_allow_html=True)
