...
with tab2:
    ...
    with L2:
        ...
        if _dca_actif5:
            ...
            _x_moy5 = [mois_f[i] for i,x in enumerate(_dca_tot5) if x>0]
            fig5.add_trace(go.Scatter(
                name=f'Moy. {_moy5:.0f}€/mois',
                x=_x_moy5, y=[_moy5]*len(_x_moy5),
                mode='lines',
                line=dict(color='#F59E0B',width=2,dash='dot'),
                hovertemplate=f'Moyenne : {_moy5:.0f} €<extra></extra>'
            ))
...
with tab3:
    ...
    with L3:
        ...
        if _dca_actif7:
            ...
            _x_moy7 = [mois_s[i] for i,x in enumerate(dca_c) if x>0]
            fig7.add_trace(go.Scatter(
                name=f'Moy. {_moy7:.0f}€/mois',
                x=_x_moy7, y=[_moy7]*len(_x_moy7),
                mode='lines',
                line=dict(color='#F59E0B',width=2,dash='dot'),
                hovertemplate=f'Moyenne : {_moy7:.0f} €<extra></extra>'
            ))
