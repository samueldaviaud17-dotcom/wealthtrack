...
with tab4:
    ...
    with _cr_col1:
        ...
        # Popover stylisé comme un lien — s'ouvre en popup flottante au clic
        with st.popover("✏️ modifier la valeur", use_container_width=False):
            st.markdown(f"**Valeur actuelle :** {st.session_state['capital_reel']:.2f} €")
            _new_cr = st.number_input("Nouveau capital réel (€)", value=float(st.session_state['capital_reel']),
