...
<th style='text-align:right;padding:4px'>Total</th></tr></thead><tbody>"""
    ...
    # ── CRYPTO ──
    with st.expander("₿ Versement Crypto", expanded=False):
        st.markdown(f"<div style='color:{C['muted']};font-size:13px;margin-bottom:12px'>Le virement bancaire arrive sur Binance et se convertit automatiquement en <b>USDC</b>. Pour acheter BTC/ETH/SOL/BNB (ou autre) ensuite, utilise le bloc <b>🔄 Conversion entre cryptos</b> juste en dessous.</div>", unsafe_allow_html=True)
        with st.form("form_crypto_dca"):
            ...
            _submit_c = st.form_submit_button("💾 Enregistrer", use_container_width=True)
        if _submit_c:
            if _montant_usdc <= 0:
    ...
    # ── CONVERSION CRYPTO (swap entre tokens, pas un flux externe) ──
    with st.expander("🔄 Conversion entre cryptos (swap)", expanded=False):
