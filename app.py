...
with tab1:
    ...
    if YF_AVAILABLE:
        with st.expander("📊 Indices de référence — variation journalière", expanded=True):
            _mkt_cols = st.columns(len(MARKET_TICKERS) + 1)  # +1 pour VIX
        ...
        <div style="font-size:13px;color:{C['muted']};margin-top:4px">—</div></div>""", unsafe_allow_html=True)
                    ...
                    _vix_var_col = C['red'] if _vix_var >= 0 else C['green']  # VIX inverse : hausse = mauvais
                    st.markdown(f"""<div style="background:{C['card']};border:1px solid {_vix_col}44;border-radius:8px;padding:10px 14px 8px 14px">
        <div style="font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em">😱 VIX — Peur</div>
        <div style="display:flex;align-items:baseline;gap:8px;margin-top:2px">
        <span style="font-family:'Space Grotesk';font-size:18px;font-weight:700">{_vix_val:.2f}</span>
        <span style="font-size:12px;font-weight:700;color:{_vix_var_col}">{'+' if _vix_var>=0 else ''}{_vix_var:.2f}%</span>
        </div>
        <div style="margin-top:6px;padding:2px 8px;border-radius:4px;background:{_vix_col}22;display:inline-block">
        <span style="font-size:11px;font-weight:700;color:{_vix_col}">{_vix_label}</span>
        </div>
        <div style="font-size:9px;color:{C['muted']};margin-top:4px">&lt;15 Calme · &lt;20 Normal · &lt;30 Prudence · &lt;40 Stress</div>
        </div>""", unsafe_allow_html=True)

            # ── Fear & Greed Index CNN — dans une nouvelle ligne ──
            _fg_data = fetch_fear_greed()
            if _fg_data is not None:
                _fg_score = int(_fg_data['score'])
                _fg_prev  = int(_fg_data['prev_close'])
                _fg_rating_raw = _fg_data['rating'].lower()
                # Labels FR + couleurs
                _FG_LABELS = {
                    'extreme fear': ('Peur Extrême', '#EF4444'),
                    'fear':         ('Peur',         '#F97316'),
                    'neutral':      ('Neutre',       C['gold']),
                    'greed':        ('Avidité',      '#84CC16'),
                    'extreme greed':('Avidité Extrême', '#22C55E'),
                }
                _fg_label, _fg_col = _FG_LABELS.get(_fg_rating_raw, ('—', C['muted']))
                _fg_delta = _fg_score - _fg_prev
                _fg_delta_col = C['red'] if _fg_delta < 0 else C['green']
                # Gauge SVG (demi-cercle)
                _angle = (_fg_score / 100) * 180  # 0=left, 180=right
                import math
                _rad = math.radians(180 - _angle)
                _cx, _cy, _r_needle = 60, 55, 42
                _nx = _cx + _r_needle * math.cos(_rad)
                _ny = _cy - _r_needle * math.sin(_rad)
                _gauge_svg = f"""<svg viewBox="0 0 120 65" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:160px;display:block;margin:4px auto 0">
  <path d="M10,55 A50,50 0 0,1 110,55" fill="none" stroke="#EF4444" stroke-width="8" stroke-dasharray="39.27 157.08"/>
  <path d="M10,55 A50,50 0 0,1 110,55" fill="none" stroke="#F97316" stroke-width="8" stroke-dasharray="39.27 157.08" stroke-dashoffset="-39.27"/>
  <path d="M10,55 A50,50 0 0,1 110,55" fill="none" stroke="{C['gold']}" stroke-width="8" stroke-dasharray="31.42 157.08" stroke-dashoffset="-78.54"/>
  <path d="M10,55 A50,50 0 0,1 110,55" fill="none" stroke="#84CC16" stroke-width="8" stroke-dasharray="39.27 157.08" stroke-dashoffset="-109.96"/>
  <path d="M10,55 A50,50 0 0,1 110,55" fill="none" stroke="#22C55E" stroke-width="8" stroke-dasharray="39.27 157.08" stroke-dashoffset="-149.23"/>
  <line x1="{_cx}" y1="{_cy}" x2="{_nx:.1f}" y2="{_ny:.1f}" stroke="white" stroke-width="2" stroke-linecap="round"/>
  <circle cx="{_cx}" cy="{_cy}" r="4" fill="white"/>
  <text x="{_cx}" y="68" text-anchor="middle" font-size="11" font-weight="700" fill="white">{_fg_score}</text>
</svg>"""
                st.markdown(f"""<div style="background:{C['card']};border:1px solid {_fg_col}44;border-radius:8px;padding:10px 14px 8px 14px;margin-top:8px">
  <div style="font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">😨 Fear &amp; Greed — CNN</div>
  <div style="display:flex;align-items:center;gap:12px">
    <div style="flex:1">{_gauge_svg}</div>
    <div style="flex:1">
      <div style="margin-top:4px;padding:2px 8px;border-radius:4px;background:{_fg_col}22;display:inline-block">
        <span style="font-size:12px;font-weight:700;color:{_fg_col}">{_fg_label}</span>
      </div>
      <div style="font-size:10px;color:{C['muted']};margin-top:4px">Veille : <b style="color:{_fg_delta_col}">{_fg_prev}</b> ({'+' if _fg_delta>=0 else ''}{_fg_delta})</div>
    </div>
  </div>
</div>""", unsafe_allow_html=True)
