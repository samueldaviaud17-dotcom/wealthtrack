...
with tab6:
    ...
    with col_form:
        ...
        # ── Choix de liste (toujours visible) ────────
        st.selectbox(
            "Ajouter dans :",
            ["📈 Actions en Portefeuille", "⚙️ Actions en Option", "👀 Actions à Surveiller"],
            key="valo_wl_target"
        )
...
<div style="font-size:10px;color:{C['muted']}">{score_detail}</div>
        ...
        if add_wl and action_name and cours_v > 0:
            ...
            target_label = st.session_state.get("valo_wl_target", "👀 Actions à Surveiller")
...
<div style="font-size:13px">Calcule une valorisation dans l'onglet <b>🔍 Valorisation</b>, choisis une liste et clique sur <b>⭐ Watchlist</b></div>
        ...
        tri_row = st.columns([5, 1])
        with tri_row[1]:
            asc_now = st.session_state.get("wl_sort_asc", False)
            if st.button("Écart ↑ Asc" if asc_now else "Écart ↓ Desc", key="wl_sort_btn", use_container_width=True):
        ...
        TH = lambda col: f"<th style='padding:8px 12px;text-align:right;border-bottom:1px solid {C["border"]};font-size:10px;color:{C["muted"]};text-transform:uppercase;letter-spacing:.06em'>{col}</th>"
        THL = lambda col: f"<th style='padding:8px 12px;text-align:left;border-bottom:1px solid {C["border"]};font-size:10px;color:{C["muted"]};text-transform:uppercase;letter-spacing:.06em'>{col}</th>"
        THC = lambda col: f"<th style='padding:8px 12px;text-align:center;border-bottom:1px solid {C["border"]};font-size:10px;color:{C["muted"]};text-transform:uppercase;letter-spacing:.06em'>{col}</th>"
        ...
        for list_key, list_label, list_color in SUB_LISTS:
            ...
            tbl += f"<table style='width:100%;border-collapse:collapse;font-size:12px;font-family:DM Sans'>"
            tbl += f"<thead><tr style='background:#1C2333'>"
            tbl += THL("Action") + THC("Ticker") + TH("PRU") + TH("Prix Actuel") + TH("Prix d'Achat avec Marge") + TH("Écart (%) ▼▲") + THC("Verdict")
            tbl += "</tr></thead><tbody>"
