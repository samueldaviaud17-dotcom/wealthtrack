    ...
    with R2:
        ...
        for i in range(15):
            ...
            if act and act not in ["Action",""] and net>0:
                div_rows.append({"Action":act,"Pays":pays,"Net (€)":net})
        if div_rows:
            df_div=pd.DataFrame(div_rows).sort_values("Net (€)",ascending=False).reset_index(drop=True)
            df_div["Net (€)"]=df_div["Net (€)"].map(lambda x:f"{x:.2f} €")
