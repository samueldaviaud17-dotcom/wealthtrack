...
<b>✅ Fait — TRI Global vs Benchmarks (migré dans Performance)</b><br>
Le TRI (XIRR) "Mon PF (Global)" combine tous les flux réels : DCA Bourse (depuis 02/2022),
DCA Crypto (depuis 08/2022, montant constant = capital investi / nb de mois, versé le 4 de
chaque mois), et dépôts IBKR réels (Options, via <code>depots_detail</code>). Comparé au TRI
simulé si ces mêmes flux avaient été investis dans <b>SP500 TR</b> (^SP500TR), <b>CAC40 GR</b>
(PX1GR.PA, fallback ^FCHI), <b>STOXX 600</b> (^STOXX). Remplace l'ancien "CAGR vs Benchmarks"
(Sheet lignes 13-16) et le KPI "CAGR ANNUALISÉ" (Vue d'ensemble + Performance, ex-<code>sv(df_p,5,9)</code>).
Code mutualisé : <code>xirr</code>, <code>dca_months</code>, <code>price_at_or_before</code>,
<code>fetch_index_history</code> (helpers globaux), calcul unique à l'init dans
<code>st.session_state['tri_global']</code>.
