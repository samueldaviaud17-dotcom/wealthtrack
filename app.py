    ...
    with col_form:
        ...
        if ticker_input and len(ticker_input) >= 2:
            ...
            if hist_data and hist_data.get("years"):
                ...
                # ── Choix de la liste puis bouton ────
                st.selectbox("Ajouter dans :", 
                    ["📈 Actions en Portefeuille", "⚙️ Actions en Option", "👀 Actions à Surveiller"],
                    key="valo_wl_target", label_visibility="visible")
                add_wl = st.button("⭐ Ajouter à la Watchlist", use_container_width=True, key="valo_add")
            else:
                st.selectbox("Ajouter dans :",
                    ["📈 Actions en Portefeuille", "⚙️ Actions en Option", "👀 Actions à Surveiller"],
                    key="valo_wl_target_2", label_visibility="visible")
...
<div style="font-size:10px;color:{C['muted']}">{score_detail}</div>
        ...
        if add_wl and action_name and cours_v > 0:
            ...
            # Resolve target list from selectbox (either key)
            target_label = (st.session_state.get("valo_wl_target") or
                            st.session_state.get("valo_wl_target_2") or
...
<tbody>'''
  ...
  <td style="padding:7px 12px;text-align:right;font-weight:700;color:{C["text"]};background:{'#0D2A0D' if pru_w > 0 and cours_w <= pru_w else ('#2A0D0D' if pru_w > 0 and cours_w > pru_w else 'transparent')};border-radius:4px">{cours_w:,.2f} €</td>
