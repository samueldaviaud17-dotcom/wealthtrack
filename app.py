...
<span style="font-size:12px;font-weight:700;color:{_vcol}">{'+' if _snap['var_pct']>=0 else ''}{_snap['var_pct']:.2f}%</span>
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start">
    <div>
      ...
      <div style="background:{C['border']};border-radius:3px;height:4px;margin-top:6px;width:110px">
        <div style="background:{_dca_col};height:100%;width:{min(_pct_dca,100):.0f}%;border-radius:3px"></div>
      </div>
    </div>
    <div style="text-align:right;font-size:12px;line-height:2.0">
      <div>📈 <b style="color:{C['blue']}">{fmt(_dca_b_ce_mois)}</b></div>
      <div>₿ <b style="color:{C['gold']}">{fmt(_dca_c_ce_mois)}</b></div>
      <div>⚙️ <b style="color:{C['purple']}">{fmt(_dca_o_real)}</b></div>
    </div>
  </div>
</div>""", unsafe_allow_html=True)
            with st.popover("✏️", use_container_width=False):
  ...
  <div style="background:{C['border']};border-radius:3px;height:4px">
    <div style="background:{C['blue']};height:100%;width:{_pct_yr:.0f}%;border-radius:3px"></div>
        ...
        with _o4:
            ...
            _avance_label = "En avance ✅" if _avance >= 0 else "En retard ⚠️"
            st.markdown(f"""<div style="background:{C['card']};border:1px solid {C['border']};border-radius:8px;padding:12px 14px">
  <div style="font-size:11px;color:{C['muted']};text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">📊 Trajectoire objectif</div>
  <div style="font-family:'Space Grotesk';font-size:26px;font-weight:700;color:{_avance_col}">{'+' if _avance>=0 else ''}{fmt(_avance)}</div>
  <div style="font-size:10px;color:{C['muted']};margin:3px 0 6px"><span style="color:{_avance_col};font-weight:700">{_avance_label}</span> · attendu {fmt(_obj_expected)}</div>
  <div style="background:{C['border']};border-radius:3px;height:4px">
